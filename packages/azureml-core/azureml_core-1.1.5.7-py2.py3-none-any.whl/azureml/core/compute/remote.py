...
class RemoteCompute(ComputeTarget):
    ...
    def _attach(workspace, name, config):
        ...
        :raises: :class:`azureml.exceptions.ComputeTargetException`
        """
        attach_payload = RemoteCompute._build_attach_payload(config.resource_id, config.address,
    ...
    @staticmethod
    def attach_configuration(username, resource_id=None, address=None, ssh_port=22, password='', private_key_file='',
                             private_key_passphrase=''):
        ...
        * US South Central
        ...
        """
        config = RemoteComputeAttachConfiguration(username, resource_id, address, ssh_port, password, private_key_file,
                                                  private_key_passphrase)
        return config
    ...
    def _build_attach_payload(resource_id, address, ssh_port, username, password=None, private_key_file=None,
        ...
        if address is not None and resource_id is not None:
...
class RemoteComputeAttachConfiguration(ComputeTargetAttachConfiguration):
    ...
    def __init__(self, username, resource_id, address,
                 ssh_port=22, password='', private_key_file='', private_key_passphrase=''):
    ...
    def validate_configuration(self):
        ...
        if self.address is not None and self.resource_id is not None:
            raise ComputeTargetException('only one of address or resource_id must be provided but not both.')
        if not self.address and not self.resource_id:
            raise ComputeTargetException('address or resource_id must be provided.')
