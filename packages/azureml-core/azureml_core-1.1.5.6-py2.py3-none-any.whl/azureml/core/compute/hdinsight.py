...
class HDInsightCompute(ComputeTarget):
    ...
    def _attach(workspace, name, config):
        ...
        :raises: :class:`azureml.exceptions.ComputeTargetException`
        """
        attach_payload = HDInsightCompute._build_attach_payload(config.resource_id, config.address,
    ...
    def attach_configuration(username, resource_id=None, address=None, ssh_port='22', password='', private_key_file='',
        ...
        * US South Central
        ...
        """
        config = HDInsightAttachConfiguration(username, resource_id, address, ssh_port, password, private_key_file,
                                              private_key_passphrase)
        return config
