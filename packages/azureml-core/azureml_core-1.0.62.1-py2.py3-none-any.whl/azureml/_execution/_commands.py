    ...
    try:
        ...
        if execute_with_zip:
            if prepare_only:
                ...
                files = [
                    ("runDefinitionFile", ("definition.json", json.dumps(definition))),
...
def _get_telemetry_values(telemetry_values):
    ...
    if os.path.exists(notebook_instance_file) and os.path.isfile(notebook_instance_file):
        is_notebook_vm = True
        try:
            with open(notebook_instance_file, "r") as config_file:
                config = json.load(config_file)
                notebook_instance_name = config["instance"]
        except:
            pass

    telemetry_values["notebookVM"] = is_notebook_vm
    if notebook_instance_name:
        telemetry_values["notebookInstanceName"] = notebook_instance_name
