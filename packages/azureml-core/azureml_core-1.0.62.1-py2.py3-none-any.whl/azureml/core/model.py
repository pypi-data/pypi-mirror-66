...
class Model(object):
            ...
            if '/datasetSnapshotName' in dataset_id:
                ...
                try:
                    ...
                    try:
                        ...
                        self.datasets.setdefault(dataset_scenario, []).append(legacy_dataset)
                    except Exception as e:
    ...
    def register(workspace, model_path, model_name, tags=None, properties=None, description=None,
        ...
        :param datasets: A list of tuples representing a pairing of purpose to dataset object.
        :type datasets: builtin.list[tuple[str, TabularDataset | FileDataset]]
    ...
    def _resolve_to_model_ids(workspace, models, name):
        """Convert a mixed list of model object, IDs, or local file paths to a list of just model IDs."""
        model_objs = []
        model_number = 1
        for model in models:
    ...
    def update(self, tags=None, description=None):
        """Perform an inplace update of the model.

        :param tags: A dictionary of tags to update the model with. Will replace what currently exists
        :type tags: dict[str, str]
        :param description: Replace the description of the model
        :type description: str
        :raises: azureml.exceptions.WebserviceException
        """
    ...
    def add_dataset_references(self, datasets):
        ...
        :type datasets: builtin.list[(str, Dataset | DatasetSnapshot)]
        :raises: azureml.exceptions.WebserviceException
        """
        headers = {'Content-Type': 'application/json-patch+json'}
        headers.update(self._auth.get_authentication_header())
    ...
    def deploy(workspace, name, models, inference_config, deployment_config=None, deployment_target=None):
        ...
        if deployment_config and (type(deployment_config) is LocalWebserviceDeploymentConfiguration):
            service = deployment_config._webservice_type._deploy(workspace,
                                                                 name,
                                                                 models,
            ...
            # make sure we don't deploy IotWebservice with environment, only support Aci and Aks
            from azureml._model_management._constants import IOT_WEBSERVICE_TYPE
            if deployment_config and \
               deployment_config._webservice_type._webservice_type != IOT_WEBSERVICE_TYPE:
            ...
            if use_env_path:
                ...
                if deployment_config._webservice_type._webservice_type == MIR_WEBSERVICE_TYPE:
                    raise WebserviceException("MIR Webservices must be deployed with an environment")
                else:
                    return Webservice.deploy_from_model(workspace, name, models, inference_config,
                                                        deployment_config, deployment_target)

    @staticmethod
    def package(workspace, models, inference_config, generate_dockerfile=False):
        ...
        :type inference_config: :class:`azureml.core.model.InferenceConfig`
        ...
        :rtype: :class:`azureml.core.model.ModelPackage`
        """
    ...
    def _deploy_with_environment(workspace, name, models, inference_config, deployment_config, deployment_target):
        ...
        if 'computeName' in webservice_payload:
...
class InferenceConfig(object):
    ...
    def validate_configuration(self):
        ...
        if self.environment:
            ...
            if not self.environment.docker.base_image.startswith('mcr.microsoft.com/azureml/base') and \
                    not self.environment.inferencing_stack_version:
                module_logger.warning('Warning, custom base image detected without a specified '
...
class ModelPackage(object):
    ...
    :type workspace: :class:`azureml.core.Workspace`
    :param operation_id: Internal ID of the package creation operation.
    ...
    def __init__(self, workspace, operation_id, environment):
        ...
        :type environment: :class:`azureml.core.Environment`
        """
        self.workspace = workspace
    ...
    def get_container_registry(self):
        ...
        :return: Address and login credentials for the container registry.
        :rtype: :class:`azureml.core.ContainerRegistry`
        """
        ...
        if self.generate_dockerfile:
            ...
            registry_details = image_details.dockerImage['registry']
    ...
    def save(self, output_directory):
        ...
        :param output_directory: Local directory that will be created to contain the contents of the package.
    ...
    def wait_for_creation(self, show_output=False):
        ...
        Wait for package creation to reach a terminal state. Will throw a WebserviceException if it reaches a
