...
class LocalWebservice(Webservice):
    ...
    CONTAINER_LABEL_INFERENCE_CONFIG = 'amlInferenceConfig'
    ...
    CONTAINER_ENVVAR_MODEL_PATH = "AZUREML_MODEL_DIR"
    ...
    def update(self, models=None, image_config=None, deployment_config=None, wait=False, inference_config=None):
        ...
        self._deployable |= (self.workspace is not None and
                             ...
                             (self._image_config is not None or self._inference_config is not None))
    ...
    def _generate_docker_context(self):
        ...
        if self._image_config is not None:
            ...
            # MMS-generated files
            package = Model.package(self.workspace, [], self._inference_config, generate_dockerfile=True)
        ...
        if self._models_dir is not None:
            shutil.copytree(self._models_dir.name, os.path.join(assets_dir, MODEL_PACKAGE_MODELS_DIR))
    ...
    def _get_container_inference_config(container):
        inference_config = pickle.loads(b64decode(container.labels[LocalWebservice.CONTAINER_LABEL_INFERENCE_CONFIG]))
        if inference_config is not None:
    ...
    def _run_container(self, wait):
        ...
        labels = {
            ...
            LocalWebservice.CONTAINER_LABEL_INFERENCE_CONFIG:
        ...
        model_path_envvar = posixpath.join(MODEL_PACKAGE_MODELS_DIR, self._model_path_dir)
        environment = {
            LocalWebservice.CONTAINER_ENVVAR_MODEL_PATH: model_path_envvar
        }
