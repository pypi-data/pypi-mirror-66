...
def collect_datasets_usage(logger, message, datasets, workspace, compute, custom_dimensions=None):
    ...
    try:
        ...
        dimensions.update(custom_dimensions or {})

        ds_infos = [_get_dataset_info(ds) for ds in datasets]
        for ds_info in ds_infos:
            ds_info.update(dimensions)
...
def _get_dataset_info(dataset):
    ...
    if isinstance(dataset, DatasetConsumptionConfig):
        ...
        return {
            'input_type': 'Unknown:' + type(dataset).__name__
        }
