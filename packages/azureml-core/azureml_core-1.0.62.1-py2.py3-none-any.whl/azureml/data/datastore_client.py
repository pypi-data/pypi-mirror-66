...
class _DatastoreClient:
    ...
    @staticmethod
    def _grant_msi_workspace_access(subscription_id, resource_group, workspace, account_name):
        ...
        # Construct body
        workspace_resource = _commands.get_workspace(workspace._auth, subscription_id, resource_group,
                                                     workspace._workspace_name)
        """ The role Definition used here is for Storage Blob Data Owner role : b7e6dc6d-f1e8-4753-8033-0f276bb0955b"""
        properties = {}
        properties["roleDefinitionId"] = "/subscriptions/" + workspace._subscription_id + \
                                         "/providers/Microsoft.Authorization/roleDefinitions/" \
                                         "b7e6dc6d-f1e8-4753-8033-0f276bb0955b"
        properties["principalId"] = workspace_resource.identity_principal_id
        data = {}
        ...
        if response.status_code not in [200, 201, 409]:
            module_logger.error("Please contact an admin to manually assign the "
                                "Storage Blob Data Owner role to the Workspace MSI."
                                "Trying to grant Workspace MSI access to the Storage "
                                "Account failed with error status '{}' and error message '{}'"
                                .format(response.status_code, _DatastoreClient._get_error_message(response)))
            raise Exception("Please contact an admin to manually assign the Storage Blob Data Owner role to the"
                            " Workspace MSI.Trying to grant Workspace MSI access to the Storage Account failed "
                            "with error code '{}' and error '{}'"
                            .format(response.status_code, _DatastoreClient._get_error_code(response)))
    ...
    def _check_or_grant_access(subscription_id, resource_group, workspace, account_name):
        ...
        if response.status_code not in [200]:
            ...
            for role in json_obj['value']:
                # If Storage Blob Data Owner or Contributor Role Id present
                # in the roles for the workspace is present, do nothing
                if 'b7e6dc6d-f1e8-4753-8033-0f276bb0955b' in role['properties']['roleDefinitionId'] or \
                        'b24988ac-6180-42a0-ab88-20f7382dd24c' in role['properties']['roleDefinitionId']:
                    return

        else:
            module_logger.warning("User does not have permissions to check the Workspace MSI access to the Storage "
                                  "container. error '{}'.We will try with best effort to grant access to the Workspace"
                                  "MSI for the storage account.".format(rest_client._deserialize('ErrorResponse',
                                                                                                 response)))

        _DatastoreClient._grant_msi_workspace_access(subscription_id, resource_group, workspace, account_name)
    ...
    def register_azure_blob_container(workspace, datastore_name, container_name, account_name, sas_token=None,
        ...
        """
        if grant_workspace_access and subscription_id is not None and resource_group is not None:
    ...
    def _get(ws, name, auth=None, host=None):
        ...
        datastore = client.data_stores.get(name, ws._subscription_id, ws._resource_group, ws._workspace_name,
    ...
    def _register(ws, dto, create_if_not_exists, skip_validation, overwrite, auth, host):
        try:
            ...
            if e.response.status_code == 400 and overwrite and CONFLICT_MESSAGE in e.message:
                ...
                client = _DatastoreClient._get_client(ws, auth, host)
                client.data_stores.update(dto.name, ws._subscription_id, ws._resource_group,
    ...
    def _delete(ws, name, auth=None, host=None):
        ...
        client = _DatastoreClient._get_client(ws, auth, host)
        client.data_stores.delete(name, ws._subscription_id, ws._resource_group, ws._workspace_name,
                                  custom_headers=_DatastoreClient._custom_headers)
    ...
    def _set_default(ws, name, auth=None, host=None):
        ...
        client = _DatastoreClient._get_client(ws, auth, host)
        client.data_stores.set_default(name, ws._subscription_id, ws._resource_group, ws._workspace_name,
                                       custom_headers=_DatastoreClient._custom_headers)
    ...
    def _dto_to_datastore(ws, datastore):
        ...
        if datastore.data_store_type == constants.AZURE_MYSQL:
