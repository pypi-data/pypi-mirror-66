...
from .constants import (PAGE_SIZE_KEY, TOP_KEY, CALLER_KEY, CUSTOM_HEADERS_KEY, RequestHeaders,
                        FILTER_KEY, ORDER_BY_KEY, QUERY_PARAMS_KEY)
from .contracts.query_params import create_query_params
...
def _generate_client_kwargs(has_query_params=False, **kwargs):
    if not kwargs:
        return None
    ...
    if has_query_params:
        ...
        query_params = create_query_params(filter=kwargs.pop(FILTER_KEY, None), top=kwargs.pop(TOP_KEY, None),
