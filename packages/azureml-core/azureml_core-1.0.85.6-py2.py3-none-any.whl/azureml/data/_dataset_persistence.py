...
from azureml.data.dataset_factory import TabularDatasetFactory, FileDatasetFactory, _normalize_path
...
def create_dataset_from_file(workspace, def_path, validate, register):
    ...
    if validate and not is_dataprep_installed():
        _raise_dataprep_required_error('Cannot perform validation', 'set --skip-validation')
    ...
    def _create_tabular(self, parameters, validate):
        ...
        if source_type == 'delimited_files':
            ...
            if infer_column_types:
                _raise_dataprep_required_error(
                    'Cannot infer column types',
                    'set {} to false in definition file'.format(self._prop_infer_column_types))
            return self._create_dataset_from_blocks([
                _Block.craft_get_file_block(path),
...
def _raise_dataprep_required_error(reason, fix):
    install_cmd = '"{}" -m pip install azureml-dataprep'.format(sys.executable)
    message = '{}. Please install required package by running `{}`, or {}.'.format(reason, install_cmd, fix)
    raise RuntimeError(message)
