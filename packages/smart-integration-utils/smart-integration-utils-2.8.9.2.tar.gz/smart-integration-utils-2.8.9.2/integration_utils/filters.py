...
class __Validate(object):
    ...
    def generate_mongo_query(self, parsed: dict) -> dict:
        ...
        field = field.replace('!', '')
        if operator == 'in_':
            query[f'{field}__regex' if not exclude else f'{field}__regex_ne'] = parsed['value']
        elif operator == 'not_in':
            query[f'{field}__regex_ne' if not exclude else f'{field}__regex'] = parsed['value']
        elif operator == 'start':
            query[f'{field}__startswith' if not exclude else f'{field}__not_startswith'] = parsed['value']
        elif operator == 'end':
            query[f'{field}__endswith' if not exclude else f'{field}__not_endswith'] = parsed['value']
        elif operator == 'eq':
    ...
    def generate_query(self, mongo: bool = True) -> list:
        ...
        for parsed in self.parsed_filters:
            if isinstance(parsed, list):
                ...
                queries.append(
                    self.generate_mongo_query(parsed) if mongo else self.generate_django_orm_query(parsed)
                )
        return queries
    ...
    def generate_django_orm_query(self, parsed: dict) -> dict:
        ...
        field = field.replace('!', '')
        if operator == 'in_':
            query['filter' if not exclude else 'exclude'][f'{field}__iexact'] = parsed['value']
        elif operator == 'not_in':
            query['exclude' if not exclude else 'filter'][field] = parsed['value']
        elif operator == 'start':
            query['filter' if not exclude else 'exclude'][f'{field}__startswith'] = parsed['value']
        elif operator == 'end':
            query['filter' if not exclude else 'exclude'][f'{field}__endswith'] = parsed['value']
        elif operator == 'eq':
