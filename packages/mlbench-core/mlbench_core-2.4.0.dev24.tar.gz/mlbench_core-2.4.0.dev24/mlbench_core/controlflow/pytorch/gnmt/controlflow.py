...
MockDataPoint = namedtuple("MockDataPoint", "src trg")
...
class GNMTTrainer:
    ...
    def optimize(self, batch_idx, data, num_batches_per_device_train, update=False, track=True):

        # Whether to update the weights at this iteration
        # update = (batch_idx % self.iter_size) == self.iter_size - 1
        if self.tracker and track:
    ...
    def preallocate(self, batch_size, max_length):
        ...
        if self.prealloc_mode == 'always' or (self.prealloc_mode == 'once' and
            ...
            logger.info('Executing preallocation')
            ...
            src = torch.full(shape, 4, dtype=torch.int64, device=device)
            tgt = torch.full(shape, 4, dtype=torch.int64, device=device)
            src = src, src_length
            tgt = tgt, tgt_length

            data = MockDataPoint(src, tgt)
            self.optimize(batch_idx=0, data=data, update=False, num_batches_per_device_train=0, track=False)
    ...
    def train_round(
        ...
        self.preallocate(train_loader.batch_size, train_loader.dataset.max_len)

        for batch_idx, data in enumerate(train_loader):
            update = (batch_idx % self.iter_size) == self.iter_size - 1
            self.optimize(batch_idx, data, num_batches_per_device_train, update=update, track=True)
