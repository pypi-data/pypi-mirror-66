...
def build_collate_fn(batch_first, sort):
    def collate_seq(seq):
        ...
        for i, s in enumerate(seq):
            end_seq = lengths[i]
            segmented = torch.Tensor(s[:end_seq])
...
def get_loader(data, batch_size, batch_first, num_workers, sort, pin_memory):
    collate_fn = build_collate_fn(batch_first=batch_first, sort=sort)
    return DataLoader(
        data,
...
class WMT14Dataset(Dataset):
    ...
    def _process_data(self, path, filter_pred, lazy=False, max_size=None):
    ...
    def _parse_example(self, example):
        ...
        return self.fields['src'].parse_line(src_line), self.fields['trg'].parse_line(trg_line)

    def __getitem__(self, item):
        if self.lazy:
            return self._parse_example(self.examples[item])
        else:
