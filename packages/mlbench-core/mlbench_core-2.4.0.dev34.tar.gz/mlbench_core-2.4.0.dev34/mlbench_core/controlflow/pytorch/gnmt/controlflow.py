...
class GNMTTrainer:
    ...
    def train_round(
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            logger.info("Summary for {} (before): {}".format(batch_idx, summary))
            self.optimize(batch_idx, data, num_batches_per_device_train)

            summary = torch.cuda.memory_summary()
            logger.info("Summary for {} (after): {}".format(batch_idx, summary))
            # if torch.cuda.is_available():
            #     start = datetime.now()
            #     torch.cuda.empty_cache()
            #     end = datetime.now()
            #     logger.info("Empty cache for {} took {}".format(batch_idx, end - start))
