...
def _maybe_cuda(example, use_cuda=False):
    src, trg = example
    if use_cuda:
        src = src[0].cuda(), src[1].cuda()
        trg = trg[0].cuda(), trg[1].cuda()
    return src, trg
...
class GNMTTrainer:
    ...
    def compute_model_output(self, src, trg):
        ...
        if self.batch_first:
            output = self.model(src, src_length, trg[:, :-1])
        else:
            output = self.model(src, src_length, trg[:-1])
    ...
    def compute_loss(self, src, trg, output):
        ...
        Returns:
            (float, float, float, dict): Total loss, loss per token, loss per sentence, num tokens
        """
        src, src_length = src
        trg, trg_length = trg
    ...
    def train_round(
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            logger.info("Batch {} took {}".format(batch_idx, end - start))
            data = _maybe_cuda(data, use_cuda=self.use_cuda)
    ...
    def validate(self, loader):
        ...
        with torch.no_grad():
            ...
            for data in loader:
                src, trg = _maybe_cuda(data, use_cuda=self.use_cuda)
                output = self.compute_model_output(src, trg)
                ...
                # Update loss
                losses.update(loss_per_token, num_toks["trg"])
