...
def unpack_tensors(aggregated, indices, sizes, convert_cuda=False):
    """
    ...
    Args:
        ...
        convert_cuda (bool): Convert the resulting tensors to cuda
    ...
    for i, (start, end) in enumerate(zip(start_index, end_index)):
        if convert_cuda:
            tensors.append(aggregated[start: end].view(sizes[i]).cuda())
        else:
...
class Aggregation(object):
    ...
    def _agg_weights_by_model(self, model, op):
        ...
        convert_cuda = next(model.parameters()).is_cuda and not self.use_cuda
        tensors = unpack_tensors(aggregated, indices, sizes, convert_cuda=convert_cuda)
        # Unpack
