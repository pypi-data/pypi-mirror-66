...
#         if use_cuda:
#             data = data[0].cuda().long(), data[1].cuda()
#             target = target[0].cuda().long(), target[1]
#         yield data, target
...
class GNMTTrainer:
    ...
    def _maybe_cuda(self, data):
        src, trg = data
        if self.use_cuda and torch.cuda.is_available():
            src = (src[0].cuda(), src[1].cuda())
            trg = (trg[0].cuda(), trg[1])
    ...
    def train_round(
        ...
        for batch_idx, data in enumerate(train_loader):
            logger.info("Loaded batch {}".format(batch_idx))
            time.sleep(10)
