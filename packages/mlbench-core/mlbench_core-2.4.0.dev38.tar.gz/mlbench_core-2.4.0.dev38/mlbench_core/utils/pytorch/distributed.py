...
def pack_tensors(tensors, use_cuda=False):
    ...
    logger.info("Tensor types grad: {} {}, packed {} {}, size = ".format(tensors[0].dtype,
                                                                         tensors[0].is_cuda,
                                                                         vec.dtype,
                                                                         vec.is_cuda,
                                                                         vec.nelement() * vec.element_size()))
...
class Aggregation(object):
    ...
    def _agg_gradients_by_model(self, model, op):
        ...
        logger.info("Recevied tensor {} size {} cuda {}".format(aggregated.dtype, aggregated.nelement() * aggregated.element_size(),
                                                                aggregated.is_cuda))
        # Unpack
        tensors = unpack_tensors(aggregated, indices, sizes)
        for i, param in enumerate(model.parameters()):
