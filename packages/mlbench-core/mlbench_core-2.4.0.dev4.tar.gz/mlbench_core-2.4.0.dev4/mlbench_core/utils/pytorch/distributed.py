...
def pack_tensors(tensors, use_cuda=False):
    ...
    vec = torch.empty(
        pointers[-1],
        device=tensors[0].device if use_cuda else "cpu",
    )
    ...
    for tensor, start_idx, end_idx in zip(tensors, pointers[:-1], pointers[1:]):
        if not use_cuda and tensor.is_cuda: # Tensor is on GPU, but we don't use GPU for comm => convert
            vec[start_idx:end_idx] = tensor.data.view(-1).cpu()
        elif use_cuda == tensor.is_cuda:  # either both are false or both are true => no conversion
            vec[start_idx:end_idx] = tensor.data.view(-1)
        else: # Not supported
            raise ValueError("Cannot use cuda for communication if model is not on GPU")
    return vec, indices, tensor_sizes


def unpack_tensors(aggregated, indices, sizes, num_params, convert_cuda=False):
    ...
    for i in range(num_params):
        if convert_cuda:
            tensors.append(aggregated[start_index[i] : end_index[i]].view(sizes[i]).cuda())
        else:
            tensors.append(aggregated[start_index[i] : end_index[i]].view(sizes[i]))
...
class Aggregation(object):
    ...
    def _agg_gradients_by_model(self, model, op):
        ...
        tensors = unpack_tensors(aggregated, indices, sizes, num_params, convert_cuda=model.is_cuda and not self.use_cuda)
        for i, param in enumerate(model.parameters()):
