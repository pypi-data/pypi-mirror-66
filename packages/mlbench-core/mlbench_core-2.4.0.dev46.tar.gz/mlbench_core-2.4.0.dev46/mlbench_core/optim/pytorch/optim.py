...
class PowerSGD(Optimizer):
    ...
    def reduce(self, grad_in, grad_out, memory_out):
        ...
        rank1_tensor_list = pack_tensors([tensor for (tensor, _, _) in rank1_tensors])

        rank1_handle = dist.all_reduce(rank1_tensor_list, async_op=True)
        ...
        rank1_handle.wait()
        rank1_tensor_list[:] /= self.n_workers
        unpacked = unpack_tensors(rank1_tensor_list)
