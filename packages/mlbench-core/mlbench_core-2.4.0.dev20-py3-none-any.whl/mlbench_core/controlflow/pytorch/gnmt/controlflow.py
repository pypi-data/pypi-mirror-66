...
class GNMTTrainer:
    ...
    def optimize(self, batch_idx, data, num_batches_per_device_train):
        ...
        # Opt step
        if update:
            self.fp_optimizer.step(batch_idx)
            if self.tracker:
            ...
            if self.schedule_per == "batch":
                self.scheduler.step()
                if self.tracker:
                    self.tracker.record_batch_step("lr_schedule")
    ...
    def train_round(
        ...
        for batch_idx, data in enumerate(train_loader):
            start = datetime.now()
            logger.info("Loading for {} took {}".format(batch_idx, start - ss))
            self.optimize(batch_idx, data, num_batches_per_device_train)
            end = datetime.now()
            logger.info("Optimization for {} took {}".format(batch_idx, end - start))
