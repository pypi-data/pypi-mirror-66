...
import gaeio.src.segpy.reader as syreader
import gaeio.src.segpy.writer as sywriter
import gaeio.src.segpy.packer as sypacker
...
def readSeisInfoFromSegy(segyfile, traceheaderformat=TraceHeaderRev1, endian='>'):
    ...
    with open(segyfile, 'rb') as sid:
        ...
        if (dat.dimensionality > 3 and dat.dimensionality <= 0):
            vis_msg.print('ERROR in readSeisInfoFromSegy: ' + segyfile + ' contains no seismic survey',
                          type='error')
            sys.exit()

        if (dat.dimensionality == 1):
            vis_msg.print('WARNING in readSeisInfoFromSegy: ' + segyfile + ' contains 1D seismic survey',
                          type='warning')
            inlstart = 0
            inlend = 0
            inlnum = 1
        ...
        if (dat.dimensionality == 2):
            vis_msg.print('WARNING in readSeisInfoFromSegy: ' + segyfile + ' contains 2D seismic survey',
                          type='warning')
            inlstart = 0
            inlend = 0
            inlnum = 1
...
def readSeis3DMatFromSegyNoInfo(segyfile, traceheaderformat=TraceHeaderRev1, endian='>',
    ...
    with open(segyfile, 'rb') as sid:
        ...
        #
        if (dat.dimensionality > 3 and dat.dimensionality <= 0):
            vis_msg.print('ERROR in readSeis3DMatFromSegyNoInfo: ' + segyfile + ' contains no seismic survey',
                          type='error')
            sys.exit()
        ...
        if (dat.dimensionality == 2):
            vis_msg.print('WARNING in readSeis3DMatFromSegyNoInfo: ' + segyfile + ' contains 2D seismic survey',
                          type='warning')
            inlstart = 0
            inlend = 0
            inlnum = 1
...
def writeSeis2DMatToSegyWithRef(seis2dmat, seisfile,
    ...
    with open(refsegy, 'rb') as refid, \
        ...
        for itrace in seisdata.trace_indexes():
            ...
            if itrace >= tracenum:
                vis_msg.print('WARNING in writeSeis2DMatToSegyWithRef: Reference segy file ' + refsegy +
                              ' contains more traces than 2D seismic matrix', type='warning')
                break
        if verbose:
...
def writeSeis3DMatToSegyWithRef(seis3dmat, seisfile,
    ...
    with open(refsegy, 'rb') as refid, \
        ...
        for itrace in seisdata.trace_indexes():
            ...
            if itrace >= tracenum:
                vis_msg.print('WARNING in writeSeis3DMatToSegyWithRef: Reference segy file ' + refsegy +
                              ' contains more traces than input 3D seismic matrix', type='warning')
                break
        if verbose:
            print('Done --> output file saved as ' + seisfile)
        #
...
def writeSeis2DMatToSegyNoRef(seis2dmat, seisfile,
    ...
    if np.ndim(seis2dmat) != 2:
        ...
        sys.exit()
    if datacol < 0 or len(seis2dmat[0, :]) <= datacol:
        vis_msg.print('ERROR in writeSeis2DMatToSegyNoRef: No data column found in 2D seismic matrix',
                      type='error')
        sys.exit()
...
def createSegyBinaryHeader(znum, zstep):
    ...
    if znum <= 0:
        vis_msg.print('ERROR in createSegyBinaryHeader: Positive num of samples per trace expected',
                      type='error')
        sys.exit()
    if zstep >= 0:
        vis_msg.print('ERROR in createSegyBinaryHeader: Negative sample interval in milliseconds (ms) expected',
                      type='error')
        sys.exit()
...
def createSegyTraceHeader(inlidx, xlidx, xcoor, ycoor,
    ...
    if znum <= 0:
        ...
        sys.exit()
    if zstep >= 0:
        vis_msg.print('ERROR in createSegyTraceHeader: Negative sample interval in milliseconds (ms) expected',
                      type='error')
        sys.exit()
    if zlag > 0:
        vis_msg.print('ERROR in createSegyTraceHeader: Non-positive vertical lag-time in millisecond (ms) expected',
                      type='error')
        sys.exit()
