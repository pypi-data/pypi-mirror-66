...
class RTI(Emitter):
    ...
    def __init__(self, application_id="python", application_version=None, instance_id=None, federation_id=None, secret=None, incognito=False, suppress_own_messages=False, connect=True):
        super().__init__()
    ...
    def publish_json(self, channel_name: str, message: object):
        try:
            self._add_channel(channel_name, data_type="json")
            if self.federation_id and not channel_name.startswith("@") and channel_name != constants.federations_channel:
                channel_name = self.federation_id + "/" + channel_name
