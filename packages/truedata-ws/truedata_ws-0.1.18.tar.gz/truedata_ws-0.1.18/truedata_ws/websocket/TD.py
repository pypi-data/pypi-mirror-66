...
from .support import LiveData, TouchlineData
...
class TDLiveDataError(LookupError):
    def __str__(self):
        return f"{Style.BRIGHT}{Fore.RED}Something's wrong with the historical data...{Style.RESET_ALL}"
...
class LiveClient(WebSocketApp):
    ...
    def handle_message_data(self, msg):
        ...
        if msg['message'] == 'TrueData Real Time Data Service':  # Connection success message
            print(f"You can trade on {msg['segments']}")
    ...
    def add_contract_details(self, contracts_list):
        for contract in contracts_list:
            ...
            for ticker_id in self.parent_app.symbol_mkt_id_map[symbol]:
                self.parent_app.touchline_data[ticker_id] = TouchlineData()
                ...
                self.parent_app.touchline_data[ticker_id].close = float(contract_details[5])
                ...
                self.parent_app.touchline_data[ticker_id].turnover = float(contract_details[9])
    ...
    def handle_trade_data(self, trade_tick):
        try:
            ...
            for ticker_id in self.parent_app.symbol_mkt_id_map[symbol]:
                ...
                self.parent_app.live_data[ticker_id].ltp = ltp = float(trade_tick[2])
                ...
                self.parent_app.live_data[ticker_id].turnover = float(trade_tick[6])
                try:
                    ...
                    self.parent_app.live_data[ticker_id].special_tag = special_tag = str(trade_tick[11])
                    if self.parent_app.live_data[ticker_id].special_tag != "":
                        if special_tag == 'H':
                            print(f"{Style.BRIGHT}{Fore.GREEN}NEW DAY'S HIGH for {symbol}...{Style.RESET_ALL}")
                            self.parent_app.live_data[ticker_id].day_high = ltp
                        elif special_tag == 'L':
                            print(f"{Style.BRIGHT}{Fore.RED}NEW DAY'S LOW for {symbol}...{Style.RESET_ALL}")
                            self.parent_app.live_data[ticker_id].day_low = ltp
                except IndexError:
...
class TD:
    ...
    def connect(self):
        ...
        print(f"\t\t\twss://{self.url}:{self.live_port}?user={self.login_id}&password={self.password}{fyers_append}")
    ...
    def start_live_data(self, resolved_contract, req_id=None):
        global DEFAULT_MARKET_DATA_ID
        if req_id is None:
            req_id = DEFAULT_MARKET_DATA_ID
        if type(resolved_contract) == str:
            ...
            return req_id
        elif type(resolved_contract) == list:
            req_ids = []
            if type(req_id) == list:
                if len(req_id) == len(resolved_contract):
                    req_ids = req_id
                else:
                ...
                if req_id is None:
                    ...
                    for i in range(0, len(resolved_contract)):
                        req_ids.append(curr_req_id + i)
                    DEFAULT_MARKET_DATA_ID = DEFAULT_MARKET_DATA_ID + len(resolved_contract)
                else:
                    curr_req_id = req_id
                    for i in range(0, len(resolved_contract)):
                        req_ids.append(curr_req_id + i)
            for j in range(0, len(req_ids)):
                print(j)
                self.live_data[req_ids[j]] = LiveData(resolved_contract[j])
                try:
                    print('Adding to existing')
                    self.symbol_mkt_id_map[resolved_contract[j].upper()].append(req_ids[j])
                    print('Adding to existing... SUCCESS')
                except KeyError:
                    self.symbol_mkt_id_map[resolved_contract[j].upper()] = [req_ids[j]]
                    print('Creating new... SUCCESS')
                self.live_websocket.send(f'{{"method": "addsymbol", "symbols": ["{resolved_contract[j]}"]}}')
            return req_ids
