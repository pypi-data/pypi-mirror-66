...
class Logger(object):
    def __init__(self, name="Main", print_level=0, dating="%H:%M:%S:%3N  %Y-%m-%d", filing="%Y-%m-%d"):
        self.name = name
    ...
    def log(self, data, level=2):
        if level == 1:
            ...
            ex = "INFO"
        full_data = "[ " + ex + " ] - " + datetime.datetime.strftime(self.dating) + " >> " + str(data)
        if self.print_level == level or self.print_level == 3 or debug_level == 3 or debug_level == level:
            print(full_data)
        file_ops.write_file(os.path.join(self.workDir, datetime.datetime.strftime(self.filing) + ".log"), full_data, True)
