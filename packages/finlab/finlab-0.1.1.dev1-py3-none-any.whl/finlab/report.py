...
class Report():
    ...
    def __repr__(self):
        ...
        if IN_COLAB:
            ...
            tb = cwidgets.TabBar(['performance', 'position', 'edge cases', 'stock chart'], location='top')
            self.tb = tb
            with tb.output_to('position'):
                self.generate_table_according_to_date(str(max(self.data['transaction'].end_date).date()))
                display(self.stock_table_panel)
                
            with tb.output_to('edge cases'):
                max_list, min_list = self.generate_edge_cases()
                display(max_list)
                display(min_list)
                
            with tb.output_to('performance'):
        ...
        self.tab.children = [self.performance_panel, widgets.Output(layout=widgets.Layout(width='100%')), self.stock_chart_panel, self.edge_cases_panel]
    ...
    def generate_figure(self, figure_type='widget'):
        ...
        fig.add_scatter(x=performance.index, y=performance.strategy-1, name='strategy', row=1, col=1, legendgroup='performnace', fill='tozeroy')
        fig.add_scatter(x=performance.index, y=performance.benchmark-1, name='benchmark', row=1, col=1, legendgroup='performance', line={'color': 'gray'})
        ...
        def update_ds_image(layout, x_range):
            ...
            data0 = performance.strategy.values / sratio - 1
            data1 = performance.benchmark.values / bratio - 1
            ...
            fig.data[0]['y'] = data0
            fig.data[1]['y'] = data1
            fig.layout.yaxis.range = [y_min-1, y_max-1]
    ...
    def generate_table_according_to_date(self, d):
        ...
        panel_exist = self.stock_table_panel is not None
        ...
        # update gui panel
        if self.stock_table_panel:
            self.stock_table_panel.children = (self.stock_table_panel.children[0], table)
            self.stock_table_panel.children[0].children[0].value = d
            self.stock_table_panel.children[0].children[1].value = daily_return
            return table
        ...
        self.stock_table_panel = widgets.VBox([widgets.HBox([date_slider, widgets.Text(daily_return)]), table])

        # jupyter configuration
        if self.tab:
            self.tab.children = (self.tab.children[0], self.stock_table_panel, self.tab.children[2], self.tab.children[3])
        return table
    ...
    def generate_stock_table(self, display_table, d=None):
        ...
        def call_chart(id):
            def temp(e):
                self.generate_stock_chart(int(id))
                if self.tab:
                    self.tab.selected_index = 2
            return temp

        for i, (tid, info) in enumerate(display_table.iterrows()):
            btns[i+1].on_click(call_chart(tid))
            btns[i+1].description = 'view'#str(tid)
        table = widgets.HBox([
            table_html, 
    ...
    def generate_stock_chart(self, case_id):
        ...
        if self.__data_query is None:
            return ''

        if self.ohlc is None:
            close = self.__data_query.get('close')
            open_ = self.__data_query.get('open')
            high = self.__data_query.get('high')
            low = self.__data_query.get('low')
            vol = self.__data_query.get('volume')
        ...
        IN_COLAB = 'google.colab' in sys.modules
        if IN_COLAB and self.tb:
            with self.tb.output_to('stock chart'):
                display(IPython.display.HTML(filename="render.html"))
        return html