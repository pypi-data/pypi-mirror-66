...
class Data():
    ...
    def talib(self, func_name, amount=0, freq="d", **args):
        ...
        close = self.get('close', amount)
        open_ = self.get('open', amount)
        high  = self.get('high', amount)
        low   = self.get('low', amount)
        volume= self.get('volume', amount)
...
class Data(Data):
    ...
    def get_item_names(self):
        res = requests.get('https://github.com/koreal6803/fdata')
        names = re.findall('title="\w+.pkl"', res.text)
...
def backtest(strategy):
    ...
    if 'message' in res_data:
        pickled = res_data['message']
        return report.Report(pickle.loads(codecs.decode(pickled.encode(), 'base64')), data=data)
