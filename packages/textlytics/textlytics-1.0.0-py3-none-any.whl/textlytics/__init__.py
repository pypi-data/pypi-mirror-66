...
class Config:
    def __init__(self, language):
        ...
        self.incidence = 1000
        self.nlpnet_pospt = 'textlytics_data/pos-pt/'
    ...
    def getPosPtDir(self):
        return self.nlpnet_pospt

    def setPosPtDir(self,dir=None):
        if (dir == None):
            try:
                nlpnet.set_data_dir(self.nlpnet_pospt)
            except OSError:
...
def download(pkg_name):
    ...
    files_dir = requests.get('https://gitlab.com/jorgeluizfigueira/python-textlytics/-/raw/master/downloader.json').json()
    if (pkg_name in files_dir):
        chunk_size = 1024

        directory = './textlytics_data/'+pkg_name+'/'
        ...
        for file_path in files_dir[pkg_name]:
            url = file_path
            desc = 'Downloading from '+file_path
            ...
            try:
                ...
                with open(directory+filename, 'wb') as f:
                    for data in tqdm(iterable = r.iter_content(chunk_size = chunk_size), total = total_size/chunk_size, unit = 'KB',desc=desc):
                        f.write(data)    
                print("Download complete!")
            except:
                filename = url.split('/')[-1]
                with open(directory+filename, 'wb') as f:
                    for data in tqdm(iterable = r.iter_content(chunk_size = chunk_size), unit = 'KB',desc=desc):
                        f.write(data)    
                print("Download complete!")
    else:
        print("Invalid Package Request")
