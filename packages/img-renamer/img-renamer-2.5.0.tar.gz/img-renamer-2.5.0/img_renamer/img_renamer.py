...
from click import command, argument, option, version_option, Choice
...
def main(folder, logfile, y, nro, zeroes, mode):
    ...
    <https://gitlab.com/miicat/img_renamer/-/wikis/>
...
def _rename_images(images_to_rename, folder, nro_total, nro_to_skip,
    ...
    if not yes:
        ...
        print("Renaming {nro} of {total}".format(nro=rename_total,
                                                 total=nro_total))
        while True:
            y = prompt("Are you sure you want to rename (y/n): ")
            if y.lower() == "n":
                print("Aborting")
                exit(0)
            elif y.lower() == "y":
        ...
        if not log:
            while True:
                y = prompt("Do you want to save a log file (y/n): ")
                if y.lower() == "y":
                    _create_log(rename)
                    break
                elif y.lower() == "n":
