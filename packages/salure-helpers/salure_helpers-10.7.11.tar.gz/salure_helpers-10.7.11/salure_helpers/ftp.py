...
class SFTP:
    def __init__(self, host, username, private_key=None, private_key_pass=None, password=None):
        """
        Init the SFTP class
    ...
    def upload_file(self, local_filepath, remote_filepath):
        ...
        if self.private_key == None:
            connection = pysftp.Connection(host=self.host, username=self.username, password=self.password, cnopts=self.cnopts)
        else:
            connection = pysftp.Connection(host=self.host, username=self.username, private_key=self.private_key, private_key_pass=self.private_key_pass, cnopts=self.cnopts)
        response = connection.put(local_filepath, remote_filepath)
        connection.close()
        return response
