...
class RandomParameterSearcher(ABC):
    ...
    def do_search_with_testset(self, param_distributions, record, X_test, y_test, budget=100, verbose=False, precomputed_vectors=None):
        ...
        with tqdm.tqdm(total=budget) as progress:
            ...
            for c in configs:
                ...
                if precomputed_vectors is None:
                    ...
                    if self.scorer == 'roc_auc_score':
                        pred = alg.algorithm.predict_proba(precomputed_vectors)
                    else:
                        pred = alg.algorithm.predict(precomputed_vectors)
