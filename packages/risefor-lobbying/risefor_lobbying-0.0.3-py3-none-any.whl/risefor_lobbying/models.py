...
class Representative(Model):
    ...
    class Meta:
        serializer_fields = ['@id', 'civility', 'firstname', 'lastname', 'get_full_name', 'function', 'title', 'protocol_role', 'protocol_person', 'political_party', 'election_department', 'election_region', 'direct_phones', 'phone_organization', 'email_direct', 'email_organization', 'address1', 'address2', 'postal_code', 'city', 'country', 'website', 'organization_type', 'organization_subtype', 'photo', 'slug', 'permanent_commission', 'additional_functions', 'parliamentary_missions', 'study_groups', 'assistants', 'actiongroups']
...
class ActionGroup(Model):
    ...
    theme = models.ForeignKey(Theme, verbose_name=u"Type d'action")
    ...
    pressreleasetitle = models.TextField(blank=True, null=True, verbose_name=u"Titre du Communiqué de Presse")
    pressreleasecontent = models.TextField(blank=True, null=True, verbose_name=u"Contenu du Communiqué de Presse")

    # elected officials
    electedofficialemailsubject = models.TextField(blank=True, null=True,
...
class Organisation(Model):
    ...
    author = models.ForeignKey(User, related_name='created_ngo', blank=True)
    ...
    lat = models.DecimalField(max_digits=15, decimal_places=12, blank=True, null=True, verbose_name=u"Latitude")
    lng = models.DecimalField(max_digits=15, decimal_places=12, blank=True, null=True, verbose_name=u"Longitude")
    organisation_type = models.ForeignKey(OrgType, verbose_name=u"Type d'organisation")
...
class Event(Model):
    ...
    theme = models.ForeignKey(Theme, verbose_name=u"Type d'événement")
    organization = models.CharField(max_length=255, blank=True, null=True, verbose_name=u"Organisation")
    actiongroup = models.ForeignKey(ActionGroup, related_name="events", blank=True, null=True)
    description = models.TextField(blank=True, null=True, verbose_name=u"Description")
...
class HelpInstall(Model):
    ...
    orgType = models.ForeignKey(HelpOrgType, verbose_name=u"Type de collectif")
    helpLevel = models.ForeignKey(HelpLevel, verbose_name=u"Level of technical help needed")
...
class ContactTeam(Model):
    ...
    contact_reason = models.ForeignKey(ContactReason, verbose_name=u"Raison du contact")
...
class TextSupporters(Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    supporter_firstname = models.CharField(max_length=100, verbose_name=u"Prénom")
    supporter_lastname = models.CharField(max_length=100, verbose_name=u"Nom")
    supporter_collectif = models.CharField(max_length=100,blank=True,null=True, verbose_name=u"Organisation")
    supported_action = models.ForeignKey(ActionGroup, related_name="supported_group", blank=True, null=True)
    #supported_email = models.ForeignKey(TextSupporters_Email, related_name="supported_email", blank=True, null=True)
    class Meta:
        serializer_fields = ['@id','supported_action','supporter_firstname','supporter_lastname','supporter_collectif']
        nested_fields=['supported_action']
        anonymous_perms = ['add','view']
        authenticated_perms = ['inherit', 'add']
        ordering=['-created_at']

    def __str__(self):
        return self.supporter_firstname + " " + self.supporter_lastname
...
class ContactEO(Model):
    ...
    # linked data
    related_action = models.ForeignKey(ActionGroup, related_name='related_text', blank=True)
