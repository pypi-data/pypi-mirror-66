...
def MapscanCommon(filename, ininame,   format, CalibrationFile=None):
    ...
    if CalibrationFile:
      data = ASCII(CalibrationFile,skip_header=3).T
      calX = Data1D(data[0,:],nmType)
      calY = Data1D(data[1,:],countType)
    else:
      calY = None
    ...
    xData = numpy.arange(center+1024*dispersion,  center-1024*dispersion,  -dispersion)
