...
class Plot1D(BasePlot):
  ...
  def plotPoints(self, x, y, label=None):
    self.bottom.errorbar(x, y, label=label, fmt="o", alpha=0.2)
...
class Plot1DSingleAxis(Plot1D):
  ...
  def addPoints(self, data: ndarray, label=None):
    self.plotPoints(self.axis.values, data, label=label)
