...
def run_job(model):
    ...
    LOGGER.warn("Lok: preprocessing following topic_ontologies %s"%str(topic_ontologies))
    topic_ontologies = spark_context.parallelize(topic_ontologies)
    l=topic_ontologies.map(lambda topic_ontology:project_documents(topic_ontology,model,True))
    l.collect()
run_job('word2vec-esa-100')
#project_documents('debatepedia')
#project_documents('strategic-intelligence')
#project_documents('wikipedia-categories')