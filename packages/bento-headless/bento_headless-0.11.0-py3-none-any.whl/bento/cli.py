...
def make_cli(prog_name: str, headless: bool = False) -> click.Command:
    @click.group(
        epilog=f"To get help for a specific command, run `{prog_name} COMMAND --help`"
    )
    @click.help_option("-h", "--help")
    @click.version_option(
        prog_name=prog_name, version=_get_version(), message="%(prog)s/%(version)s"
    )
    @click.option(
        "--base-path",
        help=f"Path to the project to run {prog_name} in.",
    ...
    @click.option("--headless", is_flag=True, default=False, hidden=True)
    @click.pass_context
    def cli(
        ctx: click.Context,
        ...
        extra = {"base_path": base_path} if base_path is not None else {}
        ctx.obj = Context(
            is_init=is_init,
            email=email,
            headless=headless,
        ...
        if not _is_running_latest() and not _is_test():
            logging.warning(f"{prog_name} client is outdated")
            click.echo(constants.UPGRADE_WARNING_OUTPUT)

    cli.add_command(check.make_check(prog_name, headless))
    cli.add_command(archive.make_archive(prog_name, headless))
