...
class DropColumns(BaseEstimator, TransformerMixin):
    """
    Simply delete columns specified from pandas DataFrame

    Parameters
    ----------
    drop_columns : list, optional default=None
    List of feature names which will be droped from fit dataframe.
    Note that it will only accept pandas DataFrame object.
    If None, no columns will be deleted
    """
...
class DropNoVariance(BaseEstimator, TransformerMixin):
    ...
    Such columns have no variance and therefore not useful
    in prediction.
...
class DropHighCardinality(BaseEstimator, TransformerMixin):
    ...
    Parameters
    ----------
    max_categories : int, optional default = 50
    Maximum number of categories to be permitted in a column.
    If number of categories in a certain column exceeds this value,
...
class DropLowAUC(BaseEstimator, TransformerMixin):
    ...
    Parameters
    ----------
    threshold : float, optional default = 0.51
    Threshold value for roc_auc. Feature with roc_auc below this value
    will be deleted.
    """
...
class DropHighCorrelation(BaseEstimator, TransformerMixin):
    ...
    Threshold value for correlation coefficient.
    Utilizes Pearson's definition.
    """
...
class ImputeNaN(BaseEstimator, TransformerMixin):
    ...
    Parameters
    ----------
    cat_strategy : string, optional default = mode
    Strategy for imputing NaN exist in categorical columns.
    If any other string apart from mode is specified, the
    NaN will be imputed by fixed string name ImputedNaN.

    num_strategy : string, optional default = mean
    Strategy for imputing NaN exist in numerical columns.
    Either mean, median or mode can be specified and if any
    other string is specified, mean imputation will be employed.
    """
...
class OneHotEncoding(BaseEstimator, TransformerMixin):
    ...
    Whether to drop first column after one hot encoding
...
class ClipData(BaseEstimator, TransformerMixin):
    ...
    Threshold value for to define upper and lower bound.
    For example, 0.99 will imply upper bound at 99% percentile
    annd lower bound at 1% percentile.
    """
...
class GroupRareCategory(BaseEstimator, TransformerMixin):
    ...
    Threshold value for defining "rare" category. For example,
    0.01 will imply 1% of the total number of data as "rare".
    """
...
class TargetMeanEncoding(BaseEstimator, TransformerMixin):
    ...
    Parameters
    ----------
    smoothing : Boolean, optional default = True
    Whether to smooth target mean with global mean using
    sigmoid function. Do not recommend smoothing=False.
    k : integer or float, optional default = 0
    hyperparameter for sigmoid function
    f : integer or float, optional default = 255
    hyperparameter for sigmoid function
    """
