...
class KPred:
    ...
    TODO:
    * Load the active feature indexes from file for each process
    * Track the training set boundaries in transformed feature space, and optionally freeze at boundaries
    * Pickle/dill the feature transformers etc. in a single file
    """
    ...
    def __init__(self, energy, process):
        ...
        self.feature_scaler = load_pickle(getpath("feature_scaler.pkl"))
        self.loxsec_scaler = load_pickle(getpath("xsec_scaler.pkl"))
        # print(self.model, self.feature_scaler, self.loxsec_scaler)
    ...
    def predict_kfac(self, slhadoc, xsec_lo, kmin=0, kmax=None):
        """\
        ...
        ## Assemble input feature list for the active process
        # TODO: embed this logic into the pickled feature selector & scaler... via lambda + dill?
        indices = None
        if self.canonical_process == "11":
            indices = [39, 10, 11, 4, 5, 13]
        elif self.canonical_process == "12":
            indices = [10, 2, 95, 11, 73, 4, 39, 99]
        elif self.canonical_process == "13":
            indices = [46, 39, 69, 10, 13, 70, 95, 102]
        elif self.canonical_process == "14":
            indices = [71, 10, 69, 72, 96, 5, 75, 73]
        elif self.canonical_process == "15":
            indices = [12, 86, 10, 42, 4, 73, 5, 2]
        elif self.canonical_process == "16":
            indices = [10, 87, 88, 85, 92, 96, 4, 95]
        elif self.canonical_process == "17":
            indices = [12, 86, 1, 10, 42, 11, 90, 5]
        elif self.canonical_process == "18":
            #indices = [87, 10, 88, 5, 4, 96, 78, 85, 7]
            indices = [87, 10, 88, 5, 4, 96, 73, 85, 99,
                       40, 70, 16, 118, 121, 90, 77, 126,
                       134, 140, 65, 45, 98, 35, 3, 105,
                       26, 37, 11, 92, 91, 46]
        elif self.canonical_process == "22":
            indices = [51, 11, 2, 44, 12, 110, 4, 13]
        elif self.canonical_process == "23":
            indices = [11, 75, 13, 39, 46, 99, 79, 4]
        elif self.canonical_process == "24":
            indices = [10, 2, 95, 11, 73, 4, 39, 99]
        elif self.canonical_process == "25":
            indices = [12, 86, 11, 4, 2, 10, 5, 95]
        elif self.canonical_process == "26":
            indices = [11, 95, 15, 12, 39, 13, 88, 46]
        elif self.canonical_process == "27":
            indices = [12, 86, 11, 4, 2, 10, 5, 95]
        elif self.canonical_process == "28":
            indices = [11, 95, 39, 99, 46, 1, 44, 88]
        elif self.canonical_process == "33":
            indices = [13, 40, 2, 79, 15, 45, 80, 5]
        elif self.canonical_process == "34":
            indices = [81, 2, 46, 14, 45, 15, 82, 41]
        elif self.canonical_process == "35":
            indices = [12, 46, 45, 0, 13, 11, 69, 5]
        elif self.canonical_process == "36":
            indices = [13, 46, 15, 2, 1, 104, 14, 118]
        elif self.canonical_process == "37":
            indices = [12, 46, 102, 0, 13, 11, 69, 75]
        elif self.canonical_process == "38":
            indices = [13, 46, 15, 2, 1, 14, 79, 120]
        elif self.canonical_process == "44":
            indices = [52, 13, 14, 15, 0, 1, 109, 77]
        elif self.canonical_process == "45":
            indices = [14, 45, 95, 4, 5, 12, 99, 78]
        elif self.canonical_process == "46":
            indices = [59, 15, 0, 14, 13, 1, 109, 46]
        elif self.canonical_process == "47":
            #indices = [12, 46, 95, 0, 77, 45, 99, 71]
            indices = [12, 46, 95, 0, 77, 45, 99, 71,
                       0, 2, 106, 113, 87, 17, 120, 63, 92,
                       22, 134, 131, 142, 143, 32, 48, 4, 11]
        elif self.canonical_process == "48":
            indices = [42, 15, 14, 52, 13, 1, 46, 2]
        elif self.canonical_process == "58":
            indices = [15, 5, 4, 12, 118, 117, 1, 119]
        elif self.canonical_process == "67": #"76":
            indices = [15, 5, 118, 12, 11, 117, 120, 10]
        else:
            # TODO: use specific exception
            raise Exception("Unsupported process {}/{}".format(self.process, self.canonical_process))

        ## Build the raw features vector, with the LO ln(xsec) as the trailing entry
        ln_xsec_lo = self.loxsec_scaler.transform(xsec_lo)
        params = SLHAFeatures(slhadoc)
        stdparams = np.array(params.stdfeatures())
        raw_features = np.concatenate((stdparams[indices], [ln_xsec_lo]))
    ...
    def predict_kfac_xsec(self, slhadoc, xsec_lo, kmin=0, kmax=None):
        "Predict a LO->NLO K-factor and the NLO cross-section, given an SLHA dataset and a LO cross-section"
        kpred = self.predict_kfac(slhadoc, xsec_lo, kmin, kmax)
        xsecpred = kpred * xsec_lo
        return kpred, xsecpred


    def predict_xsec(self, slhadoc, xsec_lo, kmin=0, kmax=None):
...
class KPreds:
    ...
    def predict_kfac(self, process, slhadoc, xsec_lo, kmin=0, kmax=None):
        "Predict a LO->NLO K-factor, given an SLHA dataset and a LO cross-section"
        return self.kpred(process).predict_kfac(slhadoc, xsec_lo, kmin, kmax)

    def predict_kfac_xsec(self, process, slhadoc, xsec_lo, kmin=0, kmax=None):
        "Predict a LO->NLO K-factor and the NLO cross-section, given an SLHA dataset and a LO cross-section"
        return self.kpred(process).predict_kfac_xsec(slhadoc, xsec_lo, kmin, kmax)

    def predict_xsec(self, process, slhadoc, xsec_lo, kmin=0, kmax=None):
        "Predict the NLO cross-section, given an SLHA dataset and a LO cross-section"
        return self.kpred(process).predict_xsec(slhadoc, xsec_lo, kmin, kmax)
