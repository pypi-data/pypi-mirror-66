...
class Document_attributesManager(CommonManagetWithLookUpFieldsManager):
    ...
    def get_or_create_attribute(attr_codes, value_str, attr_names=None, logger=None):
        ...
        settings.LOCKS.acquire('Document_attributesManager.get_or_create_attribute')
        ...
        try:
            attribute = Document_attributes.objects.row(f'''select *
                                                            from kd_document_attributes
                                                            where delete_dbl_spaces(value_str) = %s
                                                              and attr_type_id = %s
                                                              and value_int = %s  
                                                            order by id''', [delete_dbl_spaces(value_str), attr_type.id, value_int])[0]
            settings.LOCKS.release('Document_attributesManager.get_or_create_attribute')
...
class Document_attributes(AuditModel):
    ...
    def __str__(self):
        return f'ID={self.id}, attr_type=[{self.attr_type}], value_str={self.value_str.strip()}, value_int={self.value_int}'
