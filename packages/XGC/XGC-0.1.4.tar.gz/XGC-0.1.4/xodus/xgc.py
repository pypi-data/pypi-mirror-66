...
#This function will create/update a json with a specified name (Which should be the name of a script) and it will be filled with information provided in a dict format (inputs)
def update_main(
    inputs, update_script_name, update_variable_name
):
    path= PurePath(inspect.stack()[1].filename)

    with open(str((path.parent/'main').with_suffix(".json")),'w') as f:
        
        input_values[update_script_name][update_variable_name] = inputs
        json.dump(input_values, f, cls=NpEncoder)
    return
