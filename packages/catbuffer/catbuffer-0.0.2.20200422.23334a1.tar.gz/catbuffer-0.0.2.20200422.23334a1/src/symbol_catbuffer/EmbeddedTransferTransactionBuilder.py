...
class EmbeddedTransferTransactionBuilder(EmbeddedTransactionBuilder):
    ...
    def __init__(self, signerPublicKey: KeyDto, version: int, network: NetworkTypeDto, type_: EntityTypeDto, recipientAddress: UnresolvedAddressDto, mosaics: List[UnresolvedMosaicBuilder], message: bytes):
        """Constructor.
        Args:
    ...
    def loadFromBinary(cls, payload: bytes) -> EmbeddedTransferTransactionBuilder:
        ...
        return EmbeddedTransferTransactionBuilder(superObject.signerPublicKey, superObject.version, superObject.network, superObject.type_, transferTransactionBody.recipientAddress, transferTransactionBody.mosaics, transferTransactionBody.message)
