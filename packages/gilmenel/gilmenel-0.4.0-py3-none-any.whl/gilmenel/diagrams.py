...
def produce_png(instr: BaseInstrument, stars: List[Star], best_stars: List[Star]):
    ...
    transform_target.translate(instr.target.ra.deg, instr.target.dec.deg)

    # Set up an affine transformation
    transform_position_angle = Affine2D()
    transform_position_angle.rotate(instr.position_angle)  # radians
    ...
    target_radius = 5 / 60 * one_arcmin  # 5 arcseconds
    # North
    points = [
        (0, target_radius),
        (0, 0.5 * one_arcmin),
    ...
    # SCAM
    ax.add_artist(
        Circle((0, 0), 5 * one_arcmin, color='#00ff00', fill=False,)
    ).set_zorder(0.5)

    # RSS
    ax.add_artist(
        Circle((0, 0), 4 * one_arcmin, color='#00ff00', fill=False,)
    ).set_zorder(0.5)
            ...
            Line2D(
                *np.transpose(points),
                ...
                transform=Affine2D().translate(
                    +instr.slit_gap_radius.to_value(u.deg), 0,
                )
                + ax.transData,
            )
        )
