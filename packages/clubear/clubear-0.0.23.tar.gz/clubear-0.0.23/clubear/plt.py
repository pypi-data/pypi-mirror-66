...
'''
    ...
    def hist(self,x='',niter=10,xrange=[99,-99]):
        ...
        for n in range(niter):
            ...
            titlestr=titlestr+'    Time elapsed = ['+str(np.round(elapse_time,2))+'] seconds.'
            ax.set_xlabel(titlestr)
            colors = cm.rainbow(np.linspace(0, 1, len(xval)))
            sns.barplot(xval,yval)
            plt.show()    
    ...
    def eig(self,keep=[],niter=10):
        '''eig is used to place variables by their two leading principal components.'''
        ...
        for n in range(niter):
            ...
            lam,vec=np.linalg.eig(MyCorr)
            percent=np.round(np.sum(lam[:2])/np.sum(lam)*100,1)
            xval=vec[:,0];yval=vec[:,1]
            pos=np.argmax(np.abs(xval));xval=np.sign(xval[pos])*xval
            pos=np.argmax(np.abs(yval));yval=np.sign(yval[pos])*yval
            ...
            titlestr='Task accomplished = ['+str(n+1)+'/'+str(niter)+']; '
            titlestr=titlestr+'    Time elapsed = ['+str(np.round(elapse_time,2))+'] seconds;'
            titlestr=titlestr+'    Variance explained = ['+str(percent)+'%].'
            ax.set_title(titlestr)
            ax.set_xlabel('The 2nd eigenvalue')
            ax.set_ylabel('The 1st eigenvalue')
            colors = cm.rainbow(np.linspace(0, 1, len(xval)))
            ax.plot(xval, yval,' ') 
            for k in range(len(mylist)): ax.scatter(xval[k],yval[k],color=colors[k])
            for k in range(len(mylist)): ax.text(xval[k],yval[k],mylist[k])
            plt.show() 
    ...
    def box(self,y='',x='',niter=10):
        ...
        for n in range(niter):
            ...
            titlestr='Task accomplished = ['+str(n+1)+'/'+str(niter)+']; '
            titlestr=titlestr+'    Time elapsed = ['+str(np.round(elapse_time,2))+'] seconds;'
            titlestr=titlestr+'    Outlier percentage = ['+str(outlier_percent)+'%].'
            ax.set_title(titlestr)
            ax.set_xlabel(x+' <'+dtype+'>')
