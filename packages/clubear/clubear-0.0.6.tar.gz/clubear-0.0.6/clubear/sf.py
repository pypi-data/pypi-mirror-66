...
shuffle.go()        '''
    ...
    def __init__(self, original_data_path):
        ...
        if os.path.exists(self.original_data_path) == False:
            print("shuffle: [%s] does not exist! Please check the path." % self.original_data_path)
        elif os.path.isfile(self.original_data_path) == False:
            print("shuffle: [%s] is not a file! Please check the path." % self.original_data_path)
        else:
            # 文件是否为空
            self.is_empty = self.is_file_empy()

            # shuffle后的文件存放路径，默认为原始路径+.csv.shuffle
            # 若为windows路径，须将\替换为/
    ...
    def go(self, **kwargs):
        ...
        if self.is_data_exits:
            if self.is_empty:
                print("shuffle.go: [%s] is empty." % self.original_data_path)
            else:
            ...
            print("shuffle.go: Please check the path.")
        ...
        if new_path == self.original_data_path:
            ...
            if self.is_data_exits:
                ...
                for i in range(0, cache_file_num):
                    ...
                    _output.write('\rShuffle stage (2/3) accomplished: %.2f %%   ' % (status))
                ...
                # 先对所有cache，产生一个随机排序
                concat_order = list(pd.Series(range(cache_file_num)).sample(cache_file_num, replace = False))
                ...
                with open(self.original_data_path, 'r', encoding='ISO-8859-1', errors='ignore') as of:
                    ...
                    with open(new_path, 'a', encoding='ISO-8859-1') as f:
                        ...
                        for i in range(0, len(concat_order)):
                            ...
                            status = 100 * (i+1) / float(cache_file_num)
                            _output.write('\rShuffle stage (3/3) accomplished: %.2f %%   ' % (status))
        ...
        with open(self.original_data_path, 'r', encoding='ISO-8859-1', errors='ignore') as f:
            ...
            while line: # cnt < 100:
                ...
                if cnt % flush == 0:
                    ...
                    _output.write('\rShuffle stage (1/3) accomplished: %.2f %%' % status)
