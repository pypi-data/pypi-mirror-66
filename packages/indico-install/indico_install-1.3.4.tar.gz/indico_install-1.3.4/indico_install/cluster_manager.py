...
def _list_versions():
    ...
    return [i[len(UPDRAFT_BUCKET):].replace('/', '') for i in sorted(out, reverse=True)]


def list_backups():
    out = run_cmd(
        f"kubectl get cm --no-headers -o custom-columns=Name:.metadata.name 2>&1 | grep {TRACKER_CONFIGMAP_BACKUP}"
    )
...
class ClusterManager(object):
    ...
    def clean_services(self):
        ...
        if "hash" in self.cluster_config.get("frontend", {}):
