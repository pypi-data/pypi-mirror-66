...
def get_json_unwrapped_callback(callback: Callable):
    ...
    def json_unwrapper_around_callback(client: Client, userdata: KaijuMqtt, packet: MqttPacket):
        """
        ...
        callback(client, userdata, json_decoded_payload)
...
def get_custom_request_responder(logger: logging.Logger, clear_timeout: Callable, return_me: ReturnMe, handled_event: Event):
    ...
    def handle_response_to_this_request(client: Client, user_data: KaijuMqtt, packet: MqttPacket):
        ...
        if not isinstance(response, dict) or "status" in response:
            ...
            logger.debug(f"Handling a typical response: {response}")
...
class KaijuMqtt:
    ...
    def __init__(self, json_stringify: Callable = json.dumps):
    ...
    def subscribe(self, topic: str, callback: Callable, options: Optional[Dict] = None):
        """
