...
from ._typing import Any, Dict, List, Optional, Tuple, Type
from .utils import (
    get_comet_version,
    get_time_monotonic,
    local_timestamp,
...
def check_api_backend_version(backend_root_url, minimal_backend_version):
    ...
    try:
        response = requests.get(version_url, timeout=DEFAULT_TIMEOUT)
        response.raise_for_status()
...
class LowLevelHTTPClient(object):
    ...
    def __init__(
        ...
        # type: (str, Optional[Dict[str, str]], bool, int) -> None
    ...
    def post(
        ...
        # type: (str, Any, Optional[Dict[str, str]], Optional[bool], Optional[float], Optional[Any], Optional[Any], bool, Optional[Type[json.JSONEncoder]]) -> requests.Response
        if retry is None:
            retry = self.default_retry
...
class RestServerConnection(object):
    ...
    def __init__(
        self, api_key, experiment_id, server_address, default_timeout=DEFAULT_TIMEOUT
    ):
    ...
    def report(
        ...
        try:
            if event_name is not None:
                ...
                with session:
                    json_post(
                        endpoint_url, session, headers, payload, DEFAULT_TIMEOUT / 2
                    )
...
class BaseApiClient(object):
    ...
    def post(self, url, payload, headers, files=None, params=None):
        # type: (str, Any, Optional[Dict[str, str]], Optional[Any], Optional[Any]) -> requests.Response
        response = self.low_level_api_client.post(
