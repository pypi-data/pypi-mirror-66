...
class Utils(object):
	...
	def assert_docker_alive(cls):
		try:
			...
			print('No running docker client found, please ensure docker client is running.')
			print(str(e))
			exit(-1)
	...
	def docker_login(cls, scan_info):
		...
		if stdout is not None and stdout.decode('utf-8').find('Login Succeeded') >= 0:
			...
			raise DockerError('Login to docker failed: ' + stdout.decode('utf-8')) from None
	...
	def docker_pull_image(cls, docker_client, image):
		...
		try:
			...
			try:
				...
				raise DockerError('Failed to pull image: ' + str(e)) from e
	...
	def upload_output(cls, aws_config, app_id, output_file):
		...
		try:
			...
			s3.Bucket('sken-scanner-output').put_object(Key=app_id + '/' + timestamp + '/' + file_name, Body=data)
			
			print('Output file uploaded')
		except Exception as e:
			raise S3Error('Failed to upload output file: ' + str(e)) from None
