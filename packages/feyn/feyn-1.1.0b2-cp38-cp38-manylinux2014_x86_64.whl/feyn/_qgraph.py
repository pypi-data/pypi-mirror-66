...
class QGraph:
    ...
    def _update_display(self, current_ix, best_ix, show):
        status = "Examined %i of %i: Best %i with loss: %.6f" % (current_ix, len(self._graphs), best_ix, self._graphs[best_ix].loss_epoch)

        if show == "graph":
            import IPython
            dot = DotRenderer.rendergraph(self._graphs[best_ix])
            dot.attr(label="\n"+status)
    ...
    def fit(self, data, epochs:int=10, loss_function=_feyn.DEFAULT_LOSS, show:Optional[FittingShowType]="graph", threads:int=1):
        """
    ...
    def select(self, data, filter_func=None, loss_function=_feyn.DEFAULT_LOSS, sort=None, max_depth=-1, max_edges=-1, n=5):
        """
        ...
        Arguments:
            ...
            filter_func -- A callback function to filter the graph collection. It will receive a graph as argument and should return True or False to include the graph or not.
            ...
            sort -- A custom function to sort by. It receives the graph and the loss as arguments. When not specified, the collection will be sorted by loss.
        ...
        Returns:
            [List[Graph]] -- The selected list of graphs.
        """
        # Magic support for pandas DataFrame
        if type(data).__name__ == "DataFrame":
            data = {col: data[col].values for col in data.columns}
        ...
        if filter_func is not None:
            graph_collection = list(filter(filter_func, graph_collection))
        ...
        for graph in graph_collection:
            out_reg = graph[-1]
            l = loss_func(data[out_reg.label], graph.predict(data))
            data_losses[graph] = l
    ...
    def __repr__(self):
        ...
        out_label = self.out_reg_dict["label"]
