...
def addRandomEdges(G : nx.Graph, 
    ...
    """
    Add random edges to a given SEN. 
...
def randomSpecialSENs(ecoNodes : int,
    ...
    :param socName: name for the social nodes, i.e. 'Town' for getting names
        like 'Town 0', 'Town 1' etc.
    """
    assert edgesCounts[sma.properties.EDGE_TYPE_SOC_SOC] <= socNodes * (socNodes - 1), 'too many social-social edges requested, SEN size insufficient'
    assert edgesCounts[sma.properties.EDGE_TYPE_ECO_SOC] <= (socNodes + ecoNodes) * (socNodes + ecoNodes - 1), 'too many ecological-social edges requested, SEN size insufficient'
    assert edgesCounts[sma.properties.EDGE_TYPE_ECO_ECO] <= ecoNodes * (ecoNodes - 1), 'too many ecological-ecological edges requested, SEN size insufficient'
    while True:
        G, ecos, socs = _plainSEN(ecoNodes, socNodes, ecoName, socName)
...
def randomSimilarSENs(G : nx.Graph,
    ...
    :param socName: name for the social nodes, i.e. 'Town' for getting names
        like 'Town 0', 'Town 1' etc.
    """
    ecoNodes = len(list(sma.analyse.sesSubgraph(G, sma.properties.NODE_TYPE_ECO)))
    socNodes = len(G.nodes) - ecoNodes
    edgesCounts = sma.analyse.edgesCount(G)
    return randomSpecialSENs(ecoNodes, socNodes, edgesCounts, ecoName, socName)
...
def randomMultiSENs(nVertices : list, 
...
def randomSimilarMultiSENs(G : nx.Graph, names : list = sma.MULTI_DEFAULT_NAMES):
    """
    Generates random multi-level SENs with the same numbers of nodes and egdes
    per domain as the given SEN.
    
    See :py:meth:`sma.randomMultiSENs`.
    ...
    return randomMultiSENs(nVertices, nEdges, names)