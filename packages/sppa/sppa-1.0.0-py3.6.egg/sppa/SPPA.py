...
class SPPA:
    ...
    def solve(self, verbose=1, milp_msg=False):
        ...
        while not self.termination_criteria_satisfied:
            ...
            if self.iteration_number == 0 and not self.decomposition_initialized:
                self.initialize_decomposition()
            ...
            if prob_status == -1:
                if self.iteration_number == 1:
                    raise Exception('The MILP solution is infeasible. The MINLP problem can be feasible even though '
                                    'the decomposed MILP problem isn\'t. Consider setting a larger initial_n_pieces')
                else:
                    raise Exception('The MILP solution is infeasible. The solver may have run for too many iterations '
    ...
    def set_termination_criteria(self, ftol=1E-6, xtol=1E-6, computation_time=None, max_iterations=100,
                                 infeasible_allowed=False, con_tol=None):
        check_termination_criteria(ftol, xtol, computation_time, max_iterations, infeasible_allowed)
        self.check_compiled()
        self.ftol = ftol
        self.xtol = xtol
        ...
        for variable in self.variables_to_decompose.values():
            if not variable.expand and variable.var_type != BIN_VAR:
                ...
                if index_a is not None:
                    vertices = next_vertices(variable_solution, variable.vertices, index_a, index_b,
                                             variable.var_type, variable.bounds, self.n_vertices)
            ...
            if variable.expand:
                ...
                if variable.var_type == INT_VAR:
                    if variable.bounds[1] - variable.bounds[0] + 1 <= self.initial_n_vertices:
                        ...
                        vertices = squash_degenerate_vertices(vertices, variable.bounds)  # not really needed with linspace
                        variable.set_vertices(vertices)
    ...
    def get_all_variables(self, variable_order):
        all_variables = list()
