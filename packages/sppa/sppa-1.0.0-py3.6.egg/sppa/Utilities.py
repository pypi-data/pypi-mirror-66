...
def next_vertices(solution, original_vertices, index_a, index_b, var_type, bounds, n_vertices, expand_multiple=1.5,
                  contract_frac=0.7):
    #width = (original_vertices[-1] - original_vertices[0]) * contract_frac
    width = np.mean(original_vertices[1:] - original_vertices[:-1]) * (n_vertices-1) * contract_frac
...
def retrieve_variable_value(variables_values, var_info):
    ...
    if var_info.var_type == INT_VAR:
        if isinstance(x, numbers.Number) and not isinstance(x, numbers.Integral):
            #warnings.warn('variable_value is not an integer but var_type is. casting as an integer. ', SPPAWarning)
            #if abs(np.floor(x) - x) > TOL and abs(np.ceil(x) - x) > TOL:
            #    raise ValueError('variable value is not an integer for integer var_type')
            x = int(np.round(x))
...
def check_termination_criteria(ftol, xtol, computation_time, max_iterations, infeasible_allowed):
...
def check_fun(fun):
    if not isinstance(fun, Callable):
        raise TypeError('fun_nlin is not a function')
