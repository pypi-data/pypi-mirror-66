...
class Expr:
    ...
    def compute_expression(self, variables_values):
        f, self.violation, self.violated = self.evaluate_expression(variables_values, return_violation=True)
...
class Var(Expr):
    def __init__(self, var_name, low_bound=None, up_bound=None, var_type='cont', expand=False):
        ...
        self.expand = expand
        self.var_info = VarInfo(coeff, bounds, var_type, var_name, expand)
        self.lin_vars[var_name] = self.var_info


class VarInfo:
    def __init__(self, coeff, bounds, var_type, name, expand):
        self.coeff = coeff
        self.bounds = bounds
        self.var_type = var_type
        self.name = name
    ...
    def set_vertices(self, vertices):
        ...
        self.decomposed_vars = [VarInfo(1.0, [vertices[i], vertices[i+1]], self.var_type,
                                        self.name + '_var_' + str(i), self.expand)
                                for i in range(self.n_pieces)]
        self.decomposed_bins = [VarInfo(1.0, [None, None], BIN_VAR, self.name + '_bin_' + str(i), False)
                                for i in range(self.n_pieces)]
            ...
            for simplex_index_id, simplex_id in enumerate(self.simplex_ids):
                ...
                for dimension_step in simplex_id:
                    ...
                    if self.sharing_nlinexpr_decomposition:
                        ...
                        var_info = VarInfo(coeff, bounds, var_type, var_name, expand)
                ...
                if self.sharing_nlinexpr_decomposition:
                    ...
                    bin_info = VarInfo(f_vertex_cube - cum_const, [None, None], BIN_VAR, bin_name, False)
