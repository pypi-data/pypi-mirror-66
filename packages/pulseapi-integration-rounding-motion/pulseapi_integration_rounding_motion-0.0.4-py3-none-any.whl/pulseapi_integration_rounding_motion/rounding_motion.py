...
def rounding_motion(target_list):
    ...
    for i in range(len(target_list)):
        ...
        if len(target_list[i]) == 2 or target_list[i][1][0] != 0:
            startAngle, sweepAngle, circlePoint, radius = calculate_start_and_end_angle(
                ...
                convert_position2list(target_list[i + 1][0])[0], target_list[i][1][0])

            dx1 = convert_position2list(target_list[i][0])[0][0] - convert_position2list(target_list[i - 1][0])[0][0]
            dy1 = convert_position2list(target_list[i][0])[0][1] - convert_position2list(target_list[i - 1][0])[0][1]
            length = get_length(dx1, dy1)
            if radius + radius_prev > length:
                print(f'Радиус заменен на макимально возможный i в точке')
