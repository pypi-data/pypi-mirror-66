...
def production_tab(beam, acc):
    ...
    line_button = RadioButtonGroup(
        labels=['rms x', 'rms y', 'rms x Emitt', 'rms y Emitt', 'avg Energy',
                'avg Velocity', 'x β-func', 'y β-func'], active=0)
    ...
    def refresh_handler(beam=beam, acc=acc):
        ...
        if tabs.active == 1:
            df['p2'] = df['px']*df['px'] + df['py']*df['py'] + df['pz']*df['pz']
            df['xp'] = df['px']/df['pz']
            df['yp'] = df['py']/df['pz']
            df['xp2'] = df['xp']*df['xp']
            df['yp2'] = df['yp']*df['yp']
            ...
            df['avg Energy'] = df['p2'].mean()**0.5 - beam.type.mass*rp.c**2/rp.e/1e6
            df['avg Velocity'] = df['pz'].mean()/(df['avg Energy'] + beam.type.mass*rp.c**2/rp.e/1e6)
            ...
            if line_button.active == 5:
                line_plot.xaxis.axis_label = 'z [m]'
                line_plot.yaxis.axis_label = 'avg Velocity'
                line_source.data = {'x': df['z'], 'y': df['avg Velocity']}
        ...
        if tabs.active == 2:
            df_interp = pandas_interpolate(df, 'Ez', 'cubic')
            df['avg Ez'] = df['Ez'].mean()
            df['avg Ex'] = df['Ex'].mean()
            df['avg Ey'] = df['Ey'].mean()
            df['avg Bz'] = df['Bz'].mean()
            df['avg Bx'] = df['Bx'].mean()
            df['avg By'] = df['By'].mean()
            if field_button.active == 0:
                field_plot.xaxis.axis_label = 'z [m]'
                field_plot.yaxis.axis_label = 'Ez [MV/m]'
                field_source.data = {'x': df_interp['z'], 'y': df_interp['Ez']}
