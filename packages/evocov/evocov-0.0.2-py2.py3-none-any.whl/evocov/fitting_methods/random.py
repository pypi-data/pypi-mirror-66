...
class Random(FittingMethod):
    ...
    def fit(self, model, folds, budget=None, verbose=False):
        ...
        while log['fun_calls'] < max_fun_call:
            ...
            # run optimization
            value = np.inf
            model.covariance_function = individual.covariance_function
            model.hyperparameters[1] = individual.covariance_function
            try:
            ...
            if value < log['best']['value']:
                log['improvements'] += 1
                log['best']['kernel'] = str(individual)
        ...
        model.covariance_function = best_kernel
        model.hyperparameters[1] = best_kernel
