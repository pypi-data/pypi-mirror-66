...
# 删除无用文件
for bit in [32,64]:
    path = f'./ctp_risk/lib{bit}'
    for f in os.listdir(path):
        if os.path.isdir(f):
            continue
        if os.path.splitext(f)[1] not in ['.dll', '.so']:
            os.remove(f'./ctp_risk/lib{bit}/{f}')

long_description = read_file('setup.md')
long_description_content_type = "text/markdown",  # 指定包文档格式为markdown

# 生成requirements.txt 
# pipreqs --encoding=utf8 --force ctp_risk && \cp ctp_risk/requirements.txt .
# //.tar.gz //.whl
# rm -rf dist && python setup.py sdist && python setup.py bdist_wheel && twine upload dist/*
setup(
    name='ctp_risk',  # 包名
    python_requires='>=3.6.0',  # python环境
    version='0.0.3',  # 包的版本
    description="Python CTP Risk api",  # 包简介，显示在PyPI上
    ...
    url='https://gitee.com/haifengat/ctp_risk_api.git',
