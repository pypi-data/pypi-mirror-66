...
class experiment:
    ...
    def run(self, processes=1, chunksize=1, tqdm=None):
        ...
        def process():
            """Helper function"""
            if processes == 1:
                return map(runner, self._generate_arglists())
            else:
                return ProcessPool(nodes=processes).imap(
                    runner, self._generate_arglists(), chunksize=chunksize
                )

        return list(process() if tqdm is None else tqdm.tqdm(process(), total=num_jobs))
