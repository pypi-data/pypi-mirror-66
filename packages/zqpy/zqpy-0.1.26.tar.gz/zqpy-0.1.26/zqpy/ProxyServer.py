    ...
    def get_all_proxy_by_type(self, proxySourceType, isForce=False):
        ...
        for url in config['urlList']:
            ...
            if result.status_code != 200:
                continue
            print('下面一行移动到heepService')
            self.file.WriteFile('./result.html', result.text)
            ...
            for pageIndex in range(1, allPageNum):
                ...
                if pageIndex+1 == allPageNum:
                    percent = 100.0
                    print('%s \n代理收集完成 : %s [%d/%d]'%(url, str(percent)+'%',pageIndex+1,allPageNum), end='\n')
                else:
                    percent = round(1.0 * pageIndex / allPageNum * 100,2)
                    print('%s \n代理正在收集 : %s [%d/%d]'%(url, str(percent)+'%',pageIndex+1,allPageNum), end='\r')
            '''
