...
class APSchedulerServiceClass():
    ...
    def AddJob_FixedTime(self, job_function, run_date, args=None, kwargs=None, timezone=None, jitter=0, isBackground=True, jobId=None):
        '''
        添加调度器【固定时间任务】
        ...
        if not jobId or not sched:
            return False
        job = sched.add_job(job_function, 'date', args=args, kwargs=kwargs, run_date=run_date, timezone=timezone, jitter=jitter, id=jobId)
        return self.OnAddSchedInfo(jobId, job)

    def AddJob_Interval(self, job_function, args=None, kwargs=None, weeks=0, days=0, hours=0, minutes=0, seconds=0, start_date=None, end_date=None, timezone=None, jitter=0, isBackground=True, jobId=None):
        '''
        添加调度器【周期触发任务】
        ...
        if not jobId or not sched:
            return False
        job = sched.add_job(job_function, 'interval', args=args, kwargs=kwargs, weeks=weeks, days=days, hours=hours, minutes=minutes, seconds=seconds, start_date=start_date, end_date=end_date, timezone=timezone, jitter=jitter, id=jobId)
        return self.OnAddSchedInfo(jobId, job)

    def AddJob_Cron(self, job_function, args=None, kwargs=None, year=0, month=0, day=0, week=0, day_of_week=0, hour=0, minute=0, second=0, start_date=None, end_date=None, timezone=None, jitter=0, isBackground=True, jobId=None):
        '''
        添加调度器【特定周期触发任务】
        ...
        */a         所有        可被a整除的通配符。
        ...
        if not jobId or not sched:
            return False
        job = sched.add_job(job_function, 'cron', args=args, kwargs=kwargs, year=year, month=month, day=day, week=week, day_of_week=day_of_week, hour=hour, minute=minute, second=second, start_date=start_date, end_date=end_date, timezone=timezone, jitter=jitter, id=jobId)
        return self.OnAddSchedInfo(jobId, job)
