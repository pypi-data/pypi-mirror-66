...
class NLU(object):
    def __init__(self, env="", timeout=3, verbose=True):
        ...
        self.stopwords = set([word.strip() for word in open(self.base_dir + "/config/stop_words.txt", encoding="utf8")])
        if verbose:
    ...
    def getSubSentences(self, sentence, bullet=True, cut_comma=False, cut_all=False):
        """规则切句"""
        server_name = 'sentence_spliter'
        try:
            # data = {'sentence':sentence, 'mode': 0}
            # sub_sents = doTask(self.env, server_name, data, self.timeout)
            sub_sents = split_text(sentence, bullet, cut_comma, cut_all)
            return sub_sents
        except Exception as e:
            self.raise_exception(server_name, e)

    def split(self, sentence, bullet=True, cut_comma=False, cut_all=False):
        return self.getSubSentences(sentence, bullet, cut_comma, cut_all)
    ...
    # def predictDialogEmotion(self, sentences):
    #     server_name = "dialog_sentiment"
    #     try:
    #         if sentences:
    ...
    def emotion(self, sentences, prob=False, mode="default"):
        if mode == "default":
            return self.predictEmotion(sentences, prob)
        # elif mode == "dialog":
        #     return self.predictDialogEmotion(sentences)
