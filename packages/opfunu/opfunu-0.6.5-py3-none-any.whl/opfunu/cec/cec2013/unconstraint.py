...
class Model(BasicFunction):
    ...
    def F1(self, solution=None, name="Sphere Function", f_bias=-1400):
        return sum((solution - self.shift[3:4, :self.problem_size]) ** 2) + f_bias

    def F2(self, solution=None, name="Rotated High Conditioned Elliptic Function", f_bias=-1300):
        t1 = solution - self.shift[4:5, :self.problem_size]
        t2 = dot(self.matrix[:self.problem_size, :], t1.T)
        t3 = self.osz_func__(t2)
        t4 = self.elliptic__(t3)
        return t4 + f_bias

    def F3(self, solution=None, name="Rotated Bent Cigar Function", f_bias=-1200):
        t1 = solution - self.shift[2:3, :self.problem_size]
        t2 = dot(self.matrix[:self.problem_size, :], t1.T)
    ...
    def F4(self, solution=None, name="Rotated Discus Function", f_bias=-1100):
        t1 = solution - self.shift[8:9, :self.problem_size]
        t2 = dot(self.matrix[:self.problem_size, :], t1.T)
        t3 = self.osz_func__(t2)
        return self.discus__(t3) + f_bias

    def F5(self, solution=None, name="Different Powers Function", f_bias=-1000):
        t1 = solution - self.shift[1:2, :self.problem_size]
        return self.different_powers__(t1) + f_bias

    def F6(self, solution=None, name="Rotated Rosenbrock’s Function", f_bias=-900):
        t1 = 2.048 * (solution - self.shift[:1, :self.problem_size]) / 100
        t2 = dot(self.matrix[:self.problem_size, :], t1.T) + 1
        return self.rosenbrock__(t2) + f_bias

    def F7(self, solution=None, name="Rotated Schaffers F7 Function", f_bias=-800):
        t1 = solution - self.shift[2:3, :self.problem_size]
        t2 = dot(self.matrix[:self.problem_size, :], t1.T)
    ...
    def F8(self, solution=None, name="Rotated Ackley’s Function", f_bias=-700):
    ...
    def F9(self, solution=None, name="Rotated Weierstrass Function", f_bias=-600):
        t1 = 0.5 * (solution - self.shift[:1, :self.problem_size]) / 100
        t2 = dot(self.matrix[:self.problem_size, :], t1.T)
        t3 = self.asy_func__(t2, 0.5)
        t4 = self.create_diagonal_matrix__(self.problem_size, alpha=10)
        t5 = matmul(t4, self.matrix[self.problem_size: 2 * self.problem_size, :])
    ...
    def F10(self, solution=None, name="Rotated Griewank’s Function", f_bias=-500):
        t1 = 600 * (solution - self.shift[:1, :self.problem_size]) / 100
    ...
    def F11(self, solution=None, name="Rastrigin’s Function", f_bias=-400):
        t1 = 5.12 * (solution - self.shift[:1, :self.problem_size]) / 100
    ...
    def F12(self, solution=None, name="Rotated Rastrigin’s Function", f_bias=-300):
        t1 = 5.12 * (solution - self.shift[:1, :self.problem_size]) / 100
        t2 = dot(self.matrix[:self.problem_size, :], t1.T)
        t3 = self.osz_func__(t2)
    ...
    def F13(self, solution=None, name="Non-continuous Rotated Rastrigin’s Function", f_bias=-200):
        t1 = 5.12 * (solution - self.shift[:1, :self.problem_size]) / 100
        t2 = dot(self.matrix[:self.problem_size, :], t1.T)
    ...
    def F14(self, solution=None, name="Schwefel’s Function", f_bias=-100):
        t1 = 1000 * (solution - self.shift[:1, :self.problem_size]) / 100
    ...
    def F15(self, solution=None, name="Rotated Schwefel’s Function", f_bias=100):
        t1 = 1000 * (solution - self.shift[:1, :self.problem_size]) / 100
        t2 = self.create_diagonal_matrix__(self.problem_size, alpha=10)
        t3 = matmul(t2, self.matrix[:self.problem_size, :])
    ...
    def F16(self, solution=None, name="Rotated Katsuura Function", f_bias=200):
        t1 = 5 * (solution - self.shift[:1, :self.problem_size]) / 100
        t2 = dot(self.matrix[:self.problem_size, :], t1.T)
    ...
    def F17(self, solution=None, name="Lunacek bi-Rastrigin Function", f_bias=300):
        d = 1
        s = 1 - 1.0 / (2 * sqrt(self.problem_size + 20) - 8.2)
        miu0 = 2.5
        miu1 = -sqrt((miu0 ** 2 - d) / s)
        t1 = 10 * (solution - self.shift[:1, :self.problem_size]) / 100
    ...
    def F18(self, solution=None, name="Rotated Lunacek bi-Rastrigin Function", f_bias=400):
        d = 1
        s = 1 - 1.0 / (2 * sqrt(self.problem_size + 20) - 8.2)
        miu0 = 2.5
        miu1 = -sqrt((miu0 ** 2 - d) / s)
    ...
    def F19(self, solution=None, name="Rotated Expanded Griewank’s plus Rosenbrock’s Function", f_bias=500):
        t1 = 5 * (solution - self.shift[4:5, :self.problem_size]) / 100
        t2 = dot(self.matrix[:self.problem_size, :], t1.T) + 1
        return self.expanded_griewank__(t2) + f_bias

    def F20(self, solution=None, name="Rotated Expanded Scaffer’s F6 Function", f_bias=600):
        t1 = dot(self.matrix[:self.problem_size, :], (solution - self.shift[:1, :self.problem_size]).T)
