...
def load_data(
        file_path_with_daily_transaction_data,
        strategy_hyperparameters,
        file_path_with_token_data=None,
        name_of_foreign_key_in_transaction_table=None,
        name_of_foreign_key_in_token_metadata_table=None
    ):
    """Loads transactional data and metadata.

    This function loads the transactional data (daily prices) and the token
    metadata (token data). It also uses the date as an index.
    """

    # Handles different csv formats and handles program calls from root and from /backtesting
    try:
        df_prices = read_csv(
            file_path_with_daily_transaction_data,
            sep=',',
    ...
    if file_path_with_token_data is not None:
        ...
        df_prices = merge_data(
            ...
            name_of_foreign_key_in_transaction_table=name_of_foreign_key_in_transaction_table,
...
def merge_data(
        df_prices,
        df_token_metadata,
        name_of_foreign_key_in_transaction_table,
        name_of_foreign_key_in_token_metadata_table
    ):
    """Enriches transactional data with metadata about the tokens."""

    df_prices_enriched_with_metadata = merge(
        df_prices,
        df_token_metadata,
        left_on=name_of_foreign_key_in_transaction_table,
...
def prepare_data(
    ...
    df_prices = load_data(
        file_path_with_daily_transaction_data=file_path_with_daily_transaction_data,
...
def save_dataframe_to_csv(
        df_prices,
        string_name,
        string_directory='assets/results',
    ):
    ...
    try:
        ...
        string_directory = 'backtesting/' + string_directory
        result_no = len(
            [name for name in listdir(string_directory) if path.isfile(
                path.join(
                    string_directory,
