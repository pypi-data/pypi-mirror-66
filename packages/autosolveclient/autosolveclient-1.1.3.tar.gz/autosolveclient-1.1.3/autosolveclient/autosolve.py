...
class AutoSolve:
    ...
    autosolve_constants = AutoSolveConstants()
    ...
    def __init__(self, params):
        ...
        self.account_id = ""
        self.vhost = ""
        self.routing_key_receiver = self.build_prefix_with_credentials(self.autosolve_constants.RECEIVER)
        self.routing_key_cancel = self.build_prefix_with_credentials(self.autosolve_constants.CANCEL)
    ...
    def send_token_request(self, message):
        ...
        try:
            send_result = self.send(message, self.autosolve_constants.SENDER, self.autosolve_constants.DIRECT_EXCHANGE)
            ...
            self.resend(message, self.autosolve_constants.SENDER, self.autosolve_constants.DIRECT_EXCHANGE)
    ...
    def cancel_token_request(self, taskId):
        ...
        try:
            send_result = self.send(message, self.autosolve_constants.CANCEL, self.autosolve_constants.FANOUT_EXCHANGE)
            if send_result is None:
                self.log("Message with TaskId: " + message['taskId'] + " sent successfully", self.autosolve_constants.SUCCESS)
        except Exception as e:
            self.log(e, self.autosolve_constants.WARNING)
            self.resend(message, self.autosolve_constants.CANCEL, self.autosolve_constants.FANOUT_EXCHANGE)
