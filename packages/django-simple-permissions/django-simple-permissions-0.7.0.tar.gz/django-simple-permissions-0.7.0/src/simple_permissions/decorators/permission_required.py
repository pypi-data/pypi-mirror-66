...
def permission_required(perm=None, queryset_or_model=None,
    ...
    if queryset_or_model and isinstance(queryset_or_model, type) and issubclass(queryset_or_model, Model):
        queryset_or_model = queryset_or_model._default_manager.all()

    def wrapper(class_or_method):
        if inspect.isclass(class_or_method):
            from simple_permissions.decorators.classbase import permission_required as decorator
        else:
            # method_permission_required can handle method or function
            # correctly.
            from simple_permissions.decorators.methodbase import permission_required as decorator
