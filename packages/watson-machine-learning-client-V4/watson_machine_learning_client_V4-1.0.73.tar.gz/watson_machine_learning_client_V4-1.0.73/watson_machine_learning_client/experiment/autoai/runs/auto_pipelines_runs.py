...
class AutoPipelinesRuns(BaseAutoPipelinesRuns):
    ...
    def list(self) -> 'DataFrame':
        ...
        if self._wml_engine._wml_client.ICP_30:
            runs_pipeline_ids = [run['entity']['pipeline']['id'] for run in data]

        else:
            runs_pipeline_ids = [run['entity']['pipeline']['href'].split('/')[-1] for run in data]

        runs_timestamps = [run['metadata'].get('modified_at') for run in data]
