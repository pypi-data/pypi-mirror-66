...
from watson_machine_learning_client.utils.wml_client_error import ApiRequestFailure
from .errors import MissingPipeline, AutoAIComputeError, DataFormatNotSupported
        ...
        for pipeline in run_params['entity']['status'].get('metrics', []):
            ...
            if pipeline['context']['phase'] == 'global_output':
                ...
                if pipeline_name is None:
                    ...
                    pipelines_names.append(f"Pipeline_{pipeline['context']['intermediate_model']['name'][-1]}")

                elif pipeline_name == f"Pipeline_{pipeline['context']['intermediate_model']['name'][-1]}":
                    model_paths.append(model_path)
                    pipelines_names = [f"Pipeline_{pipeline['context']['intermediate_model']['name'][-1]}"]
                    break
                # --- end note
        ...
        for pipeline in run_params['entity']['status'].get('metrics', []):
            ...
            if pipeline['context']['phase'] == 'global_output':
                ...
                if pipeline_name is None:
                    ...
                    filenames.append(f"{path}/Pipeline_{pipeline['context']['intermediate_model']['name'][-1]}.pickle")
                    keys.append(model_path)
                    pipelines_names.append(f"Pipeline_{pipeline['context']['intermediate_model']['name'][-1]}")

                elif pipeline_name == f"Pipeline_{pipeline['context']['intermediate_model']['name'][-1]}":
                    buckets = [run_params['entity']['results_reference']['location']['bucket']]
                    filenames = [f"{path}/Pipeline_{pipeline['context']['intermediate_model']['name'][-1]}.pickle"]
                    keys = [model_path]
                    pipelines_names = [f"Pipeline_{pipeline['context']['intermediate_model']['name'][-1]}"]
                    break
...
def try_import_lale():
    ...
    try:
        ...
        check_call([executable, "-m", "pip", "install", "lale"])
