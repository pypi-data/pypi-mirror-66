...
class SketchAxis(Cartesian2DAxis):
    ...
    def min(self, value):
        ...
        if session.connected():
            fmt = '$!SketchAxis {}Detail {{ RangeMin = {} RangeMax = {} }}'
            cmd = fmt.format(self.name, value, self.max)
            macro.execute_command(cmd)
            Cartesian2DAxis.max.fset(self, self.max)
        Cartesian2DAxis.min.fset(self, value)
    ...
    def max(self, value):
        ...
        if session.connected():
            fmt = '$!SketchAxis {}Detail {{ RangeMin = {} RangeMax = {} }}'
            cmd = fmt.format(self.name, self.min, value)
            macro.execute_command(cmd)
            Cartesian2DAxis.min.fset(self, self.min)
        Cartesian2DAxis.max.fset(self, value)
