...
class DCMotor(_BaseModule):
    ...
    def run(self,speed):
        ...
        if self._pack.port<9:
            self._pack.data = [self._pack.port + (self._pack.slot - 1) * 8]
        else:
...
class SmartServo(_BaseModule):
    ...
    def set_led(self,idx,red,green,blue):
        self._pack.data = [2,self._pack.port,idx,red,green,blue]
        self.call(self._pack)

    def move_to(self,idx,position,speed):
        self._pack.data = [4,self._pack.port,idx]
        self._pack.data.extend(long2bytes(position))
        self._pack.data.extend(float2bytes(speed))
        self.call(self._pack)

    def move(self,idx,position,speed):
        self._pack.data = [5,self._pack.port,idx]
        self._pack.data.extend(long2bytes(position))
        self._pack.data.extend(float2bytes(speed))
        self.call(self._pack)

    def set_zero(self,idx):
        self._pack.data = [7,self._pack.port,idx]
        self.call(self._pack)

    def run(self,idx,pwm):
        self._pack.data = [6,self._pack.port,idx]
        self._pack.data.extend(short2bytes(pwm))
        self.call(self._pack)

    def set_break(self,idx,status):
        self._pack.data = [1,self._pack.port,idx,status]
        self.call(self._pack)

    def set_init(self,idx):
        self._pack.data = [8,self._pack.port,idx]
        self.call(self._pack)
...
class BLDCMotor(_BaseModule):
    ...
    def run(self,pwm):
        self._pack.data = [2,self._pack.port,pwm]
        self.call(self._pack)
...
class RGBLed(_BaseModule):
    ...
    def set_colors(self,pixels):
        """
            ...
            :type pixels: list
        """
        self._pack.data = [self._pack.port,self._pack.slot,0]
...
class LedMatrix(_BaseModule):
    ...
    def set_string(self,msg,x=0,y=0):
        """
        ...
        self._pack.data = [self._pack.port,1,int2uint(x),int2uint(8-y),len(msg)]
...
class Ultrasonic(_BaseModule):
    ...
    def __on_parse(self, pack):
        self._callback(float('{0:.1f}'.format(bytes2float(pack.data,1))))
...
class Gyro(_BaseModule):
    ...
    def __on_parse(self, pack):
        self._callback(bytes2float(pack.data,1),bytes2float(pack.data,6),bytes2float(pack.data,11))
...
class Color(_BaseModule):
    ...
    def __on_parse(self, pack):
        color = bytes2long(pack.data,1)
        self._callback({'red':color>>16,'green':(color>>8)&0xff,'blue':color&0xff})

    def read(self,callback):
        self._callback = callback
        self._pack.data = [self._pack.port,0x1,0x3]
        super().request(self._pack)
