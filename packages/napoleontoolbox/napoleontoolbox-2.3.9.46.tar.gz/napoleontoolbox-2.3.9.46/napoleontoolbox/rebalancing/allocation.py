...
def HRC(X, low_bound=0., up_bound=1.0, nb_clusters = 5):
...
def maxPerf(X,  w0=None, normalize=False, low_bound=0., up_bound=1.0):
    T, N = X.shape
    up_bound = max(up_bound, 1 / N)

    def f_performance(w, X):
        cumulated_perf = np.cumprod(X @ w + 1)
        final_cumulated_perf = cumulated_perf[-1]
        return final_cumulated_perf

    toOptimize = lambda w : f_performance(w,X)
...
def rolling_ensembling(X, w_mat_predictors_list, w_mat_target, n=252, s=63,ret=True, ensembling_model = 'other', period = 252, **kwargs):
    ...
    if ret:
        ...
        X_ = X
    if ensembling_model == 'min_sharpe':
