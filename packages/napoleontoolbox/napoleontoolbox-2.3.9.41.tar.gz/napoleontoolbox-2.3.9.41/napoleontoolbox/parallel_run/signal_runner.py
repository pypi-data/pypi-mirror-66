...
class AbstractRunner(ABC):
    def __init__(self, starting_date = None, running_date = None, drop_token=None, dropbox_backup = True, freqly_pkl_file_name='freqly_candels.pkl', local_root_directory='../data/', user = 'napoleon',nb_items_per_year=252):
        super().__init__()
        self.freqly_pkl_file_name=freqly_pkl_file_name
        ...
        self.nb_items_per_year = nb_items_per_year
...
class SimpleSignalRunner(AbstractRunner):
    def runTrial(self, saver, seed, trigger, signal_type, idios_string, transaction_costs, normalization):
        ...
        freqly_df = pd.read_pickle(self.local_root_directory+self.freqly_pkl_file_name)
        ...
        sharpe_strat = metrics.sharpe(freqly_df['perf_return'].dropna(), period= self.nb_items_per_year, from_ret=True)
        sharpe_under = metrics.sharpe(freqly_df['close_return'].dropna(), period= self.nb_items_per_year, from_ret=True)
