...
def _recording_device(flag):
    ...
    if flag:
        ...
        for device_index in range(temp.get_device_count()):
            ...
            if device_info['name'].find('立体声混音') != -1 and not device_info[
                    'hostApi']:
                return device_info
        else:
...
def _save_image(start_event, image_cache):
    global img_index
    ...
    while start_event_setted() or image_cache:
        if image_cache:
            ...
            image.save('{}\{:08x}.jpg'.format(img_temp_dir, local_index))
...
def _scan(start_event, init_threads_num, image_cache, max_fps):
...
def start_screen_recording(audio_chunk_size: float = 800,
    ...
    # 分别录音和录屏
    audio_process = Process(target=_record_audio, args=[
        start_event, audio_filename,
        audio_chunk_size, audio_format, interrecord],
        name='audio_process')
    ...
    [join_process(process) for process in processes[:-1]]
    ...
    video.write_videofile(video_filename, fps=None, codec='libx265',
                          bitrate=max_bitrate, audio=True,
                          audio_fps=int(_recording_device(interrecord)[
                              'defaultSampleRate']),
                          preset=video_compression_speed, audio_codec=None,
                          audio_bufsize=audio_chunk_size, threads=None,
