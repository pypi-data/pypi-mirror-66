...
class Initialise(object):
    def __init__(self, configfile=None, inifile=None, logfile=None
        ...
        e.g. INFO
        """
        # create logger for messages prior to full init of assetic package
        logger = logging.getLogger(__name__)

        # read xml config file if one was passed in and it is valid
        if configfile and not os.path.isfile(configfile):
            # user is passing in an invalid file so alert and return
            logger.error("Unable to locate XML configuration "
                         "file [{0}]".format(configfile))
            return None

        if configfile is None:
            # user may want to use default,so look for it
            configfile = os.path.abspath(os.environ.get("APPDATA") \
                                      + "\\Assetic\\arcmap_edit_config.xml")
            if not os.path.isfile(configfile):
                # can't find default so alert user and return
                logger.error(
                    "Unable to locate default XML configuration file [{0}]"
                    "".format(configfile))
                return None
