...
from ._data_characteristics_calculator import DataCharacteristicsCalculator
...
class DataPreparer(ABC):
    ...
    @abstractmethod
    def _parse(self, dataprep_json_obj):
        """
...
class DataPreparerFromDataSet(DataPreparer):
    ...
    def _get_fit_params(self, automl_settings_obj: AzureAutoMLSettings):
        training_data_row_count = self.data_characteristics.num_rows if self.data_characteristics else 0
    ...
    def get_preparer(dataprep_json: str, logger: logging.Logger) -> DataPreparer:
        ...
        try:
            ...
            if 'activities' in dataprep_json_obj:
                preparer = DataPreparerFromSerializedDataflows(dataprep_json_obj, logger)
...
class DataPreparerUtils:
    ...
    def _get_dict_from_dataflow(dflow: Any,
        ...
        if not automl_settings_obj.enable_streaming:
            ...
            validation_data, training_data = DataPreparerUtils._split_data_train_valid(logger, training_data,
                                                                                       train_data_row_count)

            fit_iteration_parameters_dict = DataPreparerUtils._set_dict_from_dataflow(training_data, validation_data,
                                                                                      automl_settings_obj)
    ...
    def _helper_get_data_from_dict(dataflow_dict: Dict[str, Any],
        ...
        if automl_settings_obj.enable_streaming:
            ...
            if validation_data is not None:
                ...
                validation_data, training_data = DataPreparerUtils._split_data_train_valid(logger,
                                                                                           training_data,
                                                                                           train_data_row_count)
    ...
    def _split_data_train_valid(
        ...
        if train_data_row_count > 0:
            ...
            return train_data.random_split(sample_probability, seed=42)
        else:
            logger.info('Using 10% of training data as validation data')
            return train_data.random_split(0.1, seed=42)
