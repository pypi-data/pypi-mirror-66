...
def test_remap():
    ...
    assert DotEnough.remap([f'/home/user/{p}', unchanged]) == expected
    expected = [f'--option={os.path.expanduser("~")}/{p}', unchanged]
    assert DotEnough.remap([f'--option=/home/user/{p}', unchanged]) == expected
...
def test_preserve_ownership(mocker, tmpdir):
    ...
    #
    assert DotEnough.preserve_ownership() is False
    ...
    assert DotEnough.preserve_ownership() is True
