...
def exp(table, region, format, output):
    ...
    if data:        
        if format == 'json':
            clz.write_to_json_file(data, os.path.join(output, f'{table}.json'))
        else:
            clz.write_to_csv_file(data, os.path.join(output, f'{table}.csv'))
    else:
...
def trunc(table, region):
    ...
    """
    click.confirm(f"About to truncate table '{table}'. Are you sure?", abort=True)
