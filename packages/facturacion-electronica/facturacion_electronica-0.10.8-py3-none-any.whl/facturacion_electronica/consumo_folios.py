...
<MntTotal>0</MntTotal><FoliosEmitidos>0</FoliosEmitidos><FoliosAnulados>0\
    ...
    def item_anulados(self, vals):
        folio, continuado = vals
        _item_utilizados = []
        if hasattr(self, '_item_anulados'):
            _item_anulados = self._item_anulados
        self._item_anulados = self.push_item(folio, _item_anulados, continuado)
    ...
    def item_utilizados(self, vals):
        folio, continuado = vals
        _item_utilizados = []
        if hasattr(self, '_item_utilizados'):
            _item_utilizados = self._item_utilizados
        self._item_utilizados = self.push_item(folio, _item_utilizados, continuado)
    ...
    def _rangosU(self, resumen, rangos, continuado=True):
        ...
        if 'Anulado' in resumen and resumen['Anulado'] == 'A':
            self.item_anulados = resumen['NroDoc'], continuado
            rangos['itemAnulados'] = self.item_anulados
            return rangos
        #anulados = rangos['itemAnulados'] if 'itemAnulados' in rangos else []
        #rangos.setdefault('itemUtilizados', [])
        self.item_utilizados = resumen['NroDoc'], continuado
        rangos['itemUtilizados'] = self.item_utilizados
        return rangos
