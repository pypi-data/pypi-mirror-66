    ...
    *Class managing mixture state*
    ...
    def _compute_mixture_fraction(self):
        ...
        *Compute mixture fraction of the mixture*
        ...
        nu_o2 = np.add(
            self._fuel.atoms['C'],
            self._fuel.atoms['H'] / 4)

        num_C = np.divide(self.elem_mass_frac('C'),
                          self._fuel.atoms['C'] * ATOMIC_MASS['C'])
        num_H = np.divide(self.elem_mass_frac('H'),
                          self._fuel.atoms['H'] * ATOMIC_MASS['H'])
        num_O = np.divide(self.elem_mass_frac('O', 2) - self.elem_mass_frac('O'),
                          nu_o2 * ATOMIC_MASS['O'])
        num = num_C + num_H + num_O

        den_C = np.divide(self.elem_mass_frac('C', 1),
                          self._fuel.atoms['C'] * ATOMIC_MASS['C'])
        den_H = np.divide(self.elem_mass_frac('H', 1),
                          self._fuel.atoms['H'] * ATOMIC_MASS['H'])
        den_O = np.divide(self.elem_mass_frac('O', 2),
                          nu_o2 * ATOMIC_MASS['O'])
    ...
    def _compute_far(self):
        ...
        *Compute FAR (Fuel Air Ratio) from mixture fraction*
        ...
        far = np.divide(self.mixture_fraction,
                        1.0 - self.mixture_fraction)
        return far
    ...
    def _compute_far_st(self):
        ...
        *Compute stoechiometric FAR from stoechiometric quantities*
        ...
        fuel_mass = 1. * self._fuel.molar_mass
        nu_o2 = np.add(
            self._fuel.atoms['C'],
            self._fuel.atoms['H'] / 4)

        far_st = np.divide(self.species_by_name('O2').mass_fraction(2) * fuel_mass,
