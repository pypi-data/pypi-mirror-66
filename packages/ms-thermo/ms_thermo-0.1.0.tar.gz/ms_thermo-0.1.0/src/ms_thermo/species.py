...
__all__ = ['build_thermo_from_avbp',
...
class _NasaPolys:
    ...
    def get_total_enthalpy(self, temp):
        ...
        condlist = [_temp < self.t_range["mid"],
                    np.logical_and(_temp >= self.t_range["mid"],
                                   _temp <= self.t_range["max"])]
...
def _make_props_avbp(species_description):
    ...
    props = dict()
    props["species_name"] = [line.split()[-1] for line in species_description
                             if line.startswith("species_name =")][0]
    ...
    for line in species_description:
        ...
        if line.startswith("species_enthalpy_table ="):
            ...
            number = np.array([_avbp_str_to_float(k) for k in number.split()
                               if k])
            number -= number[0]

            t_range = np.linspace(0., 5000., 51)
...
def _make_props_cantera(species_description):
    ...
    """
    props = dict()
    props["species_name"] = [line for line in species_description
                             if line.startswith("species(name")][0].strip()
    ...
    line = [k for k in "".join([' '.join(k.strip().split())
                                for k in species_description]).split("=")
            if "NASA" in k][0]
        ...
        if k:
            ...
            if len(k) == 2:
                ...
                if t_1 >= 1000.:
                    ...
                    t_range["min"] = t_1
                    t_range["mid"] = t_2
                coeffs[zone] = np.array([j for j in k[1].split() if j],
                                        dtype=float)
...
def build_thermo_from_avbp(database_file):
    ...
    *Reading all AVBP database species and storing in a dict( ) \
    ...
    for i, _ in enumerate(indexes, 0):
        ...
        species_description = [line for line in _db_stream[start: end]
                               if " = " in line]
        props = _make_props_avbp(species_description)
        species[props["species_name"]] = Species(props['molecular_weight'],
