...
class TierTypeEnum(UnityEnum):
    NONE = (0, 'None')
    NVME_EXTREME_PERFORMANCE = (5, 'NVMe_Extreme_Performance')
...
class VVolDatastoreTypeEnum(UnityEnum):
    ISCSI = (0, 'iSCSI')
    NFS = (1, 'NFS')
...
class HostOperationalStatusEnum(UnityEnum):
    UNKNOWN = (0, 'Unknown')
    OTHER = (1, 'Other')
    OK = (2, 'OK')
    DEGRADED = (3, 'Degraded')
    STRESSED = (4, 'Stressed')
    PREDICTIVE_FAILURE = (5, 'Predictive_Failure')
    ERROR = (6, 'Error')
    NON_RECOVERABLE_ERROR = (7, 'Non_Recoverable_Error')
    STARTING = (8, 'Starting')
    STOPPING = (9, 'Stopping')
    STOPPED = (10, 'Stopped')
    IN_SERVICE = (11, 'In_Service')
    NO_CONTACT = (12, 'No_Contact')
    LOST_COMMUNICATION = (13, 'Lost_Communication')
    ABORTED = (14, 'Aborted')
    DORMANT = (15, 'Dormant')
    SUPPORTING_ENTITY_IN_ERROR = (16, 'Supporting_Entity_in_Error')
    COMPLETED = (17, 'Completed')
    POWER_MODE = (18, 'Power_Mode')
    TEMP_UNMOUNTED = (19, 'Temp_Unmounted')
    PERM_UNMOUNTED = (20, 'Perm_Unmounted')
    INITIATORS_CONFIG_NOT_HA = (32768, 'Initiators_Config_not_HA')
    NO_LOGGEDIN_INITIATOR = (32769, 'No_LoggedIn_Initiator')
    INITIATORS_DATA_INCONSISTENT = (32770, 'Initiators_Data_Inconsistent')
    NETWORK_ADDRESSES_DATA_INCONSISTENT = (
        32771, 'Network_Addresses_Data_Inconsistent')
    TENANT_DATA_INCONSISTENT = (32772, 'Tenant_Data_Inconsistent')
    CREDENTIAL_INVALID = (34048, 'Credential_Invalid')
    STORAGE_NOT_FOUND = (34049, 'Storage_Not_Found')
    CERTIFICATE_INVALID = (34050, 'Certificate_Invalid')


class HostOperationalStatusEnumList(UnityEnumList):
    @classmethod
    def get_enum_class(cls):
        return HostOperationalStatusEnum


class VasaProviderStateEnum(UnityEnum):
    NOT_REGISTERED = (0, 'Not_Registered')
    REGISTERED = (1, 'Registered')
    NOT_SUPPORTED = (99, 'Not_Supported')


class HostContainerPotentialHostMatchConditionEnum(UnityEnum):
    MATCH_UNKNOWN = (0, 'Match_Unknown')
    MATCH_UUID = (1, 'Match_UUID')
    MATCH_ALLINITIATORS = (2, 'Match_AllInitiators')
    MATCH_UUID_ALLINITIATORS = (3, 'Match_UUID_AllInitiators')
    MATCH_ALLNETWORKADDRESSES = (4, 'Match_AllNetworkAddresses')
    MATCH_UUID_ALLNETWORKADDRESSES = (5, 'Match_UUID_AllNetworkAddresses')
    MATCH_ALLINITIATORS_ALLNETWORKADDRESSES = (
        6, 'Match_AllInitiators_AllNetworkAddresses')
    MATCH_UUID_ALLINITIATORS_ALLNETWORKADDRESSES = (
        7, 'Match_UUID_AllInitiators_AllNetworkAddresses')
    MATCH_SOMEINITIATORS = (8, 'Match_SomeInitiators')
    MATCH_UUID_SOMEINITIATORS = (9, 'Match_UUID_SomeInitiators')
    MATCH_ALLNETWORKADDRESSES_SOMEINITIATORS = (
        12, 'Match_AllNetworkAddresses_SomeInitiators')
    MATCH_UUID_ALLNETWORKADDRESSES_SOMEINITIATORS = (
        13, 'Match_UUID_AllNetworkAddresses_SomeInitiators')
    MATCH_SOMENETWORKADDRESSES = (16, 'Match_SomeNetworkAddresses')
    MATCH_UUID_SOMENETWORKADDRESSES = (17, 'Match_UUID_SomeNetworkAddresses')
    MATCH_ALLINITIATORS_SOMENETWORKADDRESSES = (
        18, 'Match_AllInitiators_SomeNetworkAddresses')
    MATCH_UUID_ALLINITIATORS_SOMENETWORKADDRESSES = (
        19, 'Match_UUID_AllInitiators_SomeNetworkAddresses')
    MATCH_SOMEINITIATORS_SOMENETWORKADDRESSES = (
        24, 'Match_SomeInitiators_SomeNetworkAddresses')
    MATCH_UUID_SOMEINITIATORS_SOMENETWORKADDRESSES = (
        25, 'Match_UUID_SomeInitiators_SomeNetworkAddresses')


class HostContainerPotentialHostMatchConditionEnumList(UnityEnumList):
    @classmethod
    def get_enum_class(cls):
        return HostContainerPotentialHostMatchConditionEnum


class HostContainerPotentialHostImportOptionEnum(UnityEnum):
    IMPORT_NEW = (0, 'Import_New')
    IMPORT_EXISTING = (1, 'Import_Existing')
    IMPORT_CONFLICT = (2, 'Import_Conflict')
    IMPORT_MATCH = (3, 'Import_Match')
    IMPORT_WARN = (4, 'Import_Warn')
