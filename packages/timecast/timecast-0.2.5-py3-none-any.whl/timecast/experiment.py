...
class experiment:
    ...
    def __init__(self, argnames: Union[List[str], str], arglists: List[Any]) -> None:
        ...
        for arglist in arglists:
            ...
            if arglist_length != len(self._argnames):
                raise ValueError(
                    (
                        "Each arglist must have the same length as provided argnames. "
                        "Got names {} and list {}"
                    ).format(argnames, arglist)
                )
    ...
    @property
    def df(self):
        """Access the resulting pandas dataframe object"""
        if self._df is None:
    ...
    def run(self, processes=1, tqdm=None):
        """Execute the experiment"""
        self._validate()

        num_jobs = jnp.product([len(spec[1]) for spec in self._spec])

        pool = ProcessPool(nodes=processes)
        if tqdm is not None:
            result = list(tqdm.tqdm(pool.imap(runner, self._generate_arglists()), total=num_jobs))
        else:
            result = list(pool.imap(runner, self._generate_arglists()))
        return result
