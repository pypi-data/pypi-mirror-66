...
def import_auth():
 try:
  ...
  raise hWQCs('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=hWQCP)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=hWQCg)
def get_params(args,names=[],mandatory=hWQCP):
 result=()
 for name in names:
  value=hWQCJ
  if name in args:
  ...
  if not value and mandatory:
   raise hWQCs('Please provide %s=... parameter'%name)
  result+=(value)
 if hWQCc(result)==1:
  return result[0]
 return result
