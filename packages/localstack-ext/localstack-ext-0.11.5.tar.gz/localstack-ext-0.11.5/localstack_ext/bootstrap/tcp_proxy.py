...
class AttrDict(veKap):
 def __init__(self,*args,**kwargs):
  veKac(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and veKau(socket,'IPV6_V6ONLY'):
...
class Half(veKaH):
 def __init__(self,opt,sock,addr,veKaY):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.veKaY=veKaY
  self.name='peer' if self.veKaY else 'client'
  self.queue=[]
  self.dest=veKag
  self.err=veKag
  self.ready=veKaE
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':veKaE,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
