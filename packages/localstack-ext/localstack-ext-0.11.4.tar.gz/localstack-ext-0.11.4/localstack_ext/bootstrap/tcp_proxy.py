...
class AttrDict(tULjc):
 def __init__(self,*args,**kwargs):
  tULjF(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and tULjd(socket,'IPV6_V6ONLY'):
...
class Half(tULjH):
 def __init__(self,opt,sock,addr,tULjW):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.tULjW=tULjW
  self.name='peer' if self.tULjW else 'client'
  self.queue=[]
  self.dest=tULjl
  self.err=tULjl
  self.ready=tULjw
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':tULjw,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
