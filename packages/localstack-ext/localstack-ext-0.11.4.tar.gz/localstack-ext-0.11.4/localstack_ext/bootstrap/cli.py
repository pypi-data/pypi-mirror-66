...
def import_auth():
 try:
  ...
  raise FLbvM('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=FLbvO)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=FLbvV)
def get_params(args,names=[],mandatory=FLbvO):
 result=()
 for name in names:
  value=FLbvQ
  if name in args:
  ...
  if not value and mandatory:
   raise FLbvM('Please provide %s=... parameter'%name)
  result+=(value)
 if FLbvC(result)==1:
  return result[0]
 return result
