...
class AttrDict(jOeQt):
 def __init__(self,*args,**kwargs):
  jOeQL(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and jOeQf(socket,'IPV6_V6ONLY'):
...
class Half(jOeQz):
 def __init__(self,opt,sock,addr,jOeQA):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.jOeQA=jOeQA
  self.name='peer' if self.jOeQA else 'client'
  self.queue=[]
  self.dest=jOeQP
  self.err=jOeQP
  self.ready=jOeQd
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':jOeQd,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
