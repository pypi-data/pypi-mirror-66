...
def import_auth():
 try:
  ...
  raise yrSNE('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=yrSNj)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=yrSNY)
def get_params(args,names=[],mandatory=yrSNj):
 result=()
 for name in names:
  value=yrSNu
  if name in args:
  ...
  if not value and mandatory:
   raise yrSNE('Please provide %s=... parameter'%name)
  result+=(value)
 if yrSNR(result)==1:
  return result[0]
 return result
