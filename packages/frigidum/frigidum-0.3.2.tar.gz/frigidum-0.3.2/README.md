...
I created a objective/neighbourhood strategy that is conservative on using colors, and needs to set a goal number of colors to use. We use `O_stop=0` to stop the annealing when an colering has been found.

The goal number of colors for example below is set to 250. It is possible to come closer to 222, but needs much more `repeats` or even larger `alpha`.
