...
def force_add_random_vertex(C):
	...
	if np.any(~colored_vertices):
		...
		skewed_proposal_weights = (proposal_weights / np.max(proposal_weights))**3
...
List Complrehensions:

[ np.sum(gcp.edges_matrix[c][:,uncolored_vertex]) for c in C]
[np.all( ~gcp.edges_matrix[c][:,60] ) for c in T]
