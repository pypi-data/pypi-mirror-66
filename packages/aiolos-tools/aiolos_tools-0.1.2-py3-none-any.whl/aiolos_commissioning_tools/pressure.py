...
from aiolos_commissioning_tools.tunnel_conditions import wind_speed_from_pitot
...
pressure_scanner_ranges = {ZOC334: 10 * units.INCHES_H2O,  # 2488.4 Pa = 10 inches water
...
def process_boundary_layer_data(run_id, scanner_data, tunnel_data, scanner_channels, rake, probe):
    ...
    pt = np.mean(scanner_data.pressure[:, scanner_data.port == scanner_channels.pt])
    ps = np.mean(scanner_data.pressure[:, scanner_data.port == scanner_channels.ps])
    po = np.array([np.mean(scanner_data.pressure[:, scanner_data.port == port]) for port in scanner_channels.p])
    z = np.array([rake.z[rake.port == port] for port in scanner_channels.p])
    ...
    boundary_layer_thickness = fluids.boundary_layer_thickness(rake.z, u, u_e)
    displacement_thickness = fluids.dispacement_thickness(rake.z, u, u_e)
    momentum_thickness = fluids.momentum_thickness(rake.z, u, u_e)
