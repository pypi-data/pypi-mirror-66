    ...
    def __init__(self, value=Void,
        ...
        Args:
            ...
            readonly (bool, optional): Unchangeable property. Mutable values can still be changed inplace. Defaults to False. Readonly modes can be the following:
                RO_WEAK: Property will be readonly but changeable through internal variable. Defaults to False.
                RO_STRONG: Can not be changed through internal variable because there will be no internal variable.
                RO_CLASS: Readonly at class level i.e the property itself will be readonly for the defining class.
                True: Equvalent to RO_STRONG | RO_CLASS
                False: Not readonly
            store_default (bool, optional): Whether to store the default value of the property in `Props.Defaults`.
            ...
            undead (bool, optional): Whether the property should be undead (non deletable). Defaults to False. Undead modes are following:
                UD_CLASS: undead at class level i.e the class itself won't be able to delete it.
        ...
        if readonly is True:
            ...
            raise ValueError("Invalid value for parameter 'readonly'. Check class `Prop` for assistance.")
        if undead is True:
            ...
            raise ValueError("Invalid value for parameter 'undead'. Check class `Prop` for assistance.")
...
class PropMeta(ABCMeta):
        ...
        if name == '_Props_': # if we keep this name in a single place (somewhere in some variable), it would be possible to change it.
            ...
            if not name.startswith('_'):
                ...
                if isinstance(value, Prop):
                    ...
                    This_Prop, Defaults_Prop, Keys_Prop, Ivan_Prop, Conf_Prop = self._makePropProperties(name, value, val)
    ...
    def _get_var_conf(self, name, value):
        ...
        try:
            ...
            raise TypeError("'VarConf' is a reserved attribute name. It must be a class that inherits and implements `easyvar.abc.VarConf`.")
        if isinstance(var_conf, abc.VarConf): # must do this check
            # because if some other class implementation provide get_conf method with name/value arg,
            # we do not want to accept unknown entities.
            try:
                p = var_conf.get_conf(name, value)
            except:
                raise TypeError("Bad implementation of `VarConf` class. See `easyvar.abc.VarConf`")
        else:
            raise TypeError("'VarConf' is a reserved attribute name. It must be a class that inherits and implements `easyvar.abc.VarConf`.")
        assert p is None or isinstance(p, Prop), "return value from 'get_conf' in class 'VarConf' "\
                    ...
                    "`easyvar.abc.VarConf`" % (self.__class__,)
        return p
