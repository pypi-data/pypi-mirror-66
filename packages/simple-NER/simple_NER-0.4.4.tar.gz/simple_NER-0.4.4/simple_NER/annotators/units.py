...
import logging
logging.getLogger("quantulum3.parser").setLevel("WARN")
logging.getLogger("quantulum3.classifier").setLevel("WARN")
logging.getLogger("quantulum3._lang.en_US.parser").setLevel("WARN")
...
class UnitsNER(NERWrapper):
    ...
    def annotate(self, text):
        for e in parser.parse(text):
            ...
            data["unit"] = e.unit.__dict__.copy()
            data["unit"]["entity"] = e.unit.entity.__dict__.copy()
            ...
            surface = data.pop("surface")
            yield Entity(surface, e_type, source_text=text, data=data)
