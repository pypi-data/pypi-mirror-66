...
with Context(debugMode=False) as ctx:
    ...
    s = SIMFLUX(ctx)
    sum_estim = s.CreateEstimator(psf, mod, False)
    sf_estim = s.CreateEstimator(psf, mod, True)

    ev = sf_estim.ExpectedValue(sf_theta)  
    sf_smp = np.random.poisson(ev)
    su.imshow_hstack(sf_smp[0])
    
    sf_result,diag,sf_traces = sf_estim.Estimate(sf_smp)
    sum_result,diag,sum_traces = sum_estim.Estimate(sf_smp)

    deriv, mu = sf_estim.Derivatives(sf_theta)
    for k in range(deriv.shape[1]):
         plt.figure()
         su.imshow_hstack(deriv[0,k])
         plt.title(f"Simflux psf derivative {sf_estim.colnames[k]}")
