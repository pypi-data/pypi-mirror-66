...
def test_psf_speed(ctx: Context, smp_psf:Estimator, est_psf:Estimator,
                   theta, batchSize=1024*4,repeats=1):
    ...
    queue = EstimQueue(est_psf, batchSize=batchSize, numStreams=4)
    n = 10000
...
#    test_queue_output(ctx, psf, theta)
    ...
    sf_psf = sf_api.CreateEstimator_Gauss2D(sigma, sf_mod, roisize, len(sf_mod),  simfluxEstim=True)
    ...
    test_psf_speed(ctx, sf_as_psf, sf_as_psf, as_theta, repeats=1)
    
    print('Single-Frame SIMFLUX 2D PSF:')
    sfsf_psf = sf_api.CreateSFSFEstimator(psf, sf_mod, sfsf_offsets())
    test_psf_speed(ctx, sfsf_psf, sfsf_psf, theta, repeats=1)
    ...
    if cspline_fn is not None:
        ...
        test_psf_speed(ctx,cs_psf,cs_psf, cs_theta)
    ...
    test_psf_speed(ctx,psf,phasor_est,theta,batchSize=1024*10, repeats=200)

    print('COM:')
    com_est = com.CreateEstimator(roisize, ctx)
    test_psf_speed(ctx,psf,com_est,theta,batchSize=1024*10, repeats=200)
