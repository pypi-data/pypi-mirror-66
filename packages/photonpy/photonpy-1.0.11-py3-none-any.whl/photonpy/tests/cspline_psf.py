...
with Context() as ctx:
    calib = CSpline_Calibration.from_file_nmeth(cspline_calib_fn())
    
    roisize= 20
    psf = CSpline(ctx).CreatePSF_XYZIBg(roisize, calib, True, 1)
    
    N = 200
    theta = np.repeat([[roisize/2,roisize/2,50,3000,0.1]], N, axis=0)
