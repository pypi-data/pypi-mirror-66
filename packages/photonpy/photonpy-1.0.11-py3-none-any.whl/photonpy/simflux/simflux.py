...
from photonpy.smlmlib import util as su
...
def tiff_to_rois(path, num_patterns, cfg, debugMode):
    with SMLM(debugMode=debugMode) as smlm, Context(smlm) as ctx:
        ...
        spotDetector = spotdetect.SpotDetector(sigma, roisize, minIntensity)

        calib = process_movie.create_calib_obj(cfg['gain'],cfg['offset'],imgshape,ctx)

        sf = SIMFLUX(ctx)        
        psf = sf.CreateSIMFLUX2D_Gauss2D_PSF(sigma, num_patterns, roisize, 
                                             num_patterns, False)
        roipsf = Estimator.CopyROI_Create(psf, ctx)
        roiqueue = EstimQueue(roipsf, batchSize=1024)

        process_movie.run_spot_detection_queue(imgshape, spotDetector, calib, roiqueue, 
...
#        r.Filter(np.where( (crlb[:,0]+crlb[:,1])<3 )[0])
        ...
        r.rois = r.diagnostics.reshape((len(r.ids),*psf.sampleshape))
...
def estimate_IBg(smlm, rois, roipos, sigma):
    ...
    with Context(smlm) as ctx:
        ...
        q = EstimQueue(psf, batchSize=1024)
