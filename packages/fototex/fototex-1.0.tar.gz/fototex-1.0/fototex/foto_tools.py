...
from fototex._numba import sector_average, azimuthal_average
...
def rspectrum(window, radius, window_size, nb_sample, standardize, keep_dc_component):
    """ Compute r-spectrum for given window and filter smaller ones
...
def rspectrum_per_sector(window, radius, sectors, window_size, nb_sample, nb_sectors, standardize, keep_dc_component):
    ...
    if window.shape[0] == window.shape[1] == window_size:
        ...
        return np.zeros((nb_sectors, nb_sample))
