...
PUMP_LOG_FILENAME = "pump_log.csv"
PUMP_CALIBRATION_FILENAME = "pump_calibration.csv"


class Pumps:
    def __init__(self, pe):
        self.pe = pe  # peacedeath.Experiment object
        self.dev = pe.dev  # device
        self.logfile = os.path.join(pe.directory, PUMP_LOG_FILENAME)
        self.calibration_file = os.path.join(pe.directory, PUMP_CALIBRATION_FILENAME)
    ...
    def pump_peace(self, tube, volume):
        ...
        self.log_pump_operation(pump1Volume=volume, comment="started pumping")
        ...
        time.sleep(0.5)  # release all pressure in the tubes
        self.pe.Valves.close_valve(tube)
        self.log_pump_operation(pump1Volume=volume, comment="completed pumping")

    def pump_death(self, tube, volume):
        self.pe.Valves.close_all_valves()  # redundant valve close for safety
        self.log_pump_operation(pump2Volume=volume, comment="started pumping")
        self.pe.Valves.open_valve(tube)
        time.sleep(0.5)
        ...
        time.sleep(0.5)  # release all pressure in the tubes
        self.pe.Valves.close_valve(tube)
        self.log_pump_operation(pump2Volume=volume, comment="completed pumping")

    def pump_vacuum(self, tube, volume):
        self.pe.Valves.close_all_valves()  # redundant valve close for safety
        self.log_pump_operation(pump3Volume=volume, comment="started pumping")
        self.pe.Valves.open_valve(tube)
        time.sleep(0.5)
        ...
        time.sleep(0.5)  # release all pressure in the tubes
        self.pe.Valves.close_valve(tube)
        self.log_pump_operation(pump3Volume=volume, comment="completed pumping")

    def log_pump_operation(self, pump1Volume=0, pump2Volume=0, pump3Volume=0, comment=""):
        if not os.path.exists(self.logfile):
            with open(self.logfile, "a") as f:
                f.write("Time, pump1Volume, pump2Volume, pump3Volume, comment\n")
        pump1Volume = str(pump1Volume)
        pump2Volume = str(pump2Volume)
        pump3Volume = str(pump3Volume)
        log_string = ", ".join([time.ctime(), pump1Volume, pump2Volume, pump3Volume, comment]) + "\n"
        with open(self.logfile, "a") as f:
            f.write(log_string)
