...
CURRENT_DATA_FILENAME = "current_data.csv"

class Photodiodes:
    def __init__(self, pe):
        self.pe = pe
        self.dev = pe.dev
        self.current_file = os.path.join(self.pe.name, CURRENT_DATA_FILENAME)
    ...
    def measure_current(self, tube, cycles=10, n_measurements=40, laserOnDelay=10, laserOffDelay=10,
        ...
        IFS, ISS, IFSoff, ISSoff = _measure_current(dev=self.dev, tube=tube, cycles=cycles,
                                                       ...
                                                       verbose=verbose)
        if log:
            self.log_current(IFS, ISS, IFSoff, ISSoff)
        return IFS, ISS, IFSoff, ISSoff

    def log_current(self, IFS, ISS, IFSoff, ISSoff):
        if not os.path.exists(self.current_file):
            with open(self.current_file, "w+") as f:
                f.write("time, front, side, front_dark, side_dark\n")
        current_time=time.ctime()
        with open(self.current_file,"a") as f:
            f.write("{time}, {IFS:.2f}, {ISS:.2f}, {IFSoff:.2f}, {ISSoff:.2f}/n".format(time=current_time, IFS=IFS,
                                                    ISS=ISS, IFSoff=IFSoff, ISSoff=ISSoff))
