        ...
        for ii in range(num_ranges):
            ...
            if export in ['csv', 'interlaced','binary']:
                ...
                if export in ['binary']:
                    ...
                    if export == 'csv':
                        file_handle.write(','.join([str(x) for x in sorted(list(set(channels)))]) + '\n')
        ...
        if current_type_str == 'epocs':
            ...
            if channel > 0:
                ...
                if len(data[current_type_str][current_name].chan) > 1:
                    valid_ind = data[current_type_str][current_name].chan == channel
                    if not nodata:
                    ...
                    # it's a single value
                    if channel != data[current_type_str][current_name].chan[0]:
            ...
            if current_name in sev_names:
                ...
                for jj in range(num_ranges):
                    ...
                    if len(fc) < 1:
                        continue
                    if channel > 0:
                        if np.all(data[current_type_str][current_name].filtered_chan[jj] == 1):
                            ...
                            data[current_type_str][current_name].filtered_chan[jj] = fc[valid_ind]
                        nchan = np.uint64(1)
                    else:
                        valid_ind = np.arange(len(data[current_type_str][current_name].filtered_data[jj]))
                    ...
                    fc = data[current_type_str][current_name].filtered_chan[jj]
                    these_offsets = data[current_type_str][current_name].filtered_data[jj]
                    ...
                    for f in range(len(arr)):
                        ...
                        for kk in range(len(relative_offsets)):
                            ...
                            if export in ['csv', 'interlaced', 'binary']:
                                ...
                                if ready_for_export[chan] == True:
                                    warnings.warn('This channel already ready? tev file out of order?', Warning)
                                ddd[chan, :] = tev_data[ind[kk,:].flatten().astype(np.uint64)]
                                
                                ready_for_export[chan] = True
                                if np.all(ready_for_export):
                                    ...
                                    if export == 'csv':
                                        ...
                                        for x in sorted(file_handles.keys()):
                                            ddd[x-1,:].astype(export_type, copy=False).T.tofile(file_handles[x])
                                    # reset ready indicators
                                    ready_for_export = [False for i in ready_for_export]
                            else:
                                data[current_type_str][current_name].data[jj][chan, chan_index[chan]:(chan_index[chan] + npts)] = tev_data[ind[kk,:].flatten().astype(np.uint64)]
                            chan_index[chan] += npts
                            #    data[current_type_str][current_name].data.append(tev_data[ind.flatten().astype(np.uint64)].reshape((-1, npts)))
                ...
                if channel > 0:
                    data[current_type_str][current_name].channel = channel
