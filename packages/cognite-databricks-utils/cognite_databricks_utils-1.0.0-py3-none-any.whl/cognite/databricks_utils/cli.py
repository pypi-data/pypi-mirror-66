...
loc_option = click.option("--location", "-l", default="westeurope", type=click.Choice(["westeurope", "northeurope"]))
token_option = click.option("--token", "-t", help="Databricks token", envvar="DB_TOKEN")
...
@click.option(
    ...
    "-i",
    default="users.xlsx",
    help="Relative path to excel file with user emails",
...
def get_token(token) -> None:
    if not token:
        print(
            "Expected a Databricks token to be set as a environment variable named 'DB_TOKEN' or "
            "passed in as an option '--token/-t'."
        )
        sys.exit(1)
