...
class Backend(ABC):
    ...
    def get_shots(
        ...
        **kwargs: KwargTypes,
        ...
        """Obtain the table of shots from an experiment. Can either accept a :py:class:`ResultHandle` for a processed circuit to retrieve the data from the backend's cache, or a :py:class:`Circuit` to be run and immediately returned. This will fail if the circuit does not match the device's requirements.
        ...
        :type valid_check: bool, optional
        :raises NotImplementedError: If backend implementation does not support shot table retrieval
    ...
    def get_counts(
        ...
        **kwargs: KwargTypes,
        ...
        """Obtain a summary of results, accumulating the shots for each result from an experiment. Can either accept a :py:class:`ResultHandle` for a processed circuit to retrieve the data from the backend's cache, or a :py:class:`Circuit` to be run and immediately returned. This will fail if the circuit does not match the device's requirements.
        ...
        :param valid_check: Explicitly check that the circuit satisfies all of the required predicates before running. Defaults to True
        :type valid_check: bool, optional
        :raises NotImplementedError: If backend implementation does not support counts retrieval
    ...
    def get_state(
        ...
        """Obtain a statevector from a simulation. Can either accept a :py:class:`ResultHandle` for a processed circuit to retrieve data from the backend's cache, or a :py:class:`Circuit` to be run and immediately returned. This will fail if the circuit does not match the simulator's requirements.

        :param circuit: The circuit to run, or results to retrieve
        :type circuit: Union[Circuit, ResultHandle]
        :param basis: Toggle between ILO-BE (increasing lexicographic order of bit ids, big-endian) and DLO-BE (decreasing lexicographic order, big-endian) for ordering the coefficients. Defaults to BasisOrder.ilo.
        :type basis: BasisOrder, optional
        :param valid_check: Explicitly check that the circuit satisfies all of the required predicates before running. Defaults to True
        :type valid_check: bool, optional
        :raises NotImplementedError: If backend implementation does not support statevector retrieval
        :return: A big-endian statevector for the circuit in the encoding given by `basis`; e.g. :math:`[a_{00}, a_{01}, a_{10}, a_{11}]` where :math:`a_{01}` is the amplitude of the :math:`\\left|01\\right>` state (in ILO, this means qubit q[0] is in state :math:`\\left|0\\right>` and q[1] is in state :math:`\\left|1\\right>`, and the reverse in DLO)
        :rtype: np.ndarray
        """
