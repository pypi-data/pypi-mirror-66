...
class Bot:
    ...
    def _wait_for(self, response):
        ...
        for msgJSON in self.conn:
            ...
            if i > self.APITimeout:
                log(f"{response} API response not recorded. reason: too many events before {response}.", error = True, _copy = self._copy)
                reply = None
                break
    ...
    def start(self, function = None, advanced = False):
        if not advanced and not self.advanced:
            ...
            if self.function is None and function is not None and callable(function):
                ...
                log("No valid function passed to advanced bot's start function. Using filler function", _copy = self._copy)
    ...
    def copy_to(self, roomName, password = None):
        ...
        copy.parentThreads.append(threading.current_thread())
