...
class _ConvNd(Layer):
    ...
    def __init__(self, kernel_size, num_filters, strides, auto_pad, padding,padding_mode, use_bias, dilation, groups,
        ...
        self.depthwise=kwargs.get('depthwise',False)
        self.separable = kwargs.get('separable', False)
    ...
    def build(self, input_shape):
        if self._built == False:
            ...
            elif self.depthwise and self.depth_multiplier%1==0.5 and self.input_filters%2==0:  #in channel=128=>out_channel=64   means: depth_multiplier=0.5*2 and group=128//2
                self.groups=self.input_filters//2
...
class Conv1d(_ConvNd):
    ...
    def conv1d_forward(self, x):
        return tf.nn.conv1d(x, filters=self.weight, strides=(1,)+self.strides+(1,), padding=self.padding, data_format="NHWC", dilations=self.dilation, name=self._name)
...
class Conv3d(_ConvNd):
    ...
    def conv3d_forward(self, x):
        return tf.nn.conv3d(x, filters=self.weight, strides=(1,) + self.strides + (1,), padding=self.padding,data_format="NHWC", dilations=self.dilation, name=self._name)
...
class TransConv1d(_ConvNd):
    ...
    def conv1d_forward(self, x):
        ...
        return tf.nn.conv1d_transpose(x, filters=self.weight, output_shape=in_shape,strides=(1,)+self.strides+(1,), padding=self.padding, data_format="NHWC",   dilations=self.dilation, name=self._name)
...
class DepthwiseConv1d(_ConvNd):
    ...
    def conv1d_forward(self, x):
        return tf.nn.convolution(x, filters=self.weight, strides=(1,)+self.strides+(1,),padding=self.padding, data_format="NHWC",dilations=self.dilation, name=self._name)
...
class DepthwiseConv2d(_ConvNd):
    ...
    def conv2d_forward(self, x):
        return tf.nn.depthwise_conv2d(x, filters=self.weight, strides=(1,)+self.strides+(1,),padding=self.padding, data_format="NHWC",dilations=self.dilation, name=self._name)
...
class SeparableConv2d(_ConvNd):
    ...
    def conv2d_forward(self, x):
        return tf.nn.separable_conv2d(x, depthwise_filter=self.weight,pointwise_filter=self.pointwise, strides=(1,)+self.strides+(1,),padding=self.padding, data_format="NHWC",dilations=self.dilation, name=self._name)
