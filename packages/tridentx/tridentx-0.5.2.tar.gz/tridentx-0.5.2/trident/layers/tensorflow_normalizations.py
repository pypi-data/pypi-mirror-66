...
class BatchNorm(Layer):
    ...
    def build(self, input_shape):
        if self._built == False:
            ...
            if self.track_running_stats:
                self.running_mean = tf.constant(tf.zeros(shape=[self.input_filters]), name='running_mean')
                self.running_var = tf.constant(tf.ones(shape=[self.input_filters]), name='running_var')
