...
def upload_file(filename, copy_clipboard, output_url):
    with open(filename, 'rb') as img:
        r = requests.post(
            f'{config.get("url")}/images',
...
@click.command()
@click.option('--no-copy', is_flag=True, help='Copy url to clipboard after upload.')
@click.option('--no-output', is_flag=True, help='Print url to stdout after upload.')
@click.option('-i', '--image', help='Path to image to upload')
def upload_image(no_copy, no_output, image):
    if image:
        url = upload_file(image, not no_copy, not no_output)
    else:
        make_screenshot()
        url = upload_file(tmp_file, not no_copy, not no_output)
        save_file()
