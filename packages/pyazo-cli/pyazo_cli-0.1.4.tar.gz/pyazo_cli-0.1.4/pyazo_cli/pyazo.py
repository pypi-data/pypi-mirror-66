...
def upload_file(filename, copy_clipboard, output_url, private):
    with open(filename, 'rb') as img:
        request_url = f'{config.get("url")}/images'
        if private:
            request_url = f'{request_url}?private=True'
...
def upload_image(no_copy, no_output, image, private):
    if image:
        url = upload_file(image, not no_copy, not no_output, private)
    else:
        make_screenshot()
        url = upload_file(tmp_file, not no_copy, not no_output, private)
        save_file()
