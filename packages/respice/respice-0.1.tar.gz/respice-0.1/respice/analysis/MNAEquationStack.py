...
class MNAEquationStack:
    ...
    def evaluate(self, v_i: np.ndarray, dt: float) -> np.ndarray:
        """
        ...
        :param dt:
            The time step (measured in seconds).
        :return:
            The result.
        """
        out_vector = []
    ...
    def lambdify(self):
        ...
        code = [
            'import numpy as np',
            'def eq(v_i: np.ndarray, dt: float) -> np.ndarray:',
            ' return np.array([',
    ...
    def jacobian(self, v_i: np.ndarray, dt: float) -> np.ndarray:
        """
    ...
    def lambdify_jacobian(self):
        ...
        code = [
            ...
            f'def jac(v_i: np.ndarray, dt: float) -> np.ndarray:',
