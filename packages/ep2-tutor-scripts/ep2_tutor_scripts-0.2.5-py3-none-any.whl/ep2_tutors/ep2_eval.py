...
def pre_eval_csv_fieldnames(ep2: Ep2, ue: int) -> [str]:
    ...
    return [KEY_STUDENT_ID, KEY_UE_ATTENDED, KEY_UE_REMARKS, KEY_UE_FEEDBACK, KEY_UE_GRADING] + sub_exercises
...
def checkout(ctx, group, ue: int):
    ...
    with open(students_file, 'r') as infile:  # read students and create entries in attendance file
        ...
        for row in reader:
            ...
            # clone or pull repo
            clone_result = ep2.clone_or_update(os.path.join(uebung_path, student_id), repo, TAG_ABGABE % ue)
            if clone_result.is_err():
                errors += [clone_result.value]
            ...
            with open(pre_eval_file, 'w') as outfile:
                ...
                for row in reader:
                    ...
                    if curr_ue_type == ExerciseType.Normal:
                        ...
                        writer.writerow({KEY_STUDENT_ID: student_id, KEY_UE_GRADING: '_', KEY_UE_REMARKS: '', KEY_UE_FEEDBACK: '', KEY_UE_ATTENDED: 0, **sub_exercises})
                    elif curr_ue_type == ExerciseType.Team:
                        writer.writerow(
