...
class ManageHandler(BaseHandler):
    ...
    async def get(self):
        ...
        entries = await self.query('users', schema={'id': int})
...
class DiskHandler(tornado.web.StaticFileHandler, BaseHandler):
    ...
    def send_html(self, html, padding=0):
        lines = [
            '<html><head>',
            '<link rel="stylesheet" href="/static/src/css/monokai_sublime.css">',
            f'</head><body style="padding:{padding}px;margin:0;height:100%;width:100%;background:#23241f;">',
            html,
    ...
    async def check(self, name):
        ...
        return (doc and name.startswith(doc.name))

    async def send(self, name, include_body=True):
        if self.app.options.git and include_body and re.search(r'\.(gz|bz2|tar|tgz|xz|z|zip|7z|rar|mp4|mp3)$', name):
            doc = await self.db.files.find_one({'name': name})
            if doc:
    ...
    async def get(self, name, include_body=True):
        ...
        if self.args.q:
            ...
            if path.suffix.lower() == '.zip':
                ...
                self.send_html(html, padding=20)
                ...
                self.send_html(f'<pre><code class="{code}">{ tornado.escape.xhtml_escape(text) }</code></pre>')
            else:
                await self.send(name, include_body)
        elif path.is_file():
            await self.send(name, include_body)
    ...
    async def post(self, name):
        ...
        if not path.exists() and self.args.action and self.args.action not in ['unshare', 'git-rm', 'merge']:
            ...
            if not re.match('.(gz|bz2|tar|tgz|xz|z|zip|7z|rar|mp4|mp3)$', path.suffix.lower()):
        ...
        if self.app.options.auth:
            ...
            for f in (self.root / 'public').rglob('*'):
                if f.resolve() == path.absolute():
                    f.unlink()
                    await self.git_rm(f.relative_to(self.root))
