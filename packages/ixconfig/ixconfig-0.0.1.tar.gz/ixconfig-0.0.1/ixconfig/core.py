import subprocess
from .util import execmatch, matchmany, as_tuple
...
class _BaseConfig:
    ...
    def get_output(self):
        ...
        return subprocess.run(
            cmdargs, check=True, capture_output=True).stdout.decode('utf-8')
    ...
    def params(self):
        ...
        out = self.get_output()
        print('output:', out)
        if any(msg.lower() in out.lower() for msg in as_tuple(self.error_msgs)):
            return {}
