...
def parse_cli_args(yamlfile=None,out_dir=None):
    ...
    parser.add_argument('--d','--dir',default=out_dir,
...
def main(yamlfile=None, p_output=None, p_generate=None, out_path=None):
    # process_list = []
    args = parse_cli_args(yamlfile,out_path)
    # multi process here for now
    # process_yaml(args.yaml, args.log_level)
    path = args.d
    
    if out_path is None and path is None:
        path = os.getcwd()
    else:
