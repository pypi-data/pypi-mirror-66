...
def upload_files(request, input_name, sequenced_filename=None, parent_directory=None):
    """
    ...
    for ff in request.FILES.getlist(input_name):
        ...
        saved_file = upload_file(ff, filename, parent_directory)
        if saved_file:
...
def upload_file(file_instance, specified_filename=None, parent_directory=None):
    """
    ...
    try:
        ...
        message_service.post_error(f"Error saving file: {original_file_name}")
        log.error(f"Error uploading file: {str(ee)}")
...
def build_s3_path(original_file_name, parent_directory, specified_filename):
    ...
    if specified_filename:
        file_path = os.path.join(file_path, f"{specified_filename}{given_extension}")
    else:
