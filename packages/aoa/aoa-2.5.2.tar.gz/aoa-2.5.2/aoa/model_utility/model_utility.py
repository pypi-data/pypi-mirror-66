...
class ModelUtility(object):
    ...
    def __init__(self, base_path: str, model_template_path: str = None):
        ...
        self.model_template_path = os.path.join(model_template_path, '') if model_template_path is not None else None
        self.logger = logging.getLogger(__name__)

    def add_model(self, model_name: str, model_desc: str, lang: str,
                  template: str, base_path: str = None, model_template_path: str = None):
        ...
        model_template_path = self.model_template_path if model_template_path is None \
            else os.path.join(model_template_path, '')
        ...
        self.__create_model_structure(model_path, model_template_path)
    ...
    def init_model_directory(self, path: str = None):
        ...
        if not os.path.isdir(path + "model_templates/"):
            self.logger.info('creating model templates')
            shutil.copytree(os.path.join(os.path.split(__file__)[0], '') + 'model_templates',
                            path + 'model_templates')
        else:
            self.logger.info('model templates already exists...skipping')
    ...
    def __create_model_structure(self, model_path, model_template_path: str = None):
        model_dir = model_path + self.model["id"]
        if model_template_path is None:
            template_dir = self.template_catalog + self.model["language"] + "/" + self.model_template
        else:
            template_dir = model_template_path + self.model["language"] + "/" + self.model_template
    ...
    def get_all_template_catalog(self):
        ...
        if self.model_template_path is None:
            pass
        else:
            custom_cat = self.__get_template_catalog(self.model_template_path)
