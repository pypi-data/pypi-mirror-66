...
class AutoMLConfig(object):
    ...
    DEFAULT_EXPERIMENT_TIMEOUT = 7 * 24 * 60
    ...
    def __init__(self,
        ...
        self.user_settings["experiment_timeout_minutes"] = int(experiment_timeout_hours * 60) \
            if experiment_timeout_hours is not None else AutoMLConfig.DEFAULT_EXPERIMENT_TIMEOUT
    ...
    def _validate_config_settings(self, workspace: Optional[Workspace] = None) -> None:
            ...
            if value is not None:
                if key != 'cv_splits_indices':
                    ...
                    if not isinstance(cv_splits_indices, list):
                        raise ConfigException.create_without_pii("cv_splits_indices should be a list")
        ...
        if has_single_dataset_input:
            if has_feature_label_input:
                raise ConfigException.create_without_pii(
                    "The input data can either be combined (Features & Label column) or separate Features dataset "
                    "and Label column.")
