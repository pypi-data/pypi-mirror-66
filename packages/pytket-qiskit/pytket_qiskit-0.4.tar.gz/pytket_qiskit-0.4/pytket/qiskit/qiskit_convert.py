...
import qiskit.extensions.standard as qiskit_gates
...
_known_qiskit_gate = {
    qiskit_gates.IdGate: OpType.noop,
    ...
    qiskit_gates.Barrier: OpType.Barrier,
...
def append_tk_command_to_qiskit(op, args, qcirc, qregmap, cregmap) -> Instruction:
    optype = op.type
    if optype == OpType.Measure:
        ...
        qargs = [qregmap[q.reg_name][q.index[0]] for q in args]
        g = qiskit_gates.Barrier(len(args))
        return qcirc.append(g, qargs=qargs)
        ...
        instruction = append_tk_command_to_qiskit(
            op.op, args[width:], qcirc, qregmap, cregmap
        )
        ...
        qargs = [qregmap[q.reg_name][q.index[0]] for q in args]
        params = [p * pi for p in op.params]
        g = gatetype(*params)
        return qcirc.append(g, qargs=qargs)
...
def tk_to_qiskit(tkcirc: Circuit) -> QuantumCircuit:
    ...
    for reg_name, size in creg_sizes.items():
        ...
        qcirc.add_register(qis_reg)
    for command in tkc:
        append_tk_command_to_qiskit(command.op, command.args, qcirc, qregmap, cregmap)
    return qcirc
