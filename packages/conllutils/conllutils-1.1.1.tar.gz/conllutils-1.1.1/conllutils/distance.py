...
def _treedist(i, j, l1, l2, nodes1, nodes2, TD, TD_oprs, cost, return_oprs):
    ...
    for x in range(1, n):
        ...
        if return_oprs:
            _merge(x, 0, x-1, 0, [(DEL, nodes1[x+i_off].index)])

    for y in range(1, m):
        d[0, y] = d[0, y-1] + cost(None, nodes2[y+j_off], INS) # insert
        if return_oprs:
            _merge(0, y, 0, y-1, [(INS, nodes2[y+j_off].index)])
    ...
    for x in range(1, n):
        for y in range(1, m):
            ...
            if l1[i] == l1[xi] and l2[j] == l2[yj]:
                ...
                if return_oprs:
                    if min_cost == costs[0]:
                        _merge(x, y, x-1, y, [(DEL, nodes1[xi].index)])
                    elif min_cost == costs[1]:
                        _merge(x, y, x, y-1, [(INS, nodes2[yj].index)])
                    else:
                        opr = [(SUB, nodes1[xi].index, nodes2[yj].index)] if d[x, y] != d[x-1, y-1] else []
...
def tree_edit_distance(t1, t2, cost=_default_node_cost, normalize=False, return_oprs=False):
    ...
    if n == 0 and m == 0:
        ...
        if return_oprs:
            oprs = [(DEL, nodes1[i].index) for i in range(n)]
    elif n == 0 and m != 0:
        dist = sum(cost(None, node, INS) for node in nodes2)
        if return_oprs:
            oprs = [(INS, nodes2[j].index) for j in range(m)]
    else:
