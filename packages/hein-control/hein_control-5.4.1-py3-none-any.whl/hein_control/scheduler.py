...
from .sequencing import ActionList
...
class SamplingScheduler(object):
    ...
    # class-level list of default actions
    default_actions: ActionList = ActionList()
    ...
    def __init__(self,
        ...
        if len(actions) > 0:
            # todo consider whether this should override or extend
            self.instance_default_actions: ActionList = ActionList(*actions)
        else:
            # todo consider how to extend default list
            self.instance_default_actions: ActionList = copy.copy(self.default_actions)
    ...
    def _determine_actions(self,
        ...
        if additional_actions is not None:
            actions = copy.copy(self.instance_default_actions)
            actions.extend(additional_actions)
        elif specified_actions is not None:
            actions = ActionList(*specified_actions)
        else:
            actions = copy.copy(self.instance_default_actions)
        return actions
    ...
    def as_dict(self) -> dict:
        ...
        out = {
            ...
            'instance_default_actions': [action.name for action in self.instance_default_actions],
            'class_default_actions': [action.name for action in self.default_actions],
