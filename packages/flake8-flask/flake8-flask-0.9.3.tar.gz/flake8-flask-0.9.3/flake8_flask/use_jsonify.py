...
class JsonifyVisitor(FlaskBaseVisitor):
    ...
    def visit_Return(self, return_node: ast.Return):
        ...
        self.report_nodes.append(
            {
                "node": return_node,
                "message": f"{self.name} json.dumps() shouldn’t be used as a return value from Flask routes. Use flask.jsonify() instead, which returns a Response object with the application/json mimetype set."
            }
        )
