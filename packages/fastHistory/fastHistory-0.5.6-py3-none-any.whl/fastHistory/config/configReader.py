...
class ConfigReader:
    ...
    def __init__(self, path_data_folder, path_code_folder, is_from_installer, config_file, version_file):
        ...
        self.path_version_file = path_data_folder + version_file
        self.path_default_version_file = path_code_folder + "/config/default_" + version_file
        self.skip_bash_checks = is_from_installer
    ...
    def check_config(self):
        ...
        if self.path_data_folder is None:
            ...
            version_file_data = self._get_content_version_file(self.path_version_file)
            version_file_code = self._get_content_version_file(self.path_default_version_file)
            if version_file_data is None:
                self._checkError = [False, "data version file cannot be read"]
            elif version_file_data != version_file_code:
                self._checkError = [True, "fastHistory was updated, data folder needs to be updated (%s -> %s)" %
                                    (version_file_data, version_file_code)]
            ...
            elif not self.skip_bash_checks and str(os.environ[self._BASH_VAR_VERSION]) != version_file_code:
                self._checkError = [None, "Please restart your terminal! old bash is still used (%s -> %s)" %
                                    (os.environ[self._BASH_VAR_VERSION], version_file_code)]
            else:
                try:
