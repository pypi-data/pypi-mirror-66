...
def localNightlyHelp(prog, err, project, platform, args, error=True):
    ...
    try:
        ...
        all_slots = set(
            ...
            for entry in load(
                urlopen(
                    COUCHDB_ROOT + "_design/names/_view/slots?group=true", timeout=5
                )
            ).get("rows", [])
        )
        ...
        try:
            slot_info = load(
                urlopen("{}{}.{}".format(COUCHDB_ROOT, err.slot, build_id), timeout=5)
            )
        except HTTPError:
...
class LbRun(xenv.Script):
    ...
    def _makeEnv(self):
        ...
        try:
            if self.project.lower() == "lhcbdirac":
                ...
                from LbEnv.ProjectEnv.LHCbDirac import (
                    getLHCbDiracEnv,
                    LHCbDiracEnvError,
                )

                # different convention between lb-run and lhcbdirac
                for item in getLHCbDiracEnv(
                    "pro" if self.version == "prod" else self.version
                ).items():
    ...
    def runCmd(self):
        if self.opts.container == "singularity":
            ...
            wrapper = ["singularity", "exec", "--bind", "/cvmfs", "--userns"]
