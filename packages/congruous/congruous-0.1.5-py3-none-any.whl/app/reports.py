...
def match_records(document, file_name, hcd_records, ocrd_records, report):
	...
	if document == "pan":
		...
		try:
			...
			None if report else echo_all_fields() 
...
def generate_match_report(document):
	...
	# Set piechart
	pdf.drawInlineImage(chart_image, 100, 25)
...
def show_pie_charts(document):
	...
	if document == 'pan':
		...
		img = Image.open(cong_report[document]['report_id'] + '.png')

		# respective sizes for width and height
		width = 400
		height = 570
		
		img.resize((width, height), Image.ANTIALIAS)
		resized_image = img.resize((width, height), Image.ANTIALIAS)
		os.remove(cong_report[document]['report_id'] + '.png')
		resized_image.save(cong_report[document]['report_id'] + '.png', "PNG", quality=90)
