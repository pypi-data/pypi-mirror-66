...
def stations_cli(main, galileo: GalileoSdk):
    ...
    def ls(index, id, name, mid, user_role, volume, description, page, items):
        ...
        if isinstance(index, int):
            stations_list = r["stations"][index]
        else:
            stations_list = r["stations"]
    ...
    def create(name, description, userid):
        ...
        station_df = pandas.json_normalize(station)
    ...
    @click.option(
        ...
        required=True,
        help="Machines to add to station.",
    )
    ...
    def add_volume(id, name, mountpath, access):
        ...
        print(
            pandas.json_normalize(
                galileo.stations.add_volumes_to_station(id, name, mountpath, access)[
                    "volumes"
                ]
            )
        )
    ...
    def add_hostpath(id, vid, mid, hostpath):
        ...
        print(
            pandas.json_normalize(
                galileo.stations.add_host_path_to_volume(id, vid, mid, hostpath)[
                    "volume"
                ]
            )
        )
