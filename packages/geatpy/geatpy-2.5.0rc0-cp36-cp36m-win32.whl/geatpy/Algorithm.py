...
"""
    ...
    def call_aimFunc(self, pop):
        ...
        self.evalsNum += pop.sizes # 更新评价次数
        # 检测非法值
...
class MoeaAlgorithm(Algorithm): # 多目标优化算法模板父类
    ...
    def stat(self, population): # 分析记录，更新进化记录器，population为传入的种群对象
        feasible = np.where(np.all(population.CV <= 0, 1))[0] if population.CV is not None else np.array(range(population.sizes)) # 找到可行解个体的下标
        if len(feasible) > 0:
