...
from momotor.rpc.exception import raise_message_exception, RPCException, AuthException, StreamTerminatedError, GRPCError
...
retry_connection.__doc__ = """
    ...
    * :py:class:`momotor.rpc.exception.GRPCError`
...
def version_check(server_info: "ServerInfoResponse"):
...
async def _retry_message(method, request, timeout=None):
    try:
        logger.debug(f"RPC call {method.name} {format_msg(request)}")
        response = await asyncio.wait_for(method(request), timeout=timeout)
        logger.debug(f"RPC call {method.name} {format_msg(request)} response: {format_msg(response)}")
        ...
        logger.error(f"RPC call {method.name} {format_msg(request)} timeout")
        raise

    except AuthException:
        logger.error(f"RPC call {method.name} {format_msg(request)} authentication error")
        raise

    except RPCException as exc:
        logger.error(f"RPC call {method.name} {format_msg(request)} failed: {exc}")
        raise

    except (ConnectionError, StreamTerminatedError, UnexpectedEndOfStream) as exc:
        logger.error(f"RPC call {method.name} {format_msg(request)} connection interrupted: {exc}")
        raise

    except Exception:
        logger.exception(f"RPC call {method.name} {format_msg(request)} failed")
        raise
...
class BrokerConnection:
    ...
    async def _connect(self):
        async with self._connect_lock:
            while not self._channel:
                logger.debug("connecting to broker for shared channel")
    ...
    async def _disconnect(self, reset):
        ...
        if reset:
            logger.debug('deleting cached auth-token')
    ...
    async def get_stub(self, private_channel=False):
        ...
        if not private_channel:
            ...
            logger.debug("connecting to broker for private channel")
    ...
    async def _get_server_info(self) -> "ServerInfoResponse":
        logger.debug("retrieving serverInfo")
    ...
    async def multi_job_status_stream(self, *,
        ...
        async with self.get_stub(private_channel) as stub:
            logger.debug("RPC call multiJobStatusStream")
            async with stub.multiJobStatusStream.open(timeout=connect_timeout) as stream:
                try:
                    ...
                    logger.error(f"RPC call multiJobStatusStream (send) connection interrupted: {exc}")
                    connected = False
                ...
                while connected and stub == self._client_stub:
                    try:
                        try:
                            ...
                            logger.debug(f"RPC call multiJobStatusStream recv timeout")
                            connected = False
                        ...
                        logger.error(f"RPC call multiJobStatusStream authentication error")
                        await self.disconnect(True)
                        connected = False

                    except asyncio.CancelledError:
                        logger.debug(f"RPC call multiJobStatusStream cancelled")
                        connected = False

                    except (ConnectionError, StreamTerminatedError, UnexpectedEndOfStream) as exc:
                        logger.error(f"RPC call multiJobStatusStream (recv) connection interrupted: {exc}")
                        connected = False

                try:
                    logger.debug(f"RPC call multiJobStatusStream cancel stream")
                    await stream.cancel()
                    logger.debug(f"RPC call multiJobStatusStream cancel stream done")
                except Exception as exc:
                    logger.debug(f"RPC call multiJobStatusStream cancel stream failed: {exc}")

            logger.debug("RPC call multiJobStatusStream done")
