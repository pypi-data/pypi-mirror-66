...
matrix_cutoff = 0.22    # the cutoff for matrix comparison
...
def test_knotnet_directly():
    ...
    if not check_cuda():
        print("No CUDA deteted. Skipping.")
        return
...
def test_matrix_format():
    ...
    conway('data/knots/1j85.pdb', matrix=True, closure=Closure.CLOSED, output_format=OutputFormat.KnotProt,
...
def test_matrix_plot_cutoff():
    ...
    for cutoff in cutoffs:
        ...
        conway('data/knots/1j85.pdb', tries=10, plot_ofile=ofile, matrix_plot=True, matrix_cutoff=cutoff, translate=True)
...
def test_matrix_plot_polynomials():
    print("Testing matrix plotting with different polynomials.")
    times = {}
    ...
    for algorithm in algorithms.keys():
        ...
        algorithms[algorithm]('data/knots/1j85.pdb', tries=3, plot_ofile=ofile, matrix_plot=True, translate=True)
        times[algorithm] = time.time() - t0
        time.sleep(7)   # some multiprocessing bug!
    print("Times:")
    for algorithm in algorithms.keys():
        assert os.path.getsize('matrix_' + algorithm.replace('/', '') + '.png') > 0
        print(algorithm, times[algorithm])
    print("========\n")
    return
...
def test_matrix_plot_formats():
    print("Testing matrix plotting formats.")
    ...
    for plot_format in dir(PlotFormat):
        ...
        print(plot_format)
        ofile = '1j85_matrix'
        plot_matrix(data, plot_format=plot_format, plot_ofile=ofile)
...
def test_gln_matrix():
    ...
    for code in gln_codes.keys():
        ...
        gln(f, chain1_boundary=bridges[0], chain2_boundary=bridges[1], matrix_plot=True, matrix_plot_fname=plot_ofile)
...
def test_plamkas():
    ...
    result = alexander('data/composite.xyz', matrix=True, closure=Closure.CLOSED, translate=True, cuda=False,
                       matrix_plot=True, plot_ofile=spots_ofile)
