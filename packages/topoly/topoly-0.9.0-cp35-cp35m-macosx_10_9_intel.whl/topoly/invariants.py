...
from topoly.params import Closure, ReduceMethod, PlotFormat, TopolyException, OutputFormat, Colors, GlnMode
...
class Invariant(Graph):
    ...
    def find_additional(self, matrix_results, density=1, level=0, debug=False):
        additional = []
        central_points = []
...
class GlnGraph(Graph):
    ...
    def calculate(self, chain2=None, boundary=(-1, -1), boundary2=(-1, -1), mode=GlnMode.BASIC,
        ...
        if mode == GlnMode.MAX:
            ...
            result = {'whole chains': [], 'subchain of chain 2': [], 'subchain of chain 1': [], 'local maximum': []}
            current = ''
            while value:
                element = value.pop()
                if element == 'wh:':
                    current = 'whole chains'
                elif element == 'max_wh_comp1:':
                    current = 'subchain of chain 2'
                elif element == 'max_wh_comp2:':
                    current = 'subchain of chain 1'
        ...
        elif mode == GlnMode.AVG:
            if debug:
                print("Calculating the average GLN value on " + str(avg_tries) + " tries.")
            return round(c_gln_average(arc1, arc2, boundary[0], boundary[1], boundary2[0], boundary2[1], avg_tries),
                         precision)

        elif mode == GlnMode.MATRIX:
            if debug:
