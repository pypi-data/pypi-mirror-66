...
class Graph:
    ...
    def tmp_create_code(self, yamada_mode):
        import subprocess
        import os
        files = []
        indices = []
        for k in range(len(self.arcs)):
            ...
            arc = self.arcs[k]
            for atom in arc:
                coords = ' '.join([str(x) for x in self.coordinates[atom]])
                if coords in indices:
                    ind = indices.index(coords)
                else:
                    ind = len(indices)
                    indices.append(coords)
                chain += str(ind) + ' ' + coords + '\n'
            files.append('_arc' + str(k))
            with open(files[-1], 'w') as myfile:
                myfile.write(chain)
        command = ['../build/homflylink', '-i'] + files
        if yamada_mode:
            command.append('-y')
    ...
    def generate_em_from_coords(self):
        ...
        if all([termini[key] >= 2 for key in list(termini.keys())]):
            yamada_mode = True
        if pawels_mac:
            code = self.tmp_create_code(yamada_mode)
        else:
            if yamada_mode:
    ...
    def generate_orientation(self):
        ...
        checked = []
        tries = 0
        while len(checked) < edges_num and tries < 100:
            tries += 1
            for vert in self.vertices:
            ...
            while path and cross and len(checked) + len(path) < edges_num:
                ...
                for c in self.crossings:
                    ...
                    if path[-1] in c and c[c.index(path[-1]) - 2] == path[0] and c != cross:
                        b = True
                        break
                if b or path[-1] in vertex_edges and path[0] in vertex_edges:
                    b = True
                    break
                if b:
                    break
        ...
        if tries == 100:
            print('Problem running generate orientations!!!! Reached 50000 tries, so far found edges: ' + str(len(checked)) + '/' + str(edges_num))
    ...
    def find_links_abstract(self, output_type=OutputType.PDcode):
        ...
        loops = []
        for v in vertices:
            for path in find_path(v, v, self.abstract_graph):
                edges = set(path)
                if edges in found:
                    continue
                loops.append(path)
        ...
        for v1, v2 in combinations(vertices, 2):
            for arc1, arc2 in combinations([path for path in find_path(v1, v2, self.abstract_graph)], 2):
                loop = arc1 + list(reversed(arc2))
                if not set(arc1) & set(arc2) and set(loop) not in found:
                    loops.append(loop)
                    found.append(set(loop))
        found = []
        for loop1, loop2 in combinations(loops, 2):
            link = set(loop1 + loop2)
            if set(loop1) & set(loop2) or link in found:
                continue
            g = Graph(self.generate_code_from_edges(loop1 + loop2))
            g.closed = True
            g.parse_closed()
            g.identifier = (cut_arc(loop1), cut_arc(loop2))
            result = g.print_data(output_type=output_type)
            found.append(link)
            del g
            yield result

    def find_links_coords(self, output_type=OutputType.PDcode):
        vertices = list(self.spatial_graph.keys())
        found = []
        loops = []
        for v in vertices:
            for path in find_path(v, v, self.spatial_graph):
                edges = set(path)
                if edges in found:
                    continue
                loops.append(path)
                found.append(edges)
        for v1, v2 in combinations(vertices, 2):
            for arc1, arc2 in combinations([path for path in find_path(v1, v2, self.spatial_graph)], 2):
                arc1 = [v1] + arc1[:-1]
                loop = arc1 + list(reversed(arc2))
                if not set(arc1) & set(arc2) and set(loop) not in found:
                    loops.append(loop)
        ...
        for loop1, loop2 in combinations(loops, 2):
            ...
            if set(loop1) & set(loop2) or link in found:
                continue
            g = Graph(self.generate_coords_from_atom_list([loop1, loop2]))
            g.closed = True
            g.parse_closed()
            g.identifier = (cut_arc(loop1), cut_arc(loop2))
            result = g.print_data(output_type=output_type)
    ...
    def parse_substructure(self, arcs, abstract=False, output_type=OutputType.TOPOLOGY, output='list',
                           file_prefix='tmp', folder_prefix='', loop_nr=0, arc_bridges=1):
        if abstract:
        ...
        del g
        if output == 'file':
            fname = folder_prefix + '/' + file_prefix + '_' + str(loop_nr)
    ...
    def find_loops(self, output_type=OutputType.TOPOLOGY, arc_bridges=1, output='list', file_prefix='loop',
                        folder_prefix=''):
        ...
        for v in vertices:
            for path in find_path(v, v, graph):
                ...
                if edges in found:
                    continue
                result = self.parse_substructure([path], abstract=abstract, output_type=output_type, output=output,
                                                 file_prefix=file_prefix, folder_prefix=folder_prefix,
                                                 loop_nr=len(loops), arc_bridges=arc_bridges)
                if result:
                    loops.append(result)
                    found.append(edges)
                    # if output == 'generator':
        ...
        for v1, v2 in combinations(vertices, 2):
            for arc1, arc2 in combinations([path for path in find_path(v1, v2, graph)], 2):
                if not abstract:
                    arc1 = [v1] + arc1[:-1]
                loop = arc1 + list(reversed(arc2))
                if set(arc1) & set(arc2) or set(loop) in found:
                    continue
                result = self.parse_substructure([loop], abstract=abstract, output_type=output_type, output=output,
                                                 file_prefix=file_prefix, folder_prefix=folder_prefix,
                                                 loop_nr=len(loops), arc_bridges=arc_bridges)
                if result:
                    loops.append(result)
    ...
    def find_links(self, output_type=OutputType.TOPOLOGY, arc_bridges=1, output='list', file_prefix='loop',
                   folder_prefix='', components=2):
        if self.coordinates:
            abstract = False
        ...
        for pot_link in combinations(self.find_loops(output_type=o_type, arc_bridges=arc_bridges, output='list'),
                                     components):
            ...
            if abstract:
                ...
                arcs = list(pot_link)
            edges = [edge for arc in arcs for edge in arc]
            result = self.parse_substructure(arcs, abstract=abstract, output_type=output_type, output=output,
                                             file_prefix=file_prefix, folder_prefix=folder_prefix,
                                             loop_nr=len(links), arc_bridges=0)
            if result:
                links.append(result)
                found.append(edges)
        return links

    def find_handcuffs(self, output_type=OutputType.TOPOLOGY, arc_bridges=1, output='list', file_prefix='loop',
                       folder_prefix=''):
        if self.coordinates:
            abstract = False
            o_type = OutputType.ATOMS
        ...
        for pot_hand in combinations(self.find_loops(output_type=o_type, arc_bridges=arc_bridges, output='list'), 2):
            if abstract:
                arcs = [list(set([int(_) for _ in re.sub('[VX\[\]]', '', component).replace(';', ',').split(',')]))
                        for component in pot_hand]
            else:
                arcs = list(pot_hand)
            for path in self.find_loop_connections(arcs, abstract):
                arcs.append(path)
                edges = [edge for arc in arcs for edge in arc]
                result = self.parse_substructure(arcs, abstract=abstract, output_type=output_type, output=output,
                                                 file_prefix=file_prefix, folder_prefix=folder_prefix,
                                                 loop_nr=len(handcuffs), arc_bridges=0)
                if result:
                    handcuffs.append(result)
                    found.append(edges)
        return handcuffs

    def find_thetas(self, output_type=OutputType.TOPOLOGY, arc_bridges=1, output='list', file_prefix='loop',
                    folder_prefix=''):
        if not self.coordinates:    # abstract code
            graph, abstract = self.abstract_graph, True
            if output_type not in [OutputType.PDcode, OutputType.EMcode]:
        ...
        for v1, v2 in combinations(vertices, 2):
            for arc1, arc2, arc3 in combinations([path for path in find_path(v1, v2, graph)], 3):
                ...
                edges = set(arc1 + arc2 + arc3)
                arcs = [arc1, arc2, arc3]
                verts = {v1, v2}
                if set(edges) not in found and set(arc1) & set(arc2) <= verts and set(arc2) & set(arc3) <= verts and\
                        set(arc1) & set(arc3) <= verts:
                    result = self.parse_substructure(arcs, abstract=abstract, output_type=output_type, output=output,
                                                     file_prefix=file_prefix, folder_prefix=folder_prefix,
                                                     loop_nr=len(thetas), arc_bridges=0)
                    if result:
                        thetas.append(result)
                        found.append(edges)
        return thetas

    def find_loops_abstract(self, output_type=OutputType.PDcode):
        vertices = list(self.abstract_graph.keys())
        found = []
        for v in vertices:
            for path in find_path(v, v, self.abstract_graph):
                ...
                if edges in found:
                    continue
                g = Graph(self.generate_code_from_edges(path))
                g.identifier = (cut_arc(path))
                result = g.print_data(output_type=output_type)
                found.append(edges)
                del g
                yield result
        for v1, v2 in combinations(vertices, 2):
            for arc1, arc2 in combinations([path for path in find_path(v1, v2, self.abstract_graph)], 2):
                loop = arc1 + list(reversed(arc2))
                if not set(arc1) & set(arc2) and set(loop) not in found:
                    g = Graph(self.generate_code_from_edges(loop))
                    g.identifier = (cut_arc(arc1 + [v2]), cut_arc([v1] + arc2))
                    result = g.print_data(output_type=output_type)
                    found.append(set(loop))
                    del g
                    yield result

    def find_loops_coords(self, output_type=OutputType.PDcode):
        vertices = list(self.spatial_graph.keys())
        found = []
        for v in vertices:
            for path in find_path(v, v, self.spatial_graph):
                ...
                if edges in found:
                    continue
                g = Graph(self.generate_coords_from_atom_list([path]))
                g.closed = True
                g.parse_closed()
                g.identifier = (cut_arc(path))
                result = g.print_data(output_type=output_type)
        ...
        for v1, v2 in combinations(vertices, 2):
            for arc1, arc2 in combinations([path for path in find_path(v1, v2, self.spatial_graph)], 2):
                ...
                loop = arc1 + list(reversed(arc2))
                if not set(arc1) & set(arc2) and set(loop) not in found:
                    g = Graph(self.generate_coords_from_atom_list([loop]))
                    g.identifier = (cut_arc(arc1 + [v2]), cut_arc([v1] + arc2))
                    g.close(method=Closure.CLOSED)
                    g.parse_closed()
                    result = g.print_data(output_type=output_type)
    ...
    def find_thetas_abstract(self, output_type=OutputType.PDcode):
        if output_type != OutputType.PDcode:
            raise TopolyException("Cannot generate the coordinates! (yet)")
        vertices = list(self.abstract_graph.keys())
        found = []
        for v1, v2 in combinations(vertices, 2):
            for arc1, arc2, arc3 in combinations([path for path in find_path(v1, v2, self.abstract_graph)], 3):
                theta = [item for arc in [arc1, arc2, arc3] for item in arc]
                if set(theta) not in found and len(set(arc1) | set(arc2) | set(arc3)) == len(arc1) + len(arc2) + len(arc3):
                    g = Graph(self.generate_code_from_edges(theta))
                    g.identifier = (cut_arc(arc1), cut_arc(arc2), cut_arc(arc3))
                    result = g.print_data(output_type=output_type)
                    found.append(set(theta))
                    del g
                    yield result
        return

    def find_thetas_coords(self, output_type=OutputType.PDcode):
        vertices = list(self.spatial_graph.keys())
        found = []
        for v1, v2 in combinations(vertices, 2):
            for arc1, arc2, arc3 in combinations([path for path in find_path(v1, v2, self.spatial_graph)], 3):
                arc1 = [v1] + arc1
                arc2 = [v1] + arc2
                arc3 = [v1] + arc3
                theta = [arc1, arc2, arc3]
                if set(arc1 + arc2 + arc3) not in found and len(set(arc1[1:-1]) | set(arc2[1:-1]) |
                                            set(arc3[1:-1])) == len(arc1[1:-1]) + len(arc2[1:-1]) + len(arc3[1:-1]):
                    g = Graph(self.generate_coords_from_atom_list(theta))
                    g.identifier = (cut_arc(arc1), cut_arc(arc2), cut_arc(arc3))
                    result = g.print_data(output_type=output_type)
                    found.append(set(arc1 + arc2 + arc3))
                    del g
                    yield result

    def find_handcuffs_abstract(self, output_type=OutputType.PDcode):
        if output_type != OutputType.PDcode:
            raise TopolyException("Cannot generate the coordinates! (yet)")
        vertices = list(self.abstract_graph.keys())
        found = []
        for v1, v2 in combinations(vertices, 2):
            for path in [path for path in find_path(v1, v2, self.abstract_graph)]:
                for loop1 in [path for path in find_path(v1, v1, self.abstract_graph)]:
                    for loop2 in [path for path in find_path(v2, v2, self.abstract_graph)]:
                        handcuff = [item for arc in [loop1, path, loop2] for item in arc]
                        if set(handcuff) not in found and len(set(loop1) | set(loop2) | set(path)) == len(loop1) + len(path) + len(loop2):
                            g = Graph(self.generate_code_from_edges(handcuff))
                            g.identifier = (cut_arc(loop1), cut_arc(path), cut_arc(loop2))
                            result = g.print_data(output_type=output_type)
                            found.append(set(handcuff))
                            del g
                            yield result
        return

    def find_handcuffs_coords(self, output_type=OutputType.PDcode):
        vertices = list(self.spatial_graph.keys())
        found = []
        for v1, v2 in combinations(vertices, 2):
            for path in [path for path in find_path(v1, v2, self.spatial_graph)]:
                path = [v1] + path
                for loop1 in [path for path in find_path(v1, v1, self.spatial_graph)]:
                    loop1 = [v1] + loop1
                    for loop2 in [path for path in find_path(v2, v2, self.spatial_graph)]:
                        loop2 = [v2] + loop2
                        handcuff = [loop1, path, loop2]
                        if set(loop1 + path + loop2) not in found and len(set(loop1[1:-1]) | set(path[1:-1]) |
                                                    set(loop2[1:-1])) == len(loop1[1:-1]) + len(path[1:-1]) + len(loop2[1:-1]):
                            g = Graph(self.generate_coords_from_atom_list(handcuff))
                            g.identifier = (cut_arc(loop1), cut_arc(path), cut_arc(loop2))
                            result = g.print_data(output_type=output_type)
                            found.append(set(loop1 + path + loop2))
                            del g
                            yield result
    ...
    ''' simplification'''
    def reduce(self, method=ReduceMethod.KMT, steps=1000, debug=False):
    ...
    while fringe:
        ...
        for next_state, edge_list in graph[state]:
            if edge_list and any([edge in path for edge in edge_list]):
                continue
            if next_state == start and len(path) == 1:
                continue
