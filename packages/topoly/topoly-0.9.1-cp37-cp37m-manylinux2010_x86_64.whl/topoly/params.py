...
class GlnMode:
    """
    Modes for GLN calculations in gln() function.
    """
    BASIC = 0
    """ GLN value between two whole chains is calculated. """
    MAX = 1
    """ Additionally some info about the maximal absolute GLN value 
    between fragments of two chains is calculated.
        
    The dictionary with the maximal absolute GLN value between chain1 
    and pieces of chain2 (with indices) and vice versa is returned. In 
    the dictionary there is also GLN value between whole chains and 
    local maximum.

    Local maximum is calculated when another argument of function gln() 
    -- density_for_max (d) -- is larger than 0 (by default is -1) and 
    maximal absolute GLN value between all pairs of fragments of two
    chains that have lengths being a multiplication of d is calculated.
    Thus for d=1 all pairs of fragments are analyzed. For longer chains 
    d=1 is highly unrecommended due to high time and space complexity 
    of computations ( O(N^4/d^4) ).
    
    Further arguments of function gln() used in this mode: density_for_max. 
    """
    AVG = 2
    """ Additionally the average GLN value after number of random 
    closures of both chains is calculated. 

    Further arguments of function gln() used in this mode: avg_tries. 
    """
    MATRIX = 3
    """ Whole GLN matrix between chain1 and all subchains of chain2 is 
    calculated. As a result the matrix (M) of the size of chain2_length 
    x chain2_length is returned, and M[i][j] is GLN value between chain1
    and fragment of chain2 from i-th to j-th atom.
    
    Further arguments of function gln() used in this mode: 
    matrix_plot_format, matrix_plot_fname, matrix_output_format, 
    matrix_output_fname. 
    """
...
class OutputType:
    ...
    IDENT = 'ident'
    """ indexes of first and last atom of each arc of the structure """
    TOPOLOGY = 'topology'
    """ identified topology type """
    ATOMS = 'atoms'
    """  """
...
class Colors:
    ...
    """
    Knots = {'name': 'knot', '51': 'Reds', '61': 'Blues', 
             '31': 'Greens', '52': 'Purples', '41': 'Oranges'}
