...
class Lasso(Invariant):
    ...
    def __init__(self, coordinates, loop_indices):
        if loop_indices:
            ...
            super().__init__(coordinates, bridges_type=Bridges.SSBOND)
#            self.loop_indices = 
    ...
    def lasso_type(self, smoothing=0, precision=0, dens=1,
                   ...
                   output_prefix='', output_type=OutputTypeLasso.MainType, control_dist=0):
        ...
        output_type_to_cpp = -1 if output_type==OutputTypeLasso.MainType else 1
        resD = {}
        if (len(self.loop_indices) == 0): return resD
