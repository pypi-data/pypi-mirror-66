...
from topoly.params import DensitySurface, PrecisionSurface, OutputTypeLasso, SurfacePlotFormat
import pytest
import os
...
lassos = {
    'curve1': {'data': 'data/lassos/lasso_01.xyz',
               ...
               'surface': {'VMD': {(1, 35): 'data/lassos/surface_lasso_01.xyz_1_35_VMD.tcl',
                                   (5, 50): 'data/lassos/surface_lasso_01.xyz_5_50_VMD.tcl'},
                           'JSMOL': {(1, 35): 'data/lassos/surface_lasso_01.xyz_1_35_JSMOL.jms',
                                     (5, 50): 'data/lassos/surface_lasso_01.xyz_5_50_JSMOL.jms'},
                           'MATHEMATICA': {(1, 35): 'data/lassos/surface_lasso_01.xyz_1_35_MATHEMATICA.m',
                                           (5, 50): 'data/lassos/surface_lasso_01.xyz_5_50_MATHEMATICA.m'}},
...
def add_lasso(f, bridges):
    ...
    lasso_data['detailed'] = lasso_type(f, bridges, output_type=OutputTypeLasso.MoreInfo)
...
def prepare_lassos_basic():
    ...
    for lasso in lassos.keys():
        ...
        print(lassos[lasso]['data'], lassos[lasso]['bridges'], lasso_data[lasso])
    return lasso_data
    ...
    for lasso in lassos.keys():
        ...
        for reduction in reduction_pattern:
            lasso_data[lasso][reduction] = lasso_type(lassos[lasso]['data'], lassos[lasso]['bridges'],
                                                      min_dist=reduction)
            print(lassos[lasso]['data'], lassos[lasso]['bridges'], reduction, lasso_data[lasso][reduction])
    return lasso_data
    ...
    for lasso in lassos.keys():
        ...
        for s_value in smooth_parameters:
            lasso_data[lasso][s_value] = lasso_type(lassos[lasso]['data'], lassos[lasso]['bridges'],
                                                    smooth=s_value)
            print(lassos[lasso]['data'], lassos[lasso]['bridges'], s_value, lasso_data[lasso][s_value])
    return lasso_data
...
def test_lasso_identify_bridges():
    print("Testing the bridges identification")
...
def test_lasso_basic():
    print("Testing the lasso basic output")
...
def test_lasso_detailed():
    print("Testing the lasso detailed output")
...
def test_lasso_reduction():
    print("Testing the lasso output dependence on reduction")
...
def test_lasso_surface_files():
    print("Testing the creation of the lasso surface files")
    lasso_data = prepare_lassos_surfaces()
    for lasso in lassos.keys():
        name = lassos[lasso]['data'].split('/')[-1]
        for bridge in lassos[lasso]['bridges']:
            for plot_type in surface_plot_formats:
...
def test_lasso_smoothing():
    print("Testing the lasso smoothing")
...
def test_make_surfaces():
    print("Testing the creation of the lasso surface files with make_surface")
