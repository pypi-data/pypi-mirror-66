...
class DelUserDialog(QDialog):
    ...
    def all_users_fill(self):
        ...
        self.selector.addItems([i[0] for i in self.db.users_list()])

    def remove_user(self):
        """Метод - обработчик удаления пользователя."""
        self.db.remove_user(self.selector.currentText())
        if self.selector.currentText() in self.server.names.keys():
            client = self.server.names[self.selector.currentText()]
