...
OptionParser.version_template = '%%prog %s (Docutils %s%s, Python %s, on %s)' % (
    ...
    docutils.__version_details__ and ' [%s]' % docutils.__version_details__
    or '', sys.version.split()[0], sys.platform)
...
class WrapTopTitle(Transform):
    ...
    def apply(self):
        ...
        for child in self.document.children:
            if not (isinstance(child, nodes.target)
                    and child.get('refid') in section['ids']):
...
class HTML5Writer(writers.Writer):
    ...
    settings_spec = (
        ...
        'For the rst2html5 writer the default tab width value is 4.', (
            ...
            ('Specify a stylesheet URL to be included in the output HTML file. '
             '(This option can be used multiple times)', ['--stylesheet'], {
                 'metavar': '<URL or path>',
                 'default': None,
                 'action': 'append',
             }),
            ...
            ('Specify a script URL to be included in the output HTML file. '
             '(This option can be used multiple times)', ['--script'], {
                 'metavar': '<URL or path>',
                 'default': None,
                 'dest': 'script',
                 'type': 'string',
            ...
            ('Specify a script URL with a defer attribute '
             ...
             '(This option can be used multiple times)', ['--script-defer'], {
                 'metavar': '<URL or path>',
                 'dest': 'script',
                 'type': 'string',
                 'action': 'callback',
            ...
            ('Specify a script URL with a async attribute '
             'to be included in the output HTML file. '
             '(This option can be used multiple times)', ['--script-async'], {
                 'metavar': '<URL or path>',
                 'dest': 'script',
                 'type': 'string',
                 'action': 'callback',
            ...
            ('Specify a html tag attribute. '
             '(This option can be used multiple times)', ['--html-tag-attr'], {
                 'metavar': '<attribute>',
                 'default': None,
                 'dest': 'html_tag_attr',
                 'action': 'append',
            ...
            ('Define a case insensitive identifier to be used with ifdef and ifndef directives. '
             ...
             '(This option can be used multiple times)', ['--define'], {
                 'metavar': '<identifier>',
                 'dest': 'identifiers',
                 'default': None,
                 'action': 'append',
    ...
    def translate(self):
        self.parts['pseudoxml'] = self.document.pformat(
        )  # get pseudoxml before HTML5.translate
...
class HTML5Translator(nodes.NodeVisitor):
    ...
    rst_terms = {
        ...
        'citation_reference':
        ('a', 'visit_citation_reference', 'depart_reference', True, False),
        ...
        'doctest_block':
        ('pre', 'visit_literal_block', 'depart_literal_block', True),
        ...
        'literal_block':
        ('pre', 'visit_literal_block', 'depart_literal_block'),
    ...
    def _map_terms_to_functions(self):
        ...
        for term, spec in self.rst_terms.items():
            ...
            depart_func = spec[2] and getattr(self, spec[2],
    ...
    def __init__(self, document):
        ...
        self.heading_level = int(
            getattr(self.document.settings, 'initial_header_level', 0))
    ...
    def _parse_params(self):
        ...
        if stylesheets_inline:
            self.stylesheets.append(
                tag.style(Markup(''.join(stylesheets_inline))))
        self.scripts = []
    ...
    def _compacted_paragraph(self, node):
        ...
        )) or node['classes'] or 'paragraph' != node.__class__.__name__)):
            return False
    ...
    def visit_field(self, node):
        self.docinfo[node.children[0].astext()] = self._strip_spaces(
            node.children[1].astext())
        raise nodes.SkipNode

    def visit_bibliographic_field(self, node):
        self.docinfo[node.__class__.__name__] = self._strip_spaces(
            node.astext())
        raise nodes.SkipNode

    def visit_address(self, node):
        self.docinfo[node.__class__.__name__] = ', '.join(
            node.astext().split('\n'))
        raise nodes.SkipNode
