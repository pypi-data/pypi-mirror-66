...
def convert_json_to_csv(json_file, csv_file):
    ...
    if "format" in json_data:
        # Create header
        for n, elem in enumerate(json_data['format']):
            csv_file.write("{0}{1}".format('' if n == 0 else ',', elem))
        csv_file.write('\n')
    ...
    # Non-compact format
        ...
        if "data" in json_data:
            for n, (key, value) in enumerate(json_data['data'][0].items()):
                csv_file.write("{0}{1}".format('' if n == 0 else ',', key))
            csv_file.write('\n')

            for row in json_data['data']:
                for n, (key, value) in enumerate(row.items()):
