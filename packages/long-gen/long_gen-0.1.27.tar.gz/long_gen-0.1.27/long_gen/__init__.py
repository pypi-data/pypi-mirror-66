...
def get_sample_times(bucket,measurment_occaisons,feature_values,abnormal_ratio,measure_max,sampling_function=None):
    if bucket == "equal":
        return(np.linspace(0,((1-abnormal_ratio)+abnormal_ratio*(measurment_occaisons/measure_max)),measurment_occaisons))
    elif bucket == "random":
...
def data_sorter(b_value,quantile_cutoff,quantile_sub):
...
class patient:
    ...
    def __init__(self,pat_id,b_values,features,coefficient_values,extraneous_variables,colinearity,stationarity_change_points,measurements,sampling_bucket,link_fn,sigma_e,stationarity_trend_bucket,sampling_function,probability_threshold,abnormal_ratio,measure_max):
...
class long_data_set:
    ...
    def create_data_set(self):
        ...
        measures=lognorm.rvs(0.75,loc=self.num_measurements-5, scale=5, size=self.num_of_patients, random_state=None)
        ...
        if len(self.random_effects) > 0:
            if "intercept" in self.b_dict:
                ...
                b_df['sort_col']=b_df["intercept"].apply(lambda v: data_sorter(v,cut_off,sub_value))
                ...
                b_df['sort_col']=b_df["time"].apply(lambda v: data_sorter(v,cut_off,sub_value))
                ...
                b_df['sort_col']=b_df["trend-time"].apply(lambda v: data_sorter(v,cut_off,sub_value))
            else:
                ...
                b_df['sort_col']=b_df["sort_col"].apply(lambda v: data_sorter(v,cut_off,sub_value))
        ...
        for p_id in range(self.num_of_patients):
            ...
            pat=patient(p_id,b_values,self.features,self.coefficient_values,self.extraneous_variables,ro_x,self.change_points,measures[p_id],self.sampling_bucket,self.link_fn,self.sigma_e,self.stationarity_trend_bucket,self.sampling_function,self.probability_threshold,abnormal_ratio,np.max(measures))
            if first:
                first=False
