...
def get_sample_times(bucket,measurment_occaisons,feature_values,abnormal_ratio,sampling_param=None,sampling_function=None,):
    if bucket == "equal":
        ...
        min_step_size=mean_measures/measurment_occaisons

        step_size=(((1-abnormal_ratio)+abnormal_ratio*precent_ab)/(measurment_occaisons-1))*np.ones(measurment_occaisons)
...
class patient:
    ...
    def __init__(self,pat_id,b_values,features,coefficient_values,extraneous_variables,colinearity,stationarity_change_points,measurements,sampling_bucket,link_fn,sigma_e,stationarity_trend_bucket,sampling_function,probability_threshold,abnormal_ratio):
        ...
        if (sampling_bucket=="not-random") | (sampling_bucket=="custom-feature-values"):
            ...
            for variable in extraneous_variables:
                ...
                index+=1
        time_points=get_sample_times(sampling_bucket,self.measure_count,abnormal_ratio,feature_values,sampling_function)
...
class long_data_set:
    ...
    def create_data_set(self):
        ...
        for p_id in range(self.num_of_patients):
            ...
            pat=patient(p_id,b_values,self.features,self.coefficient_values,self.extraneous_variables,ro_x,self.change_points,measures[p_id],self.sampling_bucket,self.link_fn,self.sigma_e,self.stationarity_trend_bucket,self.sampling_function,self.probability_threshold,abnormal_ratio)
            if first:
                first=False
