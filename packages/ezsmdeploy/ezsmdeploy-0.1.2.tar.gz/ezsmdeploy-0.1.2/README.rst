...
2. a python script with two functions: a) *load_model(modelpath)* - loads a model from a modelpath and returns a model object and b) *predict(model,input)* - performs inference based on a model object and input data
