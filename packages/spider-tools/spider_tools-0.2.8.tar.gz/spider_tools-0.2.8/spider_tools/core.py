...
class Auto_indb():
    ...
    def insert_data(self, items):
        ...
        self.conn.commit()
        print("**************************  insert {} table success  **********************".format(self.table_name))
...
class Auto_sinsert():
    def __init__(self, host='127.0.0.1', username='root', password='mysql', db='test',
        ...
        try:
            ...
            raise ValueError(f'连接数据库 db={db} 失败')
        ...
        if item:
            ...
            if item_key:
                ...
                for i in range(len(item_key)):
                    sss += '%s,'
                insert = 'insert ignore into {}('.format(table_name) + ','.join(item_key) + ')' + 'values({})'.format(sss[:-1])
...
def get_ip():
    ...
    if time.time() - start > 180 or not ip_list:
        while True:
            ...
            start = time.time()
            if '每1秒可提取一次' in json.dumps(ip_list,ensure_ascii=False):
                time.sleep(2)
            else:
                break
    while True:
