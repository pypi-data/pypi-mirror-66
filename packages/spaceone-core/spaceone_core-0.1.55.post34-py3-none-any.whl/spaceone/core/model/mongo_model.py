...
class MongoModel(Document, BaseModel):
            ...
            for key, value in row.items():
                ...
                if key == '_id' and isinstance(row[key], dict):
                    for aggr_key, aggr_value in row[key].items():
                        data[aggr_key] = aggr_value
                else:
                    if isinstance(value, datetime):
                        data[key] = f'{value.isoformat()}Z'
                    else:
                        data[key] = value
    ...
    def _make_group_rule(cls, options):
        ...
        if len(_keys) == 0 and len(_fields) == 0:
            raise ERROR_REQUIRED_PARAMETER(key='aggregate.group.keys or aggregate.group.fields')
        ...
        for condition in _fields:
            ...
            _group_fields[name] = 1

            if operator == 'size':
                _exist_size_operator = True
                _group_fields[name] = {
                    '$size': f'${name}'
                }
        ...
        if _exist_size_operator:
            _rules.append({
                '$project': _group_fields
            })
    ...
    @classmethod
    def _get_project_field(cls, field):
        key = field.get('key', field.get('k'))
        name = field.get('name', field.get('n'))

        if key is None:
            raise ERROR_REQUIRED_PARAMETER(key='aggregate.project.fields.key')

        if name is None:
            raise ERROR_REQUIRED_PARAMETER(key='aggregate.project.fields.name')

        return key, name

    @classmethod
    def _make_project_rule(cls, options, group_keys):
        rule = {
            '$project': {
                '_id': 0
            }
        ...
        for field in options.get('fields', []):
            key, name = cls._get_project_field(field)

            if key in group_keys:
                rule['$project'][name] = f'$_id.{key}'
            else:
                if 'operator' in field:
                    operator = field['operator']
                    rule['$project'][name] = {f'${operator}': f'${key}'}
                else:
                    rule['$project'][name] = f'${key}'
    ...
    def _make_aggregation_rules(cls, aggregate):
        ...
        if 'group' not in aggregate:
            raise ERROR_REQUIRED_PARAMETER(key='aggregate.group')
        ...
        if 'project' in aggregate:
            rule = cls._make_project_rule(aggregate['project'], _group_keys)
            _aggregation_rules.append(rule)
        """
