...
class MongoModel(Document, BaseModel):
    ...
    def _make_group_rule(cls, options):
        ...
        return _rule, _group_keys, _reference_keys
    ...
    def _make_aggregation_rules(cls, aggregate):
        ...
        for options in aggregate.get('unwind', []):
            rule = cls._make_unwind_rule(options)
            _aggregation_rules.append(rule)

        if 'group' in aggregate:
            rule, group_keys, ref_keys = cls._make_group_rule(aggregate['group'])
            _aggregation_rules.append(rule)
