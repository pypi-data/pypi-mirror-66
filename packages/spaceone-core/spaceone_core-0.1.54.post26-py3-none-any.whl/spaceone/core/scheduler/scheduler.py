...
class BaseScheduler(Process):
    ...
    def push_task(self):
        ...
        _LOGGER.debug(f'[push_task] task: {list(self.task)}')
        if self.task:
            try:
                validate(self.task, schema=SPACEONE_TASK_SCHEMA)
                json_task = json.dumps(self.task)
                queue.put(self.queue, json_task)
            except Exception as e:
                _LOGGER.debug(f'[push_task] Task schema: {self.task}, {e}')
            finally:
                self.task = None
