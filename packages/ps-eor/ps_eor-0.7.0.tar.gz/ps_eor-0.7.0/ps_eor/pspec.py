...
class BasePowerSpectra(object):
    ...
    def _compute_delays(self):
        ...
        #     idx = abs(self.k_par) > self.config.filter_kpar_min
        #     self.k_par = self.k_par[idx]
        #     self.delay = self.delay[idx]
...
class PowerSpectraCart(BasePowerSpectra):
    ...
    def get_ps3d_with_noise(self, kbins, ft_cube, noise_cube, with_cov_err=True, weighted='default'):
        ...
        ps_err_norm = self.get_ps_err_norm(ft_cube)
        ps_err_norm_signal = self.get_ps_err_norm(ft_cube, with_pb=True)

        k_per_full = psutil.l_to_k(ft_cube.ru * 2 * np.pi, self.z)

        k_per = np.repeat(k_per_full[np.newaxis, :], len(self.k_par), axis=0)
        k_par = np.repeat(self.k_par[:, np.newaxis], len(ft_cube.ru), axis=1)
        ks = np.sqrt(k_per ** 2 + k_par ** 2)

        vis_count = pscart.get_vis_count(ks, kbins)
        d_err = np.sqrt(2 / vis_count) * d.data

        d.err = np.sqrt(2 * (n.err * ps_err_norm) ** 2 + (d_err * ps_err_norm_signal) ** 2)
        d.n_eff = n.n_eff
    ...
    def get_cross_ps3d(self, kbins, data_cube1, data_cube2, with_cov_err=True, weighted='default'):
        ...
        k_mean, dsp, dsp_err, n_eff = pscart.get_3d_cross_power_spectre(
            dft_cube1, dft_cube2, kbins, k_per, k_par, weight_cube=weight_cube)
