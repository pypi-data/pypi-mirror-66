...
def make_rnd_imgfits(tmp_dir, df, shape, res, psf=False, n_chann=1, freq_width=None, wscnormf=1):
    ...
    wcs.wcs.cdelt = [-np.degrees(res), np.degrees(res), freq_width, 1]
...
def test_image_meta_data():
    ...
    assert meta.to_header(add_origin=True)['ORIGIN'] == 'ps_eor v_%s' % datacube.__version__
...
def test_load_from_fits_image_and_psf(tmp_path):
    ...
    assert cube.meta.int_time == 2
    assert cube.meta.total_time == 20
    assert np.allclose(cube.meta.freq_width, 0.4)
    assert np.allclose(cube.weights.meta.freq_width, 0.4)
    ...
    np.random.seed(0)
    f1, fits_n3, data_n3 = make_rnd_imgfits(tmp_path, 0.5, (100, 100), 1e-3, n_chann=3, freq_width=0.4)
    _, fits_psf_n3, data_psf_n3 = make_rnd_imgfits(tmp_path, 0.5, (100, 100), 1e-3, psf=True, n_chann=3, freq_width=0.4)
    ...
    assert np.allclose(cube.weights.data[0, 0], 3 * cube_n3.weights.data[0, 0])
