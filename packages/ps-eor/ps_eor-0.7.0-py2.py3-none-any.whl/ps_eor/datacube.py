...
class ImageMetaData(object):
    ...
    def to_header(self, add_origin=False):
        ...
        if add_origin:
            header['ORIGIN'] = 'ps_eor v_%s' % get_version()
...
class CartDataCube(DataCube):
    ...
    def load_from_fits_image(files, umin, umax, theta_fov, imager_scale_factor=None,
        ...
        for i, file in enumerate(files):
            ...
            if window_function is not None:
                mask = window_function.generate_window(cart_map.shape[0])
                cart_map = cart_map * mask
            ...
            if idx_ft is None:
                uu, vv, idx_ft = psutil.get_ungrid_vis_idx(cart_map.shape, res, umin, umax)

            ft = psutil.img_to_vis(cart_map)

            uu = uu.flatten()
            vv = vv.flatten()
            ft = ft[idx_ft].flatten()
    ...
    def filter_min_weight(self, min_weight, replace=False):
        if self.weights is not None and min_weight > 0:
            if replace:
                ...
                idx_uv = self.weights.get().min(axis=0) >= min_weight
                n_filt = np.sum(~idx_uv)
    ...
    def average_freqs(self, n_freqs=None, freq_bw=None):
        freqs = self.freqs
        if freq_bw is None:
            bins = np.arange(len(freqs) // n_freqs)
            digi = np.repeat(bins, n_freqs)[:len(freqs)]
        else:
            df = psutil.robust_freq_width(freqs)
            digi = (np.cumsum(np.repeat(df, len(freqs))) / freq_bw).astype(int)
            bins = np.arange(np.max(digi) + 1)
        ...
        return self.new_with_data(new_data, weights=new_weight_cube, freqs=new_freqs)
