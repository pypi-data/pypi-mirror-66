...
class Locust(object):
    ...
    min_wait = None
    """Deprecated: Use wait_time instead. Minimum waiting time between the execution of idapload tasks"""
    
    max_wait = None
    """Deprecated: Use wait_time instead. Maximum waiting time between the execution of idapload tasks"""
    
    wait_time = None
    """
    Method that returns the time (in seconds) between the execution of idapload tasks. 
    Can be overridden for individual TaskSets.
    
    Example::
    
        from idapload import Locust, between
        class User(Locust):
    ...
    Method that returns the time between the execution of idapload tasks in milliseconds
    """
    
    task_set = None
    """TaskSet class that defines the execution behaviour of this idapload"""

    weight = 10
    """Probability of idapload being chosen. The higher the weight, the greater is the chance of it being chosen."""
...
class TaskSet(object, metaclass=TaskSetMeta):
    ...
    tasks = []
    """
    List with python callables that represents a idapload user task.
    ...
    Minimum waiting time between the execution of idapload tasks. Can be used to override 
    ...
    """
    Deprecated: Use wait_time instead. 
    Maximum waiting time between the execution of idapload tasks. Can be used to override 
    ...
    """
    Deprecated: Use wait_time instead.
    Function used to calculate waiting time between the execution of idapload tasks in milliseconds. 
    ...
    def execute_task(self, task, *args, **kwargs):
        # check if the function is a method bound to the current idapload, and if so, don't pass self as first argument
...
class TaskSequence(TaskSet):
    ...
    *max_wait* milliseconds. It will then schedule the `index + 1 % len(tasks)` task for execution and so on.
    ...
    def __init__(self, parent):
        ...
        self._index = 0
        self.tasks.sort(key=lambda t: t.idapload_task_order if hasattr(t, 'idapload_task_order') else 1)
