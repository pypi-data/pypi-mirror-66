...
def update_guide_plate():
    ...
    shaft_hole_wire = vmi.ccWire(vmi.ccEdge(
        vmi.ccCircle(kirs_hole_radius, head_cs.mpt([0, 0, head_z + 20]), head_cs.axis(2), head_cs.axis(0))))
    shaft_hole = vmi.ccPrism(vmi.ccFace(shaft_hole_wire), -head_cs.axis(2), 200)
    vmi.pdSaveFile_STL(vmi.ccPd_Sh(shaft_hole))
    ...
    for i in range(3):
        ...
        kirs_hole_center = head_cs.mpt([0, -LR * arm_lengths[i], head_z + 20])
        kirs_hole = vmi.ccPrism(vmi.ccFace(vmi.ccWire(vmi.ccCircle(
            kirs_hole_radius, kirs_hole_center, head_cs.axis(2)))), -head_cs.axis(2), 200)
        vmi.pdSaveFile_STL(vmi.ccPd_Sh(kirs_hole))
...
def case_snapshot():
    ...
    with tempfile.TemporaryDirectory() as p:
        ...
        snapshots.append(pelvis_APP_snapshot)
        snapshots.append(pelvis_MPP_snapshot)
...
def LeftButtonPressRelease(**kwargs):
    ...
    global body_angle, body_xr, body_yr, match_style
    if kwargs['picked'] is dicom_box:
        ...
        r = vmi.askFloat(-10.0, cup_RA, 40.0, 1, 0.1)
        if r is not None:
            cup_RA = r
            cup_RA_box.draw_text('前倾角：{:.1f}°'.format(cup_RA))
        ...
        r = vmi.askFloat(30.0, cup_RI, 50.0, 1, 0.1)
        if r is not None:
            cup_RI = r
            cup_RI_box.draw_text('外展角：{:.1f}°'.format(cup_RI))
        ...
        if pelvis_MPP_box.text() == '标记对称面':
            ...
            with tempfile.TemporaryDirectory() as p:
                ...
                main.scrollArea().ensureWidgetVisible(pelvis_view)
                pelvis_view.snapshot_PNG(str(f))
                global pelvis_MPP_snapshot
                pelvis_MPP_snapshot = f.read_bytes()
        ...
        if pelvis_APP_box.text() == '标记前平面':
            ...
            with tempfile.TemporaryDirectory() as p:
                ...
                main.scrollArea().ensureWidgetVisible(pelvis_view)
                pelvis_view.snapshot_PNG(str(f))
                global pelvis_APP_snapshot
                pelvis_APP_snapshot = f.read_bytes()
            pelvis_cs_indicator_prop.setVisible(False)
...
def NoButtonWheel(**kwargs):
    ...
    if kwargs['picked'] is bone_value_box:
        ...
        cup_radius = min(max(cup_radius + kwargs['delta'], 15), 40)
        cup_radius_box.draw_text('半径：{:.0f} mm'.format(cup_radius))
        update_cup()
        clear_guide_plate()
    elif kwargs['picked'] is cup_RA_box:
        cup_RA = min(max(cup_RA + 0.1 * kwargs['delta'], -10), 40)
        cup_RA_box.draw_text('前倾角：{:.1f}°'.format(cup_RA))
        update_cup_orientation()
        update_cup()
        clear_guide_plate()
    elif kwargs['picked'] is cup_RI_box:
        cup_RI = min(max(cup_RI + 0.1 * kwargs['delta'], 30), 50)
        cup_RI_box.draw_text('外展角：{:.1f}°'.format(cup_RI))
        update_cup_orientation()
        update_cup()
        clear_guide_plate()
