...
def init_voi():
    ...
    td_prop[0].setData(vtk.vtkPolyData())
    td_prop[1].setData(vtk.vtkPolyData())
...
def init_pelvis():
    global cup_center, cup_radius, ace_center, pre_image
...
def LeftButtonPressMove(**kwargs):
    ...
    if kwargs['picked'] is pelvis_view:
        pelvis_view.mouseRotateFocal(**kwargs)
        if pelvis_APP_box.text() == '确定':
            update_pelvis_APP()
            return True
