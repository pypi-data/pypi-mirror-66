...
PROVIDER_ENDPOINT = getattr(settings, 'AUTH_OPENID_PROVIDER_ENDPOINT', 'https://example.com/a/')
...
PROVIDER_END_SESSION_ENDPOINT = getattr(settings, 'AUTH_OPENID_PROVIDER_END_SESSION_ENDPOINT', None)
...
# 'HS256' or 'RS256'.
PROVIDER_SIGNATURE_ALG = getattr(settings, 'AUTH_OPENID_PROVIDER_SIGNATURE_ALG', 'HS256')
PROVIDER_SIGNATURE_KEY = getattr(settings, 'AUTH_OPENID_PROVIDER_SIGNATURE_KEY', None)
...
CLIENT_ID = getattr(settings, 'AUTH_OPENID_CLIENT_ID', None)
CLIENT_SECRET = getattr(settings, 'AUTH_OPENID_CLIENT_SECRET', None)
...
USE_STATE = getattr(settings, 'AUTH_OPENID_USE_STATE', True)
...
STATE_LENGTH = getattr(settings, 'AUTH_OPENID_STATE_LENGTH', 32)

# The 'SCOPES' setting defines the OpenID Connect scopes to request during login.
SCOPES = getattr(settings, 'AUTH_OPENID_SCOPES', 'openid email')
...
USE_NONCE = getattr(settings, 'AUTH_OPENID_USE_NONCE', True)
...
NONCE_LENGTH = getattr(settings, 'AUTH_OPENID_NONCE_LENGTH', 32)

# The 'ID_TOKEN_MAX_AGE' setting defines the amount of time an id_token should be considered a
# valid token.
ID_TOKEN_MAX_AGE = getattr(settings, 'AUTH_OPENID_ID_TOKEN_MAX_AGE', 600)
...
ID_TOKEN_INCLUDE_USERINFO = getattr(settings, 'AUTH_OPENID_ID_TOKEN_INCLUDE_USERINFO', False)
...
# "next" parameter.
AUTHENTICATION_REDIRECT_URI = getattr(settings, 'AUTH_OPENID_AUTHENTICATION_REDIRECT_URI', '/')
...
USER_DETAILS_HANDLER = getattr(settings, 'AUTH_OPENID_USER_DETAILS_HANDLER', None)
...
# JumpServer config
# 用户登录的路径名
AUTH_LOGIN_URL_NAME = getattr(settings, 'AUTH_OPENID_AUTH_LOGIN_URL_NAME', None)

# 用户登录的Callback路径名
AUTH_LOGIN_CALLBACK_URL_NAME = getattr(settings, 'AUTH_OPENID_AUTH_LOGIN_CALLBACK_URL_NAME', None)

# 用户登出的路径名
AUTH_LOGOUT_URL_NAME = getattr(settings, 'AUTH_OPENID_AUTH_LOGOUT_URL_NAME', None)
...
PROVIDER_CLAIMS_USERNAME = getattr(settings, 'AUTH_OPENID_PROVIDER_CLAIMS_USERNAME', None)
PROVIDER_CLAIMS_EMAIL = getattr(settings, 'AUTH_OPENID_PROVIDER_CLAIMS_EMAIL', None)

# 总是更新用户信息
ALWAYS_UPDATE_USER_INFORMATION = getattr(settings, 'AUTH_OPENID_ALWAYS_UPDATE_USER_INFORMATION', True)
