...
class SecretGroupManager(BaseManager):
    ...
    def list_secret_groups(self, query, secret_id):
        # if secret_id is not None:
        #     query = self._add_secret_filter_query(query, secret_id)
    ...
    def _add_secret_filter_query(self, query, secret_id):
        map_query = {
            'filter': [{
                'k': 'secret_id',
                'v': secret_id,
        ...
        map_vos, total_count = self.secret_group_map_model.query(map_query)
