...
class SecretManager(BaseManager):
    ...
    def create_secret(self, params):
        ...
        secret_model: Secret = self.locator.get_model('Secret')
        secret_vo: Secret = secret_model.create(params)
    ...
    def get_secret(self, secret_id, domain_id, only=None):
        secret_model: Secret = self.locator.get_model('Secret')
        return secret_model.get(secret_id=secret_id, domain_id=domain_id, only=only)

    def list_secrets(self, query):
        secret_model: Secret = self.locator.get_model('Secret')
        return secret_model.query(**query)
