  ...
  #  rows=row2
    ...
    for y in range(len(rows)):
        ...
        for x in range(x_max):
            ...
            _world[(x, y)] = None if tile_name == '' else getattr(__import__('escapetheforest.tiles'), tile_name)(x, y)

            if tile_name == '':
                _world[(x,y)] = None

            else:
                _world[(x, y)] = getattr(__import__('escapetheforest.tiles'), tile_name)(x, y)
