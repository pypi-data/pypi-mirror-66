...
class DomainManager(BaseManager):
    ...
    def delete_domain(self, domain_id):
        ...
        # TODO: meta is Not implemented yet
        '''
        project_model = self.locator.get_model('Project')
        project_group_model = self.locator.get_model('ProjectGroup')

        domain_filter = [
            {'k': 'domain_id', 'v': domain_id, 'o': 'eq'}
        ]
        project_group_vos, total_count = project_group_model.query(filter=domain_filter)
        if total_count > 0:
            raise ERROR_EXIST_RESOURCE(key='project_group_id', value=project_group_vos[0].project_group_id)

        project_vos, total_count = project_model.query(filter)
        if total_count > 0:
            raise ERROR_EXIST_RESOURCE(key='project_id', value=project_vos[0].project_id)
