...
class Fit_Weibull_Exponential:
    ...
    def __init__(self, failures=None, failure_stress=None, right_censored=None, right_censored_stress=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        if type(failure_stress) != np.ndarray:
            raise TypeError('failure_stress must be a list or array of failure_stress data')
        if len(failure_stress) != len(failures):
            raise ValueError('The number of elements in failures does not match the number of elements in failure_stress')
        if right_censored is not None:
            ...
            if type(right_censored_stress) != np.ndarray:
                raise TypeError('right_censored_stress must be a list or array of right censored failure_stress data')
            if len(right_censored_stress) != len(right_censored):
                raise ValueError('The number of elements in right_censored does not match the number of elements in right_censored_stress')
        ...
        warnings.filterwarnings('ignore')  # necessary to suppress the warning about the jacobian when using the nelder-mead optimizer
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Weibull_Exponential.LL), guess, args=(failures, right_censored, failure_stress, right_censored_stress), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Weibull-Exponential model. Try a better initial guess by specifying the parameter initial_guess = [a,b]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2]]
            LL2 = 2 * Fit_Weibull_Exponential.LL(guess, failures, right_censored, failure_stress, right_censored_stress)
        ...
        hessian_matrix = hessian(Fit_Weibull_Exponential.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress)), np.array(tuple(right_censored_stress)))
        ...
        self.beta_upper = self.beta * (np.exp(Z * (self.beta_SE / self.beta)))  # a and b can be +- but beta is strictly + so the formulas here are different for beta
        self.beta_lower = self.beta * (np.exp(-Z * (self.beta_SE / self.beta)))

        Data = {'Parameter': ['a', 'b', 'beta'],
    ...
    def LL(params, t_f, t_rc, T_f, T_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Weibull_Exponential.logf(t_f, T_f, params[0], params[1], params[2]).sum()  # failure times
        LL_rc += Fit_Weibull_Exponential.logR(t_rc, T_rc, params[0], params[1], params[2]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Weibull_Eyring:
    ...
    def __init__(self, failures=None, failure_stress=None, right_censored=None, right_censored_stress=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Weibull_Eyring.LL), guess, args=(failures, right_censored, failure_stress, right_censored_stress), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Weibull-Eyring model. Try a better initial guess by specifying the parameter initial_guess = [a,c]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2]]
            LL2 = 2 * Fit_Weibull_Eyring.LL(guess, failures, right_censored, failure_stress, right_censored_stress)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Weibull_Eyring.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress)), np.array(tuple(right_censored_stress)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        ...
        self.beta_upper = self.beta * (np.exp(Z * (self.beta_SE / self.beta)))  # a and c can be +- but beta is strictly + so the formulas here are different for beta
        self.beta_lower = self.beta * (np.exp(-Z * (self.beta_SE / self.beta)))

        Data = {'Parameter': ['a', 'c', 'beta'],
    ...
    def LL(params, t_f, t_rc, T_f, T_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Weibull_Eyring.logf(t_f, T_f, params[0], params[1], params[2]).sum()  # failure times
        LL_rc += Fit_Weibull_Eyring.logR(t_rc, T_rc, params[0], params[1], params[2]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Weibull_Power:
    ...
    def __init__(self, failures=None, failure_stress=None, right_censored=None, right_censored_stress=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Weibull_Power.LL), guess, args=(failures, right_censored, failure_stress, right_censored_stress), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Weibull-Power model. Try a better initial guess by specifying the parameter initial_guess = [a,n]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2]]
            LL2 = 2 * Fit_Weibull_Power.LL(guess, failures, right_censored, failure_stress, right_censored_stress)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Weibull_Power.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress)), np.array(tuple(right_censored_stress)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        ...
        self.beta_upper = self.beta * (np.exp(Z * (self.beta_SE / self.beta)))  # a and n can be +- but beta is strictly + so the formulas here are different for beta
        self.beta_lower = self.beta * (np.exp(-Z * (self.beta_SE / self.beta)))

        Data = {'Parameter': ['a', 'n', 'beta'],
    ...
    def LL(params, t_f, t_rc, T_f, T_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Weibull_Power.logf(t_f, T_f, params[0], params[1], params[2]).sum()  # failure times
        LL_rc += Fit_Weibull_Power.logR(t_rc, T_rc, params[0], params[1], params[2]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Weibull_Dual_Exponential:
    ...
    def __init__(self, failures=None, failure_stress_1=None, failure_stress_2=None, right_censored=None, right_censored_stress_1=None, right_censored_stress_2=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        if use_level_stress is not None:
            if len(use_level_stress) != 2:
                raise ValueError('use_level_stress must be a 2 element list or array. eg. [use_temperature, use_humidity]')
        # adjust inputs to be arrays
        if type(failures) == list:
            failures = np.array(failures)
        if type(failures) != np.ndarray:
        ...
        if type(failure_stress_1) != np.ndarray:
            raise TypeError('failure_stress_1 must be a list or array of failure_stress data')
        if type(failure_stress_2) == list:
            failure_stress_2 = np.array(failure_stress_2)
        if type(failure_stress_2) != np.ndarray:
            raise TypeError('failure_stress_2 must be a list or array of failure_stress data')
        if len(failure_stress_1) != len(failures):
            raise ValueError('The number of elements in failures does not match the number of elements in failure_stress_1')
        if len(failure_stress_2) != len(failures):
            raise ValueError('The number of elements in failures does not match the number of elements in failure_stress_2')
        if right_censored is not None:
            if type(right_censored) == list:
                right_censored = np.array(right_censored)
            if type(right_censored) != np.ndarray:
            ...
            if type(right_censored_stress_1) != np.ndarray:
                raise TypeError('right_censored_stress_1 must be a list or array of right censored failure_stress data')
            ...
            if type(right_censored_stress_2) != np.ndarray:
                raise TypeError('right_censored_stress_2 must be a list or array of right censored failure_stress data')
            if len(right_censored_stress_1) != len(right_censored):
                raise ValueError('The number of elements in right_censored does not match the number of elements in right_censored_stress_1')
            if len(right_censored_stress_2) != len(right_censored):
                raise ValueError('The number of elements in right_censored does not match the number of elements in right_censored_stress_2')
        ...
        ALT_fit_2 = ALT_probability_plot_Weibull(failures=failures, right_censored=right_censored, failure_stress=failure_stress_2, right_censored_stress=right_censored_stress_2, print_results=False, show_plot=False, common_shape_method='average')
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Weibull_Dual_Exponential.LL), guess, args=(failures, right_censored, failure_stress_1, failure_stress_2, right_censored_stress_1, right_censored_stress_2), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Weibull-Dual-Exponential model. Try a better initial guess by specifying the parameter initial_guess = [a,b,c]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2], params[3]]
            LL2 = 2 * Fit_Weibull_Dual_Exponential.LL(guess, failures, right_censored, failure_stress_1, failure_stress_2, right_censored_stress_1, right_censored_stress_2)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Weibull_Dual_Exponential.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress_1)), np.array(tuple(failure_stress_2)), np.array(tuple(right_censored_stress_1)), np.array(tuple(right_censored_stress_2)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.b_SE = abs(covariance_matrix[1][1]) ** 0.5
        ...
        if show_plot is True:
            ...
            data = {'times': TIMES, 'stress_1': STRESS_1, 'stress_2': STRESS_2, 'cens_codes': CENS_CODES}
            df = pd.DataFrame(data, columns=['times', 'stress_1', 'stress_2', 'cens_codes'])
            df['stress_pair'] = df['stress_1'].map(str) + ' , ' + df['stress_2'].map(str)  # this combines each stress to make a "stress pair" which is treated as a unique stress combination
            df_sorted = df.sort_values(by=['cens_codes', 'stress_1', 'stress_2', 'times'])
            is_failure = df_sorted['cens_codes'] == 1
            is_right_cens = df_sorted['cens_codes'] == 0
            f_df = df_sorted[is_failure]
            rc_df = df_sorted[is_right_cens]
            ...
            if right_censored is not []:
                ...
                for item in unique_stresses_rc:  # check that there are no unique right_censored stresses that are not also in failure stresses
                    if item not in unique_stresses_f:
                        raise ValueError('The right_censored_stress_1 and right_censored_stress_2 arrays contains pairs of values that are not found in the failure_stress_1 and failure_stress_2 arrays. This is equivalent to trying to fit a distribution to only censored data and cannot be done.')
            # within this loop, each list of failures and right censored values will be unpacked for each unique stress to find the common beta parameter
            for i, stress_pair in enumerate(unique_stresses_f):
                ...
                if right_censored is not []:
                    if stress_pair in unique_stresses_rc:
                        right_cens_current_stress_df = rc_df[rc_df['stress_pair'] == stress_pair]
                        RIGHT_CENSORED = right_cens_current_stress_df['times'].values
                    else:
                        RIGHT_CENSORED = None
                else:
    ...
    def LL(params, t_f, t_rc, S1_f, S2_f, S1_rc, S2_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Weibull_Dual_Exponential.logf(t_f, S1_f, S2_f, params[0], params[1], params[2], params[3]).sum()  # failure times
        LL_rc += Fit_Weibull_Dual_Exponential.logR(t_rc, S1_rc, S2_rc, params[0], params[1], params[2], params[3]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Weibull_Power_Exponential:
    ...
    def __init__(self, failures=None, failure_stress_thermal=None, failure_stress_nonthermal=None, right_censored=None, right_censored_stress_thermal=None, right_censored_stress_nonthermal=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        if use_level_stress is not None:
            if len(use_level_stress) != 2:
                raise ValueError('use_level_stress must be a 2 element list or array. eg. [use_temperature, use_voltage]')
        # adjust inputs to be arrays
        if type(failures) == list:
            failures = np.array(failures)
        if type(failures) != np.ndarray:
        ...
        if type(failure_stress_thermal) != np.ndarray:
            raise TypeError('failure_stress_thermal must be a list or array of thermal failure_stress data')
        ...
        if type(failure_stress_nonthermal) != np.ndarray:
            raise TypeError('failure_stress_nonthermal must be a list or array of nonthermal failure_stress data')
        if len(failure_stress_thermal) != len(failures):
            raise ValueError('The number of elements in failures does not match the number of elements in failure_stress_thermal')
        if len(failure_stress_nonthermal) != len(failures):
            raise ValueError('The number of elements in failures does not match the number of elements in failure_stress_nonthermal')
        if right_censored is not None:
            if type(right_censored) == list:
                right_censored = np.array(right_censored)
            if type(right_censored) != np.ndarray:
            ...
            if type(right_censored_stress_thermal) != np.ndarray:
                raise TypeError('right_censored_stress_thermal must be a list or array of right censored thermal failure_stress data')
            if type(right_censored_stress_nonthermal) == list:
                right_censored_stress_nonthermal = np.array(right_censored_stress_nonthermal)
            if type(right_censored_stress_nonthermal) != np.ndarray:
                raise TypeError('right_censored_stress_nonthermal must be a list or array of right censored nonthermal failure_stress data')
            if len(right_censored_stress_thermal) != len(right_censored):
                raise ValueError('The number of elements in right_censored does not match the number of elements in right_censored_stress_thermal')
            if len(right_censored_stress_nonthermal) != len(right_censored):
                raise ValueError('The number of elements in right_censored does not match the number of elements in right_censored_stress_nonthermal')
        ...
        common_shape_1 = ALT_fit_1.common_shape
        ALT_fit_2 = ALT_probability_plot_Weibull(failures=failures, right_censored=right_censored, failure_stress=failure_stress_nonthermal, right_censored_stress=right_censored_stress_nonthermal, print_results=False, show_plot=False, common_shape_method='average')
        common_shape_2 = ALT_fit_2.common_shape
        common_shape = np.average([common_shape_1, common_shape_2])
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Weibull_Power_Exponential.LL), guess, args=(failures, right_censored, failure_stress_thermal, failure_stress_nonthermal, right_censored_stress_thermal, right_censored_stress_nonthermal), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Weibull-Power-Exponential model. Try a better initial guess by specifying the parameter initial_guess = [a,c,n]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2], params[3]]
            LL2 = 2 * Fit_Weibull_Power_Exponential.LL(guess, failures, right_censored, failure_stress_thermal, failure_stress_nonthermal, right_censored_stress_thermal, right_censored_stress_nonthermal)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Weibull_Power_Exponential.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress_thermal)), np.array(tuple(failure_stress_nonthermal)), np.array(tuple(right_censored_stress_thermal)), np.array(tuple(right_censored_stress_nonthermal)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.c_SE = abs(covariance_matrix[1][1]) ** 0.5
        ...
        if show_plot is True:
            TIMES = np.hstack([failures, right_censored])
            STRESS_1 = np.hstack([failure_stress_thermal, right_censored_stress_thermal])
            STRESS_2 = np.hstack([failure_stress_nonthermal, right_censored_stress_nonthermal])
            CENS_CODES = np.hstack([np.ones_like(failures), np.zeros_like(right_censored)])
            ...
            if right_censored is not []:
                ...
                for item in unique_stresses_rc:  # check that there are no unique right_censored stresses that are not also in failure stresses
                    if item not in unique_stresses_f:
                        raise ValueError('The right_censored_stress_thermal and right_censored_stress_nonthermal arrays contains pairs of values that are not found in the failure_stress_thermal and failure_stress_nonthermal arrays. This is equivalent to trying to fit a distribution to only censored data and cannot be done.')
            # within this loop, each list of failures and right censored values will be unpacked for each unique stress to find the common beta parameter
            for i, stress_pair in enumerate(unique_stresses_f):
                failure_current_stress_df = f_df[f_df['stress_pair'] == stress_pair]
                FAILURES = failure_current_stress_df['times'].values
    ...
    def LL(params, t_f, t_rc, S1_f, S2_f, S1_rc, S2_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Weibull_Power_Exponential.logf(t_f, S1_f, S2_f, params[0], params[1], params[2], params[3]).sum()  # failure times
        LL_rc += Fit_Weibull_Power_Exponential.logR(t_rc, S1_rc, S2_rc, params[0], params[1], params[2], params[3]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Lognormal_Exponential:
    ...
    def __init__(self, failures=None, failure_stress=None, right_censored=None, right_censored_stress=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Lognormal_Exponential.LL), guess, args=(failures, right_censored, failure_stress, right_censored_stress), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Lognormal-Exponential model. Try a better initial guess by specifying the parameter initial_guess = [a,b]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2]]
            LL2 = 2 * Fit_Lognormal_Exponential.LL(guess, failures, right_censored, failure_stress, right_censored_stress)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Lognormal_Exponential.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress)), np.array(tuple(right_censored_stress)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.b_SE = abs(covariance_matrix[1][1]) ** 0.5
        ...
        self.b_upper = self.b + (Z * self.b_SE)
        self.b_lower = self.b + (-Z * self.b_SE)
        self.sigma_upper = self.sigma * (np.exp(Z * (self.sigma_SE / self.sigma)))  # a and b can be +- but sigma is strictly + so the formulas here are different for sigma
        self.sigma_lower = self.sigma * (np.exp(-Z * (self.sigma_SE / self.sigma)))

        Data = {'Parameter': ['a', 'b', 'sigma'],
        ...
        if show_plot is True:
            ...
            for i, stress in enumerate(unique_stresses_f):
                ...
                life = self.b * np.exp(self.a / stress)
                fitted_dist_params = __make_fitted_dist_params_lognormal(mu=np.log(life), sigma=self.sigma)
                original_fit = Fit_Lognormal_2P(failures=FAILURES, right_censored=RIGHT_CENSORED, show_probability_plot=False, print_results=False)
                original_fit.distribution.CDF(color=color_list[i], label=str(stress), xvals=xvals)
    ...
    def LL(params, t_f, t_rc, T_f, T_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Lognormal_Exponential.logf(t_f, T_f, params[0], params[1], params[2]).sum()  # failure times
        LL_rc += Fit_Lognormal_Exponential.logR(t_rc, T_rc, params[0], params[1], params[2]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Lognormal_Eyring:
    ...
    def __init__(self, failures=None, failure_stress=None, right_censored=None, right_censored_stress=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Lognormal_Eyring.LL), guess, args=(failures, right_censored, failure_stress, right_censored_stress), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Lognormal-Eyring model. Try a better initial guess by specifying the parameter initial_guess = [a,c]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2]]
            LL2 = 2 * Fit_Lognormal_Eyring.LL(guess, failures, right_censored, failure_stress, right_censored_stress)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Lognormal_Eyring.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress)), np.array(tuple(right_censored_stress)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.c_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.sigma_SE = abs(covariance_matrix[2][2]) ** 0.5
        ...
        self.c_upper = self.c + (Z * self.c_SE)
        self.c_lower = self.c + (-Z * self.c_SE)
        self.sigma_upper = self.sigma * (np.exp(Z * (self.sigma_SE / self.sigma)))  # a and c can be +- but sigma is strictly + so the formulas here are different for sigma
        self.sigma_lower = self.sigma * (np.exp(-Z * (self.sigma_SE / self.sigma)))

        Data = {'Parameter': ['a', 'c', 'sigma'],
    ...
    def LL(params, t_f, t_rc, T_f, T_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Lognormal_Eyring.logf(t_f, T_f, params[0], params[1], params[2]).sum()  # failure times
        LL_rc += Fit_Lognormal_Eyring.logR(t_rc, T_rc, params[0], params[1], params[2]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Lognormal_Power:
    ...
    def __init__(self, failures=None, failure_stress=None, right_censored=None, right_censored_stress=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Lognormal_Power.LL), guess, args=(failures, right_censored, failure_stress, right_censored_stress), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Lognormal-Power model. Try a better initial guess by specifying the parameter initial_guess = [a,n]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2]]
            LL2 = 2 * Fit_Lognormal_Power.LL(guess, failures, right_censored, failure_stress, right_censored_stress)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Lognormal_Power.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress)), np.array(tuple(right_censored_stress)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.n_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.sigma_SE = abs(covariance_matrix[2][2]) ** 0.5
        ...
        self.n_upper = self.n + (Z * self.n_SE)
        self.n_lower = self.n + (-Z * self.n_SE)
        self.sigma_upper = self.sigma * (np.exp(Z * (self.sigma_SE / self.sigma)))  # a and n can be +- but sigma is strictly + so the formulas here are different for sigma
        self.sigma_lower = self.sigma * (np.exp(-Z * (self.sigma_SE / self.sigma)))

        Data = {'Parameter': ['a', 'n', 'sigma'],
    ...
    def LL(params, t_f, t_rc, T_f, T_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Lognormal_Power.logf(t_f, T_f, params[0], params[1], params[2]).sum()  # failure times
        LL_rc += Fit_Lognormal_Power.logR(t_rc, T_rc, params[0], params[1], params[2]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Lognormal_Dual_Exponential:
    ...
    def __init__(self, failures=None, failure_stress_1=None, failure_stress_2=None, right_censored=None, right_censored_stress_1=None, right_censored_stress_2=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        common_shape_1 = ALT_fit_1.common_shape
        ALT_fit_2 = ALT_probability_plot_Lognormal(failures=failures, right_censored=right_censored, failure_stress=failure_stress_2, right_censored_stress=right_censored_stress_2, print_results=False, show_plot=False, common_shape_method='average')
        common_shape_2 = ALT_fit_2.common_shape
        common_shape = np.average([common_shape_1, common_shape_2])
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Lognormal_Dual_Exponential.LL), guess, args=(failures, right_censored, failure_stress_1, failure_stress_2, right_censored_stress_1, right_censored_stress_2), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Lognormal-Dual-Exponential model. Try a better initial guess by specifying the parameter initial_guess = [a,b,c]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2], params[3]]
            LL2 = 2 * Fit_Lognormal_Dual_Exponential.LL(guess, failures, right_censored, failure_stress_1, failure_stress_2, right_censored_stress_1, right_censored_stress_2)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Lognormal_Dual_Exponential.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress_1)), np.array(tuple(failure_stress_2)), np.array(tuple(right_censored_stress_1)), np.array(tuple(right_censored_stress_2)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.b_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.c_SE = abs(covariance_matrix[2][2]) ** 0.5
    ...
    def LL(params, t_f, t_rc, S1_f, S2_f, S1_rc, S2_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Lognormal_Dual_Exponential.logf(t_f, S1_f, S2_f, params[0], params[1], params[2], params[3]).sum()  # failure times
        LL_rc += Fit_Lognormal_Dual_Exponential.logR(t_rc, S1_rc, S2_rc, params[0], params[1], params[2], params[3]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Lognormal_Power_Exponential:
    ...
    def __init__(self, failures=None, failure_stress_thermal=None, failure_stress_nonthermal=None, right_censored=None, right_censored_stress_thermal=None, right_censored_stress_nonthermal=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        common_shape_1 = ALT_fit_1.common_shape
        ALT_fit_2 = ALT_probability_plot_Lognormal(failures=failures, right_censored=right_censored, failure_stress=failure_stress_nonthermal, right_censored_stress=right_censored_stress_nonthermal, print_results=False, show_plot=False, common_shape_method='average')
        common_shape_2 = ALT_fit_2.common_shape
        common_shape = np.average([common_shape_1, common_shape_2])
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Lognormal_Power_Exponential.LL), guess, args=(failures, right_censored, failure_stress_thermal, failure_stress_nonthermal, right_censored_stress_thermal, right_censored_stress_nonthermal), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Lognormal-Power-Exponential model. Try a better initial guess by specifying the parameter initial_guess = [a,c,n]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2], params[3]]
            LL2 = 2 * Fit_Lognormal_Power_Exponential.LL(guess, failures, right_censored, failure_stress_thermal, failure_stress_nonthermal, right_censored_stress_thermal, right_censored_stress_nonthermal)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Lognormal_Power_Exponential.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress_thermal)), np.array(tuple(failure_stress_nonthermal)), np.array(tuple(right_censored_stress_thermal)), np.array(tuple(right_censored_stress_nonthermal)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.c_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.n_SE = abs(covariance_matrix[2][2]) ** 0.5
    ...
    def LL(params, t_f, t_rc, S1_f, S2_f, S1_rc, S2_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Lognormal_Power_Exponential.logf(t_f, S1_f, S2_f, params[0], params[1], params[2], params[3]).sum()  # failure times
        LL_rc += Fit_Lognormal_Power_Exponential.logR(t_rc, S1_rc, S2_rc, params[0], params[1], params[2], params[3]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Normal_Exponential:
    ...
    def __init__(self, failures=None, failure_stress=None, right_censored=None, right_censored_stress=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Normal_Exponential.LL), guess, args=(failures, right_censored, failure_stress, right_censored_stress), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Normal-Exponential model. Try a better initial guess by specifying the parameter initial_guess = [a,b]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2]]
            LL2 = 2 * Fit_Normal_Exponential.LL(guess, failures, right_censored, failure_stress, right_censored_stress)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Normal_Exponential.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress)), np.array(tuple(right_censored_stress)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.b_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.sigma_SE = abs(covariance_matrix[2][2]) ** 0.5
        ...
        if show_plot is True:
            ...
            for i, stress in enumerate(unique_stresses_f):
                ...
                life = self.b * np.exp(self.a / stress)
                fitted_dist_params = __make_fitted_dist_params_normal(mu=life, sigma=self.sigma)
                original_fit = Fit_Normal_2P(failures=FAILURES, right_censored=RIGHT_CENSORED, show_probability_plot=False, print_results=False)
                original_fit.distribution.CDF(color=color_list[i], label=str(stress), xvals=xvals)
    ...
    def LL(params, t_f, t_rc, T_f, T_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Normal_Exponential.logf(t_f, T_f, params[0], params[1], params[2]).sum()  # failure times
        LL_rc += Fit_Normal_Exponential.logR(t_rc, T_rc, params[0], params[1], params[2]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Normal_Eyring:
    ...
    def __init__(self, failures=None, failure_stress=None, right_censored=None, right_censored_stress=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Normal_Eyring.LL), guess, args=(failures, right_censored, failure_stress, right_censored_stress), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Normal-Eyring model. Try a better initial guess by specifying the parameter initial_guess = [a,c]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2]]
            LL2 = 2 * Fit_Normal_Eyring.LL(guess, failures, right_censored, failure_stress, right_censored_stress)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Normal_Eyring.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress)), np.array(tuple(right_censored_stress)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.c_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.sigma_SE = abs(covariance_matrix[2][2]) ** 0.5
    ...
    def LL(params, t_f, t_rc, T_f, T_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Normal_Eyring.logf(t_f, T_f, params[0], params[1], params[2]).sum()  # failure times
        LL_rc += Fit_Normal_Eyring.logR(t_rc, T_rc, params[0], params[1], params[2]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Normal_Power:
    ...
    def __init__(self, failures=None, failure_stress=None, right_censored=None, right_censored_stress=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Normal_Power.LL), guess, args=(failures, right_censored, failure_stress, right_censored_stress), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Normal-Power model. Try a better initial guess by specifying the parameter initial_guess = [a,n]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2]]
            LL2 = 2 * Fit_Normal_Power.LL(guess, failures, right_censored, failure_stress, right_censored_stress)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Normal_Power.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress)), np.array(tuple(right_censored_stress)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.n_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.sigma_SE = abs(covariance_matrix[2][2]) ** 0.5
    ...
    def LL(params, t_f, t_rc, T_f, T_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Normal_Power.logf(t_f, T_f, params[0], params[1], params[2]).sum()  # failure times
        LL_rc += Fit_Normal_Power.logR(t_rc, T_rc, params[0], params[1], params[2]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Normal_Dual_Exponential:
    ...
    def __init__(self, failures=None, failure_stress_1=None, failure_stress_2=None, right_censored=None, right_censored_stress_1=None, right_censored_stress_2=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        common_shape_1 = ALT_fit_1.common_shape
        ALT_fit_2 = ALT_probability_plot_Normal(failures=failures, right_censored=right_censored, failure_stress=failure_stress_2, right_censored_stress=right_censored_stress_2, print_results=False, show_plot=False, common_shape_method='average')
        common_shape_2 = ALT_fit_2.common_shape
        common_shape = np.average([common_shape_1, common_shape_2])
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Normal_Dual_Exponential.LL), guess, args=(failures, right_censored, failure_stress_1, failure_stress_2, right_censored_stress_1, right_censored_stress_2), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Normal-Dual-Exponential model. Try a better initial guess by specifying the parameter initial_guess = [a,b,c]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2], params[3]]
            LL2 = 2 * Fit_Normal_Dual_Exponential.LL(guess, failures, right_censored, failure_stress_1, failure_stress_2, right_censored_stress_1, right_censored_stress_2)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Normal_Dual_Exponential.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress_1)), np.array(tuple(failure_stress_2)), np.array(tuple(right_censored_stress_1)), np.array(tuple(right_censored_stress_2)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.b_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.c_SE = abs(covariance_matrix[2][2]) ** 0.5
    ...
    def LL(params, t_f, t_rc, S1_f, S2_f, S1_rc, S2_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Normal_Dual_Exponential.logf(t_f, S1_f, S2_f, params[0], params[1], params[2], params[3]).sum()  # failure times
        LL_rc += Fit_Normal_Dual_Exponential.logR(t_rc, S1_rc, S2_rc, params[0], params[1], params[2], params[3]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Normal_Power_Exponential:
    ...
    def __init__(self, failures=None, failure_stress_thermal=None, failure_stress_nonthermal=None, right_censored=None, right_censored_stress_thermal=None, right_censored_stress_nonthermal=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        common_shape_1 = ALT_fit_1.common_shape
        ALT_fit_2 = ALT_probability_plot_Normal(failures=failures, right_censored=right_censored, failure_stress=failure_stress_nonthermal, right_censored_stress=right_censored_stress_nonthermal, print_results=False, show_plot=False, common_shape_method='average')
        common_shape_2 = ALT_fit_2.common_shape
        common_shape = np.average([common_shape_1, common_shape_2])
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Normal_Power_Exponential.LL), guess, args=(failures, right_censored, failure_stress_thermal, failure_stress_nonthermal, right_censored_stress_thermal, right_censored_stress_nonthermal), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Normal-Power-Exponential model. Try a better initial guess by specifying the parameter initial_guess = [a,c,n]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2], params[3]]
            LL2 = 2 * Fit_Normal_Power_Exponential.LL(guess, failures, right_censored, failure_stress_thermal, failure_stress_nonthermal, right_censored_stress_thermal, right_censored_stress_nonthermal)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Normal_Power_Exponential.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress_thermal)), np.array(tuple(failure_stress_nonthermal)), np.array(tuple(right_censored_stress_thermal)), np.array(tuple(right_censored_stress_nonthermal)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.c_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.n_SE = abs(covariance_matrix[2][2]) ** 0.5
    ...
    def LL(params, t_f, t_rc, S1_f, S2_f, S1_rc, S2_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Normal_Power_Exponential.logf(t_f, S1_f, S2_f, params[0], params[1], params[2], params[3]).sum()  # failure times
        LL_rc += Fit_Normal_Power_Exponential.logR(t_rc, S1_rc, S2_rc, params[0], params[1], params[2], params[3]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Exponential_Exponential:
    ...
    def __init__(self, failures=None, failure_stress=None, right_censored=None, right_censored_stress=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        if failures is None or len(failures) < 1:
            raise ValueError('Maximum likelihood estimates could not be calculated for these data. There must be at least 1 failure to calculate Exponential parameters.')
        if CI <= 0 or CI >= 1:
            raise ValueError('CI must be between 0 and 1. Default is 0.95 for 95% Confidence interval.')
        # adjust inputs to be arrays
        if type(failures) == list:
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Exponential_Exponential.LL), guess, args=(failures, right_censored, failure_stress, right_censored_stress), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Exponential-Exponential model. Try a better initial guess by specifying the parameter initial_guess = [a,b]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1]]
            LL2 = 2 * Fit_Exponential_Exponential.LL(guess, failures, right_censored, failure_stress, right_censored_stress)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Exponential_Exponential.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress)), np.array(tuple(right_censored_stress)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.b_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.a_upper = self.a + (Z * self.a_SE)
    ...
    def LL(params, t_f, t_rc, T_f, T_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Exponential_Exponential.logf(t_f, T_f, params[0], params[1]).sum()  # failure times
        LL_rc += Fit_Exponential_Exponential.logR(t_rc, T_rc, params[0], params[1]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Exponential_Eyring:
    ...
    def __init__(self, failures=None, failure_stress=None, right_censored=None, right_censored_stress=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Exponential_Eyring.LL), guess, args=(failures, right_censored, failure_stress, right_censored_stress), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Exponential-Eyring model. Try a better initial guess by specifying the parameter initial_guess = [a,c]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1]]
            LL2 = 2 * Fit_Exponential_Eyring.LL(guess, failures, right_censored, failure_stress, right_censored_stress)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Exponential_Eyring.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress)), np.array(tuple(right_censored_stress)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.c_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.a_upper = self.a + (Z * self.a_SE)
    ...
    def LL(params, t_f, t_rc, T_f, T_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Exponential_Eyring.logf(t_f, T_f, params[0], params[1]).sum()  # failure times
        LL_rc += Fit_Exponential_Eyring.logR(t_rc, T_rc, params[0], params[1]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Exponential_Power:
    ...
    def __init__(self, failures=None, failure_stress=None, right_censored=None, right_censored_stress=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Exponential_Power.LL), guess, args=(failures, right_censored, failure_stress, right_censored_stress), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Exponential-Power model. Try a better initial guess by specifying the parameter initial_guess = [a,n]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1]]
            LL2 = 2 * Fit_Exponential_Power.LL(guess, failures, right_censored, failure_stress, right_censored_stress)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Exponential_Power.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress)), np.array(tuple(right_censored_stress)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.n_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.a_upper = self.a + (Z * self.a_SE)
    ...
    def LL(params, t_f, t_rc, T_f, T_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Exponential_Power.logf(t_f, T_f, params[0], params[1]).sum()  # failure times
        LL_rc += Fit_Exponential_Power.logR(t_rc, T_rc, params[0], params[1]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Exponential_Dual_Exponential:
    ...
    def __init__(self, failures=None, failure_stress_1=None, failure_stress_2=None, right_censored=None, right_censored_stress_1=None, right_censored_stress_2=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Exponential_Dual_Exponential.LL), guess, args=(failures, right_censored, failure_stress_1, failure_stress_2, right_censored_stress_1, right_censored_stress_2), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Exponential-Dual-Exponential model. Try a better initial guess by specifying the parameter initial_guess = [a,b,c]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2]]
            LL2 = 2 * Fit_Exponential_Dual_Exponential.LL(guess, failures, right_censored, failure_stress_1, failure_stress_2, right_censored_stress_1, right_censored_stress_2)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Exponential_Dual_Exponential.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress_1)), np.array(tuple(failure_stress_2)), np.array(tuple(right_censored_stress_1)), np.array(tuple(right_censored_stress_2)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.b_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.c_SE = abs(covariance_matrix[2][2]) ** 0.5
    ...
    def LL(params, t_f, t_rc, S1_f, S2_f, S1_rc, S2_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Exponential_Dual_Exponential.logf(t_f, S1_f, S2_f, params[0], params[1], params[2]).sum()  # failure times
        LL_rc += Fit_Exponential_Dual_Exponential.logR(t_rc, S1_rc, S2_rc, params[0], params[1], params[2]).sum()  # right censored times
        return -(LL_f + LL_rc)
...
class Fit_Exponential_Power_Exponential:
    ...
    def __init__(self, failures=None, failure_stress_thermal=None, failure_stress_nonthermal=None, right_censored=None, right_censored_stress_thermal=None, right_censored_stress_nonthermal=None, use_level_stress=None, show_plot=True, print_results=True, CI=0.95, initial_guess=None):
        ...
        while delta_BIC > 0.001:
            if runs < 3:
                ...
                tol = 1e-6
            runs += 1
            result = minimize(value_and_grad(Fit_Exponential_Power_Exponential.LL), guess, args=(failures, right_censored, failure_stress_thermal, failure_stress_nonthermal, right_censored_stress_thermal, right_censored_stress_nonthermal), jac=True, tol=tol, method='nelder-mead', options={'maxiter': 5000})
            if result.success is False:
                raise RuntimeError('Fitting using Autograd FAILED for the Exponential-Power-Exponential model. Try a better initial guess by specifying the parameter initial_guess = [a,c,n]. Alternatively, try another life-stress model.')
            params = result.x
            guess = [params[0], params[1], params[2]]
            LL2 = 2 * Fit_Exponential_Power_Exponential.LL(guess, failures, right_censored, failure_stress_thermal, failure_stress_nonthermal, right_censored_stress_thermal, right_censored_stress_nonthermal)
            BIC_array.append(np.log(n) * k + LL2)
            delta_BIC = abs(BIC_array[-1] - BIC_array[-2])
        ...
        # confidence interval estimates of parameters
        Z = -ss.norm.ppf((1 - CI) / 2)
        hessian_matrix = hessian(Fit_Exponential_Power_Exponential.LL)(np.array(tuple(params)), np.array(tuple(failures)), np.array(tuple(right_censored)), np.array(tuple(failure_stress_thermal)), np.array(tuple(failure_stress_nonthermal)), np.array(tuple(right_censored_stress_thermal)), np.array(tuple(right_censored_stress_nonthermal)))
        covariance_matrix = np.linalg.inv(hessian_matrix)
        self.a_SE = abs(covariance_matrix[0][0]) ** 0.5
        self.c_SE = abs(covariance_matrix[1][1]) ** 0.5
        self.n_SE = abs(covariance_matrix[2][2]) ** 0.5
    ...
    def LL(params, t_f, t_rc, S1_f, S2_f, S1_rc, S2_rc):  # log likelihood function
        LL_f = 0
        LL_rc = 0
        LL_f += Fit_Exponential_Power_Exponential.logf(t_f, S1_f, S2_f, params[0], params[1], params[2]).sum()  # failure times
        LL_rc += Fit_Exponential_Power_Exponential.logR(t_rc, S1_rc, S2_rc, params[0], params[1], params[2]).sum()  # right censored times
        return -(LL_f + LL_rc)
