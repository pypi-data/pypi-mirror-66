...
def upload_cap_uri_list(knex, org_name, lib_name, uri_list, threads=1, endpoint_url=WASABI_URL, module_prefix=''):
    ...
    upload_func = lambda args: upload_one_cap_uri(
        args[0], args[1], endpoint_url=endpoint_url, module_prefix=module_prefix
    )
    upload_args = [(lib, uri) for uri in uri_list]
    with Pool(threads) as pool:
        for field in pool.imap_unordered(upload_func, upload_args):
            yield field
