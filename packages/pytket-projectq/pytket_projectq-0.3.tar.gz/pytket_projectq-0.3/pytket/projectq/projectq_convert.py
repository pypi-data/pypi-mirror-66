...
from pytket.circuit import OpType, Op, Circuit, Command, _get_op, Bit
...
def _add_daggered_op_to_circuit(cmd: ProjectQCommand, circ: Circuit):
    ...
    if qubit_no >= circ.n_qubits:
        ...
        new_qubit = True
    circ._add_gate(Op=op, args=[qubit_no])
    return new_qubit
...
def _add_single_qubit_op_to_circuit(cmd: ProjectQCommand, circ: Circuit):
    ...
    if get_control_count(cmd) > 0:
        ...
        if type(cmd.gate) == pqo.MeasureGate:
            ...
            op = _get_op(
                OpType=_pq_to_tk_singleqs[type(cmd.gate)],
                parameters=[cmd.gate.angle / np.pi],
            )
        else:
...
class tketOptimiser(BasicEngine):
    ...
    def _optimise(
        self,
    ):  # takes the circuit and optimises it before regurgitating it as a series of ProjectQ commands
