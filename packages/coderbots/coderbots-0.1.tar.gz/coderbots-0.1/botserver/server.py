...
m       fm
mmmmmmmmmm'''
## Trailing newline is very necessary
...
class PlayerV1:
    def __init__(self,x,y,game,color,pid,direction):
        ...
        self.yv=0
        self.dist=0
        self.speed=50 ## Smaller=faster
        self.writer=None
        self.direction=direction
        self.pid=pid
        self.color=color
        self.game=game
        self.id=game.canvas.create_rectangle(x*50,y*50,(x+1)*50,(y+1)*50,outline="",fill=color)
    def run(self):
    ...
    def move(self,amnt,writer):
        if self.direction=="0":
            self.xv=self.speed/50*(amnt/abs(amnt))
        elif self.direction=="1":
            self.yv=self.speed/50*(amnt/abs(amnt))
...
class Game:
    def __init__(self,leveln,width,height):
        self.tk=Tk()
        self.canvas=Canvas(self.tk,width=500,height=500,background="white")
        self.canvas.pack()
        self.players={}
        self.toppid=1
        self.levels=[Levels.lvl1]
        self.solid=[]
        active=self.levels[leveln]
        self.startpos=[]
        self.endpos=[]
        self.fillBlockSet(active)
        for x in range(0,int(width/50)):
            self.canvas.create_line(x*50,0,x*50,height)
        for y in range(0,int(height/50)):
            self.canvas.create_line(0,y*50,width,y*50)
    ...
    def fillBlockSet(self,level):
        ...
        for ex in range(0,len(level)):
            ...
            tp=level[ex]
            if tp==" ":
                self.canvas.create_rectangle(x*50,y*50,(x+1)*50,(y+1)*50,fill="lightgreen",outline="")
            elif tp=="s":
                self.canvas.create_rectangle(x*50,y*50,(x+1)*50,(y+1)*50,fill="yellow",outline="")
                self.setStartPos(x,y)
            elif tp=="f":
                self.canvas.create_rectangle(x*50,y*50,(x+1)*50,(y+1)*50,fill="blue",outline="")
                self.setEndPos(x,y)
            elif tp=="m":
                self.solid.append(self.canvas.create_rectangle(x*50,y*50,(x+1)*50,(y+1)*50,outline="",fill="brown"))
    def run(self):
...
class Server:
    def __init__(self,leveln,width=500,height=500,host="localhost",port=8877):
        self.game=Game(leveln,width,height)
