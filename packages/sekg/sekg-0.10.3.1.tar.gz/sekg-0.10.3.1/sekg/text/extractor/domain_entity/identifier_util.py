...
class IdentifierInfoExtractor:
    ...
    def extract_from_method_name(self, method_name: str, mark_for_identifier_in_relation=IDENTIFIER_ENTITY):
        ...
        check_len = False
        check_size = False
        if method_name == "exists" or method_name.find(".exists(") > 0 or method_name.endswith(".exists"):
            check_exist = True
            operation_relation_name = "operation_" + "check if exist"
            identifier_relations.add((mark_for_identifier_in_relation, operation_relation_name, ""))
        elif method_name == "len" or method_name.find(".len(") > 0 or method_name.endswith(".len"):
            operation_relation_name = "operation_" + "get len"
            identifier_relations.add((mark_for_identifier_in_relation, operation_relation_name, ""))
            check_len = True
        elif method_name == "size" or method_name.find(".size(") > 0 or method_name.endswith(".size"):
            check_size = True
            operation_relation_name = "operation_" + "get size"
            identifier_relations.add((mark_for_identifier_in_relation, operation_relation_name, ""))
        if check_size or check_len or check_exist:
