...
def thread(outline_pts: Points,
    ...
    # def rad_for_angle(angle, total_angle, r1, r2, tooth_profile, neck_in_degrees, neck_out_degrees):
    ...
    def rad_for_angle(angle, neck_in_degrees, neck_out_start, total_angle):
    ...
    def segment_face_indices(step, total_steps, poly_sides):
        indices = []
        if step < total_steps - 1:
            ind = i * poly_sides
            for j in range(ind, ind + poly_sides - 1):
                indices.append([j, j + 1, j + poly_sides])
                indices.append([j + 1, j + poly_sides + 1, j + poly_sides])
            indices.append([ind, ind + poly_sides - 1 + poly_sides, ind + poly_sides - 1])
            indices.append([ind, ind + poly_sides, ind + poly_sides - 1 + poly_sides])                
        return indices
    
    def segment_points(tooth_outline, elev_vec, angle, inverse_thread_direction=False):
        points = []
        # create new points
        for p in tooth_outline:
            theta = radians(angle) * (-1 if inverse_thread_direction else 1)
            pt = (p + elev_vec).rotate_around(axis=euc_up, theta=theta)
            points.append(pt.as_arr())        

        return points

    for i in range(total_steps):
        angle = min(i * step_angle, total_angle)
        elevation = min(i * up_step, length)

        rad = rad_for_angle(angle, neck_in_degrees, neck_out_start, total_angle)
        elev_vec = Vector3(rad, 0, elevation)
        all_points += segment_points(euc_points, elev_vec, angle, inverse_thread_direction)
        all_tris += segment_face_indices(i, total_steps, poly_sides)
    # end polys for beginning and end of shape
    all_tris.append(list(range(poly_sides)))
    all_tris.append(list(range(len(all_points) - poly_sides, len(all_points))))
    ...
    # Make the polyhedron (convexity necessary for correct OpenSCAD render)
