...
from functional_predicates.errors import ValueAlreadySetError
from functional_predicates.errors import ValueMissingError
...
class Base(ABC):
    ...
    def __bool__(self: Base) -> bool:
        if self._has_value:
            ...
            raise ValueMissingError(f"{self} is missing a value")

    def __call__(self: BaseLike, value: Any) -> BaseLike:
        if self._has_value:
            raise ValueAlreadySetError(f"{self} already has a value")
        else:
