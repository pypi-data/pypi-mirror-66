...
class JobBase:
    ...
    def run(self, **kwargs):
        ...
        if kwargs.get('file'):
            ...
            if file_type == 'csv':
                export_file_result = self._dao.export_data_to_csv(file, project_name=self.__class__.__name__)
            elif file_type == 'parquet':
                export_file_result = None
                pass
            else:
                raise ValueError('unknown file type', str(file))
            run_result['export_file_result'] = export_file_result
        return run_result
