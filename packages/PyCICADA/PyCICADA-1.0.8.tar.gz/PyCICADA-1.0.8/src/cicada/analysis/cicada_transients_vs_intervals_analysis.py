    ...
    def run_analysis(self, **kwargs):
        ...
        for session_index, session_data in enumerate(self._data_to_analyse):
            ...
            sum_up_data = {'Age': age_list, 'Weight': weight_list, 'SubjectID': animal_id_list,
                           'Session': session_identifier_list, 'Celltype': cell_type_list, 'Rest': rest_frequency}
        ...
        if cell_to_use == "Pyramidal_cells":
            data_table = global_epoch_frequency_table.query('Celltype == "Pyramidal"')
            n_pyr = len(data_table.index)

        if cell_to_use == "Interneurons":
            data_table = global_epoch_frequency_table.query('Celltype == "Interneuron"')
        ...
        if cell_to_use == "All_cells":
            ...
            pyr_data_table = global_epoch_frequency_table.query('Celltype == "Pyramidal"')
            n_pyr = len(pyr_data_table.index)

            ins_data_table = global_epoch_frequency_table.query('Celltype == "Interneuron"')
