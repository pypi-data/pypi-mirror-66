...
_LEGACY_GWF_RE = re.compile(r'([A-Z]+)-([A-Za-z0-9_]+)-([0-9]{5,5})(.*)')
_IGWN_GWF_RE = re.compile(r'([A-Z]+)-([A-Za-z0-9_]+)-([0-9]{3,3})(.*)')
...
def kagra_offline(scope, path, rse, rse_attrs, proto_attrs):
    """
    This function maps the scope:LFN to the PFN.

    This version is a quasi-non-deterministic algorithm:
     * Strips the leading /gpfs from file paths
     * Leaves remaining name untouched

    Parameters:

    :param scope: Scope of the LFN.
    :param path: original path to the LFN.
    :param rse: RSE for PFN (ignored)
    :param rse_attrs: RSE attributes for PFN (ignored)
    :param protocol_attrs: RSE protocol attributes for PFN (ignored)
    :returns: Path for use in the PFN generation.
    ...
    # Exception for automatix test data. E.g.,
    # test.61f182e47315405ebc029599672199f2
    match = _TESTS_RE.match(os.path.basename(path))
    if match:
        return "%s/%s" % (scope, os.path.basename(path))

    pfn = path.replace('/gpfs', '')
...
if __name__ == '__main__':
    ...
    test_mapping(kagra_offline, "O3",
                 "/gpfs/data/proc/C10/O3/K1/K-K1_C10-1268604928-4096.gwf",
                 "/data/proc/C10/O3/K1/K-K1_C10-1268604928-4096.gwf")

#    test_mapping(igwn_frames, "O3", "K-K1_C10-1268039680-4096.gwf",
#                 "O3/hoft/K1/K-K1_C10-126/K-K1_C10-1268039680-4096.gwf")
