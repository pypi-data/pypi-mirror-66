...
class TPXO:
    ...
    def _fetch_tpxo_file(prefix, file):
            ...
            while 1:
                ...
                if default is not None and choice == '':
                    ...
                    sys.stdout.write("Please respond with 'yes' or 'no' "
                                     "(or 'y' or 'n').\n")
        print("*** PLEASE READ ***")
        msg = "\nThe function you are trying to use requires the TPXO file but"
        msg += 'it was not found the on system.'
        warnings.warn(msg)
        q = 'This software can automatically fetch the TPXO for you using the '
        q += 'internet. You may also cancel this operation and provide '
        ...
        q += "for the internet now?"
        a = query_yes_no(q)
        if a is False:
            raise RuntimeError('No TPXO database found.')
        else:
            msg = f'Downloding TPXO database to {str(file)}'
            msg += ' please wait... \n'
