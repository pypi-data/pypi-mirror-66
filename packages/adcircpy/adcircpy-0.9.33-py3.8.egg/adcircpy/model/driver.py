...
class AdcircRun(Fort15):
    ...
    def write(
        ...
        if fort22:
            if self.wind_forcing is not None:
                ...
                self.wind_forcing.write(path, overwrite)
        if coldstart:
            path = output_directory / coldstart
            super().write('coldstart', path, overwrite)
        if hotstart:
            path = output_directory / hotstart
            super().write('hotstart', path, overwrite)
    ...
    def _certify_spinup(spinup):
        msg = f"Error: spinup argument must be either {None}, "
        msg += f"or an instance of type {timedelta}."
        assert isinstance(
            spinup, (
                type(None),
