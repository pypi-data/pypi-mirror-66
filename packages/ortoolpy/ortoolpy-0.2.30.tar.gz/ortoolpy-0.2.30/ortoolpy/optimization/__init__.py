...
def MinimumSpanningTree(dfed, from_label='node1', to_label='node2',
    ...
    dftmp = pd.DataFrame([f'{min(i,j)}-{max(i,j)}'
        for i,j in t.edges()], columns=['FrTo_'])
    return pd.merge(dfed, dftmp).drop('FrTo_',1)

def MaximumStableSet(dfnd, dfed, node_label='id', weight_label='weight',
        from_label='node1', to_label='node2', **kwargs):
    """
    最大安定集合問題
    入力
    ...
    出力
        ...
        (重みの和は、結果のweight_labelのsum())
    """
    g,dfnd,dfed = graph_from_table(dfnd, dfed, node_label=node_label,
        from_label=from_label, to_label=to_label, **kwargs)
    lst = maximum_stable_set(g, weight=weight_label)[1]
    return dfnd[dfnd[node_label].isin(set(lst))]

def MaximumCut(dfnd, dfed, node_label='id', from_label='node1',
        to_label='node2', weight_label='weight', **kwargs):
    """
    最大カット問題
    入力
        dfnd: 点のDataFrameもしくはCSVファイル名
...
def DijkstraPath(dfed, source, target, from_label='node1', to_label='node2',
        weight_label='weight', **kwargs):
    """
    最短路問題
    入力
    ...
    return pd.concat([dfed[dfed.FrTo_==f'{min(i,j)}-{max(i,j)}']
        for i,j in pairwise(rt)]).drop('FrTo_',1)

def MaximumFlow(dfed, source, target, from_label='node1',
        to_label='node2', capacity_label='capacity',
        flow_label='flow', **kwargs):
    """
    最大流問題
    入力
        dfed: 辺のDataFrameもしくはCSVファイル名
    ...
    dftmp = pd.DataFrame([(f'{min(i,j)}-{max(i,j)}',f) for i, d in t.items()
        for j, f in d.items() if f], columns=['FrTo_',flow_label])
    return r, pd.merge(dfed, dftmp).drop('FrTo_',1)

def MinCostFlow(dfnd, dfed, node_label='id', demand_label='demand',
        from_label='node1', to_label='node2', weight_label='weight',
        capacity_label='capacity', flow_label='flow', **kwargs):
    """
    最小費用流問題
    入力
        dfnd: 点のDataFrameもしくはCSVファイル名
    ...
    出力
        辺のDataFrame
    """
    g,dfnd,dfed = graph_from_table(dfnd, dfed, True, node_label=node_label,
        from_label=from_label, to_label=to_label, **kwargs)
    t = nx.min_cost_flow(g, demand=demand_label,
        capacity=capacity_label, weight=weight_label)
    dftmp = pd.DataFrame([(i,j,f) for i, d in t.items()
        for j, f in d.items() if f], columns=[from_label,to_label,flow_label])
    return pd.merge(dfed, dftmp)

def Vrp(dfnd, dfed, nv, capa, node_label='id', demand_label='demand',
        from_label='node1', to_label='node2', cost_label='cost',
        vehicle_label='car', series_label='num', **kwargs):
    """
    運搬経路問題
    入力
        dfnd: 点のDataFrameもしくはCSVファイル名
    ...
    """
    g,dfnd,dfed = graph_from_table(dfnd, dfed, node_label=node_label,
        from_label=from_label, to_label=to_label, from_to='FrTo_', **kwargs)
    t = vrp(g.to_directed(), nv, capa, demand=demand_label, cost=cost_label)
    dftmp = pd.DataFrame([(f'{min(i,j)}-{max(i,j)}',h,k)
        for h,l in enumerate(t) for k,(i,j) in enumerate(l)],
...
def ChinesePostman(dfed, from_label='node1', to_label='node2',
        weight_label='weight', **kwargs):
    """
    中国人郵便配達問題
    入力
    ...
    出力
        ...
        (重みの和は、結果のweight_labelのsum())
    """
    g,_,dfed = graph_from_table(None, dfed, multi=True, from_label=from_label,
        to_label=to_label, from_to='FrTo_', **kwargs)
    r,t = chinese_postman(g, weight=weight_label)
    dftmp = pd.DataFrame([(f'{min(i,j)}-{max(i,j)}',) for i,j in t],
        columns=['FrTo_'])
    return r, pd.merge(dftmp, dfed).drop('FrTo_',1)

def SetCovering(df, id_label='id', weight_label='weight',
        element_label='element', is_partition=False, **kwargs):
    """
    集合被覆問題
    入力
    ...
    r = set_covering(len(g), [(r[weight_label].iloc[0],
        r[element_label].tolist()) for _,r in g], is_partition=is_partition)
    df = df[df[id_label].isin(r)].copy()
    df.loc[df[id_label]==df[id_label].shift(1), weight_label] = np.nan
    return df

def SetPartition(df, id_label='id', weight_label='weight',
        element_label='element', **kwargs):
    """
    集合分割問題
    入力
        df: 候補のDataFrameもしくはCSVファイル名
    ...
    出力
        選択された候補リストの番号リスト
    """
    return SetCovering(df, id_label, weight_label,
        element_label, is_partition=True, **kwargs)
...
def TwoMachineFlowshop(df, first_machine_label='first',
        second_machine_label='second', **kwargs):
    """
    入力
...
def ShiftScheduling(ndy, nst, shift, proh, need):
    ...
    df = pd.DataFrame(np.vectorize(lambda i: shift[i])(r),
...
def Knapsack(df, capacity, size_label='size',
        weigh_labelt='weight', **kwargs):
    """
    ナップサック問題
        価値の最大化
    入力
...
if _ip:
    _ip.register_magic_function(typical_optimization_impl,
