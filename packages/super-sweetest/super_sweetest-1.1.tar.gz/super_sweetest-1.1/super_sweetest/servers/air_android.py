...
ST.THRESHOLD_STRICT = 0.6  # assert_exists语句的默认阈值，一般比THRESHOLD更高一些
ST.THRESHOLD = 0.6  # 其他语句的伐值
ST.OPDELAY = 1  # 每一步操作后等待1秒再进行下一步操作
ST.FIND_TIMEOUT = 10  # 图像查找超时时间
ST.CVSTRATEGY = ["surf", "tpl", "brisk"]  # 修改图片识别算法顺序，只要成功匹配任意一个符合设定阙值的结果，程序就会认为识别成功
...
class AirAndroid():
    ...
    def setup_air_android(self, device_name, app_package, desired_caps):
        ...
        assert isinstance(c_t, list), "格式错误 正确格式-> 'connect_type_list':['', 'javacap', 'adbori', 'adbtouch'],"
    ...
    def teardown_air_android(self, g):
        """关闭app，停止poco实例"""
        # stop_app(g.appPackage) # 每次启动时关闭，结束不关闭app适合调试
