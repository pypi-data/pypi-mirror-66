...
def integand_tan_dth_pot(u, r, th, dens_pot, sigma_pot, qintr_pot):
    ...
    This is equation (51) of Cappellari (2020, MNRAS, arXiv:1907.09894)
...
def integrand_sph(s, t, r, th, dens_lum, sigma_lum, qintr_lum,
    ...
    from eq.(52)-(54) of Cappellari (2020, MNRAS, arXiv:1907.09894)
...
class jam_axi_intr:
    ...
    data: array_like of shape (4, p)
        Four vectors with the observed values of
        ``[sigR, sigz, sigphi, vphi]`` in km/s.
    ...
    .model: array_like with shape (4, p)
        ...
        The mnean velocity can be computed as ``vphi = np.sqrt(v2phi - sig2phi)``
    ...
    def __init__(self, dens_lum, sigma_lum, qintr_lum, dens_pot, sigma_pot, qintr_pot,
        ...
        #
        if mbh > 0:
            sigmaBH = rbh # Adopt for the BH just a very small size of (default 1pc)
        ...
        *model, nu = intrinsic_moments(
        ...
        if data is None:
            ...
            model1 = model.copy()  # For plotting (sig2r, sig2th, sig2phi, v2phi)
            model1[3] -= model[2]  # Vphi2 = V2phi - sig2phi
            model1 = np.sqrt(model1)
            d, m, e, ok = map(np.concatenate, [data, model1, errors, goodbins])

            if (ml is None) or (ml <= 0):

                ml = (np.sum(d[ok]*m[ok]/e[ok]**2)/np.sum((m[ok]/e[ok])**2))**2  # eq. (51 C08)
                m *= np.sqrt(ml)
                model1 = m.reshape(4, -1)

            chi2 = np.sum(((d[ok] - m[ok])/e[ok])**2)/ok.sum()

            if plot:
                self.plot_jam(Rbin, zbin, data, model1, nu,
...
##############################################################################
    ...
    def plot_jam(self, Rbin, zbin, data, model, nu, nodots=True, fignum=1, align='cyl'):
        ...
        if align == 'cyl':
            txt = [r"$\sigma_R$", r"$\sigma_z$", r"$\sigma_\phi$", r"$V_\phi$"]
        else:
            txt = [r"$\sigma_r$", r"$\sigma_\theta$", r"$\sigma_\phi$", r"$V_\phi$"]

        ax[0, 0].set_title("Data")
        ax[0, 1].set_title("JAM Model")
...
def satoh_solution(R, z, beta):
    ...
    sig2z = G*M*Q*(a + 2*Q)/(3*Q*(a + 2*Q) + S**2)/(2*S)        # eq.(68) Cappellari (2020)
    v2phi = sig2z*(1 - 6*(R/S)**2)/(1 - beta) + G*M*R**2/S**3   # eq.(69) Cappellari (2020)
...
def jam_axi_intr_example():
    ...
    I use as "data" the anysotropic cylindrical solution
    from Cappellari (2020) for the density by Satoh (1980)
    ...
    log_dens_lum, log_sigma_lum, qintr_lum = np.loadtxt('python2019/mge_satoh_published.txt', unpack=True)
    ...
    data = np.sqrt([sig2R, sig2z, sig2phi, v2phi - sig2phi])

    jam = jam_axi_intr(dens_lum, sigma_lum, qintr_lum, dens_pot, sigma_pot, qintr_pot,
                        mbh, Rbin, zbin, beta=beta, gamma=gamma, interp=True, align='cyl',
                        plot=1, data=data, ml=-1)

    print(f"M/L = {jam.ml:.2f}")
