...
class ConfigModel(object):
    def __init__(self, folder):
        ...
        self.ROOT_FOLDER = folder.rsplit(self.__os_sep, maxsplit=1)[0]
        self.PROJECT_NAME = self.ROOT_FOLDER.rsplit(self.__os_sep, maxsplit=1)[1]

        self.SQLITE_DEBUG_FILE = "test_mode.db"

        # Credentials
        self.CREDENTIALS = os.path.join(self.ROOT_FOLDER, 'credentials')
        self.SECRETS_FILE = f'{self.CREDENTIALS}{self.__os_sep}credentials.json'
        self.SECRETS_FILE_PYGSHEETS = f'{self.CREDENTIALS}{self.__os_sep}client_secret.json'
        self.TOKEN_FILE = f'{self.CREDENTIALS}{self.__os_sep}token.pickle'
        ...
        # Logs Config
        self.LOGS_PATH = 'logs'
        self.LOGS_FILE = os.path.join(self.ROOT_FOLDER, f'{self.LOGS_PATH}{self.__os_sep}{self.PROJECT_NAME}_{self.NOW}.log')
    ...
    def get_log(self):
        if self.test["log"]:
            if self.test["log"] == 'aws':
                logging.basicConfig(level=self.LOG_LEVEL, format=self.FORMAT)
            else:
