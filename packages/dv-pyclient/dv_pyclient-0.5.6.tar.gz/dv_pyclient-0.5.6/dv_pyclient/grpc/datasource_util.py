            ...
            for col in project_cols:
                ...
                if __isStringColumnType(col.type):
                    if row[col.name] == None:
                        ...
                        strings[string_dict[col.name]] = api.OptionalString(
                ...
                # Numbers
                    ...
                    if row[col.name] == None or math.isnan(row[col.name]):
                        ...
                        numbers[number_dict[col.name]] = api.OptionalNumber(
                ...
                # Times
                    ...
                    if row[col.name] == None or math.isnan(row[col.name].value):
                        ...
                        times[time_dict[col.name]] = api.OptionalTime(
                            value=proto.Int64Value(value=__tsToUnixEpochSeconds(row[col.name])))
...
def dataSourceQueryStreamPandas(df, request):
    ...
    project_names = list(map(lambda x: x.name, request.projectColumns))

    # Grab our string + time projections to sort byt
    string_project = list(filter(lambda x: __isStringColumnType(x.type), request.projectColumns))
    string_names = list(map(lambda x: x.name, filter(lambda x: x.name, string_project)))

    time_project = list(filter(lambda x: __isTimeColumnType(x.type), request.projectColumns))
    time_names = list(map(lambda x: x.name, filter(lambda x: x.name, time_project)))
