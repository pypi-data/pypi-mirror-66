...
class RequiredFactory(object):
    code_line: str = None
    app_name: str = None
    model_name: str = None

    def __init__(self, code_line=None, app_name=None, model_name=None):
...
class FactoryMeta(object):
    ...
    import_libs: [str] = []
    required_models: [str] = []
    required_factory: RequiredFactory = None
    ...
    def generate_char_field_code(self):
        ...
        self.code_line = \
            '{} = factories.Sequence(lambda n: \'{} {} %03d\' % n)' \
    ...
    def generate_integer_field_code(self):
        min_value = self.field.min_value \
            if self.field.min_value is not None else 0
        max_value = self.field.max_value \
            if self.field.max_value is not None else 1000
    ...
    def generate_float_field_code(self):
        min_value = self.field.min_value \
            if self.field.min_value is not None else 0.00
        max_value = self.field.max_value \
            if self.field.max_value is not None else 1000.00
    ...
    def generate_date_field_code(self):
        self.code_line = \
            '{} = factories.FuzzyDate(datetime.date(2020, 1, 1))' \
            .format(self.field.name)
    ...
    def add_required_factory(self, app_name, model_name):
        ...
        self.required_factory = RequiredFactory(
            app_name=app_name,
            model_name=model_name,
            code_line=import_line,
        )
