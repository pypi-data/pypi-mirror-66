...
@dataclasses.dataclass
class PartialDelete(PartialBase, WhereMixin, ReturningMixin):
    ...
    def __await__(self):
        ...
        logger.debug(stmt)
        return self.conn.fetch(stmt, *ctx.values).__await__()


@dataclasses.dataclass
class PartialUpdate(PartialBase, WhereMixin, ReturningMixin, SetMixin):
    ...
    def __await__(self):
        ...
        # Wait for the update.
        logger.debug(stmt)
        result = yield from self.conn.fetchrow(stmt, *ctx.values).__await__()
...
@dataclasses.dataclass
class PartialSelect(PartialBase, WhereMixin, SelectMixin):
...
@dataclasses.dataclass
class PartialInsert(PartialBase, ReturningMixin, ValuesMixin):
    """ Insert statement. """
...
@dataclasses.dataclass
class PartialCreateTable(PartialBase):
