...
def cost_steps(ch, steps):
    ...
    for i in range(nsteps):
        plt.plot(x, ch[i].cost_history, label='step ' + str(i + 1), color=col[i + 2])

    mini = ch[0].cost_history
    for i in range(1, nsteps):
        mini = np.maximum(mini, ch[i].cost_history)
    ...
    for s in range(1, nsteps + 1):
        ...
        p = params[i]
        # print('ax   ', plt)
        vals = get_values(p['name'], s, steps)
