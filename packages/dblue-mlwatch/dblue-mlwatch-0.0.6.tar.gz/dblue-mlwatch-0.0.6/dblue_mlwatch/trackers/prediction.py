...
from dblue_mlwatch.utils.json_encoder import JSONEncoder
...
class PredictionTracker(BaseTracker):
    ...
    def _create_prediction_event(
        ...
        try:
            _features = json.dumps(features, cls=JSONEncoder) if features else None
            _prediction_probs = json.dumps(prediction_probs, cls=JSONEncoder) if prediction_probs else None
    ...
    def capture_exception(self, message: str = None) -> bool:
        ...
        event = {
            ...
            'message': message,
            'exec_info': traceback.format_exception(*exc_info),
        }
    ...
    def capture_request_log(self, request: Dict, response: Dict, response_time: float) -> bool:
        ...
        try:
            _request = json.dumps(request, cls=JSONEncoder) if request else None
            _response = json.dumps(response, cls=JSONEncoder) if response else None
