...
class CorpusDiff:
    def __init__(self, domain_diff, intent_diff, entity_diff):
    ...
    def create_from_corpus(cls, x: Corpus, y: Corpus):
        ...
        entity_diff = cls._compare_entity(x, y)
        # TODO: maybe we need a document_diff
        return cls(domain_diff, intent_diff, entity_diff)
    ...
    def _compare_entity(cls, x: Corpus, y: Corpus):
        ...
        for doc_id in tqdm(doc_ids):
            ...
            x_spans = set((i.start, i.end) for i in x_doc.span_set)
            y_spans = set((i.start, i.end) for i in y_doc.span_set)
