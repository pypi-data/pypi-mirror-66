...
class Vision:
    ...
    #requires a gray scaled image as input
    def find_face(self,frame,bounding_box = True,scale_factor=1.05,min_neighbors=8,min_size=(40,40)):
        ...
        faces = self.face_cascade.detectMultiScale(frame, scaleFactor=scale_factor, minNeighbors=min_neighbors, minSize=min_size, flags=cv2.CASCADE_SCALE_IMAGE)
        if len(faces)>0:
    ...
    def find_smile(self,frame,bounding_box = True,scale_factor=1.05,min_neighbors=8,min_size=(40,40)):
        ...
        smiles = self.smile_cascade.detectMultiScale(frame, scaleFactor=scale_factor, minNeighbors=min_neighbors, minSize=min_size, flags=cv2.CASCADE_SCALE_IMAGE)
        if len(smiles)>0:
