...
@beestar_group.command(name="get")
@click.option('-c', '--credential', default='credentials.ini',
              type=click.Path(dir_okay=False, exists=True),
              prompt='Credential file', help='Configuration file that contain beestar cookie')
@click.option('-o', '--output', default='result.html',
              type=click.Path(dir_okay=False, exists=False),
              prompt='Output file', help='Output file')
@click.argument('review_details_url', metavar='{review_details_url}')
def beestar_get(credential, output, review_details_url):
    """
    sample:
    ptoolbox bs get -c ../../credentials.ini -o de2.html "https://beestar.org/exam?cmd=startexerciseconfirm&session_num3=5990178713121315&status=FINISH&descID=186&exam_num=2&check_sus=true"
    """
    cookie = Beestar.read_credential(credential)
    if not cookie:
        return
