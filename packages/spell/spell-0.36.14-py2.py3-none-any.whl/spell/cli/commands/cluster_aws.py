...
def eks_init(
    ...
    if not is_installed("eksctl"):
        raise ExitException(
            "`eksctl` is required, please install it before proceeding https://eksctl.io/"
        )
    ...
    try:
        git.Git(tmp_dir).clone("https://github.com/kubernetes-sigs/metrics-server")
        subprocess.check_call(
            (
                "kubectl",
                "apply",
                "--filename",
                os.path.join(tmp_dir, "metrics-server", "deploy", "kubernetes"),
            ),
            env=kubectl_env,
        )
