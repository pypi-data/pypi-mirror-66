...
def build_playlist_movie(
    ...
    if len(tmp_file_paths) > 0:
        ...
        for tmp_file_path, file_name in tmp_file_paths:
            ...
            print("tota", in_file["v"])
            print("toto", in_file["a"])
            audio = ffmpeg.probe(tmp_file_path, select_streams='a')
            if audio["streams"]:
                in_files.append(in_file["a"])
            else:
                joined = ffmpeg.copy(*in_files, v=1, a=1).node
                in_files.append(input("anullsrc"))
