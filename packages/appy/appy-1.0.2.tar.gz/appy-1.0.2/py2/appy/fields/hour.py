...
class Hour(Field):
    ...
    @classmethod
    def normalizeEnd(class_, start, end):
        '''Normalize p_end in such a way that p_end and p_start may be compared,
           such that end > start is True.'''
        # In other words, if p_end occurs the day after p_start, we add 24 hours
        # to it. The method returns the normalized p_end.
        if end <= start:
            r = end[0] + 24, end[1]
        else:
            r = end
    ...
    @classmethod
    def intersection(class_, rangeA, rangeB, minutes=False, normalize=True):
        '''Compute and return a new interval being the intersection between
           intervals p_rangeA and p_rangeB, or its number of minutes if
           p_minutes is True.'''
        # Every range is a tuple of the form ~(startHour, endHour)~
        startA, endA = rangeA
        startB, endB = rangeB
        # Normalize end hours when appropriate
        if normalize:
            endA = class_.normalizeEnd(startA, endA)
            endB = class_.normalizeEnd(startB, endB)
        ...
        if (startB >= endA) or (endB <= startA):
            ...
            # Return [start, end] or its number of minutes
            if minutes:
