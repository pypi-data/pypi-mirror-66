...
class Static:
    ...
    def writeUnchanged(class_, handler, response=None):
        '''Returns the adhoc response if the file has not changed since the last
           time the browser has downloaded it.'''
        code = 304
        handler.send_response(code)
        if response:
    ...
    def write(class_, handler, path, modified, content=None, response=None,
              fileInfo=None, disposition='attachment', enableCache=True):
        ...
        # account. Else, it will be ignored.
        # ~~~
        ...
        if not browserDate or (smodified > browserDate):
            ...
            if fileInfo:
                set('Content-Disposition',
                    '%s;filename="%s"' % (disposition, fileInfo.uploadName))
