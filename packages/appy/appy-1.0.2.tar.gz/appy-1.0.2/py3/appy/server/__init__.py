...
MIN_THREADS_KO = 'At least one thread must be in use.'
THREAD_LIMITS_KO = 'killThreadLimit (%d) should be > hungThreadLimit (%d)'
SPAWN_IF_KO = 'spawnIfUnder (%d) should be < than the number of threads (%d)'

INITIAL_WORKER = 'Initial worker'
ADDED_WORKER = 'Worker added in response to lack of idle workers'
ADDED_WORKER_KILLED = 'Worker added for replacing killed thread %s'
ADDED_WORKER_REPLACED = 'Voluntary replacement for thread %s'
WORK_DONE = ' > handled by thread %s (%i tasks queued).'
CHECK_ADD_CHECK = 'No idle workers for task; checking if we need to make ' \
  'more workers'
...
KILLING_THREAD = 'Killing thread with ID=%s...'
KILLED_HUNGED = 'Workers killed forcefully'
WORKER_TO_KILL = 'Killing worker %s... (working on task for %i seconds)'
HUNG_STATUS = "killHungThreads status: %d threads (%d working, %d idle, " \
  "%d starting), average time %s, maxTime %.2fsec, killed %d worker(s)."
...
STOPPING_THREAD = 'Thread %s processed %i requests (limit %s); stopping it'
SHUTDOWN_RECEIVED = 'Worker %s is asked to shutdown'
SHUTDOWN_POOL = 'Shutting down the threadpool (%d threads)'
SHUTDOWN_HUNG = "%s worker(s) didn't stop properly, and %s zombie(s)"
SHUTDOWN_OK = 'All workers stopped'
SHUTDOWN_FORCED = 'Forcefully exiting process'
SHUTDOWN_FORCED_OK = 'All workers eventually killed'
...
class Config:
    ...
    def getUrl(self, handler):
        '''Returns the base URL for this Appy server'''
        # If key 'Host' is specified as header key in the HTTP request, use it
        host = handler.headers.get('Host')
        if not host:
            host = self.address
            if self.port != 80:
                host += ':%d' % self.port
        return '%s://%s' % (self.protocol, host)
    ...
    def check(self):
        ...
        assert (self.threads > 1), MIN_THREADS_KO
        # Ensure limits are consistent
        kill = self.killThreadLimit
        hung = self.hungThreadLimit
        assert (not kill or (kill >= hung)), (THREAD_LIMITS_KO % (kill, hung))
...
class ThreadPool:
    ...
    def __init__(self, server, config):
        ...
        for i in range(config.threads):
            self.addWorker(message=INITIAL_WORKER)
    ...
    def getTracked(self):
        ...
        for id, (killed, worker) in self.dyingThreads.items():
            ...
            if culled: continue
            attr = 'zombie' if now - killed > self.dyingLimit else 'dying'
            getattr(r, attr).append(worker)
        return r
    ...
    def killHungThreads(self):
        ...
        for worker in self.workers:
            ...
            if lifetime > cfg.killThreadLimit:
                self.logger.warn(WORKER_TO_KILL % id, lifetime)
                self.killWorker(id)
                killed += 1
        ...
        try:
            while True:
                ...
                if max and (processed > max):
                    # Replace me then die
                    self.debug(STOPPING_THREAD % (id, processed, max))
                    replace = True
                    break
                ...
                if runnable is ThreadPool.SHUTDOWN:
                    ...
                    self.debug(SHUTDOWN_RECEIVED % id)
                    break
    ...
    def shutdown(self, forceQuitTimeout=0):
        ...
        for id in self.dyingThreads:
            if self.threadExists(id):
                zombies.append(id)
        if hung or zombies:
            self.debug(SHUTDOWN_HUNG % (len(hung), len(zombies)))
            if hung:
                for worker in hung:
            ...
            if forceQuitTimeout:
                ...
                if needForceQuit:
                    ...
                    self.debug(SHUTDOWN_FORCED)
                    os._exit(3)
                else:
                    self.debug(SHUTDOWN_FORCED_OK)
        else:
            self.debug(SHUTDOWN_OK)
...
class Server(HTTPServer):
    ...
    def buildUrl(self, handler, name='', base='appy', ram=False, bg=False):
    ...
    def getUrl(self, o, sub=None, relative=False, **params):
        ...
        # ----------------------------------------------------------------------
        # The base app URL
        r = '' if relative else self.config.server.getUrl(o.H())
