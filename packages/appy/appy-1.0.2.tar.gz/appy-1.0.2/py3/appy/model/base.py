...
from appy.all import String, Text, Date, Select, Selection, Computed, \
                     Ref, Search, Show, Action
...
class Base:
    ...
    def do(self, name, comment=None, doAction=True, doHistory=True,
        ...
        return transition.trigger(self, comment=comment, doAction=doAction,
                     doHistory=doHistory, doSay=False, secure=secure, data=data)
    ...
    def getCatalog(self, raiseError=False):
        ...
        r = self.H().connection.root.catalogs.get(self.class_.name)
    ...
    pxHeader = Px('''
     <table width="100%">
      <tr valign="top">
       ...
       <!-- Object navigation -->
       <td var="nav=req.nav" if="nav and (nav != 'no')" align=":dright"
           var2="self=ui.Siblings.get(nav, tool, _ctx_)">:self.pxNavigate</td>
      </tr>
     </table>
    ...
    pxButtons = Px('''
     <div class="objectButtons"
      ...
      <x if="mayAct and (page == 'main') and o.class_.showTransitions(o,layout)"
    ...
    def create(self, name, secure=False, raiseOnWrongAttribute=True,
    ...
    def createFrom(self, name, other, secure=False, executeMethods=True,
                   exclude=(), keepBase=False, fields={}):
        ...
        params = {}
        for field in other.class_.fields.values():
            ...
            if field.name in fields:
                ...
                value = field.getCopyValue(other)
            if value is not None:
                params[field.name] = value
        r = self.create(name, secure=secure, raiseOnWrongAttribute=False,
    ...
    pxTied = Px('''
     <tr valign="top" class=":rowCss"
      ...
      <td for="column in columns" width=":column.width" align=":column.align"
       ...
       <x if="(field=='_number') and (objectIndex!=None)">:ifield.pxNumber</x>
    ...
    def getInitiator(self, search=False):
        ...
        if not search:
            # Try to get a "classic" initiator, from the "nav" key
    ...
    def getFolder(self, create=False):
        ...
        return self.H().server.database.getFolder(self, create)
