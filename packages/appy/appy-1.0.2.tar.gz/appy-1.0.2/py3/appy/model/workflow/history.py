...
class Trigger(Event):
    ...
    def getLabel(self, o):
        ...
        if self.transition == '_init_':
            ...
            r = o.getWorkflow().transitions.get(self.transition).labelId 
        return r
...
class Action(Event):
    ...
    def getLabel(self, o):
        ...
        return o.fields.get(self.action).labelId
...
class History(PersistentList):
      ...
      <table width="100%" class="history">
       ...
       <tr for="event in batch.objects"
        ...
        <td><x>:_(event.getLabel(o))</x>
         <img if="isManager and event.deletable" class="clickable"
              src=":url('delete')"
     ...
     <div if="not o.isTemp()"
      ...
      <table width="100%" class="header" cellpadding="0" cellspacing="0">
       ...
       <tr if="hasHistory">
        <td colspan="2">
         <span id=":collapse.id"
          style=":collapsible and collapse.style or ''">:history.events</span>
        </td>
       </tr>
      </table>
     </div>''')
    ...
    def getBatch(self, o, size):
        ...
        size = int(size) if isinstance(size, str) else (size or 5)
        start = int(o.req.start or 0)
        return Batch(objects=self[start:start + size], total=len(self),
