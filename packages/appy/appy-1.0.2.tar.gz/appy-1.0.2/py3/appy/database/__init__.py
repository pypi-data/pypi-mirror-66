...
SEARCH_NO_CATALOG = 'Invalid operation: there is no catalog for instances ' \
  'of class "%s".'
MISSING_OBJECT_CLASS = 'Catalog for class "%s": object not found for ID "%s".'
MISSING_OBJECT = 'Object not found for ID "%s".'
MISSING_IDS_DELETED = 'Catalog "%s": ID(s) removed because no corresponding ' \
  'object(s) found: %s.'
...
class Database:
    ...
    def delete(self, o, historize=False, executeMethods=True, root=None):
        ...
        if o.class_.isIndexable() and not isTemp: o.reindex(unindex=True)
    ...
    def getObject(self, handler, id, logMissing=False, className=None):
        ...
        if (r is None) and logMissing:
            ...
            sid = str(id)
            if className:
                message = MISSING_OBJECT_CLASS % (className, sid)
            else:
                message = MISSING_OBJECT % sid
    ...
    def getObjects(self, handler, ids, className=None, start=0, size=None):
        ...
        if missing and className:
            ...
            if catalog:
                ...
                message = MISSING_IDS_DELETED % (className, ', '.join(missing))
