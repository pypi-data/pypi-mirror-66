...
from cocotbext.interfaces import Signal, InterfaceProtocolError, InterfacePropertyError
...
class StreamingInterface(BaseSynchronousInterface):
    ...
    def mask_data(self, data: BinaryValue, empty: int) -> BinaryValue:
        ...
        if not vec.is_resolvable:
            raise InterfaceProtocolError(f"Signal ({str(self.data)} is unresolvable.")
        return vec
...
class PassiveSinkModel(BaseStreamingModel):
    ...
    def valid_cycle(self) -> None:
        ...
        if self.itf.packets:
            ...
            if not self.in_pkt:
                raise InterfaceProtocolError(f"Attempted transfer outside of packet")
