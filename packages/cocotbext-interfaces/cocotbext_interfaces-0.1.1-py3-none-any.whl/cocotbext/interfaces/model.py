...
from cocotbext.interfaces import BaseInterface, InterfaceProtocolError
...
class BaseModel(HierarchicalMachine, metaclass=ABCMeta):
    ...
    def _input(self, txn: Dict) -> None:
        ...
        if self.busy:
            raise InterfaceProtocolError(f"{str(self)} cannot ingest logical input if busy")
    ...
    def _output(self) -> Dict:
        ...
        if self.busy:
            raise InterfaceProtocolError(f"{str(self)} cannot flush logical output if busy")
    ...
    def _event_loop(self) -> None:
        ...
        if not (self.get_state(self.state).is_flow or self.get_state(self.state).is_fix):
            raise InterfaceProtocolError(f"Control context invariant was violated")
