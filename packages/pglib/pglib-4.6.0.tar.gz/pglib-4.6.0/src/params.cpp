...
{
    // Bind a Python dictionary as an hstore. The wire format is:
    //
    ...
    int32_t count = PyDict_Size(param);
    ...
    Py_ssize_t pos = 0;
    PyObject *pkey, *pval;
    while (PyDict_Next(param, &pos, &pkey, &pval)) {
...
static bool BindUUID(Connection* cnxn, Params& params, PyObject* param)
{
    ...
    return params.Bind(UUIDOID, buf.buffer(), buf.size(), 1);
}
...
bool BindParams(Connection* cnxn, Params& params, PyObject* args)
{
    ...
    for (int i = 0, c = PyTuple_GET_SIZE(args)-1; i < c; i++)
    {
        ...
        else if (PyDict_Check(param) && IsHstoreRegistered())
        {
