...
class Storage(HasName):
    ...
    def getPathInAppDataStore(self, path):
        return self.getPathInTempStore(self.localPathJoin("studio", g.userId, g.appId, path))

    def getPathInAppLogsStore(self, path):
        return self.getPathInTempStore(self.localPathJoin("studio", g.userId, "logs", g.appId, path))

    def getPathInNodeDataStore(self, path):
        return self.getPathInTempStore(self.localPathJoin("studio", g.userId, g.appId, g.nodeId, path))

    def getPathInComponentDataStore(self, componentId, path):
        return self.getPathInTempStore(self.localPathJoin("studio", g.userId, "component", componentId, path))

    def getPathInGlobalDataStore(self, path):
        return self.localPathJoin(self.DEFAULT_GLOBAL_DATA_STORE, path)
