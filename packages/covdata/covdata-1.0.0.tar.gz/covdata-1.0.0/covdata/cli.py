...
def main():
    parser = argparse.ArgumentParser(description='Process some output of Covid-19 data in india.')
    ...
    parser.add_argument('-D','--daily',type=bool,
                        help='if True daily count will be shown')
    ...
    if args.options == 'show_data':
        ...
        if '_' in of:
            ...
            if args.head !=None:
                if args.save != None:
                    init.show_data(of).head(args.head).to_csv(args.save+'show_data.csv',index=True)
                else:
                    print(init.show_data(of).head(args.head))
            elif args.tail !=None:
                if args.save != None:
                    init.show_data(of).tail(args.tail).to_csv(args.save+'show_data.csv',index=True)
                else:
                    print(init.show_data(of).tail(args.tail))
            elif args.head == None:
                if args.save != None:
                    init.show_data(of).to_csv(args.save+'show_data.csv',index=True)
                else:
        ...
        if args.head !=None:
            if args.save != None:
                df.head(args.head).to_csv(args.save+f'demography_{args.place}_head_{args.head}.csv',index=True)
            else:
                print(df.head(args.head))
        elif args.tail !=None:
            if args.save != None:
                df.tail(args.tail).to_csv(args.save+f'demography_{args.place}_tail_{args.tail}.csv',index=True)
            else:
        ...
        if args.head != None:
            if args.save != None:
                df.head(args.head).to_csv(args.save+f'{args.state}_head_{args.head}.csv')
            else:
                print(df.head(args.head))
        elif args.tail != None:
            if args.save != None:
                df.tail(args.tail).to_csv(args.save+f'{args.state}_tail_{args.tail}.csv')
            else:
                print(df.tail(args.tail)) 
        elif args.head == None:
            if args.save != None:
        ...
        if args.head != None:
            if args.save != None:
                df.head(args.head).to_csv(args.save+f'{args.date.split("/")[0]}-{args.date.split("/")[1]}_head_{args.head}.csv')
            else:
                print(df.head(args.head))
        elif args.tail != None:
            if args.save != None:
                df.tail(args.tail).to_csv(args.save+f'{args.date.split("/")[0]}-{args.date.split("/")[1]}_tail_{args.tail}.csv')
            else:
                print(df.tail(args.tail)) 
        elif args.head == None:
            if args.save != None:
                df.to_csv(args.save+f'{args.date.split("/")[0]}-{args.date.split("/")[1]}.csv')
            else:
                print(df)
        elif args.tail ==None:
            if args.save != None:
        ...
        if args.head != None:
            if args.save != None:
                df.head(args.head).to_csv(args.save+f'count_by_{by.split("_")[1]}_{args.date.split("/")[0]}-{args.date.split("/")[1]}_head_{args.head}.csv')
            else:
                print(df.head(args.head))
        elif args.tail != None:
            if args.save != None:
                df.tail(args.tail).to_csv(args.save+f'count_by_{by.split("_")[1]}_{args.date.split("/")[0]}-{args.date.split("/")[1]}_tail_{args.tail}.csv')
            else:
                print(df.tail(args.tail)) 
        elif args.head == None:
            if args.save != None:
                df.to_csv(args.save+f'count_by_{by.split("_")[1]}_{args.date.split("/")[0]}-{args.date.split("/")[1]}.csv')
            else:
                print(df)
        elif args.tail ==None:
            if args.save != None:
        ...
        if args.graph == 'whole':
            obj3.whole(args.daily)
        elif args.graph =='head':
            obj3.head(num=args.rank,daily=args.daily)
        elif args.graph == 'tail':
            obj3.tail(num=args.rank,daily=args.daily)
