...
class RegionEstimator(object):
    ...
    def __init__(self, sensors, regions, actuals):
        ...
        cols = cols['timestamp'] + cols['sensor_id'] + cols.difference(['timestamp','sensor_id'])
        actuals = actuals[cols]
        #   Coerce all non numeric values in value columns to be numeric or None
        actuals[2:] = pd.to_numeric(actuals[2:], errors='coerce')
