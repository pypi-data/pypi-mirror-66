...
if __name__ == '__main__':
    scriptModes = {'Testing': {'Airtable': 'appgg2Dz6pmuRMwiQ',
                               'mySQL': 'NathanJames_Staging',
                               'slackErrorChannel': '#python_automation_alert',
                               'slackBookingChannel': '#python_automation_alert'
                               },
                   'Production': {'Airtable': 'appeHvOljHqhtu561',
                                  'mySQL': 'NathanJames',
                                  'slackErrorChannel': '#automation-errors',
                                  'slackBookingChannel': '#operations'
                                  }
                   }
    ...
    airtableBase = scriptModes.get(mode_).get('Airtable')
    errorChannel = scriptModes.get(mode_).get('slackErrorChannel')

    myPath = '{}/'.format(pathlib.Path(__file__).parent.absolute())
    _airtable = airtableToolbox(airtableBase, 'keyNmj9UPTIBu4jMR')
    _slack = slackToolbox('xoxb-67057974501-803275864324-C99xoWaLCkA8AnlCcY8xqwMt', errorChannel)

    _airtable.get_ids('PO', 'PO Name')
