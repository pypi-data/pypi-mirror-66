...
class airtableToolbox:
    ...
    def get_ids(self, table, column_name, value):
        url = "%s/%s?filterByFormula={%s}='%s'" % (self.airtableURL, table, column_name, value)
        r = requests.get(url, headers=self.airtableHeaders).json()
        _list = []
