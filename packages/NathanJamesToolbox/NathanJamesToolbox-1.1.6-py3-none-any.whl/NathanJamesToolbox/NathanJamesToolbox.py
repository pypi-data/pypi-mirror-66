...
class mySQLToolbox:
    ...
    def create_log(self, logType='log', log_=None, logMode='Testing'):
        if log_ is None or logMode == 'Testing':
            # Invalid log
            return
        ...
        log_ = '("{}", "{}", "{}", "{}", "{}")'.format(dt.datetime.utcnow(), filename, module_, logType, log_)
        log_ = 'INSERT INTO NathanJames_Monitoring.logs ' \
               '(datetime, scriptName, function, type, description) VALUES {}'.format(log_)
        db = pymysql.connect(host=self.host, user=self.user, password=self.password, db=self.db)
        cursor = db.cursor()
