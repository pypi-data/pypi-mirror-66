...
def _search_lazily(list_to_search: Iterable, search_string: str) -> list:
    ...
    """
    collist = list(list_to_search) # valid inputs: pandas.DataFrame, pandas.core.indexes.base.Index, ordinary Python list
    if type(search_string) == str:
        search_string = search_string.split()

    matchlist = []
    for var in search_string:
        try:
...
def _search_by_range(list_to_search: Iterable, search_string: str) -> list:
    ...
    pos0 = collist.index(search_string[0])
    pos1 = collist.index(search_string[1])
...
def _search_by_ranges(list_to_search: Iterable, search_string: str) -> list:
    ...
    matchlist = []
    for subsearch_string in search_string:
...
def search_iterable(list_to_search: Iterable, search_string: str, subtract: bool=False) -> list:
    ...
    typelist = []
    for x in search_string:
        if x.find('*')>-1: typelist.append('w')  # wildcard
        elif x.find('-')>-1: typelist.append('r')  # range
        else: typelist.append('l')  # lazy

    if len(set(typelist))==1 and typelist[0]=='w': matchlist = _search_by_wildcard(list_to_search, search_string)
    elif len(set(typelist))==1 and typelist[0]=='r': matchlist = _search_by_ranges(list_to_search, search_string)
    elif len(set(typelist))==1 and typelist[0]=='l': matchlist = _search_lazily(list_to_search, search_string)
    else: # handle mixed search_string (note: this apparently isn't any slower than a 'pure' search_string)
        matchlist = []
        for x in search_string:
            if x.find('*')>-1:   matchlist.extend(_search_by_wildcard(list_to_search, x))
            elif x.find('-')>-1: matchlist.extend(_search_by_ranges(list_to_search, x))
            else:                matchlist.extend(_search_lazily(list_to_search, x))
    if subtract:
