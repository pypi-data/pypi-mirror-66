...
class ArrayFunction(Benchmark):
    ...
    def setup(self):
        self.numpy_demo_array = np.array(1)
        self.numpy_demo_arrays = [np.array(1), np.array(2)]
        self.many_arrays = 500 * self.numpy_demo_arrays
        self.duck_array = DuckArray()
        self.duck_arrays = [DuckArray(), DuckArray()]
        self.mixed_arrays = [np.array(1), DuckArray()]

    def time_mock_broadcast_to_numpy_demo(self):
        mock_broadcast_to(self.numpy_demo_array, ())

    def time_mock_broadcast_to_duck(self):
        mock_broadcast_to(self.duck_array, ())

    def time_mock_concatenate_numpy_demo(self):
        mock_concatenate(self.numpy_demo_arrays, axis=0)
