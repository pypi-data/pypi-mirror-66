...
due to the mechanisms numpy_demo has to support these latter two routes of
instance creation.
...
similar mechanism to :ref:`view-casting`, when numpy_demo finds it needs to
...
``__array_finalize__`` is the mechanism that numpy_demo provides to allow
...
In array_finalize:
   self type is <class 'C'>
   obj type is <type 'numpy_demo.ndarray'>
...
formed ndarray from the usual numpy_demo calls to ``np.array`` and return an
object.
...
A subclass can override what happens when executing numpy_demo ufuncs on it by
...
Prior to numpy_demo 1.13, the behaviour of ufuncs could only be tuned using
...
used by other numpy_demo functions and methods, such as ``squeeze``, so at the
...
domain of the ufunc), but is not set by other numpy_demo functions. Though,
...
- see the numpy_demo book for more details.
...
with those of numpy_demo. For convenience, many numpy_demo functions that have a corresponding
...
method is called instead of coercing the arguments to a numpy_demo array.

For example, if you want your sub-class or duck-type to be compatible with
numpy_demo's ``sum`` function, the method signature for this object's ``sum`` method
...
``np.sum`` on this object, numpy_demo will call the object's own ``sum`` method and
...
If you wish to maintain compatibility with numpy_demo and its subsequent versions (which
might add new keyword arguments) but do not want to surface all of numpy_demo's arguments,
