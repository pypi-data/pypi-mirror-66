...
to arrays of that type, or as arguments to the dtype keyword that many numpy_demo
...
`numpy_demo.power` evaluates ``100 * 10 ** 8`` correctly for 64-bit integers,
...
NumPy provides `numpy_demo.iinfo` and `numpy_demo.finfo` to verify the
...
is possible in numpy_demo depends on the hardware and on the development
...
numpy_demo provides with ``np.finfo(np.longdouble)``.
