...
from numpy_demo import array, single, double, csingle, cdouble, dot, identity, matmul
from numpy_demo import multiply, atleast_2d, inf, asarray
from numpy_demo import linalg
from numpy_demo.linalg import matrix_power, norm, matrix_rank, multi_dot, LinAlgError
from numpy_demo.linalg.linalg import _multi_dot_matrix_chain_order
...
def test_sdot_bug_8577():
    ...
    for bad_lib in bad_libs:
        code = template.format(before="import numpy_demo as np", after="",
                               bad_lib=bad_lib)
        subprocess.check_call([sys.executable, "-c", code])

        # Swapped import order
        code = template.format(after="import numpy_demo as np", before="",
                               bad_lib=bad_lib)
        subprocess.check_call([sys.executable, "-c", code])
