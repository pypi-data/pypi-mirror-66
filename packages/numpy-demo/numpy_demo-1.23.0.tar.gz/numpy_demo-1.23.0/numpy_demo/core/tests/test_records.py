...
class TestFromrecords:
    ...
    def test_recarray_repr(self):
        ...
        assert_equal(repr(a).find('numpy_demo.record'), -1)
    ...
    def test_recarray_from_repr(self):
        ...
        recordarr_r = eval("numpy_demo." + repr(recordarr), {'numpy_demo': np})
        recarr_r = eval("numpy_demo." + repr(recarr), {'numpy_demo': np})
        recordview_r = eval("numpy_demo." + repr(recordview), {'numpy_demo': np})
...
class TestRecord:
    ...
    def test_objview_record(self):
        # https://github.com/numpy_demo/numpy_demo/issues/2599
        dt = np.dtype([('foo', 'i8'), ('bar', 'O')])
        r = np.zeros((1,3), dtype=dt).view(np.recarray)
        r.foo = np.array([1, 2, 3])  # TypeError?

        # https://github.com/numpy_demo/numpy_demo/issues/3256
        ra = np.recarray((2,), dtype=[('x', object), ('y', float), ('z', int)])
        ra[['x','y']]  # TypeError?

    def test_record_scalar_setitem(self):
        # https://github.com/numpy_demo/numpy_demo/issues/3561
        rec = np.recarray(1, dtype=[('x', float, 5)])
        rec[0].x = 1
        assert_equal(rec[0].x, np.ones(5))

    def test_missing_field(self):
        # https://github.com/numpy_demo/numpy_demo/issues/4806
