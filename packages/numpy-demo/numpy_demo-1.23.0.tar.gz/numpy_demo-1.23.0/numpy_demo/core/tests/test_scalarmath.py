...
class TestConversion:
    ...
    def test_numpy_demo_scalar_relational_operators(self):
        # All integer
...
class TestMultiply:
    def test_seq_repeat(self):
        ...
        # numpy_demo integers. And errors are raised when multiplied with others.
        ...
        for seq_type in (list, tuple):
            seq = seq_type([1, 2, 3])
            for numpy_demo_type in accepted_types:
                i = np.dtype(numpy_demo_type).type(2)
                assert_equal(seq * i, seq * int(i))
                assert_equal(i * seq, int(i) * seq)

            for numpy_demo_type in deprecated_types:
                i = np.dtype(numpy_demo_type).type()
                assert_equal(
            ...
            for numpy_demo_type in forbidden_types:
                i = np.dtype(numpy_demo_type).type()
