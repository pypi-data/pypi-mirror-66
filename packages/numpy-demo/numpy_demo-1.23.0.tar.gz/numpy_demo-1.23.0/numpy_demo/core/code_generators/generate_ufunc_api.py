...
_import_umath(void)
{
  ...
  if (numpy_demo == NULL) {
      PyErr_SetString(PyExc_ImportError,
                      "numpy_demo.core._multiarray_umath failed to import");
      return -1;
  }
  c_api = PyObject_GetAttrString(numpy_demo, "_UFUNC_API");
...
#define import_umath() \
    do {\
        ...
        if (_import_umath() < 0) {\
            PyErr_Print();\
            PyErr_SetString(PyExc_ImportError,\
                    "numpy_demo.core.umath failed to import");\
            return NULL;\
        }\
    } while(0)
...
def do_generate_api(targets, sources):
    ...
    ufunc_api_index = genapi.merge_api_dicts((
            numpy_demo_api.ufunc_funcs_api,
            numpy_demo_api.ufunc_types_api))
    genapi.check_api_dict(ufunc_api_index)

    ufunc_api_list = genapi.get_api_functions('UFUNC_API', numpy_demo_api.ufunc_funcs_api)
    ...
    for name, val in numpy_demo_api.ufunc_types_api.items():
        index = val[0]
