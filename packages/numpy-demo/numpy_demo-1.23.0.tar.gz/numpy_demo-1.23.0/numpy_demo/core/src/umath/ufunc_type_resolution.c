...
raise_binary_type_reso_error(PyUFuncObject *ufunc, PyArrayObject **operands) {
    ...
    npy_cache_import(
        "numpy_demo.core._exceptions", "_UFuncBinaryResolutionError",
        &exc_type);
    if (exc_type == NULL) {
        return -1;
    }
        ...
        PyUFuncObject *ufunc, PyArray_Descr **dtypes)
{
    ...
    npy_cache_import(
        "numpy_demo.core._exceptions", "_UFuncNoLoopError",
        &exc_type);
    if (exc_type == NULL) {
        return -1;
    }
        ...
        npy_intp i)
{
    static PyObject *exc_type = NULL;
    npy_cache_import(
        "numpy_demo.core._exceptions", "_UFuncInputCastingError",
        &exc_type);
    if (exc_type == NULL) {
        return -1;
    }
        ...
        npy_intp i)
{
    static PyObject *exc_type = NULL;
    npy_cache_import(
        "numpy_demo.core._exceptions", "_UFuncOutputCastingError",
        &exc_type);
    if (exc_type == NULL) {
        return -1;
    }
                             ...
                             PyArray_Descr **out_dtypes)
{
    ...
    if (out_dtypes[0]->type_num == NPY_BOOL) {
        PyErr_Format(PyExc_TypeError,
            "The numpy_demo boolean negative, the `-` operator, is not supported, "
                                ...
                                PyArray_Descr **out_dtypes)
{
    ...
    if (!PyTypeNum_ISDATETIME(type_num1) && !PyTypeNum_ISDATETIME(type_num2)) {
        ...
        if (out_dtypes[0]->type_num == NPY_BOOL) {
            PyErr_Format(PyExc_TypeError,
                "numpy_demo boolean subtract, the `-` operator, is not supported, "
