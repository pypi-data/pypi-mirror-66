...
#include "numpy_demo/arrayscalars.h"
...
{
    PyObject *_numpy_demo_dtype_ctypes;
    PyObject *res;
    ...
    _numpy_demo_dtype_ctypes = PyImport_ImportModule("numpy_demo.core._dtype_ctypes");
    if (_numpy_demo_dtype_ctypes == NULL) {
        return NULL;
    }
    res = PyObject_CallMethod(_numpy_demo_dtype_ctypes, "dtype_from_ctypes_type", "O", (PyObject *)type);
    Py_DECREF(_numpy_demo_dtype_ctypes);
    if (res == NULL) {
        return NULL;
    }
 ...
 * dtype names to numpy_demo scalar types.
 */
    ...
    if (PyDataType_ISUNSIZED(type)) {
        ...
        if (shape.len == 1
            ...
            if (DEPRECATE_FUTUREWARNING(
                        ...
                        "deprecated; in a future version of numpy_demo, it will be "
...
_convert_from_commastring(PyObject *obj, int align)
{
    PyObject *listobj;
    PyArray_Descr *res;
    PyObject *_numpy_demo_internal;
    assert(PyUnicode_Check(obj));
    _numpy_demo_internal = PyImport_ImportModule("numpy_demo.core._internal");
    if (_numpy_demo_internal == NULL) {
        return NULL;
    }
    listobj = PyObject_CallMethod(_numpy_demo_internal, "_commastring", "O", obj);
    Py_DECREF(_numpy_demo_internal);
    if (listobj == NULL) {
        return NULL;
    }
...
_convert_from_field_dict(PyObject *obj, int align)
{
    ...
    res = (PyArray_Descr *)PyObject_CallMethod(_numpy_demo_internal,
...
_convert_from_type(PyObject *obj) {
    ...
    if (PyType_IsSubtype(typ, &PyGenericArrType_Type)) {
         ...
         * type might override the dtype if numpy_demo does not otherwise
         * support it.
         */
 ...
 * Type-descriptor objects that are used throughout numpy_demo.
 *
...
arraydescr_name_get(PyArray_Descr *self)
{
    ...
    PyObject *_numpy_demo_dtype;
    PyObject *res;
    _numpy_demo_dtype = PyImport_ImportModule("numpy_demo.core._dtype");
    if (_numpy_demo_dtype == NULL) {
        return NULL;
    }
    res = PyObject_CallMethod(_numpy_demo_dtype, "_name_get", "O", self);
    Py_DECREF(_numpy_demo_dtype);
    return res;
}
...
arraydescr_protocol_descr_get(PyArray_Descr *self)
{
    ...
    res = PyObject_CallMethod(_numpy_demo_internal, "_array_descr", "O", self);
    Py_DECREF(_numpy_demo_internal);
    return res;
}
...
_get_pickleabletype_from_datetime_metadata(PyArray_Descr *dtype)
{
    ...
    /* Use a 4-tuple that numpy_demo 1.6 knows how to unpickle */
...
arraydescr_reduce(PyArray_Descr *self, PyObject *NPY_UNUSED(args))
{
    ...
    */
    ...
    mod = PyImport_ImportModule("numpy_demo.core._multiarray_umath");
    if (mod == NULL) {
        Py_DECREF(ret);
        return NULL;
    }
...
arraydescr_setstate(PyArray_Descr *self, PyObject *args)
{
    ...
    if (version < 0 || version > 4) {
        PyErr_Format(PyExc_ValueError,
                     "can't handle version %d of numpy_demo.dtype pickle",
                     version);
        return NULL;
    }
    /* Invalidate cached hash value */
...
arraydescr_repr(PyArray_Descr *dtype)
{
    ...
    res = PyObject_CallMethod(_numpy_demo_dtype, "__repr__", "O", dtype);
    Py_DECREF(_numpy_demo_dtype);
    return res;
}
/*
...
arraydescr_str(PyArray_Descr *dtype)
{
    ...
    res = PyObject_CallMethod(_numpy_demo_dtype, "__str__", "O", dtype);
    Py_DECREF(_numpy_demo_dtype);
    return res;
}
