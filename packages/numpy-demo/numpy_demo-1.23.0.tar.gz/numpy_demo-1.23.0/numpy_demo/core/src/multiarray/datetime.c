 ...
 */
NPY_NO_EXPORT void
numpy_demo_pydatetime_import(void)
{
    PyDateTime_IMPORT;
}
                                        ...
                                        npy_bool from_pickle)
{
    ...
    /*
     * The event metadata was removed way back in numpy_demo 1.7 (cb4545), but was
                                     ...
                                     int apply_tzinfo)
{
    ...
    if (apply_tzinfo && PyObject_HasAttrString(obj, "tzinfo")) {
        ...
        if (tmp == Py_None) {
             ...
             * Only whole minute offsets are supported by numpy_demo anyway.
             */
...
is_any_numpy_demo_datetime(PyObject *obj)
{
 ...
 */
static npy_bool
is_any_numpy_demo_timedelta(PyObject *obj)
{
 ...
 */
NPY_NO_EXPORT npy_bool
is_any_numpy_demo_datetime_or_timedelta(PyObject *obj)
{
    return obj != NULL &&
           (is_any_numpy_demo_datetime(obj) ||
            is_any_numpy_demo_timedelta(obj));
}
                ...
                PyArray_Descr *dtype)
{
    ...
    if (step != NULL && is_any_numpy_demo_datetime(step)) {
        PyErr_SetString(PyExc_ValueError,
    ...
    if (dtype != NULL) {
        ...
        if ((start && is_any_numpy_demo_datetime(start)) ||
            is_any_numpy_demo_datetime(stop)) {
    ...
    if (type_nums[0] == NPY_TIMEDELTA) {
        ...
        if (PyInt_Check(objs[1]) ||
                        ...
                        is_any_numpy_demo_timedelta(objs[1])) {
            type_nums[1] = NPY_TIMEDELTA;
        }
        else {
