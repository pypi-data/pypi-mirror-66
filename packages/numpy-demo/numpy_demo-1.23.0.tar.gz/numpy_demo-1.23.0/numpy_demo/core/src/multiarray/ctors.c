...
raise_memory_error(int nd, npy_intp const *dims, PyArray_Descr *descr)
{
    ...
    npy_cache_import(
        "numpy_demo.core._exceptions", "_ArrayMemoryError",
        &exc_type);
    if (exc_type == NULL) {
        goto fail;
    }
    ...
    if (descr->elsize != view->itemsize) {
        ...
        if (descr->elsize != view->len) {
            PyErr_SetString(
                    ...
                    "size. This is a bug both in python and numpy_demo");
            Py_DECREF(descr);
            return NULL;
        }
    }
                        ...
                        PyArrayObject **out_arr)
{
    ...
    *out_dtype = _array_find_python_scalar_type(op);
    ...
    if (!writeable && PySequence_Check(op)) {
        ...
        *out_ndim = NPY_MAXDIMS;
        ...
        if (is_object != DISCOVERED_OK) {
            ...
            npy_cache_import(
                "numpy_demo", "VisibleDeprecationWarning",
    ...
    PyErr_SetString(PyExc_RuntimeError,
                    "object cannot be viewed as a writeable numpy_demo array");
    return -1;
}
...
PyArray_ArangeObj(PyObject *start, PyObject *stop, PyObject *step, PyArray_Descr *dtype)
{
    ...
    if ((dtype != NULL && (dtype->type_num == NPY_DATETIME ||
            ...
            (dtype == NULL && (is_any_numpy_demo_datetime_or_timedelta(start) ||
                              is_any_numpy_demo_datetime_or_timedelta(stop) ||
                              is_any_numpy_demo_datetime_or_timedelta(step)))) {
