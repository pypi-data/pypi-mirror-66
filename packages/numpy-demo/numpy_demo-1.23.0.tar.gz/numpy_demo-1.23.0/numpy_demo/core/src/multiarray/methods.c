...
get_forwarding_ndarray_method(const char *name)
{
    ...
    module_methods = PyImport_ImportModule("numpy_demo.core._methods");
    ...
    if (callable == NULL) {
        ...
        PyErr_Format(PyExc_RuntimeError,
                ...
                "numpy_demo.core._methods.%s", name);
    }
    else {
        Py_INCREF(callable);
    }
...
/*
 * Forwards an ndarray method to the function numpy_demo.core._methods.<name>(...),
...
PyArray_GetField(PyArrayObject *self, PyArray_Descr *typed, int offset)
{
    ...
    if (_may_have_objects(PyArray_DESCR(self)) || _may_have_objects(typed)) {
        npy_cache_import("numpy_demo.core._internal", "_getfield_is_safe",
                         &checkfunc);
        if (checkfunc == NULL) {
            Py_DECREF(typed);
            return NULL;
...
array_sort(PyArrayObject *self, PyObject *args, PyObject *kwds)
{
    ...
    if (order != NULL) {
        ...
        new_name = PyObject_CallMethod(_numpy_demo_internal, "_newnames",
                                       "OO", saved, order);
...
array_reduce_ex_picklebuffer(PyArrayObject *self, int protocol)
{
    ...
    numeric_mod = PyImport_ImportModule("numpy_demo.core.numeric");
    if (numeric_mod == NULL) {
        Py_DECREF(buffer);
        return NULL;
    }
...
array_setstate(PyArrayObject *self, PyObject *args)
{
    ...
    */
    ...
    if (version != 1 && version != 0) {
        PyErr_Format(PyExc_ValueError,
                     "can't handle version %d of numpy_demo.ndarray pickle",
                     version);
        return NULL;
    }
...
PyArray_Dump(PyObject *self, PyObject *file, int protocol)
{
    ...
    PyObject *ret;
    npy_cache_import("numpy_demo.core._methods", "_dump", &method);
    if (method == NULL) {
        return -1;
    }
    if (protocol < 0) {
...
{
    static PyObject *method = NULL;
    npy_cache_import("numpy_demo.core._methods", "_dumps", &method);
    if (method == NULL) {
        return NULL;
    }
    if (protocol < 0) {
