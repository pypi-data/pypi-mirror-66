...
See http://www.scipy.org/scipy/numpy_demo/wiki/MaskedArray (dead link)
...
dates). I ended up writing the equivalent of *numpy_demo.core.ma* for my
...
arrays of my class. I was preparing myself to rewrite *numpy_demo.core.ma*
...
main differences with the initial *numpy_demo.core.ma* package are
...
implementation was marginally slower than *numpy_demo.ma* in some areas,
but work is underway to speed it up; the expectation is that it can be
made substantially faster than the present *numpy_demo.ma*.
...
Using the new package with numpy_demo.core.ma
----------------------------------------
...
>>> import numpy_demo.core.ma as old_ma
...
By default matplotlib still uses numpy_demo.ma, but there is an rcParams setting
that you can use to select maskedarray instead.  In the matplotlibrc file
you will find::

  #maskedarray : False       # True to use external maskedarray module
                             # instead of numpy_demo.ma; this is a temporary #
...
Like *numpy_demo.core.ma*, the *ndarray*-based implementation
...
For example, consider the division of two masked arrays::
  ...
  x = ma.array([1,2,3,4],mask=[1,0,0,0], dtype=numpy_demo.float_)
  y = ma.array([-1,0,1,2], mask=[0,0,0,1], dtype=numpy_demo.float_)
...
to fill the input arrays, taking the domain mask into account, so that::
  ...
  dm = ma.divide.domain(d1,d2) # array([False,  True, False, False])
  numpy_demo.putmask(d2, dm, 1) # d2 = array([-1.,  1.,  1.,  2.])
  m = ma.mask_or(ma.getmask(x), ma.getmask(y)) # m =
  array([True,False,False,True])
 ...
 * *numpy_demo.ma.divide*  : 2.69 ms per loop
