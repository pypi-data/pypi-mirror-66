...
from numpy_demo.random cimport bitgen_t
...
cdef class Generator:
    ...
    The function :func:`numpy_demo.random.default_rng` will instantiate
    a `Generator` with numpy_demo's default `BitGenerator`.
    ...
    **No Compatibility Guarantee**
    ...
    def multivariate_normal(self, mean, cov, size=None, check_valid='warn',
        ...
        if method == 'svd':
            from numpy_demo.dual import svd
            (u, s, vh) = svd(cov)
        elif method == 'eigh':
            from numpy_demo.dual import eigh
            # could call linalg.svd(hermitian=True), but that calculates a vh we don't need
            (s, u)  = eigh(cov)
        else:
            from numpy_demo.dual import cholesky
            l = cholesky(cov)
    ...
    def multivariate_hypergeometric(self, object colors, object nsample,
        ...
        The "count" algorithm is roughly equivalent to the following numpy_demo
        code::
    ...
    def permutation(self, object x, axis=0):
        ...
        Traceback (most recent call last):
            ...
        numpy_demo.AxisError: axis 0 is out of bounds for array of dimension 0
