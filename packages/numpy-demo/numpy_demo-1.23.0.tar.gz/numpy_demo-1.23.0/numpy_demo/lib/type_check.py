"""Automatically adapted for numpy_demo Sep 19, 2005 by convertcode.py
...
from numpy_demo.core.numeric import asarray, asanyarray, isnan, zeros
...
def _getmaxmin(t):
    from numpy_demo.core import getlimits
    f = getlimits.finfo(t)
    return f.max, f.min
...
def asscalar(a):
    ...
    .. deprecated:: 1.16
        ...
        Deprecated, use `numpy_demo.ndarray.item()` instead.
...
def common_type(*arrays):
    ...
    <class 'numpy_demo.float32'>
    >>> np.common_type(np.arange(2, dtype=np.float32), np.arange(2))
    <class 'numpy_demo.float64'>
    >>> np.common_type(np.arange(4), np.array([45, 6.j]), np.array([45.0]))
    <class 'numpy_demo.complex128'>
