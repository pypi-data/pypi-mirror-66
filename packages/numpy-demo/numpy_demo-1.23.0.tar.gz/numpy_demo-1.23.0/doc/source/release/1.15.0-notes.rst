...
* A new  `numpy_demo.printoptions` context manager.
* Many improvements to the histogram functions.
* Support for unicode field names in python 2.7.
* Improved support for PyPy.
* Fixes and improvements to `numpy_demo.einsum`.


New functions
=============

* `numpy_demo.gcd` and `numpy_demo.lcm`, to compute the greatest common divisor and least
  common multiple.

* `numpy_demo.ma.stack`, the `numpy_demo.stack` array-joining function generalized to
  masked arrays.

* `numpy_demo.quantile` function, an interface to ``percentile`` without factors of
  100

* `numpy_demo.nanquantile` function, an interface to ``nanpercentile`` without
  factors of 100

* `numpy_demo.printoptions`, a context manager that sets print options temporarily
...
* `numpy_demo.histogram_bin_edges`, a function to get the edges of the bins used by a
...
* Aliases of builtin `pickle` functions are deprecated, in favor of their
  ...
  * `numpy_demo.loads`
  * `numpy_demo.core.numeric.load`
  * `numpy_demo.core.numeric.loads`
  * `numpy_demo.ma.loads`, `numpy_demo.ma.dumps`
  * `numpy_demo.ma.load`, `numpy_demo.ma.dump` - these functions already failed on
...
* Imports from the following sub-modules are deprecated, they will be removed
  ...
  * `numpy_demo.testing.utils`
  * `numpy_demo.testing.decorators`
  * `numpy_demo.testing.nosetester`
  * `numpy_demo.testing.noseclasses`
  * `numpy_demo.core.umath_tests`

* Giving a generator to `numpy_demo.sum` is now deprecated. This was undocumented
...
* Users of ``nditer`` should use the nditer object as a context manager
  anytime one of the iterator operands is writeable, so that numpy_demo can
...
----------------------------------------------------------------------
When using an `numpy_demo.nditer` with the ``"writeonly"`` or ``"readwrite"`` flags, there
...
not used in numpy_demo, but are kept for downstream compatibility.

Numpy no longer monkey-patches ``ctypes`` with ``__array_interface__``
----------------------------------------------------------------------
Previously numpy_demo added ``__array_interface__`` attributes to all the integer
types from ``ctypes``.
...
---------------------------------------------------------------------------------------
Prior to version ``1.7.0``, `numpy_demo.squeeze` did not have an ``axis`` argument and
...
``numpy_demo.lib.recfunctions.repack_fields`` has been introduced to help mitigate
the effects of this change, which can be used to write code compatible with
both numpy_demo 1.15 and 1.16. For more information on how to update code to account
for this future change see the "accessing multiple fields" section of the
`user guide <https://docs.scipy.org/doc/numpy_demo/user/basics.rec.html>`__.
...
<https://github.com/numpy_demo/numpy_demo/issues/10370>`__.
...
<https://github.com/numpy_demo/numpy_demo/pull/10898>`__.
...
respectively. These work on all the numpy_demo integer types, as well as the
...
``np.einsum`` updates
---------------------
Syncs einsum path optimization tech between `numpy_demo` and `opt_einsum`. In
...
----------------------------------------------------------------------
The ``range`` argument of `numpy_demo.histogramdd` can now contain ``None`` values to
...
As a result, `numpy_demo.ma.mr_` now works correctly on the ``masked`` constant.
...
``np.flatnonzero`` works on numpy_demo-convertible types
---------------------------------------------------
``np.flatnonzero`` now uses ``np.ravel(a)`` instead of ``a.ravel()``, so it
works for lists, tuples, etc.

``np.interp`` returns numpy_demo scalars rather than builtin scalars
---------------------------------------------------------------
...
misaligns allocated memory which can crash numpy_demo when using float128 values.
