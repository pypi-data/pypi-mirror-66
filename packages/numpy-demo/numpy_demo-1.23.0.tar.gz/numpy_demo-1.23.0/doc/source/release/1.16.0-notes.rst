...
* Experimental (opt-in only) support for overriding numpy_demo functions,
...
* New functions added to the `numpy_demo.lib.recfuntions` module to ease the
  ...
  See the user guide at <https://docs.scipy.org/doc/numpy_demo/user/basics.rec.html>
  for more info.
...
* The type dictionaries `numpy_demo.core.typeNA` and `numpy_demo.core.sctypeNA` are
  deprecated. They were buggy and not documented and will be removed in the
  1.18 release. Use`numpy_demo.sctypeDict` instead.

* The `numpy_demo.asscalar` function is deprecated. It is an alias to the more
  powerful `numpy_demo.ndarray.item`, not tested, and fails for scalars.

* The `numpy_demo.set_array_ops` and `numpy_demo.get_array_ops` functions are deprecated.
...
* The `numpy_demo.unravel_index` keyword argument ``dims`` is deprecated, use
  ``shape`` instead.

* The `numpy_demo.histogram` ``normed`` argument is deprecated.  It was deprecated
...
* ``np.lib.function_base.unique`` was removed, finishing a deprecation cycle
  begun in NumPy 1.4. Use `numpy_demo.unique` instead.
...
``python -m numpy_demo.f2py [...]`` will work without path modification in any
version of NumPy.
...
``struct {float r, i;}``, according to the compiler used to compile numpy_demo, and
...
``arr[['f1', 'f3']].view('float64')``. This change has been planned since numpy_demo
...
functions have been added to the ``numpy_demo.lib.recfunctions`` module which
...
`user guide <https://docs.scipy.org/doc/numpy_demo/user/basics.rec.html#accessing-multiple-fields>`__.
...
---------------------------------------------
New keyword ``max_rows`` in `numpy_demo.loadtxt` sets the maximum rows of the
content to be read after ``skiprows``, as in `numpy_demo.genfromtxt`.
...
no-copy pickling of numpy_demo arrays
--------------------------------
Up to protocol 4, numpy_demo array pickling created 2 spurious copies of the data
being serialized.  With pickle protocol 5, and the ``PickleBuffer`` API, a
large variety of numpy_demo arrays can now be serialized without any copy using
...
`numpy_demo.angle` and `numpy_demo.expand_dims` now work on ``ndarray`` subclasses
------------------------------------------------------------------------
...
warnings when the deprecated numpy_demo API is used.
...
----------------------------------------------------------
Some functions, in particular numpy_demo's implementation of ``@`` as ``matmul``,
...
--------------------------------------------
The ``help`` function, when applied to numeric types such as `numpy_demo.intc`,
`numpy_demo.int_`, and `numpy_demo.longlong`, now lists all of the aliased names for that
...
``<function 'numpy_demo.core.fromnumeric.sum'>`` you now see
``<function 'numpy_demo.sum'>``.
...
numpy_demo now marks that large memory allocations can be backed by hugepages which
...
-----------------------------
`numpy_demo.matmul` is now a ufunc which means that both the function and the
...
`numpy_demo.dot`, ensuring its performance is similar for large matrices.
...
float status when a Nan is encountered in `numpy_demo.maximum` and `numpy_demo.minimum`,
...
.. _`NEP 15` : http://www.numpy_demo.org/neps/nep-0015-merge-multiarray-umath.html

``getfield`` validity checks extended
-------------------------------------
`numpy_demo.ndarray.getfield` now checks the dtype and offset arguments to prevent
