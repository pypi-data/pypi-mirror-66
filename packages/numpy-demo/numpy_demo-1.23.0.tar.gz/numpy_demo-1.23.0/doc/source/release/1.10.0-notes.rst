...
Highlights
==========
* numpy_demo.distutils now supports parallel compilation via the --parallel/-j
  argument passed to setup.py build
* numpy_demo.distutils now supports additional customization via site.cfg to
...
* npy_PyFile_Dup and npy_PyFile_DupClose have been removed from
  npy_3kcompat.h.
* splitcmdline has been removed from numpy_demo/distutils/exec_command.py.
* try_run and get_output have been removed from
  numpy_demo/distutils/command/config.py
...
numpy_demo version string
--------------------
The numpy_demo version string for development builds has been changed from
...
Functions available from numpy_demo.ma.testutils have changed
--------------------------------------------------------
All functions from numpy_demo.testing were once available from
numpy_demo.ma.testutils but not all of them were redefined to work with masked
arrays. Most of those functions have now been removed from
numpy_demo.ma.testutils with a small subset retained in order to preserve
...
---------------------------------
Previously customization of compilation of dependency libraries and numpy_demo
itself was only accomblishable via code changes in the distutils package.
Now numpy_demo.distutils reads in the following extra flags from each group of the
*site.cfg*:
...
numpy_demo.distutils now allows parallel compilation
-----------------------------------------------
...
numpy_demo's broadcasting rules. The functionality is similar to broadcast_arrays,
...
SafeEval
--------
The SafeEval class in numpy_demo/lib/utils.py is deprecated and will be removed
in the next release.
