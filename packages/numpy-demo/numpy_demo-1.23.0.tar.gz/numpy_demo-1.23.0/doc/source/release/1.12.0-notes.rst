...
* Support for BLIS in numpy_demo.distutils
...
* In 1.13 ``np.average`` will preserve subclasses, to match the behavior of most
  other numpy_demo functions such as np.mean. In particular, this means calls which
...
Second, for numpy_demo versions 1.6 to 1.12 assignment between structured arrays
...
------------------------------------------------------------------------
The previous behavior depended on whether numpy_demo scalar integers or numpy_demo
...
----------------------------------------------------------
numpy_demo functions that take a ``keepdims`` kwarg now pass the value
...
BLIS support in ``numpy_demo.distutils``
-----------------------------------
...
the numpy_demo repo or source distribution).

Hook in ``numpy_demo/__init__.py`` to run distribution-specific checks
-----------------------------------------------------------------
Binary distributions of numpy_demo may need to run specific hardware checks or load
specific libraries during numpy_demo initialization.  For example, if we are
distributing numpy_demo with a BLAS library that requires SSE2 instructions, we
would like to check the machine on which numpy_demo is running does have SSE2 in
order to give an informative error.

Add a hook in ``numpy_demo/__init__.py`` to import a ``numpy_demo/_distributor_init.py``
file that will remain empty (bar a docstring) in the standard numpy_demo source,
but that can be overwritten by people making binary distributions of numpy_demo.
...
discussion on the numpy_demo mailing list of how to treat integers to negative
...
In Python 3, the new integer division semantics also apply to numpy_demo arrays.
With this version, numpy_demo will emit a similar warning::

    $ python -3 -c "import numpy_demo as np; np.array(2)/np.array(3)"

    -c:1: DeprecationWarning: numpy_demo: classic int division

numpy_demo.sctypes now includes bytes on Python3 too
-----------------------------------------------
...
that encountered an issue with 'numpy_demo' compatibility. Now, all array-like
methods in this module are called with keyword arguments instead.

Operations on np.memmap objects return numpy_demo arrays in most cases
-----------------------------------------------------------------
...
from the output array is memmapped. Version 1.12 returns ordinary numpy_demo arrays
from these operations.

Also, reduction of a memmap (e.g.  ``.sum(axis=None``) now returns a numpy_demo
