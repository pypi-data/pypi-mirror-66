...
is not compatible with the current polynomial support in numpy_demo, but is much
...
The new modules are not automatically imported into the numpy_demo namespace,
they must be explicitly brought in with an "import numpy_demo.polynomial"
statement.
...
The following C functions have been added to the C API:
    ...
    #. PyArray_NeighborhoodIterNew - a new iterator to iterate over a
       ...
       find some examples in  the multiarray_test.c.src file in numpy_demo.core.
...
The alpha processor is now defined and available in numpy_demo/npy_cpu.h. The
...
added to numpy_demo.distutils so that 3rd party can reuse this library. See coremath
...
============
    ...
    #. numpy_demo import is noticeably faster (from 20 to 30 % depending on the
...
The following raise errors:
    ...
    #. When operating on 0-d arrays, ``numpy_demo.max`` and other functions accept
...
The numpy_demo complex types are now guaranteed to be ABI compatible with C99
