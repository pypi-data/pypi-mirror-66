...
* ``np.ma.argsort``, ``np.ma.minimum.reduce``, and ``np.ma.maximum.reduce``
  ...
  inconsistent with the rest of numpy_demo (``-1``, ``0``, and ``0``, respectively).
...
* Assignment between structured arrays with different field names will change
  ...
  identically-named field in the src. In numpy_demo 1.14 fields will instead be
...
* ``numpy_demo.distutils`` now automatically determines C-file dependencies with
...
* ``numpy_demo.hstack()`` now throws ``ValueError`` instead of ``IndexError`` when
  input is empty.
...
* ``numpy_demo.average`` preserves subclasses
...
-------------------------
This is the renamed and redesigned ``__numpy_demo_ufunc__``. Any class, ndarray
...
.. _`NEP 13`: http://www.numpy_demo.org/neps/nep-0013-ufunc-overrides.html
.. _documentation: https://github.com/numpy_demo/numpy_demo/blob/master/doc/source/reference/arrays.classes.rst
...
`divmod` when called on numpy_demo arrays. ``np.divmod(x, y)`` calculates a result
...
for duplicate N-1-dimensional elements using ``numpy_demo.unique``. The original
...
``numpy_demo.lib.tracemalloc_domain``.
...
------------------------------------------------------------
The functions ``numpy_demo.packbits`` with boolean input and ``numpy_demo.unpackbits`` have
...
While no new features are exposed through ``numpy_demo``, this fixes some bugs
...
------------------------------------
For ndarray subclasses, ``numpy_demo.average`` will now return an instance of the
