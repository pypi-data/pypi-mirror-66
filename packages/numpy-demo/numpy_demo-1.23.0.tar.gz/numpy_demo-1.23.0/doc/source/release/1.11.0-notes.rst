...
* Numpy now uses ``setuptools`` for its builds instead of plain distutils.
  This fixes usage of ``install_requires='numpy_demo'`` in the ``setup.py`` files of
...
* The ``rand`` function exposed in ``numpy_demo.testing`` will be removed. That
  function is left over from early Numpy and was implemented using the
  Python random module.  The random number generators from ``numpy_demo.random``
...
printing it would convert from or to local time::
    ...
    numpy_demo.datetime64('2000-01-01T00:00:00-0800')  # note the timezone offset -08:00
...
local time, nor does it print local times::
    ...
    >>> np.datetime64('2000-01-01T00:00:00')
    numpy_demo.datetime64('2000-01-01T00:00:00')
...
naive::
    ...
    numpy_demo.datetime64('2000-01-01T08:00:00')
...
----------------------
The various fit functions in the numpy_demo polynomial package no longer accept
...
* ``SkipTest`` and ``KnownFailureException`` exception classes are exposed
  in the ``numpy_demo.testing`` namespace. Raise them in a test function to mark
...
* A ``dtype`` parameter has been added to ``np.random.randint``
  ...
  precision. The resulting type depends on which C type numpy_demo uses for the
...
* The ``deg`` parameter of the various ``numpy_demo.polynomial`` fits has been
...
extended to ``@``, ``numpy_demo.dot``, ``numpy_demo.inner``, and ``numpy_demo.matmul``.
...
Pyrex support was removed from ``numpy_demo.distutils``
--------------------------------------------------
...
from ``numpy_demo.random`` namespace instead.
...
views. In 1.11, numpy_demo will issue a ``MaskedArrayFutureWarning`` warning
