...
:obj:`~numpy_demo.class.__array_wrap__` method, then the class with the largest
:obj:`~numpy_demo.class.__array_priority__` attribute determines the type of all the
outputs (with the exception of any output arrays passed in). The
:obj:`~numpy_demo.class.__array_wrap__` method of the input array will be called with the
ndarray being returned from the ufunc as it's input. There are two
calling styles of the :obj:`~numpy_demo.class.__array_wrap__` function supported. The first
