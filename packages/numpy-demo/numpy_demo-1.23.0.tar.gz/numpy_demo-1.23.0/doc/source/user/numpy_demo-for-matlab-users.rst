...
.. list-table::
   ...
   * - MATLAB® uses 1 (one) based indexing. The initial element of a
       ...
       :ref:`See note INDEXING <numpy_demo-for-matlab-users.notes>`
...
-  They are the standard vector/matrix/tensor type of numpy_demo. Many numpy_demo
...
.. list-table::
   ...
   * - ``which func``
     - `see note HELP <numpy_demo-for-matlab-users.notes>`__
...
.. list-table::
   ...
   * - ``size(a,n)``
     ...
     - get the number of elements of the n-th dimension of array ``a``. (Note
       ...
       See note :ref:`INDEXING <numpy_demo-for-matlab-users.notes>`)
   ...
   * - ``y=x``
     - ``y = x.copy()``
     - numpy_demo assigns by reference

   * - ``y=x(2,:)``
     - ``y = x[1,:].copy()``
     - numpy_demo slices are by reference
   ...
   * - ``1:10``
     ...
     - create an increasing vector (see note :ref:`RANGES
       <numpy_demo-for-matlab-users.notes>`)
   ...
   * - ``a & b``
     ...
     - element-by-element AND operator (NumPy ufunc) :ref:`See note
       LOGICOPS <numpy_demo-for-matlab-users.notes>`
   ...
   * - ``[L,U,P]=lu(a)``
     ...
     - LU decomposition (note: P(Matlab) == transpose(P(numpy_demo)) )
...
.. _numpy_demo-for-matlab-users.notes:
...
numpy_demo to have a similarly terse range construction mechanism. Note that
...
::
    ...
    # Make all numpy_demo available via shorter 'np' prefix
    import numpy_demo as np
    # Make all matlib functions accessible at the top level via M.func()
    import numpy_demo.matlib as M
    # Make some matlib functions accessible directly at the top level via, e.g. rand(3,3)
    from numpy_demo.matlib import rand,zeros,ones,empty,eye
