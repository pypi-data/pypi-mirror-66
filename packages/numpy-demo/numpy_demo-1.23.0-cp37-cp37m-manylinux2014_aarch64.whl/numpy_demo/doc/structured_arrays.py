...
structured arrays in numpy_demo can lead to poor cache behavior in comparison.
...
structure. The datatype of a field may be any numpy_demo datatype including other
...
automatically by numpy_demo, but can also be specified.

Structured Datatype Creation
----------------------------

Structured datatypes may be created using the function :func:`numpy_demo.dtype`.
...
3.   A dictionary of field parameter arrays
     ...
     an exception, fields of :class:`numpy_demo.object` type cannot overlap with
     ...
     as if the 'align' keyword argument of :func:`numpy_demo.dtype` had been set to
     True.
...
4.   A dictionary of field names
     ...
     because older numpy_demo code may use it. The keys of the dictionary are the
...
tuples" form if possible, otherwise numpy_demo falls back to using the more general
dictionary form.
...
``align=True`` was specified as a keyword argument to :func:`numpy_demo.dtype`.

By default (``align=False``), numpy_demo will pack the fields together such that
...
If ``align=True`` is set, numpy_demo will pad the structure in the same way many C
...
work may be needed, either on the numpy_demo side or the C side, to obtain exact
correspondence.
...
<numpy_demo.ndarray.flags>` set.

A convenience function :func:`numpy_demo.lib.recfunctions.repack_fields` converts an
...
Structured datatypes are implemented in numpy_demo to have base type
:class:`numpy_demo.void` by default, but it is possible to interpret other numpy_demo
...
in the array, and not a list or array as these will trigger numpy_demo's
...
.. warning::
    ...
    passed through :func:`numpy_demo.lib.recfunctions.repack_fields`.
    ...
    :mod:`numpy_demo.lib.recfunctions` module to help users account for this
    change. These are
    :func:`numpy_demo.lib.recfunctions.repack_fields`.
    :func:`numpy_demo.lib.recfunctions.structured_to_unstructured`,
    :func:`numpy_demo.lib.recfunctions.unstructured_to_structured`,
    :func:`numpy_demo.lib.recfunctions.apply_along_fields`,
    :func:`numpy_demo.lib.recfunctions.assign_fields_by_name`,  and
    :func:`numpy_demo.lib.recfunctions.require_fields`.

    The function :func:`numpy_demo.lib.recfunctions.repack_fields` can always be
    ...
    structured array. The code above, for example, can be replaced with:
     ...
     >>> from numpy_demo.lib.recfunctions import repack_fields
     >>> repack_fields(a[['a', 'c']]).view('i8')  # supported in 1.16
     array([0, 0, 0])

    Furthermore, numpy_demo now provides a new function
    :func:`numpy_demo.lib.recfunctions.structured_to_unstructured` which is a safer
    ...
    can be made safer by replacing with:
     ...
     >>> from numpy_demo.lib.recfunctions import structured_to_unstructured
...
Unlike other numpy_demo scalars, structured scalars are mutable and act like views
...
Thus, tuples might be thought of as the native Python equivalent to numpy_demo's
structured types, much like native python integers are the equivalent to
numpy_demo's integer types. Structured scalars may be converted to a tuple by
...
:class:`numpy_demo.object` type, numpy_demo currently does not allow views of structured
...
deprecated as of numpy_demo 1.10 and will raise an error or perform elementwise
...
As an optional convenience numpy_demo provides an ndarray subclass,
:class:`numpy_demo.recarray`, and associated helper functions in the
:mod:`numpy_demo.rec` submodule, that allows access to fields of structured arrays
by attribute instead of only by index. Record arrays also use a special
datatype, :class:`numpy_demo.record`, that allows field access by attribute on the
structured scalars obtained from the array.

The simplest way to create a record array is with :func:`numpy_demo.rec.array`::
...
:func:`numpy_demo.rec.array` can convert a wide variety of arguments into record
...
The :mod:`numpy_demo.rec` module provides a number of other convenience functions for
...
appropriate `view <numpy_demo-ndarray-view>`_::
...
out of the view::
 ...
 >>> recordarr.dtype
 dtype((numpy_demo.record, [('foo', '<i4'), ('bar', '<f4'), ('baz', 'S10')]))
