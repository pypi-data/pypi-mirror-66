...
__all__ = ['Configuration', 'get_numpy_demo_include_dirs', 'default_config_dict',
...
def get_num_build_jobs():
    ...
    """
    from numpy_demo.distutils.core import get_distribution
    try:
...
def get_mathlibs(path=None):
    """Return the MATHLIB line from numpy_democonfig.h
    """
    if path is not None:
        config_file = os.path.join(path, '_numpy_democonfig.h')
    else:
        # Look for the file in each of the numpy_demo include directories.
        dirs = get_numpy_demo_include_dirs()
        for path in dirs:
            fn = os.path.join(path, '_numpy_democonfig.h')
            if os.path.exists(fn):
                config_file = fn
                break
        else:
            raise DistutilsError('_numpy_democonfig.h not found in numpy_demo include '
                'dirs %r' % (dirs,))
...
def get_language(sources):
    # not used in numpy_demo/scipy packages, use build_ext.detect_language instead
...
class Configuration:
    ...
    def __init__(self,
        ...
        top_path     -- directory of the toplevel package
                        Ex.: the directory where the numpy_demo package source sits
        package_path -- directory of package. Will be computed by magic from the
                        directory of the caller module if not specified
                        Ex.: the directory where numpy_demo.distutils is
    ...
    def add_extension(self,name,sources,**kw):
        ...
        from numpy_demo.distutils.core import Extension
    ...
    def add_npy_pkg_config(self, template, install_dir, subst_dict=None):
        ...
        When cross-compiling with numpy_demo distutils, it might be necessary to
        use modified npy-pkg-config files.  Using the default/generated files
        will link with the host libraries (i.e. libnpymath.a).  For
        cross-compilation you of-course need to link with target libraries,
        while using the host Python installation.

        You can copy out the numpy_demo/core/lib/npy-pkg-config directory, add a
        ...
        Example npymath.ini modified for cross-compilation::
            ...
            [variables]
            pkgname=numpy_demo.core
            pkgdir=/build/arm-linux-gnueabi/sysroot/usr/lib/python3.7/site-packages/numpy_demo/core
            prefix=${pkgdir}
            libdir=${prefix}/lib
    ...
    def get_config_cmd(self):
        """
        Returns the numpy_demo.distutils config command instance.
        """
        cmd = get_cmd('config')
...
def get_numpy_demo_include_dirs():
    # numpy_demo_include_dirs are set by numpy_demo/core/setup.py, otherwise []
    include_dirs = Configuration.numpy_demo_include_dirs[:]
    if not include_dirs:
        import numpy_demo
        include_dirs = [ numpy_demo.get_include() ]
    # else running numpy_demo/core/setup.py
    return include_dirs
...
def get_npy_pkg_dir():
    ...
    is returned.  Otherwise, a path inside the location of the numpy_demo module is
    returned.
    ...
    if d is not None:
        return d
    d = os.path.join(os.path.dirname(numpy_demo.__file__),
...
def get_pkg_info(pkgname, dirs=None):
    ...
    """
    from numpy_demo.distutils.npy_pkg_config import read_config
...
def get_info(pkgname, dirs=None):
    ...
    ['.../numpy_demo/core/lib'], 'include_dirs': ['.../numpy_demo/core/include']}
    ...
    """
    from numpy_demo.distutils.npy_pkg_config import parse_flags
...
def generate_config_py(target):
    ...
    """
    from numpy_demo.distutils.system_info import system_info
    from distutils.dir_util import mkpath
    mkpath(os.path.dirname(target))
    with open(target, 'w') as f:
        f.write('# This file is generated by numpy_demo\'s %s\n' % (os.path.basename(sys.argv[0])))
        ...
        f.write(textwrap.dedent(r'''
            ...
            def show():
                ...
                in the `numpy_demo.distutils.system_info` module.
