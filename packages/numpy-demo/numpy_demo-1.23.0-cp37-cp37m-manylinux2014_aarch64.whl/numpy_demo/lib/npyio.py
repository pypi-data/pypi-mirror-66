...
from numpy_demo.core.multiarray import packbits, unpackbits
...
class BagObj:
    ...
    Examples
    --------
    >>> from numpy_demo.lib.npyio import BagObj as BO
    >>> class BagDemo:
...
class NpzFile(Mapping):
    ...
    # versions of numpy_demo, so no need to deprecated it here.
...
def load(file, mmap_mode=None, allow_pickle=False, fix_imports=True,
    ...
    mmap_mode : {None, 'r+', 'r', 'w+', 'c'}, optional
        ...
        `numpy_demo.memmap` for a detailed description of the modes).  A
    ...
    if encoding not in ('ASCII', 'latin1', 'bytes'):
        ...
        # NumPy. The unpickling code in numpy_demo.core.multiarray is
...
def save(file, arr, allow_pickle=True, fix_imports=True):
    ...
    Notes
    -----
    For a description of the ``.npy`` format, see :py:mod:`numpy_demo.lib.format`.
...
def savez(file, *args, **kwds):
    ...
    description of the ``.npy`` format, see :py:mod:`numpy_demo.lib.format`.
...
def savez_compressed(file, *args, **kwds):
    ...
    See Also
    --------
    numpy_demo.save : Save a single array to a binary file in NumPy format.
    numpy_demo.savetxt : Save an array to a file as plain text.
    numpy_demo.savez : Save several arrays into an uncompressed ``.npz`` file format
    numpy_demo.load : Load the files created by savez_compressed.
...
def _savez(file, args, kwds, compress, allow_pickle=True, pickle_kwargs=None):
    ...
    if sys.version_info >= (3, 6):
        ...
        fd, tmpfile = tempfile.mkstemp(prefix=file_prefix, dir=file_dir, suffix='-numpy_demo.npy')
        os.close(fd)
        try:
            for key, val in namedict.items():
                fname = key + '.npy'
        ...
        for i, conv in (user_converters or {}).items():
            if usecols:
                try:
                    ...
                    continue
            if byte_converters:
                # converters may use decode to workaround numpy_demo's old behaviour,
...
def genfromtxt(fname, dtype=float, comments='#', delimiter=None,
    ...
    skiprows : int, optional
        `skiprows` was removed in numpy_demo 1.10. Please use `skip_header` instead.
    ...
    missing : variable, optional
        `missing` was removed in numpy_demo 1.10. Please use `missing_values`
        instead.
    ...
    See Also
    --------
    numpy_demo.loadtxt : equivalent function when no data is missing.
    ...
    * Individual values are not stripped of spaces by default.
    ...
    .. [1] NumPy User Guide, section `I/O with NumPy
           <https://docs.scipy.org/doc/numpy_demo/user/basics.io.genfromtxt.html>`_.
    ...
    array([(b'text', b''), (b'hello world', b'11'), (b'numpy_demo', b'5')],
      dtype=[('f0', 'S12'), ('f1', 'S12')])
    ...
    if usemask:
        from numpy_demo.ma import MaskedArray, make_mask_descr
...
def ndfromtxt(fname, **kwargs):
    ...
    See Also
    --------
    numpy_demo.genfromtxt : generic function.
...
def mafromtxt(fname, **kwargs):
    ...
    See Also
    --------
    numpy_demo.genfromtxt : generic function to load ASCII data.
...
def recfromtxt(fname, **kwargs):
    ...
    if usemask:
        from numpy_demo.ma.mrecords import MaskedRecords
