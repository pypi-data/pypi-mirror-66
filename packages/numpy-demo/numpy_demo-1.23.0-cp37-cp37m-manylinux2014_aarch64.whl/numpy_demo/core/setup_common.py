...
from numpy_demo.distutils.misc_util import mingw32
...
def is_released(config):
    """Return True if a released version of numpy_demo is detected."""
...
def get_api_versions(apiversion, codegen_dir):
    ...
    try:
        m = __import__('genapi')
        numpy_demo_api = __import__('numpy_demo_api')
        curapi_hash = m.fullapi_hash(numpy_demo_api.full_api)
...
def check_api_version(apiversion, codegen_dir):
    ...
    # To compute the checksum of the current API, use numpy_demo/core/cversions.py
