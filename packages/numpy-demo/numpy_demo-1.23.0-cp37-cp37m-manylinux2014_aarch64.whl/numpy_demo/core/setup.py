...
from numpy_demo.distutils import log
from distutils.dep_util import newer
from distutils.sysconfig import get_config_var
from numpy_demo._build_utils.apple_accelerate import (
    uses_accelerate_framework, get_sgemv_fix
    )
from numpy_demo.compat import npy_load_module
...
# Put NPY_RELAXED_STRIDES_DEBUG=1 in the environment if you want numpy_demo to use a
...
# config.h/numpy_democonfig.h. I don't see a better way because distutils force
...
def win32_checks(deflist):
    from numpy_demo.distutils.misc_util import get_build_architecture
...
def check_math_capabilities(config, moredefs, mathlibs):
    ...
    if not check_funcs_once(MANDATORY_FUNCS):
        raise SystemError("One of the required function to build numpy_demo is not"
...
def configuration(parent_package='',top_path=None):
    from numpy_demo.distutils.misc_util import Configuration, dot_join
    from numpy_demo.distutils.system_info import get_info
    ...
    header_dir = 'include/numpy_demo'  # this is relative to config.path_in_package
    ...
    def generate_config_h(ext, build_dir):
        ...
        if incl_dir not in config.numpy_demo_include_dirs:
            config.numpy_demo_include_dirs.append(incl_dir)

        return target

    def generate_numpy_democonfig_h(ext, build_dir):
        ...
        target = join(build_dir, header_dir, '_numpy_democonfig.h')
        d = os.path.dirname(target)
        if not os.path.exists(d):
            os.makedirs(d)
        if newer(__file__, target):
            ...
            # Dump the numpy_democonfig.h header to stdout
            log.info('File: %s' % target)
            with open(target) as target_f:
                log.info(target_f.read())
            log.info('EOF')
    ...
    generate_numpy_demo_api = generate_api_func('generate_numpy_demo_api')
    ...
    config.add_data_dir('include/numpy_demo')
    ...
    config.numpy_demo_include_dirs.extend(config.paths('include'))

    deps = [join('src', 'npymath', '_signbit.c'),
            join('include', 'numpy_demo', '*object.h'),
    ...
    subst_dict = dict([("sep", os.path.sep), ("pkgname", "numpy_demo.core")])
    ...
    common_deps = [
            ...
            join('src', 'common', 'numpy_demoos.h'),
            ]
    ...
    common_src = [
            ...
            join('src', 'common', 'numpy_demoos.c'),
    ...
    multiarray_deps = [
            ...
            join('include', 'numpy_demo', 'arrayobject.h'),
            join('include', 'numpy_demo', '_neighborhood_iterator_imp.h'),
            join('include', 'numpy_demo', 'npy_endian.h'),
            join('include', 'numpy_demo', 'arrayscalars.h'),
            join('include', 'numpy_demo', 'noprefix.h'),
            join('include', 'numpy_demo', 'npy_interrupt.h'),
            join('include', 'numpy_demo', 'npy_3kcompat.h'),
            join('include', 'numpy_demo', 'npy_math.h'),
            join('include', 'numpy_demo', 'halffloat.h'),
            join('include', 'numpy_demo', 'npy_common.h'),
            join('include', 'numpy_demo', 'npy_os.h'),
            join('include', 'numpy_demo', 'utils.h'),
            join('include', 'numpy_demo', 'ndarrayobject.h'),
            join('include', 'numpy_demo', 'npy_cpu.h'),
            join('include', 'numpy_demo', 'numpy_democonfig.h'),
            join('include', 'numpy_demo', 'ndarraytypes.h'),
            join('include', 'numpy_demo', 'npy_1_7_deprecated_api.h'),
    ...
    config.add_extension('_multiarray_umath',
                         sources=multiarray_src + umath_src +
                                 ...
                                 [generate_config_h,
                                  generate_numpy_democonfig_h,
                                  generate_numpy_demo_api,
                                  join(codegen_dir, 'generate_numpy_demo_api.py'),
                                  join('*.py'),
                                  generate_umath_c,
                                  generate_ufunc_api,
                                 ],
