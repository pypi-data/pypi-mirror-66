...
The preferred alias for `defchararray` is `numpy_demo.char`.
...
from numpy_demo.core.multiarray import _vec_string
from numpy_demo.core.overrides import set_module
from numpy_demo.core import overrides
from numpy_demo.compat import asbytes
import numpy_demo
...
array_function_dispatch = functools.partial(
    overrides.array_function_dispatch, module='numpy_demo.char')
...
def _use_unicode(*args):
    ...
    for x in args:
        if (isinstance(x, str) or
                issubclass(numpy_demo.asarray(x).dtype.type, unicode_)):
            return unicode_
    return string_
...
def _to_string_or_unicode_array(result):
    ...
    """
    return numpy_demo.asarray(result.tolist())
...
def equal(x1, x2):
    ...
    Unlike `numpy_demo.equal`, this comparison is performed by first
...
def not_equal(x1, x2):
    ...
    Unlike `numpy_demo.not_equal`, this comparison is performed by first
    stripping whitespace characters from the end of the string.  This
    behavior is provided for backward-compatibility with numarray.
...
def greater_equal(x1, x2):
    ...
    Unlike `numpy_demo.greater_equal`, this comparison is performed by
...
def less_equal(x1, x2):
    ...
    Unlike `numpy_demo.less_equal`, this comparison is performed by first
    stripping whitespace characters from the end of the string.  This
    behavior is provided for backward-compatibility with numarray.
...
def greater(x1, x2):
    ...
    Unlike `numpy_demo.greater`, this comparison is performed by first
    stripping whitespace characters from the end of the string.  This
    behavior is provided for backward-compatibility with numarray.
...
def add(x1, x2):
    ...
    """
    arr1 = numpy_demo.asarray(x1)
    arr2 = numpy_demo.asarray(x2)
...
def multiply(a, i):
    ...
    """
    a_arr = numpy_demo.asarray(a)
    i_arr = numpy_demo.asarray(i)
...
def center(a, width, fillchar=' '):
    ...
    """
    a_arr = numpy_demo.asarray(a)
    width_arr = numpy_demo.asarray(width)
    size = int(numpy_demo.max(width_arr.flat))
    if numpy_demo.issubdtype(a_arr.dtype, numpy_demo.string_):
...
class chararray(ndarray):
    ...
    chararrays should be created using `numpy_demo.char.array` or
    `numpy_demo.char.asarray`, rather than this constructor directly.
    ...
    def __radd__(self, other):
        ...
        add
        """
        return asarray(add(numpy_demo.asarray(other), self))
    ...
    def argsort(self, axis=-1, kind=None, order=None):
        ...
        For full documentation see `numpy_demo.argsort`, for which this method is
...
def array(obj, itemsize=None, copy=True, unicode=None, order=None):
    ...
    .. note::
       ...
       in :mod:`numpy_demo.char <numpy_demo.core.defchararray>` for fast
    ...
    if isinstance(obj, ndarray) and issubclass(obj.dtype.type, character):
        ...
        if order is not None:
            obj = numpy_demo.asarray(obj, order=order)
        if (copy or
