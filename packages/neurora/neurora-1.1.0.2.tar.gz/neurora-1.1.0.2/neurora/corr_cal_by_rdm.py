...
' a function for calculating the Similarity/Correlation Cosfficient between RDMs based on EEG/MEG/fNIRS/ECoG/electrophysiological RDMs and a demo RDM'

def rdms_corr(demo_rdm, eeg_rdms, method="spearman", rescale=False):

    """
    Calculate the Similarities between EEG/MEG/fNIRS/ECoG/electrophysiological RDMs and a demo RDM
    ...
    demo_rdm : array [n_cons, n_cons]
        A demo RDM.
    eeg_rdms : array
        The EEG/MEG/fNIRS/ECoG/electrophysiological RDM(s).
    ...
    -------
    corrs : array
        The similarities between EEG/MEG/fNIRS/ECoG/electrophysiological RDM and a demo RDM
    ...
    if len(eeg_rdms.shape) == 3:
        ...
        for i in range(n1):
            if method == "spearman":
                corrs[i] = rdm_correlation_spearman(demo_rdm, eeg_rdms[i], rescale=rescale)
            elif method == "pearson":
                corrs[i] = rdm_correlation_pearson(demo_rdm, eeg_rdms[i], rescale=rescale)
            elif method == "kendall":
                corrs[i] = rdm_correlation_kendall(demo_rdm, eeg_rdms[i], rescale=rescale)
            elif method == "similarity":
...
' a function for calculating the Similarity/Correlation Cosfficient between RDMs based on fMRI RDMs and a demo RDM'
