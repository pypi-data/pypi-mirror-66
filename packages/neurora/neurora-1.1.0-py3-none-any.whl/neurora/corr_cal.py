...
def bhvANDeeg_corr(bhv_data, eeg_data, sub_opt=0, chl_opt=0, time_opt=0, time_win=5, time_step=5, method="spearman", rescale=False):
...
def bhvANDecog_corr(bhv_data, ele_data, time_win=5, time_step=5, ecog_opt="allin", method="spearman", rescale=False):
    ...
    # ecog_opt='time'
        ...
        # calculate the ecog_rdms
        ecog_rdms = ecogRDM(ele_data, time_win=time_win, time_step=time_step, opt="time")
...
' a function for calculating the Similarity/Correlation Cosfficient between behavioral data and fMRI data for searchlight'

def bhvANDfmri_corr(bhv_data, fmri_data, ksize=[3, 3, 3], strides=[1, 1, 1], method="spearman", rescale=False):
...
' a function for calculating the Similarity/Correlation Cosfficient between behavioral EEG/MEG/fNIRS and fMRI data for searchlight'
