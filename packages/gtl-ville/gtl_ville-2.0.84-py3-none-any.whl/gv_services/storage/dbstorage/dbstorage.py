...
class DbStorage:
    ...
    async def get_data_quality(self, datatypeeid):
        result = {}
        if datatypeeid == TOMTOMFCDDATATYPE or datatypeeid == TOMTOMFCDDATATYPE:
            if datatypeeid in self.datatypeseidtoid:
                result = dbrequest.select_data_points_data_quality(self.dbpool, self.datatypeseidtoid[datatypeeid])
        elif datatypeeid == ROADSDATATYPE:
            result = dbrequest.select_roads_data_quality(self.dbpool)
        return result
