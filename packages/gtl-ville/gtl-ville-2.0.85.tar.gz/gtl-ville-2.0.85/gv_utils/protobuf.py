...
async def encode_data_quality(dataquality):
    loop = asyncio.get_event_loop()
    with ProcessPoolExecutor() as executor:
        data = await loop.run_in_executor(executor, _encode_data_quality, dataquality)
    return DataQuality(dataquality=data)


def _encode_data_quality(dataquality):
    return json.dumps(dataquality).encode(csv.ENCODING)
