...
def lot(swarm, objective_func, **kwargs):
    # 计算 Y
    Y = []

    for i in range(swarm.dimensions):
        sorted_index = sorted(range(len(swarm.pbest_cost)), key=lambda k: swarm.pbest_pos[k, i])
        sorted_pos = swarm.pbest_pos[sorted_index]
        sorted_cost = swarm.pbest_cost[sorted_index]

        # 寻找所有的极小值点
        y = []
        if sorted_cost[0] <= sorted_cost[1]:
            y.append(sorted_pos[0][i])

        for j in range(1, len(sorted_cost) - 1):
            if sorted_cost[j] <= sorted_cost[j - 1] and sorted_cost[j] <= sorted_cost[j + 1]:
                y.append(sorted_pos[j][i])

        if sorted_cost[len(sorted_cost) - 1] <= sorted_cost[len(sorted_cost) - 2]:
            y.append(sorted_pos[len(sorted_cost) - 1][i])

        # 将所有的极小值点存储在 Y 中
        Y.append(y)

    # 在 Y 中，将维度随机组合，生成新的 pos
    new_position = []
    for i in range(swarm.dimensions):
        index = np.random.randint(0, len(Y[i]), size=swarm.n_particles)
        new_position.append([Y[i][j] for j in index])
    new_position = np.array(new_position).transpose()
    # 比较 pos 与 swarm.pbest_pos, 如果目标值更好的话, 替换该粒子
    new_cost = objective_func(new_position, **kwargs)
    new_pbest = []
    new_pbest_position = []
    for i in range(swarm.n_particles):
        if new_cost[i] < swarm.pbest_cost[i]:
            swarm.position[i] = new_position[i]
            swarm.current_cost[i] = new_cost[i]
            new_pbest.append(new_cost[i])
            new_pbest_position.append(new_position[i])
            if new_cost[i] < swarm.best_cost:
                swarm.best_cost = new_cost[i]
                swarm.best_pos = np.array(new_position[i])
        else:
            new_pbest.append(swarm.pbest_cost[i])
            new_pbest_position.append(swarm.pbest_pos[i])
    swarm.pbest_cost = np.array(new_pbest)
    swarm.pbest_pos = np.array(new_pbest_position)


def lot_velocity(swarm, clamp, vh, i, iters, bounds=None):
    try:
        # Prepare parameters
        swarm_size = swarm.position.shape
        c1 = swarm.options["c1"]
        c2 = swarm.options["c2"]
        w_max = swarm.options["w_max"]
        w_min = swarm.options["w_min"]
