...
from ..backend.operators import compute_pbest, compute_objective_function, lot, lot_velocity
...
class CLPSO_LOT(SwarmOptimizer):
    def __init__(
        self,
        n_particles,
        dimensions,
    ...
    def optimize(self, objective_func, iters, n_processes=None, **kwargs):
        ...
        for i in self.rep.pbar(iters, self.name):
            # g = self.swarm.options['g']
            # Compute cost for current position and personal best
            # fmt: off
            self.swarm.current_cost = compute_objective_function(self.swarm, objective_func, pool=pool, **kwargs)
            self.swarm.pbest_pos, self.swarm.pbest_cost = compute_pbest(self.swarm)
            lot(self.swarm, objective_func, **kwargs)
            ...
            self.swarm.velocity = lot_velocity(
                self.swarm, self.velocity_clamp, self.vh, i, iters, self.bounds
            )
