...
class App(Thread):
	...
	def __init__(self,size_x,size_y):
		...
		self.key = ''
		print('Starting App...')
		stream = Popen('java -jar processing-py.jar i3_jython.py',cwd=os.path.dirname(os.path.realpath(__file__))+'\\processing.py',stdin=PIPE, stdout=PIPE,stderr=PIPE)
	...
	def get_global_variables(self):
		try:
			file = open(os.path.dirname(os.path.realpath(__file__))+'\\processing.py\\global_variables.txt','r')
