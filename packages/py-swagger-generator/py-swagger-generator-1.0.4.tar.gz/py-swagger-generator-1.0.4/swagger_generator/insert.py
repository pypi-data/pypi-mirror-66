...
def insert_route_data_into_document_data_at_end(route_data : dict, document_data : dict):
    try:
        ...
        try:
            ...
            Exception(f"The key at {get_first_key_in_dict(route_data)}/{get_first_key_in_dict(unwrap_first_dict(route_data))} already exists")
        except KeyError:
...
def insert_data_into_document_data_at_end(data : dict, document_data : dict):
    try:
        ...
        Exception(f"The key {get_first_key_in_dict(data)} already exists in the document.")
    except KeyError:
