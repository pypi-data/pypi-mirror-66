...
class TestQuantity(unittest.TestCase):
    ...
    def test_vectorize_00(self):
        
        @vectorize
        def positive_length(l):
            if l < 0*m:
                return 0*m
            return l
        
        # with "scalar"
        self.assertEqual(positive_length(-1*m), 0*m)
        self.assertEqual(positive_length(0*m), 0*m)
        self.assertEqual(positive_length(1*m), 1*m)
    
        # with 1D array
        arr_m = np.array([-1, 0, 1])*m
        self.assertEqual(positive_length(arr_m),
                        np.array([0, 0, 1])*m)
        
        # wth 2D array
        arr_m = np.array([-1, 0, 1],
                         [-1, 0, 1],
                         [-1, 0, 1],)*m
        self.assertEqual(positive_length(arr_m),
                        np.array([0, 0, 1],
