...
class Control(object):
    ...
    def econ_calc(self, cost_elec='lcoe', cost_sub=0, print_out=True):
        ...
        # get revenue
        rev_cf = self.algo.ld.enr_tot*cost_elec/(
            (1+self.disc)**np.arange(self.yr_proj+1)
        )
        rev_cf[0] = 0
        ...
        # get NPV
        self.npv = self.rev_tot-self.npc
        ...
        nomcf_tot = (rev_cf-cost_cf)*ann_inv
        ...
        # get UCME
        self.ucme = (self.lcoe-cost_sub)*self.algo.ld.enr_tot
    ...
    def cashflow_plot(
        self, cost_elec='lcoe', cost_sub=0, fig_size=(12, 5)
    ):
        ...
        if not self.is_econ:
            self.econ_calc(cost_elec, cost_sub, False)
        ...
        for cp, sub in zip(self.cf_dict, self.cf_dict.values()):
            ...
            if cp == 'Revenue':
                ...
                # plot revenue
                plt.bar(t, sub, color='#33CC33', label='Revenue')
    ...
    def cashflow_csv(self, file, cost_elec='lcoe', cost_sub=0):
