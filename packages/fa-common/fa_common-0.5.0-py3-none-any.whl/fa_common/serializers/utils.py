...
from pydantic import Required, create_model, fields
    ...
    if hasattr(Meta, "model") and Meta.model is not None:
        for f, t in Meta.model.__fields__.items():
            if f not in excluded:
                ...
                _type = None
                if t.shape == fields.SHAPE_LIST:
                    _type = List[t.type_]
                elif t.shape == fields.SHAPE_SET:
                    _type = Set[t.type_]
                elif t.shape == fields.SHAPE_MAPPING:
                    _type = Mapping[t.key_field.type_, t.type_]
                elif t.shape == fields.SHAPE_TUPLE:
                    _type = t.type_
                elif t.shape == fields.SHAPE_TUPLE_ELLIPSIS:
                    _type = Tuple[t.type_, ...]
                elif t.shape == fields.SHAPE_SEQUENCE:
