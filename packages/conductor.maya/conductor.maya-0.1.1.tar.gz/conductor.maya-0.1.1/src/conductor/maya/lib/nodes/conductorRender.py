...
class conductorRender(om.MPxNode):
    ...
    def postConstructor(self):
        ...
        fn_node.findPlug(self.aAutosaveTemplate, True).setString(k.DEFAULT_AUTOSAVE_TEMPLATE)
    ...
    def set_default_asset_scrapers(cls, fn_node):
        ...
        scrapers_path = os.path.join(
            pm.moduleInfo(
                path=True, moduleName="conductor"), "scripts", "scrapers"
        )
    ...
    def make_frames_atts(cls):
        ...
        cls.aUseScoutFrames = nAttr.create(
            "useScoutFrames", "usf", om.MFnNumericData.kBoolean, False
        )
        nAttr.writable = True
        nAttr.keyable = True
        nAttr.storable = True
    ...
    # @classmethod
    # def get_system_paths(cls, data):
    #     ws = pm.Workspace()
    #     return [ws.expandName(pm.sceneName())]
