...
class EurekaClient:
    ...
    def __init__(self,
        ...
        atexit.register(lambda: self._loop.run_until_complete(
            self._SESSION.close()))
    ...
    async def register(self, *, metadata: Optional[Dict[str, Any]] = None,
        ...
        payload = {
            'instance': {
                ...
                'homePageUrl': f'http://{self._hostname}:{self._port}/',
    ...
    def get_app_sync(self, app_name: Optional[str] = None) -> Dict[str, Any]:
        app_name = app_name or self._app_name
        url = '/apps/{}'.format(app_name)
        return self._do_req_sync(url)
    ...
    def _do_req_sync(self, path: str, *, method: str = 'GET',
                     data: Optional[Any] = None):
        """
        Performs a request against the instance eureka server.
        :param path: URL Path, the hostname is prepended automatically
