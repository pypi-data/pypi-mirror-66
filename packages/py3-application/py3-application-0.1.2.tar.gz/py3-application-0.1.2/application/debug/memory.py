    ...
    def __str__(self):
        ...
        while cycle:
            ...
            if cycle and cycle[0] is getattr(obj, '__dict__', None):
                ...
                try:
                    if cycle:
                        string += ' .%s' % next((key for key,
                                                 value in d.items() if value is cycle[0]))
                    else:
                        string += ' .%s' % next((key for key,
                                                 value in d.items() if value is first_obj))
...
def memory_dump(show_cycles=True, show_objects=False):
    ...
    if show_cycles:
        ...
        for obj in garbage:
            nodes[id(obj)].successors = tuple(nodes[id(s)]
                                              for s in gc.get_referents(obj) if id(s) in nodes)
            nodes[id(obj)].visitable_successors = deque(
                nodes[id(obj)].successors)
            ...
            while path:
                ...
                if node.visitable_successors:
                    ...
                    if successor in path:
                        cycles.add(
                            Cycle(n.object for n in path[path.index(successor):]))
                    else:
    ...
    if show_objects:
        try:
            ...
            import sys
            import termios
            console_width = struct.unpack('HHHH', fcntl.ioctl(
                sys.stdout.fileno(), termios.TIOCGWINSZ, struct.pack('HHHH', 0, 0, 0, 0)))[1]
        except Exception:
            console_width = 80
