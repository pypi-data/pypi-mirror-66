...
def stopCluster(chars, emrName, keyFile):
    hostnameList, chief, _ = getHostnameList_Chief_Hostnum(emrName)
    for node in hostnameList:
        if node == chief:
            os.system("ps -ef | grep python | grep %s | awk '{print \"kill -9 \" $2}' | bash -v" % chars)
        else:
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh.connect(hostname=node, port=port, username=UserName, key_filename=keyFile)
            _ = ssh.exec_command("ps -ef | grep python | grep %s | awk '{print \"kill -9 \" $2}' | bash -v" % chars)
            time.sleep(0.1)
            ssh.close()
