...
class TFCluster(object):
  ...
  def shutdown(self, ssc=None, grace_secs=0, timeout=259200):
    ...
    Args:
      ...
      :grace_secs: Grace period to wait after all executors have completed their tasks before terminating the Spark application, e.g. to allow the chief worker to perform any final/cleanup duties like exporting or evaluating the models.  Default is 0.
...
def run(sc, map_fun, tf_args, num_executors, num_ps, tensorboard=False, input_mode=InputMode.TENSORFLOW,
  ...
  Args:
    ...
    :tf_args: ``argparse`` _args, or command-line ``ARGV``.  These will be passed to the ``map_fun``.
  ...
  defaultFS = sc._jsc.hadoopConfiguration().get("client.defaultFS")
