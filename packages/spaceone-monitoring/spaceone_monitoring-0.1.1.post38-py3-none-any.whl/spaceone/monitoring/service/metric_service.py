...
class MetricService(BaseService):
    ...
    def list(self, params):
        ...
        for resource_id, resource_info in resources_info.items():
            ...
            if resources['metrics'] is None:
                resources['metrics'] = metrics_info.get('metrics', [])
    ...
    def _merge_metric_keys(metrics_info, metrics_dict, and_metric_keys):
        ...
        for metric_info in metrics_info.get('metrics', []):
            if 'key' in metric_info:
                ...
                resource_metric_keys.append(metric_key)

            if and_metric_keys is None:
                and_metric_keys = set(resource_metric_keys)
            else:
                and_metric_keys = and_metric_keys and set(resource_metric_keys)
