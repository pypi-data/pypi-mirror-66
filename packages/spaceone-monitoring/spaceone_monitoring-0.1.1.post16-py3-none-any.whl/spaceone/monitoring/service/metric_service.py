...
class MetricService(BaseService):
    ...
    def list(self, params):
        ...
        for resource_id, resource_info in resources_info.items():
            ...
            if response['metrics'] is None:
                response['metrics'] = metrics_info['metrics']
    ...
    def get_data(self, params):
        ...
        for resource_id, resource_info in resources_info.items():
            ...
            secret_data = self._get_secret_data(resource_id, resource_info, data_source_vo, domain_id)
            metric_data_info = self.plugin_mgr.get_metric_data(plugin_options, secret_data, resource_key, params['start'],
                                                               params['end'], params.get('period'), params.get('stat'))

            if response['labels'] is None:
                response['labels'] = metric_data_info['labels']

            response['resource_values'][resource_id] = metric_data_info['values']
