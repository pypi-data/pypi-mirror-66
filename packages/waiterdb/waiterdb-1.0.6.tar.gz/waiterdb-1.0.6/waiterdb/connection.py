...
from waiterdb.avatica.client import OPEN_CONNECTION_PROPERTIES
from waiterdb.cursor import Cursor
from waiterdb.errors import ProgrammingError
...
class Connection(object):
    ...
    You should not construct this object manually, use :func:`~waiterdb.connect` instead.
    """
    ...
    def cursor(self, cursor_factory=None):
        ...
        :param cursor_factory:
            ...
            :class:`~waiterdb.cursor.Cursor` (for example :class:`~waiterdb.cursor.DictCursor`).
            A default factory for the connection can also be specified using the
            :attr:`cursor_factory` attribute.

        :returns:
            A :class:`~waiterdb.cursor.Cursor` object.
        """
        if self._closed:
            raise ProgrammingError('the connection is already closed')
    ...
    def set_session(self, autocommit=None, readonly=None):
        ...
        response = self._client.connection_sync(self._id, props)
        props = response['connProps']
        self._autocommit = props['autoCommit']
        self._readonly = props['readOnly']
        self._transactionisolation = props['transactionIsolation']
