...
import logging
import collections
from waiterdb.errors import ProgrammingError, InternalError, NotSupportedError
...
ColumnDescription = collections.namedtuple('ColumnDescription', 'name type_code precision scale null_ok')
...
class Cursor(object):
    ...
    You should not construct this object manually, use :meth:`Connection.cursor() <waiterdb.connection.Connection.cursor>` instead.
    """
    ...
    def _set_signature(self, signature):
        ...
        for column in signature['columns']:
            dtype = type(column['columnClassName'])
            self._column_data_types.append(dtype)
        #TODO
        # for parameter in signature['parameters']:
        #     dtype = TypeHelper.from_class(parameter['className'])
    ...
    def _process_results(self, results):
        if results:
            ...
            self._set_signature(result.get('signature'))
            self._set_frame(result.get('firstFrame'))
    ...
    def execute(self, sql):
        ...
        results = self._connection._client.prepare_and_execute(
            self._connection._id, self._id,
            sql, first_frame_max_size=self.itersize)
    ...
    def executemany(self, operation, seq_of_parameters):
        raise NotSupportedError('executemany is unsupported')
        # if self._closed:
        #     raise ProgrammingError('the cursor is already closed')
        # self._updatecount = -1
        # self._set_frame(None)
    ...
    @property
    def connection(self):
        """Read-only attribute providing access to the :class:`Connection <waiterdb.connection.Connection>`
