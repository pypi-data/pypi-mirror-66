...
class Pin(ParametrizedChildEntity):
    ...
    def __str__(self):
        return "Pin {} (w={}, xs={})".format(
            self.name, self.width, self.xsection
        )
...
class BB(ParametrizedEntity):
    ...
    def get_pin_data(self, pin_name) -> Dict:
        if pin_name in self.pin_names:
            return self._pins[pin_name][0]
        else:
            raise ValueError(
                f"Pin '{pin_name}' is not defined in the {self.name} BB."
            )
    ...
    def _make_cell(self):
        ...
        try:
            ...
            print("Error creating cell for BB '': {}".format(self.full_name, e))
            return None
...
class UPDK(object):
    ...
    def get_building_block_data(self, bb_name) -> Dict:
        if bb_name in self.building_block_names:
            return self._bbs[bb_name][0]
        else:
            raise ValueError(
