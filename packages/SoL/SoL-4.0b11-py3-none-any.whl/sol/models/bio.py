    ...
    def addTourney(self, stourney):
        ...
        if tourney is None:
            ...
            for m in matches:
                ...
                if 'misses1' in m:
                    misses = m.pop('misses1')
                    avg = misses // championship.trainingboards
                    for i in range(1, championship.trainingboards):
                        m[f'misses1_{i}'] = avg
                        misses -= avg
                    m[f'misses1_{championship.trainingboards}'] = misses
                if 'misses2' in m:
                    misses = m.pop('misses2')
                    avg = misses // championship.trainingboards
                    for i in range(1, championship.trainingboards):
                        m[f'misses2_{i}'] = avg
                        misses -= avg
                    m[f'misses2_{championship.trainingboards}'] = misses
...
def save_changes(sasess, request, modified, deleted, clogger=changes_logger):
    ...
    for entity, key, fvalues in inserted:
        try:
            ...
            clogger.exception('Unexpected error trying to insert %r: %r', entity, fvalues)
            raise
