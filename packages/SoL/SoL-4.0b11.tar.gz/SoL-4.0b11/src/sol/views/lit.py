...
@view_config(route_name="training_match", renderer="lit/training_match.mako")
def training_match_form(request):
    ...
    if cnum == 1:
        already_entered = m.misses1_1 is not None or m.score1
    else:
        already_entered = m.misses2_1 is not None or m.score2
...
def store_training_match(request):
    ...
    errors = tuple(int(e) for e in errors)
    for i, e in enumerate(errors, 1):
        setattr(m, f'misses{cnum}_{i}', e)

    if m.misses1_1 is not None and m.misses2_2 is not None:
        total1 = sum(getattr(m, f'misses1_{i}') for i in range(1, tboards+1))
        total2 = sum(getattr(m, f'misses2_{i}') for i in range(1, tboards+1))
        m.score1, m.score2 = errors_to_scores(total1, total2, tboards)

    changes_logger.info('%r self updated his errors for %r: %s => %s', player, m,
                        ', '.join(str(e) for e in errors), sum(errors))
