...
def MC_vanilla(prices, payoff, rd, rf, term, bstrap=False):
    ...
    Inputs:
        ...
        bstrap - if True, use bootstrap resampling to get better estimate of
                 monte-carlo error.
    ...
    if bstrap:
        ...
        for _ in range(min(n, 10000)):
            idxs = np.random.randint(0, n - 1, n)
...
def MC_american(paths, payoff, term, rd, rf,
    ...
    if basis.upper() == "POLY":
        def extrcontval(x, y, deg):
            ...
            return np.dot(
                    B.T, np.dot(
                            np.linalg.pinv(np.dot(B, B.T)), np.dot(B, y)))
        ...
        def extrcontval(x, y, deg):
            ...
            wx = np.exp(-0.5*x*x)
            B = np.vstack(
                    (np.ones_like(x), wx*hermite_e.hermevander(x, deg).T))
            return np.dot(
                    B.T, np.dot(
                            np.linalg.pinv(np.dot(B, B.T)), np.dot(B, y)))
    ...
    for k in range(last, 0, -1):
        if k == last:
            ...
            if len(itm):
                ...
                cf[itm] = np.where(
                      extrcontval(x, y, degree) > 0.0, cf[itm], intrinsic[itm])
