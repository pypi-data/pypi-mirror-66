...
def dispatch_sparql_query(raw_sparql_query, request_args, endpoint, return_format, auth={}):
    ...
    if "page" in request_args and "off_set" in request_args:
        rewritten_query.replace("LIMIT 100", "LIMIT {}".format(request_args["per_page"]))
        rewritten_query.replace("OFFSET 0", "OFFSET {}".format(request_args["off_set"]))
