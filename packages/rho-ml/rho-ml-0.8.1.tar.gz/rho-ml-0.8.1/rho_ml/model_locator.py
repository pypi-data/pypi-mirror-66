...
def find_highest_compatible_version(
    ...
    """ Return the model locator or version of the highest compatible version.
        ...
        the full locator of the highest version that matches `serch_version`.
        ...
        must still adhere to some baked in assumptions:
            1. The model name pattern must have a single capture group OR be
               None, which will result in this assuming it's only searching
               against a version pattern.
            2. The version pattern needs to follow semver, which is what the
    ...
    def _run_search(search_list, v_index, version_elements):
    ...
    if any(v is None for v in version_elements):
        logger.warning("Unable to find compatible version for {0} from: {1}"
                       .format(search_version, search_list))
        return None
    ...
    """ Return the model locators with model names that match the search_pattern
        ...
        Args:
            ...
            delimiter_pattern (r string): Regex pattern describing delimiter
                Not recommended to modify this pattern.
        Returns:
            List[str]: List of model locators that match the search.
                e.g. if search pattern is "*Model" & available keys includes
                ['my_name_0.1.1', 'my_name_1.0.0', 'MyModel_0.1.5']
                this will return ['MyModel_0.1.5']
    """
...
class Version(object):
    ...
    def _get_new_version(version: 'Version', to_increment: str):
        ...
        if to_increment == 'major':
            ...
            minor = 0
            patch = 0
        if to_increment == 'minor':
            major = version.major
            minor = version.minor + 1
            patch = 0
        if to_increment == 'patch':
            major = version.major
            minor = version.minor
    ...
    @classmethod
    def increment_version(cls, current_version: 'Version',
    ...
    def from_string(cls, val: str):
        """ Create a Version object from a string of the form
            ...
            semver has specific guidelines around usage of "+build" and
            "-prerelease" but this level of granularity is not currently
            provided in Rho ML. It is
                major.minor.patch(optional additional of your choosing)
            where (optional additional) must start with one of the valid
            separators listed above.
        """
