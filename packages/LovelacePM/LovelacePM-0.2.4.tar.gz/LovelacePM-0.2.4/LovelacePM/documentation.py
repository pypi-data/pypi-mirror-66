...
paneller.Solid.__init__.__doc__='''__init__(sldlist=[], wraparounds=[], full_parallel=False): instantiate solid, adding the first patches as their matrixes of point arrays 
...
If flag full_parallel is selected, parallel execution of aicm calculations will be held so as to stop recursive multiprocessing on Windows. This is necessary when parallelizing the whole analysis 
(not just AIC matrix calculation which is parallelized by default)
so as not to generate child processes recursively'''
...
paneller.Solid.eulersolve.__doc__='''eulersolve(self, target=np.array([]), Uinf=1.0, M=0.0, gamma=1.4, beta=1.0, a=0.0, b=0.0, p=0.0, q=0.0, r=0.0, damper=0.0, echo=True, \
        wakeiter=0, tolerance=1e-5): computes complete euler solution through calls for 
...
M and gamma used for Cp compressibility corrections'''
...
a, b, p, q, r, Uinf, M: freestream parameters
rho, mu, gamma, g: environment parameters
m, Ixx, Iyy, Izz, Ixy, Ixz, Iyz: mass parameters
...
executes dynamic simulation according to time step dt with nstep finite difference time steps. Perturbations is a dictionary which should include disturbances between phi, theta, psi, u, v, w, p, q, r. Angles 
are all accounted in radians. Returns external_history, alpha_history, beta_history, euler_history, time_history: external history is a shape (nstep, 3) array containing evolution of aircraft position
