...
class _PumpTurbine(_Link):
    ...
    max_flow: Optional[float] = Field(
        None, ge=0.0, description="Maximum volumetric flow rate in m^3/s"
    )
    ...
    max_power: Optional[float] = Field(None, ge=0.0, description="Maximum power in W")
...
class Valve(_Link):
    ...
    coefficient: float = Field(..., ge=0.0, description="Discharge coefficient")
...
class Orifice(_Link):
    ...
    coefficient: float = Field(..., gt=0.0, description="Discharge coefficient")
    min_aperture: float = Field(..., ge=0.0, description="Minimum aperture")
    max_aperture: float = Field(..., ge=0.0, description="Maximum aperture")
