...
def get_host(rpath):
    return f"http://localhost:8000/{rpath}"
...
def sync(username, password):
    ...
    if ptype == "retask":
        rurl = get_host("syncreproject")
    elif ptype == "sptask":
        rurl = get_host("syncspproject")
    else:
...
def create_project(username, password, create_project_type, package_name):
    """
    @des: 创建工程模板
    """
    # 判断工程是否存在
    ...
    if create_project_type == "retask":
        rurl = get_host("create_re_project")
    elif create_project_type == "sptask":
        rurl = get_host("create_sp_project")
    else:
...
def main(*args, **kwargs):
    ...
    -u 用户名
    -p 密码
    -m 有该参数则是创建sptask，否则都是retask
    example:
    创建工程
    tfduck -uyx -p1 create helloworld
    同步当前工程
    tfduck -uyx -p1 sync
    """
    # print(1111)
    ...
    # print(kwargs)
    #
    opts, args = getopt.getopt(args, "hvu:p:d:m", ["sync", "create=", "help"])
    print(opts, args)
    try:
        username = None
        password = None
        ...
        if is_create:
            create_project(username, password, create_project_type, arg_value)
        if is_sync:
