...
class ExternalTaskClient(BaseClient):
    ...
    def subscribe_to_external_task_for_topic(self, topic, handler):
        if topic in self._topic_for_handler:
            ...
            self.__start_subscription(topic, handler)

    def __start_subscription(self, topic, handler):
        async def bg_job_handle_external_tasks():
            try:
                await self.handle_external_tasks(topic, handler)
    ...
    def subscribe_to_external_tasks_for_topics(self, topics_for_handlers):
        for topic in topics_for_handlers.keys():
            handler = topics_for_handlers[topic]
    ...
    async def handle_external_tasks(self, topic, handler):
        external_tasks = await self.__fetch_and_lock(topic, handler)
    ...
    async def __fetch_and_lock(self, topic, handler, options={}):
