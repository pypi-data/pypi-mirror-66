    ...
    def init_common(self, molecules, numMols, volume_factor, allow_inversion, orientations, check_atomic_distances, group, lattice, tm):
        ...
        for i, mol in enumerate(molecules):
            ...
            if type(mol) == str:
                ...
                tmp = mol.split('.')
                if len(tmp) > 1:
                    printx('Load the molecule from the given file {:}'.format(mol))
                    if tmp[-1] in ['xyz', 'gjf', 'g03', 'json']:
                        mo = mol_from_file(mol)
                    else:
                        raise NameError('{:s} is not a supported format'.format(tmp[-1]))
                else:
                    printx('Load the molecule from the built_in collections')
                ...
                if mo is not None:
                    ...
                    raise NameError("Error: Could not create molecules from given parameters.\n"
