...
class Plugin(object):
    def __init__(self, path_to_def_file, path_def_file_id, data_source, name, user_function=None, volume_path=None):
        '''
        ...
        if self._code_type == CodeType.BINOUT:
            ...
            try:
                madymo_file = h5py.File(data_source, "r")
                DataContainer.init_data_source_nodout(madymo_file, self._logger, self._dynasaur_definitions)
            except:
    ...
    def _get_data_from_dynasaur_json(self, json_object, data_offsets):
        ...
        if JsonConstants.FUNCTION in json_object.keys():  # expected name and params
            ...
            for key in parameter_def.keys():
                if type(parameter_def[key]) is dict:  # step into recursion
                    ...
                    if (key == "function_name"):
                        param_dict[key] = {"name": parameter_def[key]}
                        continue
            ...
            if function_name == StandardFunctionsDefinition.HIC15:
                ...
                return StandardFunction.SIGMOID_RISK_1(param_dict)
...
class PluginInterface(metaclass=abc.ABCMeta):
    ...
    def run_calculation(self, param_dict):
