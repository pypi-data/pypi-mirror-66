...
class StandardFunction:
    ...
    def minimum(param_dict):
        ...
        index_of_minimum = np.argmin(param_dict['data_vector'])
    ...
    def SIGMOID_RISK_1(param_dict):
        ...
        power = param_dict["power"]
        risk = 1 / (1 + np.exp(offset - scaling * ((np.abs(injuryvalue[0])) ** power)))
    ...
    def object_strain_stress_percentile(param_dict):
        ...
        elements_greater_limit = {}
        percentile_values = []
        for i in time_step_indices:
            elements_greater_limit[i] = []
        ...
        for part_id in part_ids:
            ...
            for element_id in element_ids:
                ...
                for time_step_id in np.where(result_data >= param_dict[PluginsParamDictDef.LIMIT])[0]:
                    ...
                    try:
                        elements_greater_limit[time_step_id].append(element_id)
                    except:
                        continue
