        ...
        for sec in dyn_def_key:
            ...
            for id_for_loop in dyn_def_key[sec]:
                if binout_name == TestConstants.CONTACT:
                    master_slave = id_for_loop[-1]
                    id_for_loop = int(id_for_loop[:-1])

                    if id_for_loop in ids_list:
                        temp_list.append(str(id_for_loop) + master_slave)
                elif id_for_loop in ids_list:
                    temp_list.append(id_for_loop)
                else:
                    continue
            if len(temp_list) == 0:
                return False, sec
            dyn_def_key[sec] = temp_list.copy()
        return True, "temp"
