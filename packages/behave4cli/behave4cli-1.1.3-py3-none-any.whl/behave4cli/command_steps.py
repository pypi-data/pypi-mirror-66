...
from behave4cli import command_shell, command_util, pathutil, textutil
from behave4cli.command_shell_proc import BehaveWinCommandOutputProcessor, TextProcessor
...
def step_a_file_named_filename_and_encoding_with(context, filename, encoding):
    ...
    try:
        if context.surrogate_text:
            text_to_use = context.surrogate_text
            context.surrogate_text = None
    except AttributeError:
        pass

    assert text_to_use is not None, "ENSURE: multiline text is provided."
...
def step_it_should_pass(context):
    assert_that(
        ...
        equal_to(0),
        "Expected to pass but failed with content:\n" + context.command_result.output,
    )
...
def step_it_should_fail(context):
    assert_that(
        context.command_result.returncode,
        is_not(equal_to(0)),
        "Expected to fail but passed with content:\n" + context.command_result.output,
    )
...
def step_command_output_should_contain_text(context, text):
    ...
    EXAMPLE:
        ...
        Then the command output should contain "TEXT"
    """
    expected_text = command_util.template_substitute_builtin_expressions(context, text)
...
@then('the file named "{filename}" should exist')
...
@then('the file named "{filename}" should not exist')
