...
from typing import TYPE_CHECKING, Awaitable, Callable, List, Optional, Set, Union

import discord
from fuzzywuzzy import fuzz, process

from kronbot.core import data_manager
from kronbot.core.utils.chat_formatting import box

if TYPE_CHECKING:
    from kronbot.core.bot import Kron
    from kronbot.core.commands import Command, Context

main_log = logging.getLogger("kronbot")
...
async def fuzzy_command_search(
    ...
    *,
    ...
    Parameters
    ----------
    ctx : `commands.Context <kronbot.core.commands.Context>`
    ...
    Returns
    -------
    Optional[List[`commands.Command <kronbot.core.commands.Command>`]]
...
async def format_fuzzy_results(
    ...
    matched_commands : List[`commands.Command <kronbot.core.commands.Command>`]
...
async def send_to_owners_with_preprocessor(
    ...
    *,
    content_preprocessor: Optional[
        Callable[[Kron, discord.abc.Messageable, str], Awaitable[str]]
    ] = None,
    **kwargs,
):
    """
    ...
    This acts the same as `Kron.send_to_owners`, with
    one added keyword argument as detailed below in *Other Parameters*.

    Other Parameters
    ----------------
    content_preprocessor: Optional[Callable[[Kron, discord.abc.Messageable, str], Awaitable[str]]]
...
async def send_to_owners_with_prefix_replaced(bot: Kron, content: str, **kwargs):
    """
    This sends something to all owners and their configured extra destinations.

    This acts the same as `Kron.send_to_owners`, with one addition - `[p]` in ``content`` argument
    is replaced with a clean prefix for each specific destination.
    """

    async def preprocessor(bot: Kron, destination: discord.abc.Messageable, content: str) -> str:
