...
from kronbot.core import Config, checks, commands
...
from kronbot.core.utils.chat_formatting import box, humanize_list, inline, pagify
from kronbot.core.utils.menus import start_adding_reactions
...
DEPRECATION_NOTICE = _(
    ...
    " which are marked for removal in Kron 3.4: {repo_list}.\n"
...
class Downloader(commands.Cog):
    ...
    async def _cog_reinstallreqs(self, ctx: commands.Context) -> None:
        ...
        Kron might ask user to use this when it clears contents of lib folder
    ...
    def _filter_incorrect_cogs(
        ...
        for cog in cogs:
            ...
            if (
                cog.min_bot_version > kron_version_info
                or not ignore_max
                and cog.max_bot_version < kron_version_info
            ):
        ...
        if outdated_bot_version:
            message += _(
                "\nThese cogs require different Kron version"
                " than you currently have ({current_version}): "
            ).format(current_version=kron_version_info) + humanize_list(outdated_bot_version)
    ...
    def format_findcog_info(
        ...
        if isinstance(cog_installable, Installable):
            ...
            made_by = "26 & co."
            repo_url = "https://github.com/kablekompany/Kr0nOs"
