...
from kronbot.cogs.downloader.installable import Installable, InstalledModule
from kronbot.cogs.downloader.repo_manager import ProcessFormatter, Repo, RepoManager
...
async def _session_git_repo(tmp_path_factory, event_loop):
    ...
    repo = Repo(name="kronbot-testrepo", url="", branch="master", commit="", folder_path=repo_path)
...
async def git_repo(_session_git_repo, tmp_path, event_loop):
    ...
    repo_path = tmp_path / "kronbot-testrepo"
    shutil.copytree(_session_git_repo.folder_path, repo_path)
    repo = Repo(
        name="kronbot-testrepo",
...
async def cloned_git_repo(_session_git_repo, tmp_path, event_loop):
    ...
    repo_path = tmp_path / "kronbot-cloned_testrepo"
    repo = Repo(
        name="kronbot-testrepo",
...
async def git_repo_with_remote(git_repo, tmp_path, event_loop):
    ...
    repo_path = tmp_path / "kronbot-testrepo_with_remote"
    repo = Repo(
        name="kronbot-testrepo",
