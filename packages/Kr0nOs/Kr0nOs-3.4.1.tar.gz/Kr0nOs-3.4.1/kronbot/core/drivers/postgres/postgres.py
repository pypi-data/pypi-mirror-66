...
class PostgresDriver(BaseDriver):
    ...
    async def initialize(cls, **storage_details) -> None:
        if asyncpg is None:
            raise errors.MissingExtraRequirements(
                "Kron must be installed with the [postgres] extra to use the PostgreSQL driver"
            )
    ...
    def get_config_details():
        ...
        host = (
            input(
                ...
                f"If left blank, Kron will try the following, in order:\n"
        ...
        user = (
            input(
                ...
                " - The OS name of the user running Kron (ident/peer authentication).\n"
                "> "
            )
            or None
        )
        ...
        database = (
            input(
                ...
                " - The OS name of the user running Kron.\n"
                "> "
            )
            or None
        )
    ...
    async def set(self, identifier_data: IdentifierData, value=None):
        try:
            await self._execute(
                "SELECT kron_config.set($1, $2::jsonb)",
                encode_identifier_data(identifier_data),
                json.dumps(value),
            )
    ...
    async def clear(self, identifier_data: IdentifierData):
        try:
            await self._execute(
                "SELECT kron_config.clear($1)", encode_identifier_data(identifier_data)
            )
        except asyncpg.UndefinedTableError:
            pass
    ...
    async def inc(
        ...
        try:
            return await self._execute(
                f"SELECT kron_config.inc($1, $2, $3)",
                encode_identifier_data(identifier_data),
                value,
                default,
                method=self._pool.fetchval,
    ...
    async def aiter_cogs(cls) -> AsyncIterator[Tuple[str, str]]:
        query = "SELECT cog_name, cog_id FROM kron_config.red_cogs"
        log.invisible(query)
    ...
    async def delete_all_data(
        ...
        if interactive is True and drop_db is None:
            print(
                ...
                " 2. Delete all of Kron's data within this database, without dropping the database "
                "itself."
            )
            options = ("1", "2")
            while True:
