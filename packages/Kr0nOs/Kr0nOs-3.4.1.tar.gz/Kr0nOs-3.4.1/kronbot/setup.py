...
from kronbot.core import config, data_manager, drivers
from kronbot.core.drivers import BackendType, IdentifierData
from kronbot.core.utils._internal_utils import create_backup as kron_create_backup
from kronbot.core.utils._internal_utils import safe_delete

conversion_log = logging.getLogger("kronbot.converter")

config_dir = None
appdir = appdirs.AppDirs("Kr0nOs")
...
def basic_setup():
    ...
    print()
    print(
        "Your basic configuration has been saved. Please run `kronbot <name>` to"
...
async def create_backup(instance: str, destination_folder: Path = Path.home()) -> None:
    ...
    backup_fpath = await kron_create_backup(destination_folder)
...
def cli(ctx, debug):
    ...
    base_logger = logging.getLogger("kronbot")
