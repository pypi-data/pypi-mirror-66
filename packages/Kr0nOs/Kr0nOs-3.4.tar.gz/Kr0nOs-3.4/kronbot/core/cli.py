...
async def interactive_config(kron, token_set, prefix_set, *, print_header=True):
    token = None

    if print_header:
        print("Kron - Discord Bot | Configuration process\n")
        ...
        while not prefix:
            ...
            if len(prefix) > 10:
                if not confirm("Your prefix seems overly long. Are you sure that it's correct?"):
                    prefix = ""
            if prefix:
                await kron._config.prefix.set([prefix])
...
def parse_cli_flags(args):
    parser = argparse.ArgumentParser(
        description="Kron - Discord Bot", usage="kronbot <instance_name> [arguments]"
    )
    parser.add_argument("--version", "-V", action="store_true", help="Show Kron's current version")
    parser.add_argument("--debuginfo", action="store_true", help="Show debug information.")
    parser.add_argument(
        "--list-instances",
        action="store_true",
        help="List all instance names setup with 'kronbot-setup'",
    )
    parser.add_argument(
        "--edit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "to the system that is hosting Kron should be  "
    ...
    parser.add_argument(
        "--no-cogs", action="store_true", help="Starts Kron with no cogs loaded, only core"
    )
    parser.add_argument(
        "--load-cogs",
        type=str,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Makes Kron quit with code 0 just before the "
    ...
    parser.add_argument("--token", type=str, help="Run Kron with the given token.")
    parser.add_argument(
        "--no-instance",
        action="store_true",
        help=(
            "Run Kron without any existing instance. "
            "The data will be saved under a temporary folder "
            "and deleted on next system restart."
        ),
    )
    parser.add_argument(
        "instance_name", nargs="?", help="Name of the bot instance created during `kronbot-setup`."
    )
    parser.add_argument(
