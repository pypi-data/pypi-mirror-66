...
from kronbot.core.utils.chat_formatting import (
    bold,
    escape,
    humanize_number,
    humanize_timedelta,
...
class General(commands.Cog):
    ...
    async def rps(self, ctx, your_choice: RPSParser):
        ...
        kron_choice = choice((RPS.rock, RPS.paper, RPS.scissors))
        cond = {
        ...
        if kron_choice == player_choice:
            outcome = None  # Tie
        else:
            outcome = cond[(player_choice, kron_choice)]

        if outcome is True:
            await ctx.send(
                _("{choice} You win {author.mention}!").format(
                    choice=kron_choice.value, author=author
                )
            )
