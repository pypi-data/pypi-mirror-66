...
if _sys.version_info < MIN_PYTHON_VERSION and not _os.getenv("READTHEDOCS", False):
    print(
        f"Python {'.'.join(map(str, MIN_PYTHON_VERSION))} is required to run Kron, but you have "
...
if "--debug" not in _sys.argv:
    # DEP-WARN
    # Individual warnings - tracked in https://github.com/kablekompany/Kr0nOs/issues/3529
