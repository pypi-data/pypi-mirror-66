...
from kronbot.core import config as config_module
from kronbot.core import drivers
...
@pytest.fixture()
def ctx(empty_member, empty_channel, kron):
    mock_ctx = namedtuple("Context", "author guild channel message bot")
    return mock_ctx(empty_member, empty_member.guild, empty_channel, empty_message, kron)
...
@pytest.fixture()
def kron(config_fr):
    from kronbot.core.cli import parse_cli_flags

    cli_flags = parse_cli_flags(["ignore_me"])

    description = "Kron v3 - Alpha"

    Config.get_core_conf = lambda *args, **kwargs: config_fr

    kron = Kron(cli_flags=cli_flags, description=description, dm_help=None, owner_id=None)
