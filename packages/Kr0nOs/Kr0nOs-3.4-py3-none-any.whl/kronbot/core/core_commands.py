...
from . import VersionInfo, __version__, checks, commands, config, drivers, errors, i18n
...
class CoreLogic:
    ...
    async def _version_info(cls) -> Dict[str, str]:
        """
        Version information for Kron and discord.py

        Returns
        -------
        dict
            `kronbot` and `discordpy` keys containing version information for both.
        """
        return {"kronbot": __version__, "discordpy": discord.__version__}
...
class Core(commands.Cog, CoreLogic):
    ...
    async def info(self, ctx: commands.Context):
        """Shows info about Kron"""
        author_repo = "https://github.com/Twentysix26"
        org_repo = "https://github.com/Cog-Creators"
        kron_repo = org_repo + "/Kr0nOs"
        kron_pypi = "https://pypi.org/project/Red-DiscordBot"
        kron_pypi_json = "https://pypi.org/pypi/Red-DiscordBot/json"
        ...
        kron_version = "[{}]({})".format(__version__, kron_pypi)
        app_info = await self.bot.application_info()
        if app_info.team:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(kron_pypi_json) as r:
                    data = await r.json()
        except (aiohttp.ClientError, asyncio.TimeoutError):
            outdated = None
        else:
            outdated = VersionInfo.from_str(data["info"]["version"]) > kron_version_info
        about = _(
            "This bot is an instance of [Kron, an open source Discord bot]({}) "
            "created by [Twentysix]({}) and [improved by many]({}).\n\n"
            "Kron is backed by a passionate community who contributes and "
            "creates content for everyone to enjoy. [Join us today]({}) "
            "and help us improve!\n\n"
            "(c) Cog Creators"
        ).format(kron_repo, author_repo, org_repo, support_server_url)

        embed = discord.Embed(color=(await ctx.embed_colour()))
        embed.add_field(name=_("Instance owned by"), value=str(owner))
        embed.add_field(name="Python", value=python_version)
        embed.add_field(name="discord.py", value=dpy_version)
        embed.add_field(name=_("Kron version"), value=kron_version)
        ...
        embed.add_field(name=_("About Kron"), value=about, inline=False)
    ...
    async def _restart(self, ctx: commands.Context, silently: bool = False):
        """Attempts to restart Kron

        Makes Kron quit with exit code 26
    ...
    async def setdescription(self, ctx: commands.Context, *, description: str = ""):
        ...
        The default is "Kron V3"
        """
        if not description:
            await ctx.bot._config.description.clear()
            ctx.bot.description = "Kron V3"
    ...
    async def locale(self, ctx: commands.Context, language_code: str):
        ...
        Go to Kron's Crowdin page to see locales that are available with translations:
    ...
    async def datapath(self, ctx: commands.Context):
        ...
        from kronbot.core.data_manager import basic_config
    ...
    async def debuginfo(self, ctx: commands.Context):
        ...
        if await ctx.embed_requested():
            ...
            e.title = "Debug Info for Kron"
            e.add_field(name="Kron version", value=kronver, inline=True)
...
async def license_info_command(ctx):
    """
    Get info about Kron's licenses
    """

    message = (
        "This bot is an instance of Kr0nOs (hereafter referred to as Kron)\n"
        "Kron is a free and open source application made available to the public and "
        "licensed under the GNU GPLv3. The full text of this license is available to you at "
        "<https://github.com/kablekompany/Kr0nOs/blob/master/LICENSE>"
    )
