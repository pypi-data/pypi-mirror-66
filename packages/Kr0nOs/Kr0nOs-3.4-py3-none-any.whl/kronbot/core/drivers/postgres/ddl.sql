...
CREATE SCHEMA IF NOT EXISTS kron_config;
CREATE SCHEMA IF NOT EXISTS kron_utils;

DO $$
BEGIN
  PERFORM 'kron_config.identifier_data'::regtype;
EXCEPTION
  WHEN UNDEFINED_OBJECT THEN
    CREATE TYPE kron_config.identifier_data AS (
      cog_name text,
      cog_id text,
      category text,
      pkeys text[],
...
CREATE OR REPLACE FUNCTION
   ...
   */
  kron_config.maybe_create_table(
    id_data kron_config.identifier_data
  )
    RETURNS void
    LANGUAGE 'plpgsql'
  AS $$
  ...
  BEGIN
    IF NOT schema_exists THEN
      PERFORM kron_config.create_schema(id_data.cog_name, id_data.cog_id);
    END IF;
    IF NOT table_exists THEN
      PERFORM kron_config.create_table(id_data);
    END IF;
  END;
$$;
...
CREATE OR REPLACE FUNCTION
   ...
   */
  kron_config.create_schema(new_cog_name text, new_cog_id text, OUT schemaname text)
    RETURNS text
    LANGUAGE 'plpgsql'
  AS $$
  BEGIN
    ...
    INSERT INTO kron_config.red_cogs AS t VALUES(new_cog_name, new_cog_id, schemaname)
...
CREATE OR REPLACE FUNCTION
   ...
   */
  kron_config.create_table(id_data kron_config.identifier_data)
    RETURNS void
    LANGUAGE 'plpgsql'
  AS $$
  DECLARE
    ...
    pkey_columns CONSTANT text := kron_utils.gen_pkey_columns(1, id_data.pkey_len);
    pkey_type CONSTANT text := kron_utils.get_pkey_type(id_data.is_custom);
    pkey_column_definitions CONSTANT text := kron_utils.gen_pkey_column_definitions(
...
CREATE OR REPLACE FUNCTION
  ...
  DECLARE
    ...
    pkey_type CONSTANT text := kron_utils.get_pkey_type(id_data.is_custom);
    whereclause CONSTANT text := kron_utils.gen_whereclause(num_pkeys, pkey_type);
  ...
  BEGIN
    ...
    ELSIF num_missing_pkeys = 1 THEN
      ...
      USING id_data.pkeys;
    ELSE
      -- Multiple missing primary keys: we must use our custom kron_utils.jsonb_object_agg2()
      -- aggregate function.
      missing_pkey_columns := kron_utils.gen_pkey_columns_casted(num_pkeys + 1, id_data.pkey_len);

      EXECUTE format(
        'SELECT kron_utils.jsonb_object_agg2(json_data, %s) FROM %I.%I WHERE %s',
...
CREATE OR REPLACE FUNCTION
  ...
  DECLARE
    ...
    num_missing_pkeys CONSTANT integer := id_data.pkey_len - num_pkeys;
    pkey_type CONSTANT text := kron_utils.get_pkey_type(id_data.is_custom);
    pkey_placeholders CONSTANT text := kron_utils.gen_pkey_placeholders(num_pkeys, pkey_type);
  ...
  BEGIN
    PERFORM kron_config.maybe_create_table(id_data);

    IF num_missing_pkeys = 0 THEN
      -- Setting all or part of a document
      new_document := kron_utils.jsonb_set2('{}', new_value, VARIADIC id_data.identifiers);
      ...
      EXECUTE format(
        ...
        SET
          json_data = kron_utils.jsonb_set2(t.json_data, $3, VARIADIC $4)
        $query$,
        schemaname,
        id_data.category,
        pkey_placeholders,
    ...
    ELSE
      ...
      whereclause := kron_utils.gen_whereclause(num_pkeys, pkey_type);
      missing_pkey_columns := kron_utils.gen_pkey_columns_casted(
        num_pkeys + 1, id_data.pkey_len, pkey_type);
      pkey_column_definitions := kron_utils.gen_pkey_column_definitions(num_pkeys + 1, id_data.pkey_len);
      ...
      EXECUTE format(
        ...
        INSERT INTO %I.%I AS t
          SELECT %s, json_data
          FROM kron_utils.generate_rows_from_object($2, $3) AS f(%s, json_data jsonb)
        ON CONFLICT ON CONSTRAINT %I DO UPDATE
        SET
...
CREATE OR REPLACE FUNCTION
  ...
  BEGIN
    ...
    ELSE
      ...
      DELETE FROM kron_config.red_cogs
...
CREATE OR REPLACE FUNCTION
  ...
  DECLARE
    ...
    num_identifiers CONSTANT integer := coalesce(array_length(id_data.identifiers, 1), 0);
    pkey_type CONSTANT text := kron_utils.get_pkey_type(id_data.is_custom);
    whereclause CONSTANT text := kron_utils.gen_whereclause(id_data.pkey_len, pkey_type);
  ...
  BEGIN
    ...
    IF existing_document IS NULL THEN
      ...
      new_document := kron_utils.jsonb_set2('{}', result, VARIADIC id_data.identifiers);
      pkey_placeholders := kron_utils.gen_pkey_placeholders(id_data.pkey_len, pkey_type);
    ...
    ELSE
      ...
      new_document := kron_utils.jsonb_set2(
...
CREATE OR REPLACE FUNCTION
  ...
  BEGIN
    ...
    IF existing_document IS NULL THEN
      ...
      new_document := kron_utils.jsonb_set2('{}'::jsonb, result, id_data.identifiers);
      pkey_placeholders := kron_utils.gen_pkey_placeholders(id_data.pkey_len, pkey_type);
    ...
    ELSE
      ...
      new_document := kron_utils.jsonb_set2(existing_document, result, id_data.identifiers);
...
CREATE OR REPLACE FUNCTION
  /*
   * Delete all schemas listed in the kron_config.red_cogs table.
   */
  kron_config.delete_all_schemas()
    RETURNS void
    LANGUAGE 'plpgsql'
  AS $$
  DECLARE
    cog_entry record;
  BEGIN
    FOR cog_entry IN SELECT * FROM kron_config.red_cogs t LOOP
      EXECUTE format('DROP SCHEMA %I CASCADE', cog_entry.schemaname);
    END LOOP;
    -- Clear out kron_config.red_cogs table
    DELETE FROM kron_config.red_cogs WHERE TRUE;
  END;
$$;
...
CREATE OR REPLACE FUNCTION
   ...
   * of a non-document type.
   */
  kron_utils.jsonb_set2(target jsonb, new_value jsonb, VARIADIC identifiers text[])
    RETURNS jsonb
    LANGUAGE 'plpgsql'
    IMMUTABLE
    PARALLEL SAFE
...
CREATE OR REPLACE FUNCTION
   ...
   */
  kron_utils.generate_rows_from_object(object jsonb, num_missing_pkeys integer)
  ...
  BEGIN
    ...
    ELSE
      ...
      column_definitions := kron_utils.gen_pkey_column_definitions(2, num_missing_pkeys);
      ...
      FOR pair IN SELECT * FROM jsonb_each(object) LOOP
        RETURN QUERY
          EXECUTE format(
            $query$
            SELECT $1 AS key_1, *
            FROM kron_utils.generate_rows_from_object($2, $3)
...
CREATE OR REPLACE FUNCTION
   ...
   */
  kron_utils.gen_pkey_placeholders(num_pkeys integer, pkey_type text DEFAULT 'text')
    RETURNS text
    LANGUAGE 'sql'
    IMMUTABLE
    PARALLEL SAFE
...
CREATE OR REPLACE FUNCTION
   ...
   */
  kron_utils.gen_whereclause(num_pkeys integer, pkey_type text)
    RETURNS text
    LANGUAGE 'sql'
    IMMUTABLE
    PARALLEL SAFE
...
CREATE OR REPLACE FUNCTION
   ...
   */
  kron_utils.gen_pkey_columns(start integer, stop integer)
    RETURNS text
    LANGUAGE 'sql'
    IMMUTABLE
    PARALLEL SAFE
...
CREATE OR REPLACE FUNCTION
   ...
   * to the given type.
   */
  kron_utils.gen_pkey_columns_casted(start integer, stop integer, pkey_type text DEFAULT 'text')
    RETURNS text
    LANGUAGE 'sql'
    IMMUTABLE
    PARALLEL SAFE
...
CREATE OR REPLACE FUNCTION
   ...
   */
  kron_utils.gen_pkey_column_definitions(
...
CREATE OR REPLACE FUNCTION
  kron_utils.get_pkey_type(is_custom boolean)
    RETURNS TEXT
    LANGUAGE 'sql'
    IMMUTABLE
    PARALLEL SAFE
...
DROP AGGREGATE IF EXISTS kron_utils.jsonb_object_agg2(jsonb, VARIADIC text[]);
CREATE AGGREGATE
  /*
   ...
   * reasons.
   */
  kron_utils.jsonb_object_agg2(json_data jsonb, VARIADIC primary_keys text[]) (
    SFUNC = kron_utils.jsonb_set2,
    STYPE = jsonb,
    INITCOND = '{}',
    PARALLEL = SAFE
  )
