...
from typing import List, Mapping, MutableMapping, Optional, Tuple, Union, cast
...
import lavalink
from kronbot.core import Config, bank, checks, commands
...
from kronbot.core.utils.chat_formatting import bold, box, humanize_number, inline, pagify
from kronbot.core.utils.menus import (
    DEFAULT_CONTROLS,
    close_menu,
    menu,
    next_page,
...
from kronbot.core.utils.predicates import MessagePredicate, ReactionPredicate
...
log = logging.getLogger("kron.audio")
...
class Audio(commands.Cog):
    ...
    async def _get_correct_playlist_id(
        ...
        if lazy_match or (
            ...
            if specified_user:
                correct_scope_matches_global = [
                    p for p in matches.get(PlaylistScope.USER.value) if p.author == user_to_query
                ]
            else:
    ...
    async def _playlist_copy(
        ...
        **Guild** can be one of the following:
        ...
        ​ ​ ​ ​ `[p]playlist copy MyPersonalPlaylist --from-scope user --to-author Draper#6666 --to-scope Guild --to-guild Kron - Discord Bot`
        """
    ...
    async def _playlist_download(
        ...
        *,
        ...
        Kron v2-compatible playlists can be generated by passing True
        for the v2 variable.
    ...
    async def _playlist_upload(self, ctx: commands.Context, *, scope_data: ScopeParser = None):
        ...
        **Guild** can be one of the following:
        ...
        if file_suffix != "txt":
            return await self._embed_msg(ctx, title=_("Only Kron playlist files can be uploaded."))
        try:
    ...
    async def sing(self, ctx: commands.Context):
        """Make Kron sing one of her songs."""
        ids = (
            "zGTkAVsrfg8",
            "cGMWL8cOeAU",
            "vFrjMq4aL-g",
    ...
    async def on_kron_audio_track_start(
    ...
    @commands.Cog.listener()
    async def on_kron_audio_queue_end(
        self, guild: discord.Guild, track: lavalink.Track, requester: discord.Member
    ):
