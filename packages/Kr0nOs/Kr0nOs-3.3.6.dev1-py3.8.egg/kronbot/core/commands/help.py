...
# This is not an unreasonable volume on some already existing Kron instances,
...
__all__ = ["kron_help", "KronHelpFormatter", "HelpSettings"]
...
class KronHelpFormatter:
    """
    Kron's help implementation
    ...
    async def help_filter_func(
        ...
        for obj in objects:
            if verify_checks and not show_hidden:
                # Default Kron behavior, can_see includes a can_run check.
...
async def kron_help(ctx: Context, *, thing_to_get_help_for: str = None):
    """
    I need somebody
