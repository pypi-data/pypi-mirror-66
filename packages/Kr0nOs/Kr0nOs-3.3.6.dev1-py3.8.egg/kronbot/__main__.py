...
import kronbot.logging

# Set the event loop policies here so any subsequent `new_event_loop()`
# calls, in particular those as a result of the following imports,
# return the correct loop object.
from kronbot import __version__, _update_event_loop_policy
from kronbot.core import data_manager, drivers
from kronbot.core._sharedlibdeprecation import SharedLibImportWarner
from kronbot.core.bot import ExitCodes, Kron
from kronbot.core.cli import confirm, interactive_config, parse_cli_flags
from kronbot.setup import get_data_dir, get_name, save_config

_update_event_loop_policy()


log = logging.getLogger("kronbot.main")
...
def list_instances():
    if not data_manager.config_file.exists():
        print(
            ...
            "using `kronbot-setup` before trying to run the bot!"
        )
        sys.exit(1)
    else:
...
async def edit_instance(kron, cli_flags):
    ...
    await _edit_token(kron, token, no_prompt)
    await _edit_prefix(kron, prefix, no_prompt)
    await _edit_owner(kron, owner, no_prompt)
...
async def _edit_token(kron, token, no_prompt):
    if token:
        if not len(token) >= 50:
            print(
            ...
            return
        await kron._config.token.set(token)
    elif not no_prompt and confirm("Would you like to change instance's token?", default=False):
        await interactive_config(kron, False, True, print_header=False)
        print("Token updated.\n")


async def _edit_prefix(kron, prefix, no_prompt):
    if prefix:
        prefixes = sorted(prefix, reverse=True)
        await kron._config.prefix.set(prefixes)
...
async def _edit_owner(kron, owner, no_prompt):
    if owner:
        ...
        if confirm("Are you sure you want to change instance's owner?", default=False):
            ...
            while True:
                ...
                await kron._config.owner.set(owner_id)
                print("Owner updated.")
                break
        else:
...
def handle_edit(cli_flags: Namespace):
    ...
    kron = Kron(
        cli_flags=cli_flags, description="Kron V3", dm_help=None, fetch_offline_members=True
    )
    try:
        driver_cls = drivers.get_driver_class()
        loop.run_until_complete(driver_cls.initialize(**data_manager.storage_details()))
        loop.run_until_complete(edit_instance(kron, cli_flags))
...
async def run_bot(kron: Kron, cli_flags: Namespace) -> None:
    """
    This runs the bot.
    ...
    if cli_flags.token:
        ...
        token = os.environ.get("KRON_TOKEN", None)
        if not token:
            token = await kron._config.token()

    prefix = cli_flags.prefix or await kron._config.prefix()

    if not (token and prefix):
        if cli_flags.no_prompt is False:
            new_token = await interactive_config(
                kron, token_set=bool(token), prefix_set=bool(prefix)
            )
            if new_token:
                token = new_token
        else:
    ...
    if cli_flags.dry_run:
        await kron.http.close()
        sys.exit(0)
    try:
        await kron.start(token, bot=True, cli_flags=cli_flags)
    except discord.LoginFailure:
        log.critical("This token doesn't seem to be valid.")
        db_token = await kron._config.token()
        if db_token and not cli_flags.no_prompt:
            if confirm("\nDo you want to reset the token?"):
                await kron._config.token.set("")
...
async def shutdown_handler(kron, signal_type=None, exit_code=None):
    if signal_type:
        ...
        kron._shutdown_mode = ExitCodes.CRITICAL

    if exit_code is not None:
        kron._shutdown_mode = exit_code
...
def global_exception_handler(kron, loop, context):
    """
...
def kron_exception_handler(kron, kron_task: asyncio.Future):
    """
    This is set as a done callback for Kron
    ...
    try:
        ...
        kron.loop.create_task(shutdown_handler(kron))


def main():
    kron = None  # Error handling for users misusing the bot
    ...
    try:
        ...
        if cli_flags.no_instance:
            ...
            cli_flags.instance_name = "temporary_kron"
        ...
        if os.name != "nt":
            ...
            for s in signals:
                loop.add_signal_handler(
                    s, lambda s=s: asyncio.create_task(shutdown_handler(kron, s))
                )

        exc_handler = functools.partial(global_exception_handler, kron)
        ...
        fut = loop.create_task(run_bot(kron, cli_flags))
        r_exc_handler = functools.partial(kron_exception_handler, kron)
        fut.add_done_callback(r_exc_handler)
        loop.run_forever()
    except KeyboardInterrupt:
        # We still have to catch this here too. (*joy*)
        log.warning("Please do not use Ctrl+C to Shutdown Kron! (attempting to die gracefully...)")
        log.error("Received KeyboardInterrupt, treating as interrupt")
        if kron is not None:
            loop.run_until_complete(shutdown_handler(kron, signal.SIGINT))
        ...
        if kron is not None:
            loop.run_until_complete(shutdown_handler(kron, None, exc.code))
    except Exception as exc:  # Non standard case.
        log.exception("Unexpected exception (%s): ", type(exc), exc_info=exc)
        if kron is not None:
            loop.run_until_complete(shutdown_handler(kron, None, ExitCodes.CRITICAL))
    finally:
        ...
        loop.stop()
        loop.close()
        exit_code = kron._shutdown_mode if kron is not None else 1
        sys.exit(exit_code)
