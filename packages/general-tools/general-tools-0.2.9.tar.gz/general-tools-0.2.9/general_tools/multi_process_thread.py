...
def multi_process_map(work_fun, itr_arg, pool_num=2, pool=None, if_return=False, if_close=True, if_wait=True):
    if pool == None:
        pool = Pool(pool_num)
...
# 多线程第一个线程结束则全部结束，并返回线程值
def multi_thread_first_return(work_fun, itr_arg, thread_num, if_return=False):
    executor = ThreadPoolExecutor(max_workers=thread_num)
    all_task = [executor.submit(work_fun, *args) for args in itr_arg]
