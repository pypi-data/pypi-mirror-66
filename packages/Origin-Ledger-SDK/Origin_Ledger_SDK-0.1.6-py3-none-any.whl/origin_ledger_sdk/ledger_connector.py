...
class Ledger():
    ...
    def get_batch_status(self, link: str) -> BatchStatus:
        response = requests.get(link)

        print("\nRESPONSE:", response.content, "\n\n")

        batch_status: BatchStatusResponse = batch_status_schema.loads(response.content)

        return batch_status.data[0].status
    ...
    def _send_batches(self, signed_batches) -> str:
        ...
        return handle_schema.loads(response.content).link
