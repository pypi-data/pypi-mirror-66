...
ReportRecord = collections.namedtuple('ReportRecord',
                                      ['result', 'message', 'filename', 'line', 'code', 'message_filename',
                                       'message_line'])
...
class Reporter(object):
    ...
    def report_records(self,
        ...
        if len(filtered_records) > max_records:
            ...
            _, check_file_path, check_code_hit_line_number, _, check_code_hit_line, _ = inspect.getouterframes(
                current_frame)[1]
    ...
    def __save_result_message(self,
                              result,
                              message,
                              frame,
    ...
    def warn(self, message, file_name=None, line_number=None):
        ...
        todo item
        """
        reporter_output = self.__format_message(message, message_file_name=file_name, message_line_number=line_number)
        self.__save_result_message('warning', reporter_output, inspect.currentframe(), message_file_name=file_name,
    ...
    def fail(self, message, file_name=None, line_number=None):
        ...
        """
        reporter_output = self.__format_message(message, message_file_name=file_name, message_line_number=line_number)
        self.__save_result_message('failure', reporter_output, inspect.currentframe(), message_file_name=file_name,
                                   message_line_number=line_number)
    ...
    def _ast_report(self, message, files_with_ast_results, report_function):
