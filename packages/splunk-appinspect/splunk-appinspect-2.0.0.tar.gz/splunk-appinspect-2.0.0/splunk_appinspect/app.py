...
class App(object):
    ...
    def __init__(self, location=None, package=None, python_analyzer_enable=True):
        ...
        self._trusted_libs_manager = TrustedLibsManager()
        ...
        if not platform.system() == "Windows":
            for directory, file, ext in self.iterate_files():
                ...
                try:
                    ...
                    logger.debug('Magic library reports error: {}'.format(str(e)))
                else:
        ...
        if self.python_analyzer_enable:
            try:
                ...
                self._python_analyzer_client = python_analyzer.client.Client(files_folder=self._app_temp_dir, modules_metadata=py_modules_metadata)
            except:
                logger.error("Folder %s is not found. App %s", self.package.working_app_path, self.name)
    ...
    def _get_app_info(self, stanza, option, app_conf_dir="default"):
        ...
        try:
            ...
            logger.error(logger_output)
            property_to_return = ("[MISSING `{}/app.conf` stanza `{}`]"
                                  ).format(app_conf_dir, stanza)
            raise exception
            ...
            logger.error(logger_output)
            property_to_return = ("[MISSING `{}/app.conf` stanza [{}]"
                                  " property `{}`]").format(app_conf_dir, stanza, option)
            raise exception
                ...
                if current_depth <= recurse_depth:
                    for file in files:
                        ...
                        if ((check_extensions and ext not in types) or
                            ...
                            if not self._filter_by_trusted_libs(dir_in_app, file, check_name):
    ...
    def _filter_by_trusted_libs(self, dir_in_app, file, check_name=None):
        ...
        try:
            ...
            logger.error("read file {} failed".format(filepath))
        return False