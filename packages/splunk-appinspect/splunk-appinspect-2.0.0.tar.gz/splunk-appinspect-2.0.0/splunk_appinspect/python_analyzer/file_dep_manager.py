...
class FileDepManager(object):
    ...
    def find_circular_dependency_loops(self):
        ...
        for component in self._circular_dependency:
            ...
            loops_collection.append(list(map(lambda tup: list(tup), loops)))
    ...
    def iter_all_app_python_files(self):
        for filepath in self._file_pool.keys():
            yield filepath
    ...
    def iter_non_circular_dependency_files(self):
        ...
        for _, file_node in iteritems(self._file_pool):
        ...
        while stack:
            ...
            for child_node in cur_file_node.iter_child():
                changeable_in_degree[child_node] -= 1
                if changeable_in_degree[child_node] == 0 and (child_node not in self._python_files_in_circular_dependency):
    ...
    def _populate_circular_dependency(self):
        ...
        for filepath, file_node in iteritems(self._file_pool):
    ...
    def add_file(self, filepath):
        ...
        try:
            ...
            if six.PY2:
                ...
                try:
                    ...
                    if not eval(os.getenv('APPINSPECT_DISABLE_2TO3_CONCURRENCY', 'False')) and multiprocessing.cpu_count() > 1:
                        exec_two_to_three(fixer_pkg="lib2to3.fixes", args=["-w", filepath, "--no-diffs", "-j", "2"])
                    else:
                        exec_two_to_three(fixer_pkg="lib2to3.fixes", args=["-w", filepath, "--no-diffs"])
        ...
        for _, _, _, pkg_path, line_number in deps:
            if not pkg_path \
                ...
                or (self.restriction_area and not pkg_path.startswith(self.restriction_area)):
                continue
    ...
    def add_folder(self, folder_path):
        ...
        for root, _, files in os.walk(folder_path, topdown=False,
                                      onerror=lambda error: logger.warning('Walk App folder failed , `{}`'.format(error.strerror))):
            for file_name in files:
                ...
                # temporary files, ignore them
                    ...
                    if (not platform.system() == "Windows" and magic.from_file(file_path, mime=True).lower() == 'text/x-python') or \
                            (platform.system() == "Windows" and
                             (not any(map(lambda ext: file_path.endswith(ext), window_filter_exts))) and
                             file_path.startswith(os.path.join(folder_path, 'bin'))):
                        try:
                            if six.PY2:
                                ast.parse(open(file_path, 'rU').read())
                            else:
                                ast.parse(open(file_path, mode='r', encoding="utf-8").read())
                            counter += 1
                            self.add_file(file_path)
                        except:
                            ...
                            if not six.PY2:
                                try:
                                    ...
                                    if not eval(os.getenv('APPINSPECT_DISABLE_2TO3_CONCURRENCY',
                                                      'False')) and multiprocessing.cpu_count() > 1:
                                        exec_two_to_three(fixer_pkg="lib2to3.fixes",
                                                        args=["-w", file_path, "--no-diffs", "-j", "2"])
                                    else:
                                        exec_two_to_three(fixer_pkg="lib2to3.fixes",
                                ...
                                # python2 parser is more tolerant, so we use hidden python file here
                                # template code will be covered in `check_for_hidden_python_files`
                                self._hidden_python_file.add(file_path)
    ...
    def _preprocess_file(file_path):
        ...
        shutil.copy(file_path, file_path + '.raw')
