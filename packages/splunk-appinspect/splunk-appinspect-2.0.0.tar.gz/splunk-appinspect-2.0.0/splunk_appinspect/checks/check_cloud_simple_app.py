...
def check_default_data_ui_manager_for_plain_text_credentials(app, reporter):
    ...
    if app.directory_exists("default", "data", "ui", "manager"):
        compiled_regex = re.compile(r"(pass|passwd|password|token|auth|priv|access|secret|login|community|key|privpass)\s*", re.IGNORECASE)
    ...
    if config_file_paths:
        for directory, filename in iteritems(config_file_paths):
            ...
            try:
                ...
                reporter_output = "authorize.conf is malformed. Details: {}".format(str(e))
...
def check_command_scripts_exist_for_cloud(app, reporter):
    ...
    if custom_commands.configuration_file_exists():
        ...
        for command in custom_commands.get_commands():
            ...
            is_filename_with_path_suffix = re.match(with_path_suffix_pattern, command.file_name)

            # can't find scripts in `bin/` or `<PLATFORM>/bin`
            if not is_filename_with_path_suffix and not command.file_name_exe:
                reporter_message = ("The script of command [{}] was not found "
...
def _check_v2_command(command, app, reporter, is_filename_with_path_suffix):
    ...
    count_v2_exes = command.count_win_exes() + \
    ...
    directly_executing_scripts = [script for script in command.executable_files
    ...
    if filename_is_specified and is_filename_with_path_suffix:
        ...
        reporter_message = ("The custom command [{}] doesn't use a .py, .pl, .js "
...
def check_distsearch_conf_for_concerning_replicated_file_size(app, reporter):
    ...
    if app.file_exists(path):
        try:
            ...
            reporter.fail("distsearch.conf is malformed. Details: {}".format(str(e)), path)
            return
...
def check_for_inputs_fifo_or_monitor_usage(app, reporter):
    """Check [fifo] or [monitor] stanza in `inputs.conf` is not pointing to a path
    ...
    distsearch_confs = _collect_distsearch_conf(app)
    confs_for_blacklist = _search_stanza(distsearch_confs, "replicationBlacklist")
    confs_for_whitelist = _search_stanza(distsearch_confs, "replicationWhitelist")
    if config_file_paths:
        for directory, filename in iteritems(config_file_paths):
            file_path = os.path.join(directory, filename)
            try:
                ...
                reporter_output = "inputs.conf is malformed. Details: {}".format(str(e))
                reporter.fail(reporter_output, os.path.join(directory, "inputs.conf"))
                continue
            pass_pattern = r"\$SPLUNK_HOME(?:/|\\)var(?:/|\\)log(?:/|\\)splunk"
            manchk_pattern = r"\$SPLUNK_HOME(?:/|\\)etc(?:/|\\)apps(?:/|\\)[\w-]+"

            disallowed_monitor_or_fifo_stanzas, suspicious_monitor_or_fifo_stanzas =\
                                            _find_interesting_monitor_or_fifo_stanzas(
                                                    inputs_configuration_file.sections(),
                                                    pass_pattern,
                                                    manchk_pattern)

            for stanza in disallowed_monitor_or_fifo_stanzas:
            ...
            for stanza in suspicious_monitor_or_fifo_stanzas:
                if re.search(r"apps(?:/|\\)[\w-]+(?:/|\\)(?:bin|lookup)", stanza.name):
                    if len(confs_for_blacklist) == 0:
                        reporter_output = ("{}/inputs.conf contains a stanza [{}],"
                                    " which is pointing to a file that would be replicated in cloud environment."
                                    " Please either remove this stanza or add a replication blacklist."
                                    " Refer https://docs.splunk.com/Documentation/Splunk/7.3.1/Admin/Distsearchconf#REPLICATION_BLACKLIST_OPTIONS"
                                    " for replication blacklist details."
                                    ).format(directory, stanza.name)
                        reporter.fail(reporter_output, file_path, stanza.lineno)
                    else:
                        reporter_output = ("{}/inputs.conf contains a stanza [{}],"
                                    " which is pointing to a file that may or may not be replicated in cloud environment."
                                    " This depends to the replicationBlacklist stanza in {}."
                                    " We'll verify this in manual review.").format(directory, stanza.name, confs_for_blacklist)
                        reporter.manual_check(reporter_output, file_path, stanza.lineno)
                else:
                    if len(confs_for_whitelist) == 0:
                        reporter_output = ("{}/inputs.conf contains a stanza [{}],"
                                        " which is not recommanded to use in cloud environment."
                                        ).format(directory, stanza.name)
                        reporter.warn(reporter_output, file_path, stanza.lineno)
                    else:
                        reporter_output = ("{}/inputs.conf contains a stanza [{}],"
                                    " which is pointing to a file that may or may not be replicated in cloud environment."
                                    " This depends to the replicationBlacklist stanza in {} and replicationWhitelist in {}."
                                    " We'll verify this in manual review.").format(directory, stanza.name, confs_for_blacklist, confs_for_whitelist)
                        reporter.manual_check(reporter_output, file_path, stanza.lineno)
    else:
        reporter.not_applicable("The default/inputs.conf does not exist.")

def _collect_distsearch_conf(app):
    config_file_paths = app.get_config_file_paths("distsearch.conf")
    confs = []
    if config_file_paths:
        for directory, _ in iteritems(config_file_paths):
            try:
                distsearch_conf_file = app.distsearch_conf(directory)
            except InvalidSectionError:
                # Skip malformed distsearch.conf, this exception is handled in
                # check_distsearch_conf_for_concerning_replicated_file_size
                continue
            confs.append((os.path.join(directory, "distsearch.conf"), distsearch_conf_file))
    return confs

def _search_stanza(confs, stanza_name):
    positive_conf_path = []
    for conf_path, conf in confs:
        if conf.has_section(stanza_name):
            positive_conf_path.append(conf_path)
    return positive_conf_path

def _find_interesting_monitor_or_fifo_stanzas(stanzas, pass_pattern, manchk_pattern):
    monitor_or_fifo_stanzas = [stanza
                               for stanza in stanzas
                               if re.search(r"^monitor:\/\/", stanza.name) or
                               re.search(r"^fifo:\/\/", stanza.name)]
    disallowed_monitor_or_fifo_stanzas = []
    suspicious_monitor_or_fifo_stanzas = []
    for stanza in monitor_or_fifo_stanzas:
        if re.search(pass_pattern, stanza.name):
            continue
        elif re.search(manchk_pattern, stanza.name):
            suspicious_monitor_or_fifo_stanzas.append(stanza)
        else:
            disallowed_monitor_or_fifo_stanzas.append(stanza)
    return disallowed_monitor_or_fifo_stanzas, suspicious_monitor_or_fifo_stanzas
...
def check_scripted_inputs_cmd_path_pattern(app, reporter):
    ...
    absolute_path_pattern = (r"^({0}|{1})").format('/'.join(absolute_path), '(\\\\|\\\\\\\\)'.join(absolute_path))
            ...
            for external_command_stanza in external_command_stanzas:
                ...
                if script_filename_matches:
                    ...
                    if not script_matches:
                        reporter_output = (" The `transforms.conf` stanza [{}] is using the" 
...
def check_audit_conf_black_list(app, reporter):
    ...
    and certificates.
    """
    blacklist_conf(app, reporter.fail, "audit.conf",
                    ...
                    if stanza.startswith(allowed_stanza):
                        ...
                        if stanza.startswith('roleMap_'):
                            ...
                            if authentication_conf.get_section(stanza).has_option('admin'):
                                line_number = authentication_conf.get_section(stanza).get_option('admin').lineno
                                reporter.fail(reporter_output, file_name, line_number)
                        else:
                            # check if option-value equal to 'admin' or startswith 'admin::'
                            for option_k, option_v in authentication_conf.get_section(stanza).options.items():
                                if option_v.value == 'admin' or option_v.value.startswith('admin::'):
                                    line_number = option_v.lineno
                                    reporter.fail(reporter_output, file_name, line_number)
                        break
                else:
                    reporter_output = "Only role-mapping stanza is allowed in " \
                    "authentication.conf, but [{}] is found.".format(stanza)
                    line_number = authentication_conf.get_section(stanza).lineno
                    reporter.fail(reporter_output, file_name, line_number)
        except InvalidSectionError as e:
            reporter_output = "authentication.conf is malformed. Details: {}".format(str(e))
            reporter.fail(reporter_output)
    else:
...
def check_introspection_of_cloud_filesystem(app, reporter):
    ...
    blacklist_conf(app, reporter.fail, "crawl.conf",
...
def check_datatypesbnf_conf_black_list(app, reporter):
    ...
    in Splunk Cloud.
    """
    blacklist_conf(app, reporter.fail, "datatypesbnf.conf",
...
def check_default_mode_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "default-mode.conf",
...
def check_deployment_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "deployment.conf",
...
def check_deploymentclient_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "deploymentclient.conf",
...
def check_instance_cfg_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "instance.cfg.conf",
...
def check_literals_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "literals.conf",
...
def check_messages_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "messages.conf",
...
def check_outputs_conf_black_list(app, reporter):
    ...
    permitted in Splunk Cloud.
    """
    blacklist_conf(app, reporter.fail, "outputs.conf",
...
def check_that_no_configurations_of_default_source_type_in_props_conf(app, reporter):
    ...
    try:
        ...
        reporter_output = "props.conf is malformed. Details: {}".format(str(e))
        reporter.fail(reporter_output)
    except Exception:
        raise
...
def check_pubsub_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "pubsub.conf",
...
def check_segmenters_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "segmenters.conf",
...
def check_serverclass_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "serverclass.conf",
...
def check_serverclass_seed_xml_conf_black_list(app, reporter):
    ...
    administrators.
    """
    blacklist_conf(app, reporter.fail, "serverclass.seed.xml.conf",
...
def check_source_classifier_conf_black_list(app, reporter):
    ...
    data).
    """
    blacklist_conf(app, reporter.fail, "source-classifier.conf",
...
def check_sourcetypes_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "sourcetypes.conf",
...
def check_splunk_launch_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "splunk-launch.conf",
...
def check_telemetry_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "telemetry.conf",
...
def check_user_seed_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "user-seed.conf",
...
def check_wmi_conf_black_list(app, reporter):
    ...
    """
    blacklist_conf(app, reporter.fail, "wmi.conf",
            ...
            try:
                ...
                reporter.manual_check("Please manually check {} ({} {} {})\r\n"
                                      ...
                                      .format(current_file_relative_path,
                                              getattr(e, "returncode", "N/A"),
                                              getattr(e, "cmd", "N/A"),
                                              getattr(e, "output", "N/A"),
        ...
        for path, info in iteritems(app.info_from_file):
            ...
            if inputs_conf_path == path:
                ...
                banned_stanzas = [stanza
                                  ...
                                  if re.search(r"^monitor:\/\/([a-zA-Z]\:|\.)\\", stanza.name) or
                                     re.search(r"^script:\/\/([a-zA-Z]\:|\.)\\", stanza.name) or
                                     re.search(r"^perfmon:\/\/", stanza.name) or
                                     re.search(r"^MonitorNoHandle:\/\/", stanza.name) or
                                     re.search(r"^WinEventLog:\/\/", stanza.name) or
                                     re.search(r"^admon:\/\/", stanza.name) or
                                     re.search(r"^WinRegMon:\/\/", stanza.name) or
                                     re.search(r"^WinHostMon:\/\/", stanza.name) or
                                     re.search(r"^WinPrintMon:\/\/", stanza.name) or
                                     re.search(r"^WinNetMon:\/\/", stanza.name) or
                                     re.search(r"^powershell2:\/\/", stanza.name) or
                                     re.search(r"^powershell:\/\/", stanza.name)]
...
def check_for_implementing_tscollect(app, reporter):
    ...
    """
    reporter_output = "Find `tscollect` which is not allowed in Splunk Cloud because it can eat up disk space " \
