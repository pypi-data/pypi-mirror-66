...
def check_for_secret_disclosure(app, reporter):
    ...
    special_files = [os.path.join('default', 'props.conf'), os.path.join('default', 'transforms.conf')]
...
def _secret_disclosure_values_whitelist(line_message):
    ...
    values_whitelist = ['value', 'string', 'not_set', 'str', 'password', 'enabled', 'true',
                       'false', 'Enter', 'YourSecretPassword', 'YOUR_ADMIN_PASSWORD',
                       'undefined', 'received', 'self', 'changeme', 'Heavy', 'null',
                       '0+', 'x+', 'X+', r'\*+', r'\^+']
    # (?i) case insensitive
    values_patterns = (r"((?i)((?:(key|pass|pwd|token|login|passwd|password|community|privpass))[ ]{0,10}=[ ]{0,10}((<|\"|\'|\\\"|\\\')?(%s)(>|\"|\'|\\\"|\\\')?)$))"
...
def _secret_disclosure_commands_whitelist(file_path, messages):
    ...
    '''
    commands_whitelist_in_props_file = ['SEDCMD-', 'EXTRACT-', 'REPORT-', 'TRANSFORMS-',
                                        'LOOKUP[-|_]?','lookup[-|_]?', 'EVAL-', 'FIELDALIAS-']
    ...
    if file_path == os.path.join('default', 'props.conf'):
        ...
        commands_patterns = (r"(^(?=%s))" % ('|'.join(commands_whitelist_in_transforms_file)))
    else:
...
def _combine_messages_in_special_files(file_path, lineno, result, messages_in_special_files):
...
def check_for_sensitive_info_in_url(app,reporter):
    ...
    sensitive_info_patterns = (r"((?i)[ \f\r\t\v]*[0-9a-z_\.]*(url|uri|host|server|prox|proxy_str)s?[ \f\r\t\v]*=[ \f\r\t\v]*[\"\']?https?://[^\"\'\s]*?(key|pass|pwd|token)[0-9a-z]*=[^&\"\'\s]+[\"\']?|"                     # Single line url
                               r"[ \f\r\t\v]*[0-9a-z_\.]*(url|uri|host|server|prox|proxy_str)s?[ \f\r\t\v]*=[ \f\r\t\v]*([\"\']\{\}://\{\}:\{\}@\{\}:\{\}[\"\'])\.format\([^\)]*(key|password|pass|pwd|token|cridential|secret|login|auth)[^\)]*\))")  # Multi line url
    
    sensitive_info_patterns_for_report = (r"((?i)[0-9a-z_\.]*(url|uri|host|server|prox|proxy_str)s?[ \f\r\t\v]*=[ \f\r\t\v]*[\"\']?https?://[^\"\'\s]*?(key|pass|pwd|token)[0-9a-z]*=[^&\"\'\s]+[\"\']?|"                     # Single line url
                               r"[0-9a-z_\.]*(url|uri|host|server|prox|proxy_str)s?[ \f\r\t\v]*=[ \f\r\t\v]*([\"\']\{\}://\{\}:\{\}@\{\}:\{\}[\"\'])\.format\([^\)]*(key|password|pass|pwd|token|cridential|secret|login|auth)[^\)]*\))")  # Multi line url
...
def check_for_insecure_http_calls_in_python(app, reporter):
    ...
    report_template = ("Possible insecure HTTP Connection."
                       " Match: {}"
    ...
    files_with_results = AstSearcher(app.python_analyzer_client).search(components,
                                                                        node_filter=is_arg_secure_or_keyword_secure,
