...
@splunk_appinspect.tags("splunk_appinspect", "splunk_7_1", "deprecated_feature", "splunk_7_3", "removed_feature")
...
def check_for_input_command_usage(app, reporter, target_splunk_version):
    ...
    if not target_splunk_version < "splunk_7_1":
        ...
        findings = find_spl_command_usage(app, 'input(\s*)(add|remove)')
        reporter_level, kw = (reporter.warn, 'deprecated') \
            if "splunk_7_1" <= target_splunk_version < 'splunk_7_3' else (reporter.warn, 'removed')

        reporter_output = "The input search command is {}. " \
                          "To add, enable, or disable inputs, use Splunk Web or edit the inputs.conf file".format(kw)
