...
def find_spl_command_usage(app, command):
    ...
    for conf_filename, options in checked_confs.items():
        findings_in_conf = _find_spl_command_usage_in_conf_file(
                                ...
                                conf_filename,
                                options)
        findings_in_confs += list(map(lambda lineno: (os.path.join('default', conf_filename), lineno), findings_in_conf))
    findings_in_simple_xml = list(map(lambda filepath: (filepath, None), findings_in_simple_xml))
