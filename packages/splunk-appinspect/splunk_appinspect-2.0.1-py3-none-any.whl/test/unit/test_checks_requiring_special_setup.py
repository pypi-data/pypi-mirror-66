...
def test_check_source_and_binaries__check_for_expansive_permissions():
    def setup(app_dir):
        ...
        if os.name != 'nt':
            ...
            # Give full control permissions to Everyone for app.conf
            user, _, _ = win32security.LookupAccountName("", "Everyone")
...
@pytest.mark.skipif(os.name != "nt", reason="Only runs on Windows platform.")
def test_check_source_and_binaries__check_for_expansive_permissions_good_app_conf_windows():
    def setup(app_dir):
        # Create default/
        os.makedirs(os.path.join(app_dir, "default"))
        # Make default/app.conf
        ...
        with open(app_conf_path, "w") as fout:
            fout.write("")
        sd = win32security.GetFileSecurity(app_conf_path, win32security.DACL_SECURITY_INFORMATION)
        # Create a new dacl and add ACE for below allowed users:
        # file_owner and ('SYSTEM', 'Administrators', 'Authenticated Users')
        new_dacl = win32security.ACL()
        owner_sd = win32security.GetFileSecurity(app_conf_path, win32security.OWNER_SECURITY_INFORMATION)
        file_owner_sid = owner_sd.GetSecurityDescriptorOwner()
        new_dacl.AddAccessAllowedAce(win32security.ACL_REVISION, 2032127, file_owner_sid)
        for user in ['SYSTEM', 'Administrators', 'Authenticated Users']:
            try:
                user, _, _ = win32security.LookupAccountName("", user)
                new_dacl.AddAccessAllowedAce(win32security.ACL_REVISION, 2032127, user)
            except:
                pass
