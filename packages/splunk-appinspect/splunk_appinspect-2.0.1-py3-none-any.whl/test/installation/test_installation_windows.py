...
class TestInstallationOnWindows(TestInstallationBase):
    ...
    def _verify_installation(env):
        ...
        """
        try:
            return_value = subprocess.check_call(["where", "splunk-appinspect"], env=env, shell=True)
            assert return_value == 0, "Command `where splunk-appinspect` failed!"
            return_value = subprocess.check_call(["splunk-appinspect"], env=env, shell=True)
            assert return_value == 0, "Command `splunk-appinspect` failed!"
            return_value = subprocess.check_call(["splunk-appinspect", "list", "version"], env=env, shell=True)
            assert return_value == 0, "`Command `splunk-appinspect list version` failed!"
            return_value = subprocess.check_call(["splunk-appinspect", "inspect", test_app_path], env=env, shell=True)
    ...
    def test_installation_using_pip_in_fresh_virtual_env(self):
        ...
        env = os.environ.copy()
        env["PATH"] = self.temporary_directory + "\\Scripts;" + env["PATH"]  # use virtualenv's path
        ...
        splunk_appinspect_latest_file_path = self.splunk_appinspect_builds.retrieve_splunk_appinspect_path(splunk_appinspect_version_to_install,
                                                                                                           env=env)
        virtual_pip = self.temporary_directory + "\\Scripts\\pip.exe"
    ...
    def test_installation_using_pip_over_old_installation_with_virtual_env(self):
        ...
        splunk_appinspect_1_5_0_file_path = os.path.join(repository_root_directory, 'dist', 'splunk-appinspect-1.5.0.140.tar.gz')

        if not os.path.isfile(splunk_appinspect_1_5_0_file_path):
