...
TRUSTED_CHECK_AND_LIBS_FILE = os.path.join(os.path.dirname(__file__), "lib_files", "trusted_file_hashes.csv")
UNTRUSTED_CHECK_AND_LIBS_FILE = os.path.join(os.path.dirname(__file__), "lib_files", "untrusted_file_hashes.csv")
...
class TrustedDataCollector(object):
    ...
    def _process_trusted_data(self):
        if os.path.exists(self._trusted_check_and_libs_file):
            ...
            logger.warn('trustedlibs source file `%s` is not found', self._trusted_check_and_libs_file)
        ...
        if os.path.exists(self._untrusted_check_and_libs_file):
            ...
            logger.warn('trustedlibs source file `%s` is not found', self._untrusted_check_and_libs_file)

    def _read_lib_file(self, source_file):
