...
__all__ = ["find_imports", "find_pkg_path", "get_from_import_module_name", "relative_import_dump"]
...
def find_pkg_path(filepath_perfroming_import, pkg_name, libs=None):
    ...
    search_paths = [filepath_perfroming_import] + libs if six.PY2 else libs + [filepath_perfroming_import]
...
def _find_pkg_path_helper(search_path, pkg_name):
    ...
    for name in pkg_names:
        if six.PY2:
            pkg = pkgutil.ImpImporter(parentdir).find_module(name)
        else:
            pkg = importlib.machinery.PathFinder().find_spec(name, [os.path.realpath(parentdir)])
        if not pkg:
            break

        pkg_path = pkg.get_filename() if six.PY2 else pkg.origin
...
def find_imports(filepath, libs=None):
    ...
    imports = []
    if six.PY2:
        python_code_string = open(filepath, "rU").read()
    else:
        try:
                ...
                if ast_node.module is None:
                    ...
                    if pkg_file_path and pkg_file_path != filepath:
                        ...
                        for target_import_obj in ast_node.names:
                            ...
                            if last_item_path:
                                imports.append((pkg_name, target_import_obj.name, None, last_item_path, ast_node.lineno))
                            else:
                                imports.append((pkg_name, None, target_import_obj.name, pkg_file_path, ast_node.lineno))
                    else:
...
def get_from_import_module_name(node, code_lines):
    ...
    for i in range(node.lineno - 1, len(code_lines)):
        ...
        if six.PY2:
            string = string.replace(codecs.BOM_UTF8, "")
            string = string.replace(codecs.BOM_UTF16, "")
        else:
...
def is_same_ast_tree(node1, node2, node1_extra=False):
    '''
        ...
        if current[0] is None or current[1] is None:
            ...
            if current[0].__class__ == current[1].__class__:
                if isinstance(current[0], list):
                    ...
                    child_nodes1 = [
                        child_node for child_node in ast.iter_child_nodes(
                            current[0])]
                    child_nodes2 = [
                        child_node for child_node in ast.iter_child_nodes(
                            current[1])]
...
def fetch_all_nodes_belonging_to_given_subtree(ast_root, node_set):
    ...
    if hasattr(node_set, '__iter__'):
        ...
        return set([node for node in ast.walk(ast_root) if node in node_set])
    else:
