...
def check_that_directory_name_matches_package_id(app, reporter, included_tags):
    ...
    For others, it's ok if no `id` property if 'check_for_updates = True' or no 'check_for_updates'.
    """
        ...
        if app_configuration_file.has_section("package"): # with [package] stanza
            ...
            # appinspect fail if 'check_for_updates = True' or no 'check_for_updates'
                if 'cloud' in included_tags:
                    ...
                    reporter_output = ("No `id` property found in [package] stanza. "
            ...
            if 'cloud' in included_tags:
                reporter_output = ("The `app.conf` [package] stanza does not exist. "
...
def check_splunklib_dependency_under_bin_folder(app, reporter):
    ...
    results_py_file_hashes = set(['51dacaa440c6dbeff900121662eaa849',
    ...
    for directory, file, ext in app.iterate_files(basedir='bin', types=['.py']):
        if file == 'results.py':
            filename = os.path.join(app.app_dir, directory, file)
            md5_hash = md5(filename)
            if md5_hash in results_py_file_hashes:
                reporter.warn('splunklib is found under `bin` folder, this may cause some dependency management '
