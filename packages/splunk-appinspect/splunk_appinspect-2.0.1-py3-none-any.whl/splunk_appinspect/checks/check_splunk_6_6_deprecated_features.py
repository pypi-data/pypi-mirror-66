...
def check_for_app_install_endpoint(app, reporter):
    ...
    """
    reporter_output = "apps/appinstall endpoint has been deprecated in Splunk 6.6. " \
...
@splunk_appinspect.tags("splunk_appinspect", "splunk_6_6", "removed_feature")
@splunk_appinspect.cert_version(min="1.7.1")
def check_for_autolb_setting_in_outputs_conf(app, reporter, target_splunk_version):
    ...
    """
    reporter_output = "Removed support for setting autoLB in outputs.conf," \
...
def check_for_displayrownumbers_in_simple_xml(app, reporter, target_splunk_version):
    ...
    if target_splunk_version < "splunk_6_6":
        return
    reporter_output = ("<option> elements with the attribute [name=displayRowNumbers] was detected, "
