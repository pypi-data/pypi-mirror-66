...
@splunk_appinspect.tags("splunk_appinspect", "removed_feature", 'ast', 'splunk_8_0')
...
@splunk_appinspect.tags("splunk_appinspect", "removed_feature", "splunk_8_0")
def check_for_cherry_py_custom_controller_web_conf_endpoints(app, reporter, target_splunk_version):
    ...
    if not config_file_paths:
        ...
        return
    reporter_output_a = ("Update custom CherryPy endpoints to be Python 3-compatible"
    ...
    reporter_output_b = ("CherryPy endpoints are defined in web.conf but no controller file found."
    ...
    for directory, filename in iteritems(config_file_paths):
        ...
        for section in web_conf.sections():
            if section.name.startswith("endpoint:"):
                ...
                file_path = os.path.join("appserver", "controllers", "{}.py".format(python_module_name))
...
@splunk_appinspect.tags("splunk_appinspect", "splunk_8_0")
...
@splunk_appinspect.tags("splunk_appinspect", "splunk_6_3", "deprecated_feature", "advanced_xml", "splunk_8_0", 'removed_feature')
...
def check_for_advanced_xml_module_elements(app, reporter, target_splunk_version):
    ...
    if target_splunk_version < "splunk_6_3":
        return
    if target_splunk_version < 'splunk_8_0':
        finding_action, reporter_output = reporter.warn, ("Module tag is found in your XML, which indicates your're using Advanced XML. "
    ...
    findings = check_routine.find_xml_nodes(app,
    ...
    if findings is None:
        ...
        return
    try:
        has_canary_dep = app_conf_contain_stanza(app, "dependency:app:canary").with_option("requiredVersion")
    except:
        has_canary_dep = False
    try:
        has_sideview_utils_dep = app_conf_contain_stanza(app, "dependency:app:sideview_utils").with_option("requiredVersion")
    except:
    ...
    for node_name in findings:
        for relative_filepath in findings[node_name]:
            if has_sideview_utils_dep and not has_canary_dep:
                finding_action("This app is dependent on Sideview Utils, and may not function without additional app "
                               "dependencies defined in app.conf in Splunk Enterprise 8.0 and later. "
                               "Please contact the Sideview, LLC. to learn more.", relative_filepath)
...
@splunk_appinspect.tags("splunk_appinspect", "splunk_6_4", "deprecated_feature", "web_conf", "splunk_8_0", 'removed_feature')
@splunk_appinspect.cert_version(min="1.1.11")
...
@splunk_appinspect.tags("splunk_appinspect", "deprecated_feature", "splunk_8_0")
@splunk_appinspect.cert_version(min="1.7.1")
