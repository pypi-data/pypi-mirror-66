...
def check_for_simple_xml_single_element_with_deprecated_option_names(app, reporter):
    ...
    for relative_filepath, full_filepath in xml_files:
        ...
        if(total_single_elements_with_options_found > 0 or
            ...
            reporter_output = ("{} <single> panel(s) contain the option(s): {}. "
                               "These option(s) have been deprecated in Splunk 6.4."
                               ).format(total_single_elements_with_options_found,
    ...
    try:
        ...
        for section in web_config.sections():
            for property, value, lineno in [(p, v, lineno)
                ...
                reporter_output = ("{} use the simple_xml_module_render property "
                                   "in Stanza {}, which has been deprecated in "
                                   "Splunk 6.4. ").format(web_config_file_path,
                                            section.name)
    ...
    try:
        ...
        for section in web_config.sections():
            ...
            for property, value, lineno in all_secitions_with_flash_charting:
                reporter_output = ("{} use the simple_xml_force_flash_charting property "
                                   "in Stanza {}, which has been deprecated in "
                                   "Splunk 6.4.").format(web_config_file_path, section.name)
                reporter.warn(reporter_output, web_config_file_path, lineno)
    except:
        reporter_output = ("No web.conf file found.")
        reporter.not_applicable(reporter_output)
    ...
    for relative_filepath, full_filepath in xml_files:
        ...
        for option_element in option_elements:
            ...
            if not is_number(option_content) and not is_token(option_content):
                reporter_output = ('<option name="height"> use an "[integer]px" for the value, ' 
...
def check_for_splunk_js_d3chartview(app, reporter):
    ...
    for match_file_and_line, match_object in matches_found:
        ...
        reporter_output = ("Views are importing d3chartview, which "
...
def check_for_splunk_js_googlemapsview(app, reporter):
    ...
    for match_file_and_line, match_object in matches_found:
        ...
        match_file = match_split[0]
        match_line = match_split[1]
        reporter_output = ("Views are importing googlemapsview, which "
                           "has been deprecated in Splunk 6.4.")
        reporter.warn(reporter_output, match_file, match_line)
