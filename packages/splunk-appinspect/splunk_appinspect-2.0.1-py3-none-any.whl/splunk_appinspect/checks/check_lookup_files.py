...
def check_for_lookups_file_name(app, reporter):
    '''Check that "xxx" and "xxx.default" file/directory under lookups folder don't exist at the same time.
    When installing an App in Splunk Cloud, the lookup file "xxx" will be temporarily renamed with an extra
    '.default' extension. It will run into errors if "xxx.default" file already exists.
    '''
    if app.directory_exists('lookups'):
        ...
        for path in glob.glob(base_dir + os.sep + '*.default'):
            ...
            if os.path.exists(csv_path):
                ...
                reporter.fail(" When installing an app in Splunk Cloud, the lookup file '{}'"
                              ...
                              " of them or change one of their name."
                              .format(csv_file, default_file),
                              default_path)
    else:
