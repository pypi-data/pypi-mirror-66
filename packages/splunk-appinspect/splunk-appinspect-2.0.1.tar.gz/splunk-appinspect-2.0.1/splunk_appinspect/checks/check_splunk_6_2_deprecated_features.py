...
from splunk_appinspect.check_routine import report_on_xml_findings, xml_node, find_xml_nodes


@splunk_appinspect.tags("splunk_appinspect", "splunk_6_2", "splunk_6_5", "deprecated_feature", "removed_feature")
...
def check_for_dashboard_xml_list_element(app, reporter, target_splunk_version):
    ...
    report_on_xml_findings(
                ...
                reporter, reporter_output, reporter_action)
...
def check_for_simple_xml_row_grouping(app, reporter, target_splunk_version):
    ...
    finding_action, reporter_output = reporter.warn, ("Detect grouping attribute of <row>, which is deprecated in Splunk 6.2. Please use "
                        "the <panel> node instead.")
    report_on_xml_findings(
            find_xml_nodes(app, [node], path='default/data/ui/views'),
            reporter, reporter_output, finding_action)
...
def check_for_populating_search_element_in_dashboard_xml(app, reporter):
    ...
    reporter_output = ("<{}> element was deprecated in Splunk 6.2 and supposed to be removed in future releases, " \
...
@splunk_appinspect.tags("splunk_appinspect", "splunk_6_2", "deprecated_feature")
@splunk_appinspect.cert_version(min="1.7.0")
def check_for_earliest_time_and_latest_time_elements_in_dashboard_xml(app, reporter, target_splunk_version):
