...
from splunk_appinspect.check_routine import report_on_xml_findings, find_xml_nodes, xml_node
...
def check_for_simple_xml_seed_element(app, reporter):
    ...
    report_on_xml_findings(
        ...
        reporter, reporter_output, reporter.warn)
...
def check_for_simple_xml_search_related_element(app, reporter):
    ...
    report_on_xml_findings(findings, reporter, reporter_output, reporter.warn)
...
def check_for_simple_xml_option_element_with_name_previewresults(app, reporter):
    ...
    report_on_xml_findings(
        find_xml_nodes(app, [option_node]),
        reporter, reporter_output, reporter.warn)
...
def check_for_simple_xml_chart_element_with_deprecated_option_names(app, reporter):
    ...
    for relative_filepath, full_filepath in xml_files:
        ...
        for chart_element in chart_elements:
            ...
            if option_elements:
                reporter_output = ("A <chart> was detected with "
                                   "options [charting.axisLabelsY.majorTickSize] or "
                                   "[charting.axisLabelsY.majorLabelVisibility]. "
                                   "These options have been deprecated in Splunk 6.3.")
                reporter.warn(reporter_output, relative_filepath)
            else:
                pass  # Do nothing, everything is fine


@splunk_appinspect.tags("splunk_appinspect", "splunk_6_3", "deprecated_feature", "django_bindings", 'splunk_7_3', 'removed_feature')
@splunk_appinspect.cert_version(min="1.1.11")
def check_for_django_bindings(app, reporter, target_splunk_version):
    ...
    if not target_splunk_version < 'splunk_6_3':
        if app.directory_exists("django"):
            file_path = "django"
            deprecated_or_removed, splunk_version_that_feature_involved = ('deprecated', 'Splunk 6.3') \
                if 'splunk_6_3' <= target_splunk_version < 'splunk_7_3' else ('removed', 'Splunk 7.3')
            reporter_output = ("The `django` directory was detected, which implies you're using "
                            "Django bindings feature. Django bindings has been {} in {}.").format(deprecated_or_removed, splunk_version_that_feature_involved)
    ...
    if saved_searches.configuration_file_exists():
        ...
        for section in config_file.sections():
            ...
            if not normalizeBoolean(script_option.value):
                reporter_output = ("Alert of running a script found in "
                ...
                reporter_output = ("Alert of running a script found in "
                                ...
                                "Use the custom alert action framework instead.")
            reporter.warn(reporter_output, filepath, script_option.lineno)
