...
def check_for_hidden_python_files(app, reporter):
    ...
    if platform.system() == "Windows":
        ...
        for directory, file, ext in app.iterate_files(excluded_types=['.py']):
            current_file_relative_path = os.path.join(directory, file)
            reporter.manual_check(reporter_output.format(current_file_relative_path),
        ...
        for filepath in client.get_hidden_python_files():
            if six.PY2:
                ...
                content = open(os.path.join(app.app_temp_dir, filepath), "r", encoding=encoding).read()
            content = re.sub('[\r\n]+', ' ', content)
            ...
            patterns = [network_patterns, from_import_modules, import_modules, file_manipulation, possible_injection]
            template_pairs = [('<%.*', '.*%>'), ('{{.*', '.*}}'), ('{%.*', '.*%}')]

            for search_pattern in [pair[0] + pattern + pair[1] for pattern in patterns for pair in template_pairs]:
                if re.search(search_pattern, content):
                    reporter.manual_check(reporter_output.format(filepath), file_name=filepath)
                    break
...
def check_for_possible_threading(app, reporter):
    ...
    reporter_output = "The following line will be inspected during code review. " \
                      ...
                      "File: {} " \
                      "Line: {}"
    reporter_output_for_loopcheck = "The following line will be inspected during code review. " \
                                    "Questionable usage `{}` found in loop. " \
                                    "File: {} " \
                                    "Line: {}"
    circle_check_namespace = ['os.forkpty', 'os.fork', 'thread.start_new_thread', 'os.kill', 'os.killpg',
                              'threading.Thread.start', 'multiprocessing.Process.start']
    modules = [metadata_consts.ModuleNameConsts.OS, metadata_consts.ModuleNameConsts.SUBPROCESS,
               metadata_consts.ModuleNameConsts.THREAD, metadata_consts.ModuleNameConsts.THREADING,
               metadata_consts.ModuleNameConsts._THREAD, metadata_consts.ModuleNameConsts.MULTIPROCESSING]
    check_objects = metadata_store.query().namespace_prefixes(modules).tag(
        metadata_consts.TagConsts.THREAD_SECURITY).python_compatible().collect()
            ...
            if check_object.namespace in circle_check_namespace:
                ...
                for node in loop_nodes:
                    reporter.manual_check(
                        reporter_output_for_loopcheck.format(check_object.namespace, file_path, node.lineno),
                        file_name=file_path, line_number=node.lineno)
            else:
                ...
                for node_lineno in node_linenos:
                    reporter.manual_check(reporter_output.format(check_object.namespace, file_path, node_lineno),
                                          file_name=file_path, line_number=node_lineno)
...
def check_built_in_import_function(app, reporter):
    ...
    reporter_output_template = ("The `{}` function was detected being"
    ...
    def is_non_arg_or_non_string_arg(builtin_import_call, ast_info):
        # parse __import__ module from positional arguments and keyword arguments
        arg = None
        if builtin_import_call.args:
            arg = builtin_import_call.args[0]
        for keyword in builtin_import_call.keywords:
            if keyword.arg == 'name':
                arg = keyword.value
                break
        # Arg is not exists, or it could not be parsed by ast_info as string
        # If arg could be parsed as a string, manual check won't be triggered.
        # Since we have opportunity to handle this case
        return (not arg) or (not AstVariable.is_string(ast_info.get_variable_details(arg)))

    files_with_results = AstSearcher(app.python_analyzer_client).search(import_function,
                                                                        node_filter=is_non_arg_or_non_string_arg)
...
def check_for_file_and_directory_access(app, reporter):
    ...
    function_reporter_output = "The following line will be inspected during code review. " + \
    ...
    functions = metadata_store.query().namespace_prefixes(modules) \
        .tag(metadata_consts.TagConsts.FILE_READ_AND_WRITE).python_compatible().functions()
...
@splunk_appinspect.tags("splunk_appinspect", "cloud", "security", "ast")
@splunk_appinspect.cert_version(min="1.7.0")
def check_for_python_udp_network_communications(app, reporter):
    ...
    for filepath, ast_info in client.get_all_ast_infos():
        ...
        udp_nodes = query.filter(Any(ast_info.get_module_function_call_usage('socket', 'socket', fuzzy=True))) \
            .filter(Any(ast_info.get_module_usage('socket.AF_INET'))) \
            .filter(Any(ast_info.get_module_usage('socket.SOCK_DGRAM'))).collect()
...
def check_for_unencrypted_network_communications(app, reporter):
    ...
    message = ("Please check for inbound or outbound unencrypted network communications in `{}`. "
               "All communications with Splunk Cloud must be encrypted.")

    components = metadata_store.query().tag(metadata_consts.TagConsts.APPLICATION_LAYER_PROTOCOL_CONNECTION).python_compatible().collect()
...
@splunk_appinspect.tags("splunk_appinspect", "cloud", "security", "manual", "ast")
@splunk_appinspect.cert_version(min="1.7.0")
def check_file_formats_module_IO(app, reporter):
    """Check for usages of python modules related to file formats"""
    message = "The following line will be inspected during code review. " + \
              "The `{}` module/method can be used to process files outside of the app dir. " + \
              "Function call arguments: {}, keywords: {} "
    modules = [
        metadata_consts.ModuleNameConsts.CSV,
        metadata_consts.ModuleNameConsts.PLISTLIB,
        metadata_consts.ModuleNameConsts.CONFIG_PARSER,
        metadata_consts.ModuleNameConsts.CONFIG_PARSER_PY3,
        metadata_consts.ModuleNameConsts.JSON,
        metadata_consts.ModuleNameConsts.BASE64,
        metadata_consts.ModuleNameConsts.BINHEX,
        metadata_consts.ModuleNameConsts.MIMETOOLS,
        metadata_consts.ModuleNameConsts.MIMETYPES,
        metadata_consts.ModuleNameConsts.MIMEWRITER,
        metadata_consts.ModuleNameConsts.MIMIFY,
        metadata_consts.ModuleNameConsts.MULTIFILE,
        metadata_consts.ModuleNameConsts.QUOPRI,
        metadata_consts.ModuleNameConsts.UU
    ]
    query = metadata_store.query()
    query.namespace_prefixes(modules).tag(metadata_consts.TagConsts.FILE_READ_AND_WRITE).python_compatible()
    components = query.functions() + query.classes()

    files_with_results = AstSearcher(app.python_analyzer_client).search(components, get_func_params=True)
    reporter.ast_manual_check(message, files_with_results)
...
def check_all_python_files_are_well_formed(app, reporter):
    """Check all python files are well formed under python2 standard"""
    """Check all python files are well formed under python2/3 standard"""
    error_template = ("Python script is not well formed, {} when parser try to parse. "
    ...
    for filepath in client.get_syntax_error_files():
        reporter.manual_check(error_template.format(syntax_error_message, filepath), filepath)
    for filepath in client.get_null_byte_error_files():
        reporter.manual_check(error_template.format(null_byte_error_message, filepath), filepath)
    for filepath in client.get_other_exception_files():
        reporter.manual_check(error_template.format(other_exception_message, filepath), filepath)
...
def check_for_builtin_functions(app, reporter):
    ...
    def has_args(call_node, ast_info):
        return call_node.args

    files_with_results = AstSearcher(app.python_analyzer_client).search(built_in_file_functions,
                                                                        node_filter=has_args)
    ...
    files_with_results = AstSearcher(app.python_analyzer_client).search(built_in_eval_functions,
                                                                        node_filter=has_non_string_args)
    reporter.ast_manual_check(reporter_output, files_with_results)


@splunk_appinspect.tags("splunk_appinspect", "cloud", "security", "manual", "ast")
@splunk_appinspect.cert_version(min="1.7.0")
def check_for_builtin_types(app, reporter):
    """Check for builtin types(`file`, `memoryview`, `fileinput`) usages in python files"""
    memoryview_report_template = ("Memoryview objects allow Python code to access the internal data of an object "
                                  "that supports the buffer protocol without copying"
                                  "Please manual check memoryview usage here. File: {} Line: {}")

    fileinput_report_template = (
        "FileInput object's readline method could be used to read file content outside of App Dir"
        "Please manual check `readline` usage here. File: {} Line: {}")

    file_operation_report_template = (
        "File read/write operation is detected, it could be used to read/write file content "
        "outside of the App Dir. Please manual check `{}` usage here. File: {} Line: {}")

    client = app.python_analyzer_client
    for filepath, analyzer in client.get_all_ast_infos():
        # file manipulation
        file_read_write_operations = ['next', 'read', 'readline', 'readlines', 'xreadlines'] + \
                                     ['write', 'writelines', 'truncate', 'flush']
        for operation in file_read_write_operations:
            for lineno in analyzer.get_module_function_call_usage('file', operation, lineno_only=True):
                reporter.manual_check(file_operation_report_template.format(operation, filepath, lineno),
                                      file_name=filepath, line_number=lineno)
        # memoryview usage
        for lineno in analyzer.get_module_function_call_usage(AstVariable.BUILTIN_TYPE, 'memoryview', lineno_only=True):
            reporter.manual_check(memoryview_report_template.format(filepath, lineno), file_name=filepath,
                                  line_number=lineno)
        # fileinput
        for lineno in analyzer.get_module_function_call_usage('fileinput.input', 'readline', lineno_only=True):
            reporter.manual_check(fileinput_report_template.format(filepath, lineno), file_name=filepath,
                                  line_number=lineno)
...
def check_python_untrusted_xml_functions(app, reporter):
    ...
    message = "The following lines should be inspected during code review, the xml functions not safe enough. " \
              ...
              "Module/Method is {} "
    functions = metadata_store.query().tag(metadata_consts.TagConsts.FILE_READ_AND_WRITE). \
        namespace_prefix(metadata_consts.ModuleNameConsts.XML).python_compatible().functions()
...
def check_for_data_persistence(app, reporter):
    ...
    serialization_reporter_output = (
        ...
        "outside of App dir.")
    persistence_reporter_output = ("The following lines should be inspected during code review, `{}` could be used to "
                                   "store memory data to disk. Check if in-memory data will be stored outside of App "
                                   "dir.")

    serialization_modules = [metadata_consts.ModuleNameConsts.PICKLE, metadata_consts.ModuleNameConsts.CPICKLE,
                             metadata_consts.ModuleNameConsts.MARSHAL]
    persistence_modules = [metadata_consts.ModuleNameConsts.SHELVE, metadata_consts.ModuleNameConsts.ANYDBM,
                           metadata_consts.ModuleNameConsts.DBM, metadata_consts.ModuleNameConsts.GDBM,
                           metadata_consts.ModuleNameConsts.DUMBDBM]

    serialization_functions = metadata_store.query().namespace_prefixes(serialization_modules).tag(
        metadata_consts.TagConsts.DATA_PERSISTENCE).python_compatible().functions()
    persistence_functions = metadata_store.query().namespace_prefixes(persistence_modules).tag(
        metadata_consts.TagConsts.DATA_PERSISTENCE).python_compatible().functions()
    ...
    sqlite3_functions = metadata_store.query().namespace_prefix(metadata_consts.ModuleNameConsts.SQLITE3).tag(
        metadata_consts.TagConsts.DATA_PERSISTENCE).python_compatible().functions()
...
@splunk_appinspect.tags("splunk_appinspect", "cloud", "security", "manual", "ast")
@splunk_appinspect.cert_version(min="1.7.0")
def check_for_unix_questionable_commands(app, reporter):
    """Check for unix specific service usages"""
    message = "The following lines should be inspected during code review, the function usage {} in UNIX OS may " \
              "be questionable "
    unix_questionable_modules = [metadata_consts.ModuleNameConsts.POSIX, metadata_consts.ModuleNameConsts.CRYPT,
                                 metadata_consts.ModuleNameConsts.DL, metadata_consts.ModuleNameConsts.TERMIOS,
                                 metadata_consts.ModuleNameConsts.TTY, metadata_consts.ModuleNameConsts.PTY,
                                 metadata_consts.ModuleNameConsts.PIPES, metadata_consts.ModuleNameConsts.POSIXFILE,
                                 metadata_consts.ModuleNameConsts.SYSLOG]
    functions = metadata_store.query().tag(metadata_consts.TagConsts.UNIX_SPECIFIX_SERVICES). \
        namespace_prefixes(unix_questionable_modules).python_compatible().functions()
...
def check_for_plain_text_credentials_in_python(app, reporter):
    ...
    sensitive_name_patterns = ['login', 'password', 'pwd', 'passwd', 'community', 'privpass']

    def check_variable_name(variable_name):
        return any(map(lambda pattern: re.search(pattern, variable_name, re.IGNORECASE), sensitive_name_patterns))

    reporter_output = ("The following lines should be inspected during code review, "
    ...
    for filepath, ast_info in client.get_all_ast_infos():
        all_ast_nodes = set()
        for variable_name, ast_node_set in \
                filter(lambda tuple: check_variable_name(tuple[0]), iteritems(ast_info.variable_usage)):
        ...
        for sensitive_node in sensitive_ast_nodes:
            ...
            reporter.manual_check(
                reporter_output.format(node_to_name_dict[sensitive_node], node_variable.variable_value, filepath,
                                       sensitive_node.lineno),
                file_name=filepath, line_number=sensitive_node.lineno)


@splunk_appinspect.tags("splunk_appinspect", "cloud", "security", "manual", "ast")
@splunk_appinspect.cert_version(min="1.7.0")
def check_for_circular_importing_in_python(app, reporter):
    """Check if circular importing exists in current app"""
    reporter_output = "The following lines should be inspected during code review, circular importing were detected between python modules in the following files. " \
                      "CircleFile Set: {}. " \
                      "Circle importing example: {}. " \
                      "See filepath and row number for details."
    client = app.python_analyzer_client
    circular_files = client.get_circular_dependency_loops()
    for circular_file_set, circular_file_value in circular_files.items():
        if len(circular_file_value) < 1:
            reporter.manual_check(reporter_output.format(circular_file_set, "Couldn't find one"))
        else:
            reporter.manual_check(reporter_output.format(circular_file_set, circular_file_value[0]))
...
def check_for_interprocess_communication_and_networking(app, reporter):
    ...
    inheritance_output_template = 'The following lines should be inspected during code review. ' \
                                  'Class inherited from `{}` could be used to communicate with outside services or ' \
                                  'files. '
    socket_connection_template = 'The following lines should be inspected during code review. ' \
                                 '`{}` could be used to open socket connection to outside service. '
    classes = metadata_store.query().namespace_prefixes(
        [metadata_consts.ModuleNameConsts.ASYNCHAT, metadata_consts.ModuleNameConsts.ASYNCORE]) \
        .tag(metadata_consts.TagConsts.NETWORK_CONNECTION).python_compatible().classes()
    ...
    socket_functions = metadata_store.query().namespace_prefixes(
        [metadata_consts.ModuleNameConsts.SOCKET]).tag(metadata_consts.TagConsts.NETWORK_CONNECTION).python_compatible().functions()
...
def check_for_generic_operating_system_services(app, reporter):
    ...
    message = "The following lines should be inspected during code review. `{}` could be used to receive data from " \
              "outside or log data to outside. "
    query = metadata_store.query().namespace_prefixes(
        [metadata_consts.ModuleNameConsts.ARGPARSE, metadata_consts.ModuleNameConsts.LOGGING,
...
def check_for_reverse_shell_and_backdoor(app, reporter):
    ...
    for filepath, ast_info in client.get_all_ast_infos():
        ...
        for subprocess_usage in candidate_subprocess_usage:
            reporter.manual_check(reporter_output.format(filepath, subprocess_usage.lineno),
                                  file_name=filepath, line_number=subprocess_usage.lineno)
...
def check_for_python_runtime_services(app, reporter):
    ...
    message = "The following lines should be inspected during code review, " \
              "Possible system python objects manipulation `{}` found here. "

    functions = metadata_store.query().namespace_prefixes(
        [metadata_consts.ModuleNameConsts.GC, metadata_consts.ModuleNameConsts.INSPECT]).python_compatible().functions()
...
def check_for_custom_python_interpreters(app, reporter):
    ...
    functions = metadata_store.query().namespace_prefixes([metadata_consts.ModuleNameConsts.CODE]) \
        .tag(metadata_consts.TagConsts.STRING_EXECUTION).python_compatible().functions()
...
def check_for_python_multimedia_services(app, reporter):
    ...
    query = metadata_store.query().namespace_prefixes(multimedia_modules).tag(
        metadata_consts.TagConsts.FILE_READ_AND_WRITE).python_compatible()
    components = query.functions() + query.classes()
...
def check_for_data_compression_and_archiving(app, reporter):
    ...
    message = "The following lines should be inspected during code review, " \
              ...
              "outside of app dir."
    query = metadata_store.query().namespace_prefixes(
        [metadata_consts.ModuleNameConsts.GZIP, metadata_consts.ModuleNameConsts.BZ2,
         metadata_consts.ModuleNameConsts.ZIPFILE, metadata_consts.ModuleNameConsts.TARFILE]).tag(
        metadata_consts.TagConsts.DATA_COMPRESSION).python_compatible()
    components = query.functions() + query.classes()
    files_with_results = AstSearcher(app.python_analyzer_client).search(components)
    reporter.ast_manual_check(message, files_with_results)
    ...
    for file_path, ast_info in client.get_all_ast_infos():
        for module_name in ['msilib', 'msvcrt', '_winreg']:
            for lineno in ast_info.get_module_usage(module_name, lineno_only=True):
                reporter.manual_check('The following lines should be inspected during code review, MS Windows specific services usage `{}` has been detected'.format(module_name),
                                      file_name=file_path, line_number=lineno)
...
def check_for_optional_operating_system_services(app, reporter):
    ...
    objects = metadata_store.query().namespace_prefixes([metadata_consts.ModuleNameConsts.MMAP]).tag(
        metadata_consts.TagConsts.MEMORY_MAPPING).python_compatible().collect()
    message = 'The following lines should be inspected during code review, operating system feature `{}` has been ' \
              'detected. '
...
def check_for_restricted_execution(app, reporter):
    ...
    for file_path, ast_info in client.get_all_ast_infos():
        for module_name in ['rexec', 'Bastion']:
            for lineno in ast_info.get_module_usage(module_name, lineno_only=True):
                reporter.manual_check('The following lines should be inspected during code review, restricted execution `{}` has been detected'.format(module_name),
                                      file_name=file_path, line_number=lineno)
...
def check_for_internet_protocols_and_support(app, reporter):
    ...
    objects = metadata_store.query().namespace_prefixes(
        [metadata_consts.ModuleNameConsts.WSGIREF, metadata_consts.ModuleNameConsts.SOCKET_SERVER, metadata_consts.ModuleNameConsts.SOCKET_SERVER_PY3,
         metadata_consts.ModuleNameConsts.SIMPLE_XML_RPC_SERVER, metadata_consts.ModuleNameConsts.DOC_XML_RPC_SERVER,
         metadata_consts.ModuleNameConsts.BASE_HTTP_SERVER, metadata_consts.ModuleNameConsts.HTTP]).tag(metadata_consts.TagConsts.WEB_SERVER).python_compatible().collect()
    files_with_results = AstSearcher(app.python_analyzer_client).search(objects)
    reporter.ast_manual_check(message, files_with_results)
            ...
            for ast_node in ast.walk(arg):
                ...
                if AstVariable.is_string(variable):
                    ...
                    if variable.variable_value in ['su', 'sudo']:
                        return True
                    if variable.variable_value.startswith('su ') or \
                            variable.variable_value.startswith('sudo '):
                        return True
        return False

    check_objects = metadata_store.query().tag(metadata_consts.TagConsts.EXTERNAL_COMMAND_EXECUTION).python_compatible().collect()
    files_with_results = AstSearcher(app.python_analyzer_client).search(check_objects,
                                                                        node_filter=is_sudo_and_su_usage_exists)
...
def check_for_program_frameworks(app, reporter):
    ...
    check_objects = metadata_store.query().namespace_prefix(metadata_consts.ModuleNameConsts.CMD).tag(metadata_consts.TagConsts.EXTERNAL_COMMAND_EXECUTION).python_compatible().collect()
...
def check_for_importing_modules(app, reporter):
    ...
    message = 'The following lines should be inspected during code review, Python code `{}` for dynamically importing ' \
...
def check_for_debugging_and_profiling(app, reporter):
    ...
    check_objects = metadata_store.query().namespace_prefix(metadata_consts.ModuleNameConsts.TRACE)  \
                                          .tag(metadata_consts.TagConsts.EXTERNAL_COMMAND_EXECUTION) \
                                          .python_compatible().collect()
