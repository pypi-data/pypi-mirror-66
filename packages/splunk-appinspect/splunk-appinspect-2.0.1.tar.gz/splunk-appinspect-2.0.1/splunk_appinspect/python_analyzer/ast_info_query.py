...
class AstInfoQuery(object):
    ...
    def propagate_nodes(self, ast_type = ast.AST, filter_function=None, force_propagate=True):
        '''
    ...
    def _propagate_when_possible(self, ast_type, filter_function):
        ...
        while next_queue:
            ...
            if isinstance(current_node, ast_type) and (filter_function is None or filter_function(current_node)):
                result_queue.append(current_node)
            else:
                for child_node in ast.iter_child_nodes(current_node):
                    next_queue.append(child_node)
    ...
    def _post_traverse(self, filter_logic_node, all_node_set):
        if isinstance(filter_logic_node, BinaryLogicNode):
            ...
            elif isinstance(filter_logic_node, And):
                return left_set & right_set
            else:
                raise Exception('Unknown BinaryLogicNode {}'.format(type(filter_logic_node)))
        elif isinstance(filter_logic_node, MultiLogicNode):
            if isinstance(filter_logic_node, (MultiOr, MultiAnd)):
                if filter_logic_node.nodes:
                    # base result set
                    result_set = self._post_traverse(filter_logic_node.nodes[0], all_node_set)
                    for node in filter_logic_node.nodes[1: ]:
                        if isinstance(filter_logic_node, MultiAnd):
                            result_set = result_set & self._post_traverse(node, all_node_set)
                        elif isinstance(filter_logic_node, MultiOr):
                            result_set = result_set | self._post_traverse(node, all_node_set)
                    return result_set
                else:
                    raise Exception('At least one element should be included in MultiLogicNode')
            else:
                raise Exception('Unknown MultiBinaryLogicNode {}'.format(type(filter_logic_node)))
        elif isinstance(filter_logic_node, DataSetNode):
            if isinstance(filter_logic_node, Any):
                return self._get_next_batch_candidates_for_any(all_node_set, filter_logic_node.node_set)
            elif isinstance(filter_logic_node, All):
                return self._get_next_batch_candidates_for_all(all_node_set, filter_logic_node.node_set)
            else:
                raise Exception('Unknown DataSetNode {}'.format(type(filter_logic_node)))
        elif isinstance(filter_logic_node, SingleLogicNode):
    ...
    def _get_next_batch_candidates_for_all(self, all_node_set, logic_node_set):
        ...
        while queue:
            ...
            if current_node in all_node_set:
                ...
                if parent_node:
                    queue.append(parent_node)
        for node, cnt in iteritems(visit_count):
    ...
    def persist(self):
        ...
        new_query = AstInfoQuery(self._root_ast_node, self._parent_ast_node_dict, self._subtree_types_dict)
        new_query._queue.clear()
        new_query._is_end = self._is_end
...
class Any(DataSetNode):
    def __init__(self, node_set):
        super(Any, self).__init__(node_set)


class All(DataSetNode):
    def __init__(self, node_set):
        super(All, self).__init__(node_set)
