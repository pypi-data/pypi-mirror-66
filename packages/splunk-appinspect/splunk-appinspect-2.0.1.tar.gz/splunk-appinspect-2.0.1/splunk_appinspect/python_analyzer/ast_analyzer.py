...
from splunk_appinspect.python_analyzer import ast_types
...
class AstAnalyzer(object):
    ...
    def __init__(
        ...
        # python file path
        self._module_name = os.path.splitext(os.path.basename(python_file_path))[0] \
            if python_file_path is not None else None
        # module usage
        ...
        '''
        if six.PY2:
            self._snapshot_ast_types = ast.Exec # pylint: disable=no-member
        else:
        ...
        if python_file_path:
            ...
            raise Exception(
                'python_file_path or python_code_string should be provided')
    ...
    def get_module_all_possible_usage(
        ...
        '''
        result = set()
        for module_name, usage_set in iteritems(self._module_imported_usage):
            if self._is_prefix_module(module, module_name):
                result = result | usage_set
        for module_name, usage_set in iteritems(self._module_variable_usage):
            if self._is_prefix_module(module, module_name):
                result = result | usage_set
        for module_name, usage_set in iteritems(self._module_function_call_usage):
            if self._is_prefix_module(module, module_name):
                result = result | usage_set
        for module_name, usage_set in iteritems(self._module_function_call_return_value_usage):
            if self._is_prefix_module(module, module_name):
                result = result | usage_set
    ...
    def get_module_function_call_usage(
            ...
            lineno_only=False, with_context=False, fuzzy=False):
        '''
        ...
        module_string = module_name + '.' + function_name if (function_name is not None and function_name != '') else module_name
            ...
            def function_filter(function_node):
                ...
                for module_name in module_names:
                    ...
                    if module_name.startswith(module_string) and module_name != module_string:
                        return False
                return True
            function_nodes = list(filter(lambda function_node: function_filter(function_node), function_nodes))
    ...
    def get_literal_string_usage(
        ...
        for string, node_set in iteritems(self._literal_string_usage):
    ...
    def get_code_piece_usage(
        ...
        if code_ast_root.body:
            ...
            for node in ast.walk(self._root_ast_node):
                if utilities.is_same_ast_tree(
                        node, sub_tree, node1_extra=True):
                    result_list.append(node)
    ...
    def get_variable_distribution(self, variable_name, lineno_only=False, with_context=False):
        '''
    ...
    def get_variable_details(self, ast_node):
        ...
        if ast_node in self._ast_node_to_variable:
            ...
            if isinstance(ast_node, ast.Str):
                ...
                local_context, global_context = self._context_dict.get(ast_node, None), self._global_context_dict.get(ast_node, None)
                # contexts are ready
                if local_context is not None and global_context is not None:
                    # switch global context temporally
                    backup_global_map, self._global_map = self._global_map, global_context
    ...
    def _parse_file(
        ...
        if six.PY2:
            with open(python_file_path, "rU") as f:
                python_code_string = f.read()
        else:
            try:
                python_code_string = open(python_file_path, 'r', encoding="utf-8-sig").read()
    ...
    def _prev_traverse(self):
        ...
        self._global_map = {}
        # builtin namespace
        builtin_functions = [getattr(ast_types, function_name)() for function_name in dir(ast_types)
                             if function_name.startswith('AstBuiltin') and function_name != 'AstBuiltinFunction']
        self._builtin_map = {builtin_function.name:
                             ast_types.AstVariable(
                                 None,
                                 # both `builtin` and function names would be included here
                                 {ast_types.AstVariable.BUILTIN_TYPE, ast_types.AstVariable.BUILTIN_TYPE + '.' + builtin_function.name},
                                 builtin_function)
        ...
        # build ast parent tree
        for node in ast.walk(self._root_ast_node):
            for child_node in ast.iter_child_nodes(node):
                self._parent_ast_node_dict[child_node] = node
        # build subtree types dict
        self._build_subtree_types_dict(self._root_ast_node)

    def _post_traverse(self):

        # refine variable usage set
        for variable_name, usage_set in iteritems(self._variable_usage):
            remove_set = set()
    ...
    def _traverse_ast(self, ast_root, context):
        ...
        while stack:
            ...
            if isinstance(node, ast.ClassDef):
                ...
                class_context.namespace = node.name if class_context.namespace == '' else class_context.namespace + '.' + node.name

                self._traverse_ast(node.body, class_context)
            # enter function block or async function block
            elif isinstance(node, ast.FunctionDef) or ((not six.PY2) and isinstance(node, ast.AsyncFunctionDef)):
                ...
                function_context.namespace = node.name if function_context.namespace == '' else function_context.namespace + '.' + node.name

                # add arguments to local variable
                if six.PY2:
                    args, default_args = node.args.args, node.args.defaults
                    for arg in list(filter(lambda arg: isinstance(arg, ast.Name), args)):
                        function_context.variable_map[arg.id] = ast_types.AstVariable(arg, set(), None)
                    for arg, default_arg in list(filter(lambda tuple_result: isinstance(tuple_result[0], ast.Name), zip(reversed(args), reversed(default_args)))):
                        default_variable = self._get_variable_from_node(default_arg, context)
                        # collect default_arg's module usage
                        self._update_inverted_index_dict_with_key_set(default_variable.variable_type_set, default_arg, self._module_variable_usage)
                        function_context.variable_map[arg.id] = default_variable
                else:
                    args, default_args = node.args.args, node.args.defaults
                    ...
                    for arg, default_arg in list(filter(lambda tuple_result: isinstance(tuple_result[0], ast.arg),
                                                        zip(reversed(args), reversed(default_args)))):
                        default_variable = self._get_variable_from_node(default_arg, context)
                        # collect default_arg's module usage
                        self._update_inverted_index_dict_with_key_set(default_variable.variable_type_set, default_arg,
                                                                      self._module_variable_usage)
                        function_context.variable_map[arg.arg] = default_variable

                # global function
                if isinstance(context.root_node, ast.Module):
                    self._global_map[function_context.namespace] = ast_types.AstFunction(node.name, node, ast_types.AstVariable(None, set(), None))
                # function in function
                elif isinstance(context.root_node, ast.FunctionDef):
                    context.variable_map[node.name] = ast_types.AstVariable(node, {ast_types.AstVariable.FUNCTION_TYPE}, ast_types.AstFunction(node.name, node, ast_types.AstVariable(None, set(), None)), node.name)
            ...
            # enter exception handler block
                ...
                if six.PY2:
                    node_name_string, node_type_string = utilities.get_name_from_attribute_chain(node.name), \
                        utilities.get_name_from_attribute_chain(node.type)
                else:
                    node_name_string, node_type_string = node.name, utilities.get_name_from_attribute_chain(node.type)
                ...
                if isinstance(node, list):
                    if isinstance(context.root_node, ast.ClassDef):
                        ...
                        function_dict = {function_node.name: ast_types.AstFunction(function_node.name, function_node, ast_types.AstVariable(None, set(), None)) for function_node in function_nodes}

                        ast_class = None
                        if context.parent_context is not None:
                            if isinstance(context.parent_context.root_node, ast.Module):
                                self._global_map[context.namespace] = ast_types.AstClass(context.root_node.name, context.copy(), function_dict)
                                ast_class = self._global_map[context.namespace]
                            else:
                                context.parent_context.variable_map[context.root_node.name] = \
                                    ast_types.AstVariable(context.root_node, {ast_types.AstVariable.CLASS_TYPE},
                                                          ast_types.AstClass(context.root_node.name, context.copy(), function_dict))
                        ...
                        if ast_class:
                            ...
                            for base_class in context.root_node.bases:
                                ...
                                if isinstance(base_class_obj, ast_types.AstClass):
                                    ...
                                    for variable_name, variable in iteritems(base_class_obj.class_context.variable_map):
                                    ...
                                    for function_name, function_variable in iteritems(base_class_obj.function_dict):
                                        if function_name not in ast_class.function_dict:
                                            ast_class.function_dict[function_name] = function_variable
                                    self._update_inverted_index_dict_with_key_set(base_class_obj.modules, base_class, self._module_variable_usage)
                                    ...
                                    if base_class_obj.namespace is not None:
                                        ...
                                        for i in range(len(names)):
                                            ...
                                            self._update_inverted_index_dict(name_prefix, base_class, self._module_variable_usage)
                                    ...
                                    if all_modules:
                                        ...
                                        if isinstance(current_node, ast.Name):
                                            ...
                                            for index in range(len(nodes)):
                                                ...
                                                self._update_inverted_index_dict(module_string, nodes[index], self._module_variable_usage)
                            ...
                            init_functions = list(filter(lambda func: func.name in init_function_names, function_nodes))
                            other_functions = list(filter(lambda func: func.name not in init_function_names, function_nodes))
                    ...
                    stack.extend(
                        list(reversed([child_node for child_node in ast.iter_child_nodes(node)])))
    ...
    def _visit_node(self, node, context):
        ...
        method_name = '_visit_{}_node'.format(node.__class__.__name__.lower())
    ...
    def _collect_modules_import_usage(self, module_string, node, context, alias_name=None):

        # from xx import xx
        is_import_from = True if isinstance(node, ast.ImportFrom) else False
        ...
        if not is_import_from and module_string == '__builtin__':
            ...
            if self._is_in_global_context(context):
                ...
                for function_name, builtin_function in iteritems(self._builtin_map):
        ...
        if alias_name:
            key = alias_name
        if self._is_in_global_context(context):
            if context.context_mode == ast_types.AstContext.COLLECT_MODE and \
                    key in self._global_map and \
                    isinstance(self._global_map[key], ast_types.AstVariable):
                self._global_map[key].variable_type_set |= module_names
            else:
                self._global_map[key] = ast_types.AstVariable(node, module_names, None, key)
        else:
            if context.context_mode == ast_types.AstContext.COLLECT_MODE and \
                    key in context.variable_map and \
                    isinstance(context.variable_map[key], ast_types.AstVariable):
    ...
    def _visit_assign_node(self, node, context):
        ...
        for target_node in node.targets:
            self._visit_variable_node_and_value_node(
                target_node, node.value, context)

    def _visit_with_node(self, node, context):

        if six.PY2:
            self._handle_with_item(node, context)
        else:
    ...
    def _handle_with_item(self, node, context):
        if node.optional_vars is not None and node.context_expr is not None:
            if isinstance(node.optional_vars, ast.Name):
                ...
                self._ast_node_to_variable[node.optional_vars] = self._get_variable_from_node(node.context_expr, context)
    ...
    def _visit_return_node(self, node, context):
        ...
        if node.value is not None:
            ...
            if isinstance(context.root_node, ast.FunctionDef):
                ...
                if (context.parent_context is not None) and (isinstance(context.parent_context.root_node, ast.ClassDef)):
                    # check context first
                    ...
                    if class_context is not None and class_name in class_context.variable_map and ast_types.AstVariable.is_class_instance(class_context.variable_map[class_name]) and \
                            function_name in class_context.variable_map[class_name].variable_value.function_dict:
                        function = class_context.variable_map[class_name].variable_value.function_dict[function_name]
                        temp_variable_type_set = function.return_value.variable_type_set - set(ast_types.AstVariable.SPECIAL_TYPES)
                        function.return_value = ast_variable
                        function.return_value.variable_type_set |= temp_variable_type_set
                    else:
                        if class_name in self._global_map and isinstance(self._global_map[class_name], ast_types.AstClass):
                            class_obj = self._global_map[class_name]
                            if function_name in class_obj.function_dict:
                                temp_variable_type_set = class_obj.function_dict[function_name].return_value.variable_type_set - set(
                                    ast_types.AstVariable.SPECIAL_TYPES)
                                class_obj.function_dict[function_name].return_value = ast_variable
                                class_obj.function_dict[function_name].return_value.variable_type_set |= temp_variable_type_set
                else:
                    function_name = context.root_node.name
                    if context.parent_context is not None and isinstance(context.parent_context.root_node, ast.Module):
                        if function_name in self._global_map and isinstance(self._global_map[function_name], ast_types.AstFunction):
                            temp_variable_type_set = self._global_map[function_name].return_value.variable_type_set - set(ast_types.AstVariable.SPECIAL_TYPES)
                    ...
                    elif context.parent_context is not None and \
                            function_name in context.parent_context.variable_map and \
                            ast_types.AstVariable.is_function(context.parent_context.variable_map[function_name]):
                        temp_variable_type_set = context.parent_context.variable_map[function_name].variable_value.return_value.variable_type_set - set(ast_types.AstVariable.SPECIAL_TYPES)
                        context.parent_context.variable_map[function_name].variable_value.return_value = ast_variable
                        context.parent_context.variable_map[function_name].variable_value.return_value.variable_type_set |= temp_variable_type_set
    ...
    def _visit_variable_node_and_value_node(
            self, variable_node, value_node, context):
                ...
                if index == len(variable_node_name_list) - 1:
                    ...
                    if context.context_mode == ast_types.AstContext.COLLECT_MODE and name in current_variable_map:
                        ...
                        # it is a variable assignation, some extra works included here
                            ...
                            temp_variable_type_set = (current_node.variable_type_set - set(ast_types.AstVariable.SPECIAL_TYPES))
    ...
    def _get_variable_from_node(self, node, context):
        ...
        if isinstance(node, ast.Lambda):
            ...
            if six.PY2:
                for arg, default_arg in list(filter(lambda result: isinstance(result[0], ast.Name), zip(reversed(node.args.args), reversed(node.args.defaults)))):
                    lambda_context.variable_map[arg.id] = self._get_variable_from_node(default_arg, context)
            else:
                for arg, default_arg in list(filter(lambda result: isinstance(result[0], ast.arg), zip(reversed(node.args.args), reversed(node.args.defaults)))):
            ...
            return ast_types.AstVariable(node, {ast_types.AstVariable.FUNCTION_TYPE}, ast_types.AstFunction(None, node, return_value))
        ...
        if isinstance(node, ast.Name):
            ...
            if self._access_class_attribute_with_self(node_name_list, context):
                ...
                if node_list:
                    ...
                    # function call
                    elif context.parent_context.namespace in self._global_map and isinstance(self._global_map[context.parent_context.namespace], ast_types.AstClass) and \
                            start_name in self._global_map[context.parent_context.namespace].function_dict:
                        current_ast_variable = self._global_map[context.parent_context.namespace].function_dict[start_name]
                        could_start_parse = True
                    ...
                    current_ast_variable = self._propagate_ast_variable(current_ast_variable, node_list[0], node_to_call_dict, context)

                    if could_start_parse:
                        result_variable = self._parse_call_chain(node_list[1:], current_ast_variable, node_list[0], context, node_to_call_dict)
                        if result_variable:
                            return result_variable
                else:
                    # self node only
                    if context.parent_context.namespace in self._global_map and \
                            isinstance(self._global_map[context.parent_context.namespace], ast_types.AstClass):
                        return ast_types.AstVariable(self_node, {ast_types.AstVariable.CLASS_TYPE},
            ...
            # other possible cases
                ...
                if could_start_parse:
                    variable_result = self._parse_call_chain(node_list[1:], current_ast_variable, node_list[0], context, node_to_call_dict)
                    if variable_result:
        ...
        variable_name = utilities.get_node_name_or_attr(current_node) if isinstance(current_node, (ast.Name, ast.Attribute)) else None

        return ast_types.AstVariable(current_node, modules, None, variable_name)

    def _parse_call_chain(self, node_list, current_ast_variable, current_node, context, node_to_call_dict):
        '''
        ...
        if parse_succeed:
            ...
            if isinstance(current_ast_variable, ast_types.AstClass) or ast_types.AstVariable.is_class_instance(current_ast_variable):
                ...
                if isinstance(current_ast_variable, ast_types.AstFunction):
                    return ast_types.AstVariable(last_call_node, {ast_types.AstVariable.FUNCTION_TYPE}, current_ast_variable)
                elif isinstance(current_ast_variable, ast_types.AstModule):
                    return ast_types.AstVariable(last_call_node, {ast_types.AstVariable.MODULE_TYPE}, current_ast_variable)
                elif isinstance(current_ast_variable, ast_types.AstBuiltinFunction):
                    return ast_types.AstVariable(last_call_node, {ast_types.AstVariable.BUILTIN_TYPE}, current_ast_variable)
                elif isinstance(current_ast_variable, ast_types.AstCallableFunction):
                    return ast_types.AstVariable(last_call_node, {ast_types.AstVariable.CALLABLE_FUNCTION}, current_ast_variable)
                else:
    ...
    def _gather_variable_names_in_ast_tree(self, root_node, context):
        ...
        for ast_node in ast.walk(root_node):
            ...
            if (not isinstance(self._parent_ast_node_dict.get(ast_node, None), ast.Call)) or \
                self._parent_ast_node_dict[ast_node].func != ast_node:
    ...
    def _get_variable_node_name(self, node, context):
        ...
        if name_list:
            ...
            elif self._is_in_class(context):
    ...
    def _get_modules_used_in_ast_name_and_attribute_chain(self, node, context):
        '''
        ...
        if isinstance(node, ast.Name):
            ...
            if self._access_class_attribute_with_self(node_name_list, context):
                ...
                if node_list:
                    ...
                    elif (context.parent_context.namespace in self._global_map) and \
                            isinstance(self._global_map[context.parent_context.namespace], ast_types.AstClass) and \
                            start_name in self._global_map[context.parent_context.namespace].function_dict:
                        current_ast_variable = self._global_map[context.parent_context.namespace].function_dict[start_name]
                        could_start_parse = True
                        parse_start_index = 1
            ...
            if could_start_parse:
                ...
                for i in range(parse_start_index, len(node_name_list)):
                    ...
                    # class instance variable
                        ...
                        if node_name in current_ast_variable.variable_value.class_context.variable_map:
                            current_ast_variable = current_ast_variable.variable_value.class_context.variable_map[node_name]
                    ...
                    current_ast_variable = self._propagate_ast_variable(current_ast_variable, node_list[i], node_to_call_dict, context)
                else:
                ...
                if isinstance(current_ast_variable, ast_types.AstClass):
                    ...
                    result_set |= {ast_types.AstVariable.BUILTIN_TYPE, ast_types.AstVariable.BUILTIN_TYPE + '.' + current_ast_variable.name}
                    candidate_prefixes |= {ast_types.AstVariable.BUILTIN_TYPE, ast_types.AstVariable.BUILTIN_TYPE + '.' + current_ast_variable.name}
    ...
    def _check_function_args(self, node, function_args, function_keywords):
        ...
        if function_args is not None:
            for index, arg_dict in iteritems(function_args):
                # index is illegal
        ...
        if function_keywords is not None:
            ...
            for keyword_name, keyword_value_dict in iteritems(function_keywords):
                if keyword_name in keyword_arg_dict:
                    if not self._contains_same_ast_tree_in_node_set(
                        keyword_arg_dict[keyword_name],
                        self._get_expr_ast_node_set(
                            keyword_value_dict.get(
                                'arg_values',
                                []))):
                        return False
                else:
    ...
    def _is_prefix_module(self, prefix_module, module):
        ...
        i = 0
        while i < len(prefix_string_array) and i < len(module_string_array) and (
                prefix_string_array[i] == module_string_array[i]):
            i += 1
    ...
    def _filter_result_list(self, result_list, lineno_only, with_context):
        ...
        if lineno_only:
            result_list = sorted(
                list(set(map(lambda node: node.lineno, result_list))))
            return result_list
        # with_context
        if with_context:
            result_list = list(
                map(lambda node: {'ast_node': node,
                                  'local_context': self._context_dict.get(node, None),
                                  'global_context': self._global_context_dict.get(node, None)}, result_list))
            return result_list
    ...
    def _global_map_snapshot(self):
        new_dict = {}
        for key, value in iteritems(self._global_map):
    ...
    def _local_context_snapshot(self, context):
        ...
        for variable_name, ast_variable in iteritems(context.variable_map):
    ...
    def _is_in_class_method(self, context):
        ...
        return context.parent_context is not None and isinstance(
            context.parent_context.root_node, ast.ClassDef) and isinstance(context.root_node, ast.FunctionDef)
    ...
    def _get_variable_namespace(self, ast_variable):
        ...
        elif isinstance(ast_variable, ast_types.AstClass):
            return ast_variable.class_context.variable_map
        elif ast_types.AstVariable.is_module(ast_variable):
            return ast_variable.variable_value.global_map
        elif ast_types.AstVariable.is_class_instance(ast_variable):
            return ast_variable.variable_value.class_context.variable_map
        else:
            return None

    def _store_attribute_in_current_context(self, attribute_name, attribute, ast_node, context):
        ...
        if self._is_in_global_context(context):
            ...
            if isinstance(attribute, ast_types.AstClass):
                context.variable_map[attribute_name] = ast_types.AstVariable(ast_node, {ast_types.AstVariable.CLASS_TYPE},
                                                                             attribute, name=attribute_name)
            elif isinstance(attribute, ast_types.AstFunction):
                context.variable_map[attribute_name] = ast_types.AstVariable(ast_node, {ast_types.AstVariable.FUNCTION_TYPE},
                                                                             attribute, name=attribute_name)
            elif isinstance(attribute, ast_types.AstModule):
                context.variable_map[attribute_name] = ast_types.AstVariable(ast_node, {ast_types.AstVariable.MODULE_TYPE},
                                                                             attribute, name=attribute_name)
            elif isinstance(attribute, ast_types.AstCallableFunction):
                context.variable_map[attribute_name] = ast_types.AstVariable(ast_node, {ast_types.AstVariable.CALLABLE_FUNCTION},
                                                                             attribute, name=attribute_name)
            else:
    ...
    def _update_inverted_index_dict(self, key, node, inverted_index_dict):
        ...
        if six.PY2:
            assert isinstance(key, (str, unicode)) # pylint: disable=undefined-variable
        else:
    ...
    def _update_inverted_index_dict_with_key_set(self, key_set, node, inverted_index_dict):

        for key in key_set:
            self._update_inverted_index_dict(key, node, inverted_index_dict)

    def _build_subtree_types_dict(self, root_node):
        ...
        if root_node:
            ...
            while stack:
                ...
                traverse_order.append(current_node)
                for child_node in ast.iter_child_nodes(current_node):
                    stack.append(child_node)
            traverse_order.reverse()
            # traverse in post order
            for node in traverse_order:
                self._subtree_types_dict[node] = {type(node)}
                for child_node in ast.iter_child_nodes(node):
                    self._subtree_types_dict[node] |= self._subtree_types_dict[child_node]


    def _propagate_ast_variable(self, current_ast_variable, ast_node, node_to_call_dict, context):
        ...
        while self._is_propagate_variable(current_ast_variable):
            ...
            if isinstance(current_ast_variable, ast_types.AstFunction):
                if ast_node in node_to_call_dict:
                    ...
                    break
            elif isinstance(current_ast_variable, (ast_types.AstBuiltinFunction, ast_types.AstCallableFunction)):
                if ast_node in node_to_call_dict:
                    current_ast_variable = self._simulate_function_call(current_ast_variable,
                                                                        node_to_call_dict[ast_node],
                                                                        context)
                    ast_node = node_to_call_dict[ast_node]
                else:
                    break
    ...
    def _is_propagate_variable(self, current_ast_variable):
        ...
        return isinstance(current_ast_variable, (ast_types.AstBuiltinFunction, ast_types.AstCallableFunction, ast_types.AstFunction)) or \
            ast_types.AstVariable.is_function(current_ast_variable) or \
            ast_types.AstVariable.is_callable(current_ast_variable)
    ...
    def _simulate_function_call(self, current_ast_variable, call_node, context):
        ...
        if isinstance(current_ast_variable, (ast_types.AstBuiltinFunction, ast_types.AstCallableFunction)):
            return current_ast_variable.action(call_node,
                                              self,
            ...
            raise Exception('Illegal AST variable `{}` to call'.format(type(current_ast_variable)))