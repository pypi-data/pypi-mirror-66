...
{
	static const char* defaultName = "rigidBody";
	const char* name = defaultName;
	if (self->name) name = self->name;
	return _PyUnicode_FromASCII(name, strlen(name));
}

int rigidbody_setname(rigidbody_obj* self, PyObject* value)
{
	if (!PyUnicode_Check(value)) {
		PyErr_SetString(PyExc_TypeError, "Only string value can be set to name.");
		return NULL;
	}

	if (self->name) { free(self->name); self->name = nullptr; }
	Py_ssize_t size;
	const char* str = PyUnicode_AsUTF8AndSize(value, &size);
	if (size) {
		self->name = (char*)malloc(size);
...
{
	auto& transform = ((btRigidBody*)self->btbody)->getWorldTransform();
	btVector3& pos = transform.getOrigin();
	btQuaternion rot = transform.getRotation();

	PyObject* out = PyTuple_New(2);
	PyObject* outpos = Py_BuildValue("(fff)", pos.x(), pos.y(), pos.z());
	PyObject* outrot = Py_BuildValue("(ffff)", rot.x(), rot.y(), rot.z(), rot.w());
	PyTuple_SetItem(out, 0, outpos);
	PyTuple_SetItem(out, 1, outrot);
	return out;
}
...
int rigidbody_settransform(rigidbody_obj* self, PyObject* value)
{
	...
	if (!pyObjToTransform(value, trans)) return NULL;
	return 0;
}
...
PyObject* rigidbody_getshape(rigidbody_obj* self)
{
	...
	PyObject* shape = (PyObject*)btshape->getUserPointer();
	Py_INCREF(shape);
	return shape;
}
...
PyGetSetDef rigidbody_getsets[] = {
	{ const_cast<char*>("name"), (getter)rigidbody_getname, (setter)rigidbody_setname,rigidbody_name_doc, NULL },
	...
	{ const_cast<char*>("transform"), (getter)rigidbody_gettransform, (setter)rigidbody_settransform,rigidbody_transform_doc, NULL },
...
static PyObject* dynworld_getRigidBody(world_obj* self, PyObject* args) {
	...
	Py_INCREF((PyObject*)obj->getUserPointer());
...
struct MyConvexResultCallback : public btCollisionWorld::ConvexResultCallback
{
	std::vector<ConvexTestResult>& outResults;

	MyConvexResultCallback(std::vector<ConvexTestResult>& _outResults)
		:ConvexResultCallback(), outResults(_outResults){}
	...
	virtual btScalar addSingleResult(btCollisionWorld::LocalConvexResult& convexResult, bool normalInWorldSpace)
	{
		...
		ConvexTestResult result;
		result.object = convexResult.m_hitCollisionObject;
		result.hitFraction = convexResult.m_hitFraction;
		if (normalInWorldSpace)
			result.normal = convexResult.m_hitNormalLocal;
		else
			result.normal = convexResult.m_hitCollisionObject->getWorldTransform().getBasis() * convexResult.m_hitNormalLocal;

		result.point = convexResult.m_hitPointLocal;
...
static PyObject* dynworld_convexSweepTest(world_obj* self, PyObject* args) {
	//シェイプ遷移テスト
	//shape, fromPosition, fromRotation, toPosition, toRotation, allowedCcdPenetration, mask, group

	shape_obj* shapeObj = nullptr;
	PyObject* from;
	PyObject* to;
	float allowedCcdPenetration = 0.0f;
	int mask = -1;
	int group = 1;

	if (!PyArg_ParseTuple(args, "OOO|fii", &shapeObj, &from, &to, &allowedCcdPenetration, &mask, &group))return NULL;

	if (shapeObj->ob_base.ob_type != &ShapeType || !((btCollisionShape*)shapeObj->btshape)->isConvex()) {
		PyErr_SetString(PyExc_ValueError, "Shape type must be convex shape object.");
		return NULL;
	}

	btTransform a, b;
	if (!pyObjToTransform(from, a)) return NULL;
	if (!pyObjToTransform(to, b)) return NULL;

	std::vector<ConvexTestResult> outResults;
	MyConvexResultCallback cb(outResults);
	cb.m_collisionFilterMask = mask;
	cb.m_collisionFilterGroup = group;
	((btDiscreteDynamicsWorld*)self->btworld)->convexSweepTest((btConvexShape*)shapeObj->btshape, a,b,cb, allowedCcdPenetration);
	...
	PyObject* out = PyTuple_New(outResults.size());
	int i = 0;
	for (auto& it : outResults)
	{
		PyObject* point = Py_BuildValue("(fff)", it.point.x(), it.point.y(), it.point.z());
		PyObject* normal = Py_BuildValue("(fff)", it.normal.x(), it.normal.y(), it.normal.z());
		PyObject* object = (PyObject*)((btCollisionObject*)it.object->getUserPointer());
		PyObject* _res =
			Py_BuildValue(
				"{s:O,s:O,s:O,s:f}",
