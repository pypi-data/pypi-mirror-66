...
def get_fret_naming(chord_name):
  ...
  for _ in list(range(0, len(name_list))):
...
def draw_root_frets(draw, instrument, text_top_area, text_bottom_area, image_height, palette):
  ...
  for fret_num in list(range(1, instrument.number_of_frets)):
...
def draw_root_strings(draw, instrument, text_top_area, image_width, palette):
  ...
  for string_num in list(range(instrument.number_of_strings)):
    ...
    draw.line((string_pos_x, string_pos_y, image_width, string_pos_y), palette.string_colour, width=int(instrument.string_width/2))
  return draw
...
def draw_root_finder(instrument, palette, chord, image_dimentions, output_file):
  ...
  draw.line((int(instrument.margin/2), text_top_area, int(instrument.margin/2), image_height-text_bottom_area), palette.nut_colour, width=(int(instrument.nut_width/2)))
