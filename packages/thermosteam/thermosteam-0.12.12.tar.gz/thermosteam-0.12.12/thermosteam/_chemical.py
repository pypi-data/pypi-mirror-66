...
class Chemical:
    ...
    def Zc(self):
        ...
        critical_point = (self.Tc, self.Pc, self.Vc)
    ...
    def _init_data(self, CAS, atoms, MW):
        self.MW = MW
        self.Tm = normal_melting_point_temperature(CAS)
        self.Tb = Tb = normal_boiling_point_temperature(CAS)

        # Critical Point
        self.Tc = Tc = critical_point_temperature(CAS)
        self.Pc = Pc = critical_point_pressure(CAS)
        self.Vc = critical_point_volume(CAS)
        self.omega = acentric_factor(CAS, Tb, Tc, Pc)

        # Triple point
        self.Pt = triple_point_pressure(CAS)
        self.Tt = triple_point_temperature(CAS)

        # Energy
        self.Hfus = heat_of_fusion(CAS)
        self.Hsub = heat_of_sublimation(CAS)

        # Chemistry
        self.Hf = heat_of_formation(CAS)
        try:
    ...
    def _init_properties(self, CAS, MW, Tm, Tb, Tc, Pc, Zc, Vc, Hfus, omega,
        ...
        self.Psat = Psat = vapor_pressure_handle(data)
        if not Tb: Tb = self.Tsat(101325)
        
        # Volume
        sdata = (CAS,)
        ldata = (CAS, MW, Tb, Tc, Pc, Vc, Zc, omega, Psat, eos, dipole, has_hydroxyl)
        gdata = (CAS, Tc, Pc, omega, eos)
        self.V = V = volume_handle(sdata, ldata, gdata)
        ...
        self.sigma = surface_tension_handle(data)
        
        # Other
        self.epsilon = permitivity_handle((CAS, V.l,))
    ...
    def _estimate_missing_properties(self):
        ...
        if not Tt and Tm:
            self.Tt = Tt = Tm
        if not Pt and Tt:
            self.Pt = Pt = self.Psat.try_out(Tt)
        
        # Find missing critical property, if only two are given.
        critical_point = (Tc, Pc, Vc)
        N_values = sum([bool(i) for i in critical_point])
        if N_values == 2:
            self.Tc, self.Pc, self.Vc = Tc, Pc, Vc = Ihmels(*critical_point)
            
        Tb = self.Tb
        if not Tb:
            self.Tb = Tb = self.Tsat(101325)
            
        omega = self.omega
        if not omega and Pc and Tc:
            P_at_Tr_seventenths = self.Psat.try_out(0.7 * Tc)
    ...
    def default(self, slots=None):
        ...
        ['Dortmund', 'Hfus', 'Hsub', 'Hvap', 'InChI', 'InChI_key', 'PSRK', 'Pc', 'Psat', 'Pt', 'Tb', 'Tc', 'Tm', 'Tt', 'UNIFAC', 'V', 'Vc', 'combustion', 'common_name', 'dipole', 'eos', 'eos_1atm', 'formula', 'iscyclic_aliphatic', 'iupac_name', 'omega', 'pubchemid', 'similarity_variable', 'smiles']
    ...
    def get_missing_slots(self, slots=None):
        ...
        ['eos', 'eos_1atm', 'InChI', 'InChI_key', 'common_name', 'iupac_name', 'pubchemid', 'smiles', 'formula', 'Dortmund', 'UNIFAC', 'PSRK', 'combustion', 'S_excess', 'H_excess', 'mu', 'kappa', 'V', 'S', 'H', 'Cn', 'Psat', 'Hvap', 'sigma', 'epsilon', 'MW', 'Tm', 'Tb', 'Tt', 'Tc', 'Pt', 'Pc', 'Vc', 'Hf', 'LHV', 'HHV', 'Hfus', 'Hsub', 'omega', 'dipole', 'similarity_variable', 'iscyclic_aliphatic']
