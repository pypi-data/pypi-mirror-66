...
def raise_helpful_handle_error(handle):
    ...
    if isinstance(handle, PhaseHandle):
        raise TypeError(f"cannot set '{var}'; use the `add_model` "
                        f"and the `set_model_priority` methods of each phase "
                        f"handle to modify the thermodynamic properties for "
                        f"each phase (e.g. {var}.l.add_model(...))")
    elif isinstance(handle, ThermoModelHandle):
        raise TypeError(f"cannot set '{var}'; use the `{var}.add_model` "
                        f"and the `{var}.set_model_priority` methods to "
                        f"modify the thermodynamic property instead")
    else:
        raise Exception('expected a PhaseHandle or a ThermoModelHandle object; '
                       f'got a {type(handle).__name__} instead')

def raise_helpful_energy_functor_error(energy_functor):
    var = energy_functor.var
    raise TypeError(f"cannot set '{var}'; use the `reset_free_energies` "
...
class Chemical:
    ...
    def blank(cls, ID, CAS=None, phase_ref=None, phase=None, **data):
        ...
        **data : 
        ...
        if phase:
            for i in ('kappa', 'mu', 'V'):
                handle = TPDependentModelHandle(i)
                handle._chemical = self
                setfield(self, '_' + i, handle)
            self._Cn = TDependentModelHandle('Cn')
        else:
            for i in ('kappa', 'mu', 'V'):
                handle = PhaseTPHandle(i)
                handle.s._chemical = handle.l._chemical = handle.g._chemical = self
                setfield(self, '_' + i, handle)
            self._Cn = Cn = PhaseTHandle('Cn')
            Cn.s._chemical = Cn.l._chemical = Cn.g._chemical = self
        for i in ('sigma', 'epsilon', 'Psat', 'Hvap'):
            handle = TDependentModelHandle(i)
            handle._chemical = self
            setfield(self, '_' + i, handle)
    ...
    @Tt.setter
    def Tt(self, Tt):
        reset_constant(self, 'Tt', float(Tt))
    ...
    @Pt.setter
    def Pt(self, Pt):
        reset_constant(self, 'Pt', float(Pt))
    ...
    @Hfus.setter
    def Hfus(self, Hfus):
        reset_constant(self, 'Hfus', float(Hfus))
    ...
    @Hsub.setter
    def Hsub(self, Hsub):
        reset_constant(self, 'Hsub', float(Hsub))
    ...
    def has_hydroxyl(self):
        """Whether or not chemical contains a hydroxyl functional group,
    ...
    def atoms(self):
        ...
        return parse_simple_formula(formula) if formula else None
    ...
    def reset(self, CAS, eos=PR, phase_ref=None,
              ...
              HHV=None, Hfus=None, Hsub=None, dipole=None,
        ...
        self._init_data(CAS, MW, Tm, Tb, Tc, Pc,
                        Vc, omega, Pt, Tt, Hfus, Hsub, Hf, LHV,
                        HHV, combustion, dipole, similarity_variable,
                        iscyclic_aliphatic)
        self._init_eos(eos, self._Tc, self._Pc, self._omega)
        self._init_handles(CAS, self._MW, self._Tm, self._Tb, self._Tc,
                           self._Pc, self.Zc, self._Vc, self._Hfus,
    ...
    def reset_constants(self, **data):
        """Reset constant data (e.g. Tb, Tm, Hfus, etc.)"""
        setfield = setattr
        getfield = getattr
        if 'Hf' in data:
            self._Hf = data.pop('Hf')
        if 'LHV' in data:
            self._LHV = data.pop('LHV')
        if 'HHV' in data:
            self._HHV = data.pop('HHV')
        if 'combustion' in data:
            self._combustion = data.pop('combustion')
        for i in data:
            if i in _functor_data: 
                setfield(self, '_' + i, data[i])
            else:
                raise ValueError(f"invalid data key {i}")
        for handle in _handles: getfield(self, handle).load_data_from(data)
    ...
    def _init_data(self, CAS, MW, Tm, Tb, Tc, Pc,
                   Vc, omega, Pt, Tt, Hfus, Hsub, Hf, LHV,
                   HHV, combustion, dipole, similarity_variable,
                   iscyclic_aliphatic):
        self._MW = MW
        ...
        # Energy
        self._Hfus = Hfus or heat_of_fusion(CAS)
        self._Hsub = Hsub or heat_of_sublimation(CAS)

        # Chemistry
        self._Hf = Hf or heat_of_formation(CAS)
        atoms = self.atoms
        if not all([LHV, HHV, combustion]) and atoms and Hf:
            CD = CombustionData.from_chemical_data(
                atoms, self._CAS, MW, self._Hf)
            LHV = CD.LHV
            HHV = CD.HHV
    ...
    def _init_handles(self, CAS, MW, Tm, Tb, Tc, Pc, Zc, Vc, Hfus, omega,
        ...
        self._Hvap = Hvap = heat_of_vaporization_handle(data)
        ...
        self._kappa = kappa = thermal_conductivity_handle(None, ldata, gdata)
        
        # Surface tension
        if Tb:
            Hvap_Tb = Hvap.try_out(Tb)
            if Hvap_Tb: Hvap_Tb /= MW
            Cn_l = Cn.l.try_out(Tb)
            if Cn_l: Cn_l /= MW
            V_l = V.l.try_out(Tb, 101325.)
            rho_l = fn.V_to_rho(V_l, MW) if V_l else None
        else:
            Hvap_Tb = Cn_l = rho_l = None
        data = (CAS, MW, Tb, Tc, Pc, Vc, Zc,
                omega, self.Stiel_Polar, Hvap_Tb, rho_l, Cn_l)
        self._sigma = sigma = surface_tension_handle(data)
        
        # Other
        self._epsilon = epsilon = permitivity_handle((CAS, V.l,))
        
        for handle in (Psat, Hvap, sigma, epsilon):
            handle._chemical = self
        for phase_handle in (V, Cn): 
            phase_handle.s._chemical = \
            phase_handle.l._chemical = \
            phase_handle.g._chemical = self
        for phase_handel in (mu, kappa):
            phase_handle.l._chemical = \
            phase_handle.g._chemical = self
    ...
    def default(self, properties=None):
        ...
        ['Dortmund', 'Hfus', 'Hsub', 'Hvap', 'PSRK', 'Pc', 'Psat', 'Pt', 'Tb', 'Tc', 'Tm', 'Tt', 'UNIFAC', 'V', 'Vc', 'dipole', 'iscyclic_aliphatic', 'omega', 'similarity_variable']
    ...
    def get_missing_properties(self, properties=None):
        ...
        ['Cn', 'Dortmund', 'H', 'HHV', 'H_excess', 'Hf', 'Hfus', 'Hsub', 'Hvap', 'LHV', 'MW', 'PSRK', 'Pc', 'Psat', 'Pt', 'S', 'S_excess', 'Tb', 'Tc', 'Tm', 'Tt', 'UNIFAC', 'V', 'Vc', 'combustion', 'dipole', 'epsilon', 'iscyclic_aliphatic', 'kappa', 'mu', 'omega', 'sigma', 'similarity_variable']
