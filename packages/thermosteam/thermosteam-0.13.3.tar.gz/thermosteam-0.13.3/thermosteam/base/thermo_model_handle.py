...
from .functor import Functor, functor_lookalike
...
class ThermoModelHandle:
    ...
    def load_data_from(self, data:dict):
        for model in self._models: model.load_data_from(data)
    ...
    def add_model(self, evaluate=None,
        ...
        **funcs : function
            Other functions to give the model (e.g. integrate_by_T).

        Note
        ----
        This method can be used as a function decorator, but the function is
        not replace with model.
...
class TDependentModelHandle(ThermoModelHandle):
    ...
    def __call__(self, T, P=None):
        ...
        raise ValueError(f"{no_valid_model(self._chemical, self._var)} "
                         f"at T={T:.2f} K")
