...
@H.g
def Lastovka_Shaw_Integral(T, MW, similarity_variable, iscyclic_aliphatic=False):
    a = similarity_variable
    term = calculate_Lastovka_Shaw_term(iscyclic_aliphatic, a)
    B11 = 0.73917383
    B12 = 8.88308889
...
@S.g
def Lastovka_Shaw_Integral_Over_T(T, MW, similarity_variable,
                                  iscyclic_aliphatic=False):
    a = similarity_variable
    term = calculate_Lastovka_Shaw_term(iscyclic_aliphatic, a)
    a2 = a*a
...
def Dadgostar_Shaw_Indefinte_Integral(T, MW, similarity_variable, terms=None):
    if not terms:
...
def Dadgostar_Shaw_Integral(Ta, Tb, MW, similarity_variable):
    terms = calculate_Dadgostar_Shaw_terms(MW, similarity_variable)
    return MW*(Dadgostar_Shaw_Indefinte_Integral(Tb, MW, similarity_variable, terms)
               - Dadgostar_Shaw_Indefinte_Integral(Ta, MW, similarity_variable, terms))


def Dadgostar_Shaw_Over_T_Indefinte_Integral(T, MW, similarity_variable, terms=None):
    if not terms:
        terms = calculate_Dadgostar_Shaw_terms(MW, similarity_variable)
    first, second, third = terms
    first *= 3*R*(151.8675/T)**2*exp(151.8675/T)/(exp(151.8675/T)-1)**2
...
def Dadgostar_Shaw_Over_T_Integral(Ta, Tb, MW, similarity_variable):
    terms = calculate_Dadgostar_Shaw_terms(MW, similarity_variable)
    return MW*(Dadgostar_Shaw_Over_T_Indefinte_Integral(Tb, MW, similarity_variable, terms)
               - Dadgostar_Shaw_Over_T_Indefinte_Integral(Ta, MW, similarity_variable, terms))
...
heat_capacity_handle = PhaseTPropertyBuilder('Cn', heat_capacity_solid_handle, heat_capacity_liquid_handle, heat_capacity_gas_handle)
