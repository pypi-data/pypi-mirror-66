...
@sigma(doc='auto-merge', ref='[13]_')
def Aleem(T, MW, Tb, rho_l, Hvap_Tb, Cp_l):
    r'''
    Parameters
    ----------
    T : float
    ...
    Hvap_Tb : float
        Enthalpy of vaporization at the normal boiling point [J/g]
    Cp_l : float
        Liquid heat capacity of the chemical at T [J/kg/K]
    
    Notes
    -----
    Calculates vapor-liquid surface tension using the correlation derived by
    [13]_ based on critical property CSP methods.
    ...
    >>> f = Aleem(MW=16.04246, Tb=111.6, rho_l=458.7, Hvap_Tb=510870.,
    ... Cp_l=2465.)
    >>> f(T=90)
    0.01669970221165325
    ...
    return sphericity*MW**(1/3.)/(6.*N_A**(1/3.))*rho_l**(2/3.)*(Hvap_Tb + Cp_l*(Tb-T))
...
def surface_tension_handle(handle, CAS, MW, Tb, Tc, Pc, Vc, Zc, omega, StielPolar, Hvap_Tb, rho_l, Cp_l_Tb):
    ...
    data = (MW, Tb, rho_l, Hvap_Tb, Cp_l_Tb)
    if all(data):
        Tmax = Tb + Hvap_Tb/Cp_l_Tb
