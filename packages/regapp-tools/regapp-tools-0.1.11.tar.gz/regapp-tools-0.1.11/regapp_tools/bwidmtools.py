...
def get_external_id_from_username(parameter):
    ...
    if not bwidmorgid_specified: # Then we must only continue, if there is a single entry returned!!!
        if len(resps_json) != 1:
            logger.warning("You have not specified a bwidmOrgId, and I got multiple entries for the specified username. I refuse to continue. This may be normal for example for a root login.")
            raise ValueError
