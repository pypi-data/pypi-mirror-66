...
from pwbus.services.dynamic_service import DynamicService
...
class EngineDispatch():
    ...
    def route(self, channel_registry, request=None):
        ...
        try:
            ...
            if flow_in.isServiceEnabled():
                field_service_id = flow_in.getMessage().getServiceId()
                if field_service_id is not None:
                    if DEBUG:
                        log_debug(
                            f'EngineDispatch - Flow running for service_id [{field_service_id}]')

                    try:
                        for service_id in field_service_id.split(','):
                            service = DynamicService(
                                service_id, transformed_payload, DEBUG)
                            service.getInstance().execute()
                    except:
                        EngineMonitorEvent().incrValue("pwbus_service_errors")
                        pass
