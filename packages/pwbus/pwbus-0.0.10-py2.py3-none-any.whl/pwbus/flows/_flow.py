...
class _Flow:
    ...
    # _Flow.isServiceEnabled
    #
    def isServiceEnabled(self):
        return self.registry["flow.execute_services"] \
            if "flow.execute_services" in self.registry else True
...
class FlowIn(_Flow):
    ...
    def prepareMessage(self, payload):
        ...
        service_id = headers['Pwbus-Service-Id'] \
            if 'Pwbus-Service-Id' in headers else None
...
class FlowOut(_Flow):
    ...
    def prepareMessage(self, message, headers):
        ...
        self.getMessage().setServiceId(
            headers['Pwbus-Service-Id'] if 'Pwbus-Service-Id' in headers else None
        )
