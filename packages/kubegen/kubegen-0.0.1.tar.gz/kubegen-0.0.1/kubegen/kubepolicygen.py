...
# @click.command()
# @click.option('--kind', '-k', help='what kind of k8s policy file are you trying to create. support type includes deployment, ingress and svc.')
# @click.option('--data', '-d', help='Supply payload for the policy file in jsonstring format e.g {"name": "app-1", "version": "v1"} ')
# def main(kind, data):
...
#     policy = validate_yaml(response['data'])
...
class KubePolicyGen:
    ...
    def get_service_yaml_parameters(input_json):
        ...
        protocol = input_json.get('service_type', 'TCP')
