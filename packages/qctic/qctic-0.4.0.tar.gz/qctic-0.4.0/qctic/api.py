...
class QCticAPI:
    ...
    def __init__(self, host=DEFAULT_HOST):
        ...
        self._defaults = {"Content-Type": "application/json"}
    ...
    def client(self):
        ...
        AsyncHTTPClient.configure(None, defaults=self._defaults)
    ...
    def has_auth(self):
        ...
        token = self._defaults.get("headers", {}).get("Authorization")
        user = self._defaults.get("auth_username")
        passwd = self._defaults.get("auth_password")

        return token or (user and passwd)

    def _build_url(self, part):
        return urljoin(self._host, part)

    def _clean_auth(self):
        self._defaults.pop("auth_username", None)
        self._defaults.pop("auth_password", None)
        self._defaults.get("headers", {}).pop("Authorization", None)
    ...
    def auth_token(self, token):
        ...
        self._defaults["headers"] = self._defaults.get("headers", {})

        self._defaults["headers"].update({
