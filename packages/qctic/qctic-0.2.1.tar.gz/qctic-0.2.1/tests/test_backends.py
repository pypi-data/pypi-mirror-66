...
from qctic.backend import (CticErwinQasmSimulator,
                           CticErwinStatevectorSimulator,
                           CticErwinUnitarySimulator)
...
def test_backend_qasm(provider, httpserver):
    ...
    def assert_func(circuits, user_qobj_header, execute_kwargs, **kwargs):
        ...
        assert user_qobj_header["backend_name"] == CticErwinQasmSimulator.NAME
    ...
    job_erwin = execute(
        circuit,
        provider.get_backend(CticErwinQasmSimulator.NAME),
        shots=shots,
...
def test_backend_statevector(provider, httpserver):
    ...
    def assert_func(circuits, user_qobj_header, execute_kwargs, **kwargs):
        assert len(circuits) == 1
        assert user_qobj_header["backend_name"] == CticErwinStatevectorSimulator.NAME
        assert execute_kwargs["backend_options"] == backend_options
    ...
    job_erwin = execute(
        circuit,
        provider.get_backend(CticErwinStatevectorSimulator.NAME),
        backend_options=backend_options)
...
def test_backend_unitary(provider, httpserver):
    ...
    def assert_func(circuits, user_qobj_header, execute_kwargs, **kwargs):
        assert len(circuits) == 1
        assert user_qobj_header["backend_name"] == CticErwinUnitarySimulator.NAME
        assert execute_kwargs["backend_options"] == backend_options
    ...
    job_erwin = execute(
        circuit,
        provider.get_backend(CticErwinUnitarySimulator.NAME),
        backend_options=backend_options)
