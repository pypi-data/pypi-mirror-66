...
def test_provider_get_backend(provider):
    ...
    qasm_backend = provider.get_backend(CticErwinQasmSimulator.NAME)
...
def test_backend_base_interface(provider):
    backend = provider.get_backend(CticErwinQasmSimulator.NAME)
    config = backend.configuration()

    assert backend.provider() == provider
    assert backend.name() == CticErwinQasmSimulator.NAME
    assert backend.status().backend_name == CticErwinQasmSimulator.NAME
