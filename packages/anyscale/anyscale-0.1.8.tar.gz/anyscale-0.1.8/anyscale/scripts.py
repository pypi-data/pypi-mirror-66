...
def get_project_session(project_id: int, session_name: Optional[str]) -> Any:
    ...
    if len(sessions) > 1:
        raise click.ClickException(
            ...
            "Please specify the one you want to refer to with --session-name.".format(
...
def anyscale_init(
    ...
    if not os.path.exists(ray_scripts.PROJECT_DIR):
        if not name:
            ...
            project_name = name
        # TODO(pcm): Make this consistent with how we do project name normalization.
        if " " in project_name:
...
@click.command(name="pull", help="Pull files from a session")
@click.argument("source-directory", type=click.Path(), nargs=1)
@click.argument("target-directory", type=click.Path(), nargs=1)
@click.option(
    "--session-name",
    help="Session to pull from. Required if more than one running session.",
    default=None,
)
def anyscale_pull(
    source_directory: str, target_directory: str, session_name: str,
) -> None:
    ...
    try:
        ...
        snapshot_id = remote_snapshot(
            project_id, session_name, [source_directory], files_only=True
        )
        resp = send_json_request("user_get_temporary_aws_credentials", {})
        print("Downloading files.")
        download_snapshot(
            snapshot_id, resp["credentials"], target_directory, overwrite=True
        )
    except Exception as e:
        raise click.ClickException(e)  # type: ignore


@click.command(name="push", help="Create a snapshot of the current project.")
@click.argument("source-directory", type=click.Path(), nargs=1)
@click.argument("target-directory", type=click.Path(), nargs=1)
@click.option(
    "--session-name",
    help="Session to push to. Required if more than one running session.",
    default=None,
)
def anyscale_push(
    source_directory: str, target_directory: str, session_name: str,
    ...
    try:
        snapshot_uuid = create_snapshot(
            ...
            additional_files=[os.path.abspath(source_directory)],
            files_only=True,
            tags=["anyscale:push"],
        )
    except click.Abort as e:
    ...
    send_json_request(
        "/api/v1/sessions/{}/push".format(session["id"]),
        {"snapshot_uuid": snapshot_uuid, "target_directory": target_directory},
        post=True,
    )
