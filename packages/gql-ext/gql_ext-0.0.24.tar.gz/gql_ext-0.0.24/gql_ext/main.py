...
def mount_ws_service_to_app(app, service_name, base_url, endpoints, proxy_headers):
    endpoints_ = {k: WSRequest(path_template=v['path'])
                  for k, v in endpoints.items() or []}

    async def start_ws(app_):
        session = ClientSession(raise_for_status=True)
        if not hasattr(app_, 'ws'):
            app_.ws = {}
        if app_.ws.get(service_name):
            raise RuntimeError(f'This name of WS service is already in use {service_name}')

        app_.ws[service_name] = await HttpApi.create(base_url, session=session,
    ...
    app.on_startup.append(start_ws)
...
class GqlExt(abc.ABC):
    ...
    def mount_app(self):
        ...
        for ws_file_paths in (self.initial_config.get('ws') or []):
            ws_data = parse_conf(os.path.join(self.base_path, ws_file_paths))
            self.set_ws_service(ws_data)
    ...
    def set_ws_service(self, ws_description):
        for ws_service_name, endpoints in ws_description.items():
            service_base_url = self.config.get_ws(ws_service_name)
            if not service_base_url:
                raise RuntimeError(f'You must specify the base address of the service for {ws_service_name}')
            proxy_headers = self.config.proxy_headers
            mount_ws_service_to_app(self.app, ws_service_name, service_base_url, endpoints, proxy_headers)
