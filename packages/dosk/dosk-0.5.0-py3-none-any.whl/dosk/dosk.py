...
def local_build(path):
    # copy directory for local build
...
def parse_args():
    ...
    verbose.add_argument('--verbose', '-v',  action="store_const", const='DEBUG', default='INFO',
                        help="Print additional information")

    verbose.add_argument('--quiet', '-q', action="store_const", dest="verbose", const='WARNING',
...
class AsyncTaskList(TaskList):
    async def run(self, args=None):
        ...
        # Loop Time
        await asyncio.wait(
            [task.run() for task in run_tasks],
