...
def scan_surface(pdbcode, threads=4, threshold=0.999, print_scores=False):
    ...
    for i in range(data2['mesh'][:,meets_threshold].shape[1]):
        predicted_waters[list(data2['mesh'][:,meets_threshold][index_if_meets_threshold[i]][i])] = data2['predictions'][:,meets_threshold][index_if_meets_threshold[i]][i]
        if print_scores:
    ...
    print_nonwaters_as_pdb(None, predicted_waters.keys(), pdbcode)
...
def rank_waters(pdbcode):
    ...
    for line in handle:
        if line[17:20]=='HOH':
            mol_ids[[float(line[30:38].strip()),float(line[38:46].strip()),float(line[46:54].strip())]] = line[21:26]
    handle.close()
    model_=path+'water_hot_spots_adv_new.h5'
    download_pdb_file(pdbcode)
    model = water_glue_model(L=30,K=4, n=1).base_model
    ...
    rank=1
    for i in results:
        handle.write('%s\t%s\t%s\n' % (rank,mol_ids[i[0].get_array()],','.join(map(str,i[0].get_array()))))
        rank+=1
    handle.close()
