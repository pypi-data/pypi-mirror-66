...
metal_res_names = ' ZN, CA,PO4, MG, MN, FE, CO, NA,  K'.split(',')
...
def print_nonwaters_as_pdb(non_waters, waters, filename):
    ...
    if non_waters:
        for non_water in non_waters:
            ...
            handle.write("%s\n" % line)
    beginning2 =  'HETATM%s O   HOH W%s'
    space = '    '
    if waters:
        for i in range(len(waters)):
            water=waters[i]
            line = (beginning2 % (100001+i, 1000+i))+space+("%8.3f" % water[0])+("%8.3f" % water[1])+("%8.3f" % water[2]) +'  1.00  0.00'
            handle.write("%s\n" % line)
    handle.close()


def cluster_predicted_waters(file_, threshold = 1.4):
    lines = open(file_).readlines()
    waters = [[float(i[30:38].strip()),float(i[38:46].strip()),float(i[46:54].strip())] for i in lines if i[17:20]=='HOH']
    waters = np.array(waters)
    added_waters = []
    handle = open('%s_clustered.pdb' % file_.split('.')[0], 'w')
    beginning =  'HETATM%s O   HOH W%s'
    space = '    '
    for i in range(len(waters)):
        water=waters[i]
        if len(added_waters) and min(LA.norm(added_waters-water, axis=1))<threshold:
            continue
        line = (beginning % (100001+len(added_waters), 1000+len(added_waters)))+space+("%8.3f" % water[0])+("%8.3f" % water[1])+("%8.3f" % water[2]) +'  1.00  0.00'
        added_waters.append(water)
        handle.write("%s\n" % line)
    handle.close()
