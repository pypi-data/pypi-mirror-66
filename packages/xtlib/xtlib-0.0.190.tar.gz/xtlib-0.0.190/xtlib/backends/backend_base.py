...
class BackendBase(implements(BackendInterface)):
    ...
    # common code
    def append(self, cmds, cmd, echo_title="running", echo_before=False, echo_after=False, expand=False):
        ...
        if echo_before:
            cmds.append("{} 'before {}: {}'".format(echo, echo_title, cmd))

        cmds.append(cmd)

        if echo_after:
            cmds.append("{} 'after {}: {}'".format(echo, echo_title, cmd))

    def get_activate_cmd(self):

        setup_def = self.config.get_setup_from_target_def(self.compute_def)
        activate_cmd = setup_def["activate"] if setup_def and "activate" in setup_def else None
    ...
    def create_download_commands(self, xt_path, create_dest_dirs, sudo_available, download_requests, 
        ...
        for md in download_requests:
            ...
            self.append(cmds, cmd, "running: ", True, True)
    ...
    def add_first_cmds(self, cmds, script_name, change_dir):
        ...
        if self.is_windows:
            self.append(cmds, "mkdir {}".format(cwd), echo_before=True)
        else:
