...
def get_run_property_dicts():
    ...
    user_to_actual = {"box": "box_name", "created": "create_time", "child": "is_child", "cluster": "cluster",
        ...
        "status": "status", "target": "compute", "username": "username", "vc": "vc", "workspace": "ws",
...
def build_run_filter_dict(run_list, user_to_actual, builder, args):
    fd = {}
    option_filters = ["job", "experiment", "target", "service_type", "box", "status", "parent", "child", "outer"]  
...
def get_run_records(store, workspace, run_names):
    ''' return run records for specified run names'''

    mongo = store.get_mongo()

    # get philly service_run_id for each run name
    filter_dict = {}
    filter_dict["run_name"] = {"$in": run_names}

    fields_dict = {"cluster": 1, "service_run_id": 1, "vc": 1}

    run_records = mongo.get_info_for_runs(workspace, filter_dict, fields_dict)

    return run_records

def get_run_record(store, workspace, run_name):
    run_records = get_run_records(store, workspace, [run_name])
    if not run_records:
