...
class Runner():
    ...
    def process_run_command(self, args):
        ...
        self.attach_if_needed(workspace, job_runs, escape, attach)
        ...
        return cmds, run_specs, using_hp, using_aml_hparam, sweeps_text, pool_info 

    # def build_static_runs_by_node(self, total_runs, total_nodes):
    #     if total_runs < total_nodes:
    #         errors.combo_error("total runs={} must be >= total_nodes={}".format(total_runs, total_nodes))

    #     rbn = {}
    #     full_count = math.ceil(total_runs/total_nodes)
    #     runs = total_runs

    #     for i in range(total_nodes):
    #         start_index = i * full_count
    #         node_runs = runs if runs < full_count else full_count

    #         dd = {"runs_remaining": node_runs, "start_index": start_index, "run_count": node_runs}
