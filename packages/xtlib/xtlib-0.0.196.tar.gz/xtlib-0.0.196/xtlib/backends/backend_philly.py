...
class Philly(BackendBase):
    ...
    def build_curl(self, cmd):
        ...
        if pc_utils.is_windows() and not "username" in explicit:
            # for windows, we use built-in authentication
            prefix = 'curl -s {} --ntlm --user : '.format(cert_cmd)
        else:
            # for linux or explict username, see if user provided a password file
    ...
    def view_console(self, job_id, job_info, info_verb, parse_result=True, extra=""):
        ...
        cluster = utils.safe_value(pool_info, "cluster")
        vc = utils.safe_value(pool_info, "vc")
