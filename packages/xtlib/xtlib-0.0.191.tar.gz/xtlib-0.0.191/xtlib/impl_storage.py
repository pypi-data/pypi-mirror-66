...
class ImplStorage(ImplBase):
    ...
    @command(kwgroup="create", kwhelp="create the specified storage object", help="creates a new XT share")
    ...
    def view_console(self, target, workspace, node_index):
        if target.startswith("job"):
            # ji_text = self.store.read_job_info_file(target)
            # job_info = json.loads(ji_text)
            # if not utils.is_philly_job(job_info):
            #     errors.combo_error("view console currently only supports Philly jobs")
            # result, philly_job_id = self.philly.get_philly_job_info(target, job_info, "log", False)
            # console.print("console output for: {} (philly {})".format(target, philly_job_id))
            # console.print(result)
            workspace = job_helper.validate_job_name_with_ws(self.store, target, True)
            fn = "node-{}/after/stdout.txt".format(node_index)

            if not self.store.does_job_file_exist(target, fn):
                console.print("job '{}' has no file'{}'".format(target, fn))
            else:
                console.print("{} for {}:\n".format(fn, target))

                text = self.store.read_job_file(target, fn)
    ...
    def view_metrics(self, run_name, workspace, export):
        is_aml = self.is_aml_ws(workspace)
        ws, run_name, full_run_name = run_helper.validate_run_name(self.store, workspace, run_name, parse_only=is_aml)

        records = self.store.get_run_log(ws, run_name)
        console.print("metrics for {}:\n".format(full_run_name))

        metric_sets = run_helper.build_metrics_sets(records)
    ...
    @command(help="copy all files associated with the run to the specified directory")
    ...
    #---- CREATE TEAM command ----
    @example(task="create a template for a new XT team", text="xt create team")
    @command(kwgroup="create", help="generate an Azure template for creating a set of resources for an XT Team")
    def create_team(self):
        ...
        fn_template = file_utils.get_xtlib_dir() + "/templates/teamResources.json"
        template = file_utils.read_text_file(fn_template)

        # # personalize it for team name
        # template = template.replace("teamx7", name)
        
        # write to local directory
        fn_team_template = "create_xt_team.json"
        file_utils.write_text_file(fn_team_template, template)
        ...
        console.print()
        console.print("template generated: {}".format(fn_team_template))
        console.print()
    ...
    @example(task="view the most recent events in the XT log", text="xt view xt log")
