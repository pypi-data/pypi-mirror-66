...
class ImplUtilities(ImplBase):
    ...
    @command(help="executes the specified command, on begins a console session, with the specified box")
    def ssh(self, name, cmd, workspace):
        ...
        if name.startswith("run"):
            ...
            exit_code, output = process_utils.sync_run_ssh(self, ssh_ip, cmd, capture_output=capture_output)
    ...
    def scp(self, cmd):
        ...
        exit_code, output = process_utils.run_scp_cmd(self, cmd, report_error=True)
        if output:
            console.print(output)
        else:
