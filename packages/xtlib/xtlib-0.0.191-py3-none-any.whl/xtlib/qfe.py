...
def command(name=None, group=None, kwgroup=None, kwhelp=None, options_before_cmd=False, keyword_optional=False, pass_by_args=False, help=""):
    '''
    ...
    def decorator_command(func):
        ...
        cmd_info =  {"name": cmd_name, "options_before_cmd": options_before_cmd, "keyword_optional": keyword_optional, "pass_by_args": pass_by_args, 
...
class Dispatcher():
    ...
    def dispatch(self, args, is_rerun=False, capture_output=False, raise_syntax_exception=False):
        ...
        # add quotes that were removed by windows/shell command processor
        # console.diag("before: args={}".format(args))
        # self.add_quotes_to_string_args(args)
        # console.diag("after: args={}".format(args))

        # #add single quotes around option values without quotes
        # args = []
        # for arg in orig_args:
        #     #if "=" in arg:
        #     if arg.startswith("--") and "=" in arg:
        #         pre, post = arg.split("=", 1)
        #         if post and "=" in post and not post[0] in ['"', "'", "{"]:
        #             arg = "{}='{}'".format(pre, post)
        #     args.append(arg)
        # console.diag("after: args={}".format(args))
        ...
        if not cmd_info["options_before_cmd"]:
            tok = self.collect_raw_options(raw_options, scanner, tok)
