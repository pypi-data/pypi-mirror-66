...
class Philly(BackendBase):
    ...
    def submit_runs_for_node(self, job_id, node_runs, workspace, compute_def, resume_name, repeat_count, using_hp, experiment, 
        ...
        if fake_submit:
            ...
            fn = file_utils.fix_slashes(fn, force_linux=True)
    ...
    def run_curl_cmd(self, curl_cmd, parse_result=True):
        ...
        run_result = subprocess.run(parts, stdout=subprocess.PIPE)  # , stderr=subprocess.PIPE)
        ...
        if parse_result:
            console.diag("run_curl_cmd: result={}".format(result))

            result = json.loads(result)    
        return result

    def view_console(self, job_id, job_info, info_verb, parse_result=True, extra=""):
        '''get snapshot of console output for specified job'''
        pool_info = job_info["pool_info"]
        cluster = pool_info["cluster"]
        vc = pool_info["vc"]
        ...
        node_info = service_info_by_node[node_index]
        philly_job_id = node_info["service_id"]  

        curl_cmd = self.build_curl('"https://philly/api/{}?clusterId={}&vcId={}&jobId={}&logType=stdout&logRev=latest&content=full&jobType=ns{}"'.format(info_verb, cluster, vc, philly_job_id, extra))

        result = self.run_curl_cmd(cur_cmd, parse_result)
