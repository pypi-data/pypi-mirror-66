...
class Trainer():
    ...
    def log_stats(self, epoch, steps, data_len, loss, total_correct, total, model, device, 
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            if test_only:
                ...
                if epoch % args.log_interval == 0:
                    ...
                    self.log_stats(epoch, steps, data_len, loss, total_correct, total, model, device, 
                        checkpoint_freq, run, train_loss, train_acc, args)
    ...
    def run(self):
        ...
        if args.eval_model:
            ...
            self.train_test_loop(run, model, device, optimizer, start_epoch, checkpoint_freq, 
                test_only=True, args=args)
        else:
            self.train_test_loop(run, model, device, optimizer, start_epoch, checkpoint_freq, 
                test_only=False, args=args)
