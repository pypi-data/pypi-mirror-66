...
class MongoRunIndex():
    ...
    def _restart_runs_for_node(self):
        ...
        while True:
            ...
            dd = self.mongo.mongo_db["__jobs__"].find_and_modify(fd , update={"$set": {"active_runs.$.status": constants.WAITING}}, new=True)
            if not dd:
                break
    ...
    def mark_child_run_completed(self, entry):
        ...
        self.mongo.mongo_db["__jobs__"].find_and_modify(fd , update={"$set": {"active_runs.$.status": constants.COMPLETED}})
