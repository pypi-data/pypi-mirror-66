...
import math
import re
from typing import Any, Dict, Iterable, List, Optional, Sequence, Tuple, Type, Union, get_type_hints
...
class Lattice:
    ...
    def __getattribute__(self, item):
        ...
        if inspect.isclass(obj) and issubclass(obj, torch.nn.Module):
            cls = partial(obj, beam=self.beam)
...
def error_script(lattice: Segment) -> str:
    ...
    for item in sequence:
        ...
        if errors and element.label is None:
            raise SerializerError(f'Element without label: {element} (errors: {item})')
        field_errors = serialize_field_errors(element)
        ...
        statements.extend(serialize_errors(errors))
        statements.extend(field_errors)
...
def serialize_errors(errors: List[AlignmentError]) -> List[dict]:
    """Convert the list of alignment errors to a corresponding `dict` representations."""
    return [Command('ealign',
                    {name: write_attribute_value(value) for name, value in sorted(get_attributes(error).items())})
            for error in errors]


def serialize_field_errors(element: LatticeElement) -> List[dict]:
    ...
    for name in element.field_errors.values():
        ...
        if not name.endswith('l'):
            ...
            name = name[:-1]
        field_errors[int(name[-1])] = value.item()
    if all(x == 0 for x in field_errors.values()):
        return []
    field_error_list = [0.] * (max(field_errors) + 1)
    for order, value in field_errors.items():
        field_error_list[order] = value
    return [Command('efcomp', {'dkn': write_attribute_value(field_error_list)})]
