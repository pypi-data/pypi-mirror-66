...
from dipas.elements import Kicker, HKicker, HMonitor, ThinQuadrupole, elements
...
class Bases:
    ...
    class TestClosedOrbit(Base):
        ...
        def test_closed_orbit(self):
            ...
            co_ref = self._get_co_ref(script).loc[f'{self.sequence}$start', :].values
            ...
            co_lattice_ref = self._get_co_ref(self._augment_script(script_ref)).loc[f'{self.sequence}$start', :].values
            ...
            co_ref_everywhere = self._get_co_ref(script)
            x0 = torch.zeros(6, 1)
        ...
        def test_transfer_maps_accumulated(self):
            script = self._get_script()
            sm_ref = self._get_transfer_maps_ref(script)
            sm_ref = zip(*sm_ref[1])
            lattice = self._customize_lattice(from_script(script))
        ...
        def test_twiss_init_beta_alpha(self):
            ...
            self._test_twiss(init=init)

        def test_twiss_init_beta_alpha_mu(self):
            init = InitialLatticeParameters(beta_x=random.uniform(10, 100), beta_y=random.uniform(10, 100),
                                            alpha_x=random.uniform(0.1, 1.0), alpha_y=random.uniform(0.1, 1.0),
                                            mu_x=random.uniform(0, 1), mu_y=random.uniform(0, 1))
            self._test_twiss(init=init)

        def test_twiss_init_dispersion(self):
        ...
        def _get_co_ref(self, script):
            twiss = run_script(script, ['twiss'], madx=self.madx)['twiss']
            twiss['NAME'] = twiss['NAME'].str.lower()
            twiss.set_index('NAME', inplace=True)
            return twiss.loc[:, ['X', 'PX', 'Y', 'PY']]

        def _get_transfer_maps_ref(self, script):
            results = run_script(script, ['sectormap.tfs', 'sectormap_acc.tfs'], madx=self.madx)
...
class Lattices:
    class ToyLattice(unittest.TestCase):
        ...
        def setUpClass(cls):
            super().setUpClass()
            elements['multipole'] = lambda knl, l, beam, label, **kwargs: ThinQuadrupole(k1l=knl[1], label=label)
