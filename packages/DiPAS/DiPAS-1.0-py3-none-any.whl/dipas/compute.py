...
def orm(lattice: Segment, *,
    ...
    -------
    orm_x, orm_y : Tensor
        Shape `len(kickers), len(monitors)`.
    """
    co_args = co_args or {}
    ...
    orm_x = torch.zeros(len(kickers), len(monitors))
    orm_y = torch.zeros(len(kickers), len(monitors))
    for ik, kicker in enumerate(kickers):
        co_values = []
        for kick in kicks:
            kicker.kick += kick
            thin = lattice.makethin({Kicker: 2}, style={Kicker: 'edge'})  # These are the settings used by MADX.
            co = thin.transfer_maps('accumulate', order=order, d0=closed_orbit(thin, order=order, **co_args))
            co_values.append([co[i][0] for i in m_indices])
            kicker.kick -= kick
        for im in range(len(monitors)):
            orm_x[ik, im] += (co_values[1][im] - co_values[0][im])[0, 0] / (kicks[1] - kicks[0])
            orm_y[ik, im] += (co_values[1][im] - co_values[0][im])[2, 0] / (kicks[1] - kicks[0])
    return orm_x, orm_y
