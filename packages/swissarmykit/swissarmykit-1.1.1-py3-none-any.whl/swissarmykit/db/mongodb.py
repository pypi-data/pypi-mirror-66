...
class BaseDocument(DynamicDocument):
    ...
    @classmethod
    def save_url(cls, url, name=None, html=None, data=None, extra=None, attr=None, level=1, update_modified_date=None,
                 force_insert=False):  # type: (str, str, str, dict, dict, dict, int, bool, bool) -> BaseDocument
    ...
    @classmethod
    def get_one(cls, limit=1, offset=0, col_name=None, desc=None, where_id=None, where_url=None, filters=None, order_by=None, have_html=None, level=1,
                is_null_data=None, no_cursor_timeout=False, timeout=False, batch_size=0):  # type: (int, int, str, bool, list, list, dict, any, bool, int, bool, bool, bool, int) -> Union[List[BaseDocument], BaseDocument]
        '''
    ...
    def save_html(self, html, level=1, update_modified_date=None):  # type: (str, int, bool) -> BaseDocument
        ...
        if FileTask(id=self.id, table=self.get_collection_name(), html=html).save(level=level):
    ...
    def get_html(self, level=1, unset_html=False): # type: (int, bool) -> str
        ...
        try:
            if getattr(self, 'has_html_%d' % level):
                return FileTask(id=self.id, table=self.get_collection_name()).load(level=level)
        except Exception as e:
            print('ERROR: ', e)
            if unset_html:
    ...
    def have_html(self, level=1, unset_html=False):
        try:
            if getattr(self, 'has_html_%d' % level):
                if FileTask(id=self.id, table=self.get_collection_name()).exists(level=level):
                    return True
        except Exception as e:
            print('ERROR: ', e)
    ...
    def get_html_size(self, level=1):
        if getattr(self, 'has_html_%d' % level):
            return FileTask(id=self.id, table=self.get_collection_name()).size(level=level)
    ...
    def save_data(self, data, attr=None, update_modified_date=None):  # type: (dict, dict, bool) -> BaseDocument
    ...
    def save_attr(self, attr, update_modified_date=None):
        self.save_data(None, attr=attr, update_modified_date=update_modified_date)
    ...
    def delete_all_html(cls):
        task = FileTask(0, cls.get_collection_name())
        task.delete_all()
        cls.update_html_to_1()
        print('INFO: Delete all folder %s' % task.get_path())

    @classmethod
    def get_tmp_class(cls, class_name='', level=1): # type: (str, int) -> BaseDocument
    ...
    @classmethod
    def get_class(cls, class_name='', version=1): # type: (str, int) -> BaseDocument
