...
## from punctuation.logs.logger import logging_function # Inherit the logger from the calling function
...
def load_corpus(path='data/pickle/punctuation_stylometry.p'):
    res = try_to_load_as_pickled_object_or_None(path)
    if res is not None:
        return res
    else:
        print('Downloading the corpus.. please wait..')
        url = "https://zenodo.org/record/3605100/files/punctuation_stylometry.p?download=1"
        wget.download(url, path)
        return try_to_load_as_pickled_object_or_None(path)
#    if 'genre' in path:
