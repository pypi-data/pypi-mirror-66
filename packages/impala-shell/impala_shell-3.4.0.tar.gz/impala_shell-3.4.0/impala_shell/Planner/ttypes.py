...
class TPlanFragment:
  ...
  def __hash__(self):
    ...
    value = (value * 31) ^ hash(self.display_name)
    value = (value * 31) ^ hash(self.plan)
    value = (value * 31) ^ hash(self.output_sink)
    value = (value * 31) ^ hash(self.partition)
    value = (value * 31) ^ hash(self.backend_min_mem_reservation_bytes)
    value = (value * 31) ^ hash(self.instance_min_mem_reservation_bytes)
    value = (value * 31) ^ hash(self.instance_initial_mem_reservation_total_claims)
    value = (value * 31) ^ hash(self.backend_initial_mem_reservation_total_claims)
    value = (value * 31) ^ hash(self.produced_runtime_filters_reservation_bytes)
    value = (value * 31) ^ hash(self.consumed_runtime_filters_reservation_bytes)
    value = (value * 31) ^ hash(self.thread_reservation)
    return value
...
class TScanRangeLocation:
  ...
  def __hash__(self):
    value = 17
    value = (value * 31) ^ hash(self.host_idx)
    value = (value * 31) ^ hash(self.volume_id)
    value = (value * 31) ^ hash(self.is_cached)
    return value
...
class TScanRangeSpec:
  ...
  thrift_spec = (
    None, # 0
    (1, TType.LIST, 'concrete_ranges', (TType.STRUCT,(TScanRangeLocationList, TScanRangeLocationList.thrift_spec)), None, ), # 1
    (2, TType.LIST, 'split_specs', (TType.STRUCT,(PlanNodes.ttypes.TFileSplitGeneratorSpec, PlanNodes.ttypes.TFileSplitGeneratorSpec.thrift_spec)), None, ), # 2
  )
  ...
  def __hash__(self):
    value = 17
    value = (value * 31) ^ hash(self.concrete_ranges)
    value = (value * 31) ^ hash(self.split_specs)
    return value
...
class TPlanFragmentTree:
  ...
  def __hash__(self):
    value = 17
    value = (value * 31) ^ hash(self.cohort_id)
